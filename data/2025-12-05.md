...
Method: 开发平台无关的计算机视觉软件，通过连续自定义屏幕捕获技术，在用户查看数字切片图像时提供实时AI推理。软件为单个可执行文件，可在消费级PC上本地运行，无需复杂集成。

Result: 在2500多张公开全切片图像和临床数字病理案例中验证了软件的有效性，展示了在脑肿瘤分类、有丝分裂检测、免疫组化染色定量等常规病理任务中的稳健性。内置多模态聊天助手提供可验证的图像描述，支持实时显微镜摄像头输入。

Conclusion: OnSight Pathology能够跨多种病理学流程提供实时AI推理，消除了AI工具在组织病理学中采用的关键障碍，具有成本效益和安全性，适用于研究、临床甚至远程病理学环境。
...
TL;DR: LAPA是一个基于Transformer的端到端多相机点跟踪架构，通过注意力机制联合推理跨视图和时间，将外观匹配与几何约束集成，避免传统流水线中的误差传播问题。
...
Method: 提出LAPA架构：1) 使用跨视图注意力机制建立软对应关系，结合几何先验；2) 通过注意力加权聚合构建3D点表示，而非传统三角测量；3) 使用Transformer解码器建模长程时间依赖，保持身份一致性。

Result: 在TAPVid-3D-MC和PointOdyssey-MC数据集上显著优于现有方法，分别达到37.5%和90.3%的APD，在复杂运动和遮挡场景中表现尤为突出。

Conclusion: LAPA通过统一的端到端Transformer架构成功整合了外观匹配和几何约束，有效解决了多相机点跟踪中的误差传播和时间一致性问题，为挑战性场景提供了鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类自然地将连续经验感知为时间嵌套事件的层次结构，但计算机视觉需要能够前瞻性和层次性地分割视频的模型。现有方法缺乏对无监督、预测性、层次化事件结构学习的能力。

Method: PARSE采用统一框架，通过多尺度递归预测器学习事件结构：底层建模短期动态，高层通过基于注意力的反馈整合长期上下文。事件边界通过预测误差的瞬态峰值自然出现，形成时间连贯的嵌套部分整体关系。

Result: 在Breakfast Actions、50 Salads和Assembly 101三个基准测试中，PARSE在流式方法中达到最先进性能，在时间对齐（H-GEBD）和结构一致性（TED, hF1）方面可与离线基线媲美。

Conclusion: 不确定性下的预测学习为人类类似的时间抽象和组合事件理解提供了可扩展的路径，证明了无监督层次化事件分割的有效性。
...
TL;DR: MoReGen是一个基于牛顿运动控制的文本到视频生成框架，通过多智能体LLM、物理模拟器和渲染器生成物理精确的视频，并提出了MoReSet基准数据集和评估指标来量化物理有效性。
...
Method: 提出了MoReGen框架，整合多智能体LLM、物理模拟器和渲染器，在代码域中从文本提示生成可重现的物理准确视频。同时提出了对象轨迹对应性作为直接评估指标，并创建了MoReSet基准数据集，包含1,275个人工标注的视频，涵盖九类牛顿现象。

Result: 实验表明，现有最先进的文本到视频模型在保持物理有效性方面存在困难，而MoReGen为物理连贯的视频合成提供了一个有原则的方向。通过MoReSet基准评估，验证了MoReGen在物理准确性方面的优势。

Conclusion: MoReGen框架通过整合LLM、物理模拟器和渲染器，为物理精确的文本到视频生成提供了系统解决方案。提出的MoReSet基准和评估指标为量化物理有效性提供了重要工具，揭示了当前模型的局限性并指出了改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散和Transformer的内在图像分解模型依赖于合成数据集的配对监督，但在多样化真实场景中的泛化能力有限

Method: 使用多模态大语言模型作为感知评判器提供相对内在比较，将这些比较作为GRPO奖励用于在无标签真实图像上微调内在分解模型，通过奖励模型输出与评判器关系评估之间的一致性来对齐条件内在预测器

Result: 在多种基础架构和模态上，ReasonX带来显著改进：IIW反照率上WHDR降低9-25%，ETH3D深度准确率提升高达46%

Conclusion: ReasonX展示了MLLM引导的比较监督在连接低层和高层视觉推理方面的潜力，该框架与模型无关，可应用于不同的内在预测器
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估常见解剖结构的性能，无法捕捉罕见解剖变异带来的挑战。临床工作流程中需要处理各种解剖变异，但当前视觉语言模型在这方面的表现尚未得到系统评估。

Method: 研究者创建了AdversarialAnatomyBench基准测试，包含跨多种成像模态和解剖区域的自然发生罕见解剖变异。使用该基准测试评估了22个最先进的视觉语言模型，并分析了模型在基本医学感知任务上的表现。

Result: 1. 在罕见解剖结构上，平均准确率从典型解剖的74%下降到29%，性能下降45个百分点；2. 即使表现最好的模型（GPT-5、Gemini 2.5 Pro、Llama 4 Maverick）也出现41-51%的性能下降；3. 模型错误与预期的解剖偏见密切相关；4. 模型缩放和干预措施（包括偏见感知提示和测试时推理）无法解决这些问题。
...
Method: 采用两阶段设计：第一阶段使用新颖表示方法桥接3D布局和图像条件信号；第二阶段进行图像条件多视角生成，引入布局感知极线注意力机制增强一致性。还提出迭代框架支持文本到场景生成。

Result: 实验结果表明，MVRoom在3D场景生成方面实现了高保真度和可控性，在定量和定性评估中均优于现有基线方法。消融研究验证了关键组件的有效性。
...
TL;DR: UniLight提出统一的潜在空间表示，将文本、图像、辐照度和环境贴图等多种光照表示模态对齐，支持跨模态的光照检索、生成和控制任务。
...
Method: 提出UniLight联合潜在空间表示，通过对比学习训练模态特定编码器（文本、图像、辐照度、环境贴图），对齐不同模态的表示，并加入球谐函数预测任务增强方向性理解。

Result: 实验表明该表示能捕捉一致且可迁移的光照特征，支持光照检索、环境贴图生成和基于扩散模型的图像合成中的光照控制等任务。

Conclusion: UniLight通过统一的潜在空间表示成功解决了多种光照模态的兼容性问题，实现了跨模态的灵活光照操作和转换。
...
TL;DR: 提出GRU-SNF框架，通过MCMC采样增强GRU-NF在推理时的多样性，用于视频运动传输中的多模态未来预测
...
Method: 提出GRU-SNF框架，在GRU-NF推理过程中引入MCMC步骤（受SNF启发），使模型能够探索更丰富的输出空间，无需重新训练即可更好地近似真实数据分布。在基于关键点的视频运动传输管道中进行验证。

Result: 实验表明GRU-SNF在生成多样化输出方面优于GRU-NF，且不牺牲准确性，即使在更长的预测时间范围内也能有效捕捉多模态行为。

Conclusion: 将随机动力学与基于流的序列模型相结合，为生成式时间序列预测提供了潜力，通过推理时注入随机性可以更有效地捕捉多模态行为。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型日益复杂，输入数据规模不断扩大，尽管专用硬件有所进步，但数据加载仍然是限制训练和推理速度的主要瓶颈。现有研究主要关注识别任务，本文旨在探索这种方法在单图像超分辨率（SISR）恢复任务中的有效性。

Method: 提出一个轻量级超分辨率流程，直接在JPEG离散余弦变换（DCT）系数上操作，在频域进行处理，避免了完整的JPEG解码过程。

Result: 该方法实现了2.6倍的数据加载加速和2.5倍的训练加速，同时保持了与标准SISR方法相当的视觉质量。

Conclusion: 直接在JPEG编码特征上训练超分辨率模型是有效的，能够显著提升效率而不牺牲质量，为深度学习中的数据处理瓶颈提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统单目深度估计方法通常会过度平滑道路几何特征（如颠簸、坡度和表面不规则性），丢失对运动规划和车辆稳定性至关重要的信息。

Method: GfM方法预测主导路面平面和残差变化，使用gamma（垂直偏离平面的无量纲度量）表示局部细节。仅需相机离地高度，通过闭式解确定性地恢复度量深度，无需完整外部标定，适合自监督学习。

Result: 在KITTI和RSRD数据集上评估，GfM在近场深度和gamma估计方面达到最先进精度，同时保持竞争力的全局深度性能。轻量级模型（8.88M参数）能适应不同相机设置，是首个在RSRD上评估的自监督单目方法。

Conclusion: GfM通过解耦全局和局部结构，有效解决了单目重建中的投影模糊问题，能够准确恢复道路几何细节，为车辆控制提供关键信息，且无需大量标注数据。
...
Method: 首先分析文本提示调优方法在联邦学习下的校准表现，评估现有训练中校准技术在不同全局聚合方法中的效果，提出FL²oRA方法——一种基于LoRA的简单方法，自然改善联邦学习中的校准性能

Result: 实验表明，文本提示调优在联邦学习下会降低校准指标，现有校准技术改进有限。FL²oRA方法在多个基准测试中能持续产生良好校准的模型，减少对显式校准过程的需求

Conclusion: 联邦学习中CLIP校准的关键挑战不仅在于如何聚合或校准，更在于选择微调哪些组件。FL²oRA方法能有效改善联邦学习环境下的模型校准性能
...
TL;DR: TOMcap是一种无需对齐图像-文本对训练的文本训练方法，通过CLIP表示和检索增强技术改进图像描述生成，性能优于其他无训练和纯文本方法。
...
Method: 基于预训练语言模型解码器，使用经过模态差距减少处理的CLIP表示进行提示，结合检索的文本描述示例和潜在向量表示来引导生成过程。

Result: TOMcap在实验中优于其他无训练和纯文本方法，并对检索增强和模态差距减少组件的不同配置选择进行了分析。

Conclusion: 该方法展示了无需对齐图像-文本对训练的有效图像描述生成能力，为减少对标注数据依赖提供了有前景的解决方案。
...
TL;DR: 开发了一个基于计算机视觉和物理执行的低成本实时系统，用于自动化按性别分选家蟋蟀，在资源受限设备上实现了86.8%的分选准确率。


<details>
  <summary>Details</summary>
Motivation: 全球对可持续蛋白质来源的需求推动了对食用昆虫的兴趣，家蟋蟀被认为是最适合工业化生产的物种之一。当前养殖实践通常在混合性别群体中进行，缺乏自动化性别分选，尽管选择性育种、优化繁殖比例和营养差异化等方面存在潜在益处。

Method: 结合计算机视觉和物理执行，开发了一个低成本实时系统。系统集成了Raspberry Pi 5与官方Raspberry AI摄像头，使用自定义的YOLOv8 nano目标检测模型，配合伺服驱动的分选臂。

Result: 模型在测试中达到了0.977的mAP@0.5（平均精度均值），在实际蟋蟀群体实验中实现了86.8%的整体分选准确率。
...
TL;DR: Mind-to-Face：首个从非侵入式脑电图信号直接解码为高保真面部表情的框架，通过同步EEG和多视角面部视频数据训练，实现神经信号到视觉表达的映射。


<details>
  <summary>Details</summary>
Motivation: 当前的表情化虚拟形象系统过度依赖视觉线索，当面部被遮挡或情绪内化时失效。需要一种能够从神经信号直接解码面部表情的方法，实现更真实的情感表达。

Method: 构建双模态记录装置获取同步的EEG和多视角面部视频数据；使用CNN-Transformer编码器将EEG信号映射到密集的3D位置图（可采样超过65k顶点）；通过改进的3D高斯溅射渲染管道实现逼真的视角一致性结果。

Result: EEG信号能够可靠预测动态、个性化的面部表情，包括细微的情感反应；神经信号包含比先前假设更丰富的情感和几何信息；实现了神经驱动虚拟形象的新范式。
...
TL;DR: DisentangleFormer提出了一种解耦空间和通道维度的Vision Transformer架构，通过并行处理空间token和通道token流，实现结构依赖和语义依赖的独立建模，在超光谱成像任务中取得SOTA性能。
...
Method: 1) 并行解耦：独立处理空间token流和通道token流，实现空间和光谱维度的去相关特征学习；2) 压缩token增强器：自适应校准模块，动态融合空间和通道流；3) 多尺度FFN：补充全局注意力，捕获细粒度结构和语义依赖。

Result: 在超光谱基准测试（Indian Pine、Pavia University、Houston、BigEarthNet）和红外病理数据集上达到SOTA性能。在ImageNet上保持竞争力的同时，计算成本降低17.8% FLOPs。

Conclusion: DisentangleFormer通过信息论指导的空间-通道解耦设计，实现了鲁棒的多通道视觉表示，在超光谱成像任务中表现出色，同时计算效率更高。
...
Method: 首先计算每个视频的密集4D特征轨迹和跨视频轨迹对应关系（使用融合Gromov-Wasserstein最优传输方法）；然后进行全局帧级时间对齐以最大化匹配4D轨迹的重叠运动；最后通过基于运动样条支架表示的多视频4D高斯泼溅实现亚帧同步。

Result: 在Panoptic Studio和SyncNeRF Blender数据集上评估，平均时间误差低于0.26帧，达到亚帧同步精度；在Panoptic Studio数据集上获得26.3 PSNR的高保真4D重建分数。

Conclusion: SyncTrack4D是首个通用的非同步视频集4D高斯泼溅方法，无需假设预定义场景对象或先验模型，实现了高质量的动态3D场景重建与时间同步。
...
TL;DR: 本文提出了一种用于开放集人脸伪造检测的双层证据方法，能够识别新型伪造类别，在检测未知伪造类型上比基线模型平均提升20%性能。


<details>
  <summary>Details</summary>
Motivation: 随着人脸伪造生成算法的快速发展，新型伪造类别不断出现，现有的人脸伪造检测方法通常局限于二元真伪分类或已知伪造类别的识别，无法检测新型伪造的出现。这在实际应用中构成了重大挑战。

Method: 提出双层证据人脸伪造检测方法，通过不确定性估计来解决开放集人脸伪造检测问题。该方法在空间和频率两个层次上收集和融合类别特定的证据来估计预测不确定性。

Result: 在多种实验设置下的广泛评估表明，DLED方法在检测新型伪造类别方面实现了最先进的性能，比各种基线模型平均提升20%。在传统的真伪人脸伪造检测任务上，DLED方法也表现出有竞争力的性能。

Conclusion: 本文成功解决了开放集人脸伪造检测问题，提出的双层证据方法能够有效识别新型伪造类别，为人脸伪造检测领域提供了更适应现实场景的解决方案。
...
### [19] [When Robots Should Say "I Don't Know": Benchmarking Abstention in Embodied Question Answering](https://arxiv.org/abs/2512.04597)
*Tao Wu,Chuhao Zhou,Guangyu Zhao,Haozhi Cao,Yewen Pu,Jianfei Yang*

Main category: cs.CV

TL;DR: 论文提出AbstainEQA数据集，关注具身问答中的弃权能力，即智能体知道何时因信息不足而拒绝回答，而非强制回答所有问题。


<details>
  <summary>Details</summary>
Motivation: 现有具身问答基准假设所有问题都必须回答，但实际应用中智能体应知道何时因信息不足而拒绝回答。研究发现32.4%的人类查询存在缺失或未明确指定的上下文，需要弃权机制。

Method: 基于500个人类查询研究和人类沟通错误认知理论，提出五类需要弃权的情况：可操作性限制、指代不明确、偏好依赖、信息不可用、错误预设。通过将OpenEQA中的明确问题转化为模糊变体，构建包含1,636个弃权案例的AbstainEQA数据集。

Result: 评估显示，即使最好的前沿模型弃权召回率仅为42.79%，而人类达到91.17%。扩展、提示和推理仅带来边际改进，微调模型容易过拟合到文本线索。

Conclusion: 弃权是具身环境中可靠交互的基本前提，也是有效澄清的必要基础。当前模型在弃权能力上仍有很大提升空间，需要更鲁棒的解决方案。
...
### [20] [Mitigating Object and Action Hallucinations in Multimodal LLMs via Self-Augmented Contrastive Alignment](https://arxiv.org/abs/2512.04356)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在生成视频描述时存在严重的事实不准确问题，导致幻觉现象。虽然已有工作探索缓解静态图像的幻觉问题，但针对动态视频中的视觉物体和时序动作幻觉的联合缓解仍是一个未解决的挑战性任务。

Method: 提出自增强对比对齐框架，通过幻觉自增强方案识别MLLM中的潜在幻觉，将原始描述转换为对比负样本；开发轨迹短语对比对齐方法，将区域物体和关系引导的动作与其对应的视觉和时序短语进行匹配。

Result: 大量实验表明，SANTA在缓解物体和动作幻觉方面优于现有方法，在幻觉检测基准上取得了优越性能。

Conclusion: SANTA框架通过消除虚假关联并强调视觉事实，有效提升了多模态大语言模型在视频描述中的物体和动作忠实性。
...
### [21] [FASTer: Toward Efficient Autoregressive Vision Language Action Modeling via neural Action Tokenization](https://arxiv.org/abs/2512.04952)
...
TL;DR: FASTer框架通过可学习的动作分词器和自回归策略，在机器人操作任务中实现了更高的推理效率和任务性能平衡


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视觉-语言-动作模型在动作分词化过程中存在重建保真度与推理效率之间的权衡问题，需要更高效的统一框架

Method: 提出FASTer框架，包含FASTerVQ（将动作块编码为单通道图像的分词器）和FASTerVLA（基于该分词器的块级自回归解码和轻量级动作专家）

Result: FASTerVQ提供优越的重建质量、高令牌利用率和强大的跨任务/跨具身泛化能力；FASTerVLA在推理速度和任务性能上均超越现有最先进的VLA模型

Conclusion: FASTer框架通过高效的动作分词化和自回归策略，在机器人学习任务中实现了更好的效率-性能平衡，具有实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 相机只能捕捉物体的部分观测，特别是在遮挡情况下，物体重建具有挑战性。需要利用额外信息来减少视觉信号的歧义性。

Method: 结合两种信息源：1) 生成模型学习常见物体形状先验，可以对未见几何部分进行合理猜测；2) 从视频和物理交互中获取的接触信息提供几何边界的稀疏约束。通过接触引导的3D生成方法结合这两种信息，灵感来源于生成模型中的拖拽式编辑。

Result: 在合成数据和真实世界数据上的实验表明，该方法相比纯3D生成和基于接触的优化方法，在重建效果上有显著改进。

Conclusion: 通过结合生成模型先验和接触信息，能够有效减少视觉信号的歧义性，提高从部分观测中重建物体几何的准确性。
...
TL;DR: FMA-Net++是一个联合视频超分辨率和去模糊框架，专门处理现实世界中由运动与动态变化曝光耦合产生的复杂退化问题，通过曝光时间感知调制和流引导动态滤波实现高效恢复。


<details>
  <summary>Details</summary>
Motivation: 现实世界视频恢复面临运动与动态变化曝光耦合产生的复杂退化问题，这是先前工作忽视的关键挑战，也是自动曝光或低光拍摄的常见伪影。

Method: 采用序列级架构，基于分层细化与双向传播块实现并行长程时序建模。每个块包含曝光时间感知调制层，将特征按每帧曝光条件化，驱动曝光感知流引导动态滤波模块推断运动和曝光感知的退化核。将退化学习与恢复解耦，前者预测曝光和运动感知先验来指导后者。

Result: 仅使用合成数据训练，FMA-Net++在REDS-ME、REDS-RE和GoPro基准测试中实现了最先进的准确性和时序一致性，在恢复质量和推理速度上都优于近期方法，并能很好地泛化到具有挑战性的真实世界视频。

Conclusion: FMA-Net++通过显式建模运动与动态变化曝光的耦合效应，有效解决了现实世界视频恢复中的复杂退化问题，在准确性和效率方面都有显著提升，并具有良好的泛化能力。
...
### [24] [NeuralRemaster: Phase-Preserving Diffusion for Structure-Aligned Generation](https://arxiv.org/abs/2512.05106)
*Yu Zeng,Charles Ochoa,Mingyuan Zhou,Vishal M. Patel,Vitor Guizilini,Rowan McAllister*

Main category: cs.CV

TL;DR: 提出相位保持扩散（φ-PD），通过保留输入相位同时随机化幅度，实现结构对齐的生成，无需修改架构或增加参数，适用于需要几何一致性的任务。


<details>
  <summary>Details</summary>
Motivation: 标准扩散使用高斯噪声破坏数据的相位成分，这会破坏空间结构，不适用于需要几何一致性的任务，如重新渲染、仿真增强和图像到图像转换。

Method: 提出相位保持扩散（φ-PD），这是一种模型无关的扩散过程重构，保留输入相位同时随机化幅度。进一步提出频率选择性结构（FSS）噪声，通过单一频率截止参数提供对结构刚度的连续控制。

Result: φ-PD在照片级真实感和风格化重新渲染，以及驾驶规划器的仿真到真实增强中产生可控、空间对齐的结果。应用于CARLA仿真器时，将CARLA到Waymo规划器性能提升50%。
...
### [25] [Fourier-Attentive Representation Learning: A Fourier-Guided Framework for Few-Shot Generalization in Vision-Language Models](https://arxiv.org/abs/2512.04395)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模预训练视觉语言模型通常学习整体表征，其中图像的不变结构特征与领域特定风格特征隐式纠缠，这限制了模型的泛化能力，需要进一步解耦这些视觉线索。
...
TL;DR: 该研究对六种预训练深度学习模型在胸部X光疾病检测任务上进行迁移学习性能评估，发现InceptionV3表现最佳，ResNet系列随深度增加性能提升，并探讨了模型选择的影响因素。
...
Method: 使用六种预训练模型（AlexNet、VGG16、ResNet18、ResNet34、ResNet50和InceptionV3）在自定义胸部X光数据集上进行迁移学习评估，包括不确定性分析和运行时间比较。

Result: InceptionV3在所有标准指标上表现最佳；ResNet系列随深度增加性能逐步提升；VGG16和AlexNet表现合理但准确率较低；迁移学习在数据有限时特别有益，但改进程度受模型架构、数据集大小和领域相似性等因素影响。

Conclusion: 迁移学习对医学图像分类有效，特别是数据有限时；InceptionV3是最佳选择；ResNet系列随深度增加性能提升；使用训练良好的特征提取器，仅需轻量级前馈模型即可实现高效预测；研究为基于特定需求选择合适模型提供了指导。
...
### [27] [Dual-Stream Spectral Decoupling Distillation for Remote Sensing Object Detection](https://arxiv.org/abs/2512.04413)
...
<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法在遥感图像目标检测中存在特征混合问题，且忽略了由细微特征变化引起的差异，导致知识混淆。遥感图像具有密集小目标检测的挑战，需要更有效的蒸馏方法。

Method: 提出DS2D2方法：1）基于一阶小波变换进行光谱分解，保留关键空间特征；2）设计密度无关尺度权重（DISW）处理密集小目标；3）通过全频和高频放大器提取隐式知识，将特征差异映射到预测偏差。
...
TL;DR: 提出一个可解释的多模态框架，整合RGB和深度数据用于帕金森病步态分析，通过双编码器提取特征，结合多尺度局部全局提取模块和跨空间融合机制，并利用冻结大语言模型生成临床可理解的解释。


<details>
  <summary>Details</summary>
Motivation: 现有帕金森病步态分析方法存在单模态输入、鲁棒性差和缺乏临床透明度等局限性，需要开发更准确、鲁棒且可解释的多模态分析框架。

Method: 使用双YOLOv11编码器分别提取RGB和深度模态特征，通过多尺度局部全局提取模块和跨空间颈部融合机制增强时空表示，最后利用冻结大语言模型将视觉嵌入和结构化元数据转换为临床文本解释。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶轨迹规划研究存在两个方向：轨迹生成导向的方法侧重于生成高质量轨迹但决策机制简单；轨迹选择导向的方法能进行多维度评估但生成能力不足。需要一种能够整合高质量轨迹生成与综合决策推理的框架。

Method: 提出MindDrive框架，包含两个核心组件：1) 基于世界行动模型(WaM)的未来感知轨迹生成器(FaTG)，通过自我条件"假设"模拟预测潜在未来场景并生成有远见的轨迹候选；2) 基于视觉语言模型的评估器(VLoE)，利用大语言模型的推理能力在安全、舒适和效率维度进行多目标评估。
...
TL;DR: StreamEQA是首个面向具身智能场景的流式视频问答基准，评估MLLM在具身和流式两个维度上的能力，包含42个任务和约21K个带时间戳的问答对，评估显示现有模型在具身流式视频理解方面仍有困难。


<details>
  <summary>Details</summary>
Motivation: 随着具身智能向真实世界部署发展，需要模型能够持续感知和推理流式视觉输入，但现有基准缺乏专门针对具身场景中流式视频问答的评估标准。

Method: 构建StreamEQA基准，包含具身维度（感知、交互、规划三个层次）和流式维度（后向、实时、前向推理三种模式），基于156个独立长视频，通过自动生成和人工精炼的混合流程创建约21K个带时间戳的问答对。

Result: 评估13个最先进的视频-LLM发现，尽管在传统基准上表现良好，但这些模型在具身场景的流式视频理解方面仍然存在困难。

Conclusion: StreamEQA基准将促进具身应用中的流式视频理解研究，填补了现有评估体系的空白，为模型在真实世界动态环境中的能力评估提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于生成建模的真实噪声合成方法通常需要相机元数据和大量目标特定的噪声-干净图像对，且在不同设置间泛化能力有限。为减少这些前提条件，需要一种更简单有效的噪声合成方法。

Method: 提出GuidNoise方法，仅使用单个噪声/干净图像对作为引导，无需额外元数据。引入引导感知仿射特征修改（GAFM）和噪声感知细化损失，利用扩散模型的内在潜力，通过细化反向过程生成更真实的噪声分布。

Result: GuidNoise能在多样噪声环境下合成高质量噪声图像，无需训练和推理时的额外元数据。在推理时能高效生成噪声-干净图像对，通过自增强显著提升去噪性能，特别是在轻量模型和有限训练数据的实际场景中。

Conclusion: GuidNoise通过单对引导的扩散模型实现了广义噪声合成，减少了数据需求，提高了去噪性能，为实际应用中的训练数据增强提供了有效解决方案。
...
TL;DR: dVLM-AD：基于扩散的视觉语言模型，用于端到端自动驾驶，通过双向注意力机制提升推理与规划的一致性，相比自回归模型在长尾场景中表现更优。
...
Method: 提出dVLM-AD，基于离散扩散的视觉语言模型，利用双向注意力机制和迭代去噪过程，统一感知、结构化推理和低层规划，实现更可控可靠的端到端驾驶。

Result: 在nuScenes和WOD-E2E数据集上评估，dVLM-AD产生更一致的推理-动作对，规划性能与现有驾驶VLM/VLA系统相当，在长尾WOD-E2E场景中相比自回归基线提升9%的行为-轨迹一致性和6%的RFS。

Conclusion: 扩散基VLM为可扩展的端到端自动驾驶提供了更可控可靠的途径，双向注意力机制和迭代去噪过程能更好地保持推理与规划的一致性。
...
Result: UniTS在低层和高层时间序列任务中表现出卓越的生成和认知能力，显著优于现有方法，特别是在严重云污染、模态缺失和预测物候变化等挑战性场景中表现优异。

Conclusion: UniTS为多种时间序列遥感任务提供了一个统一的生成模型框架，通过创新的架构设计和高质量数据集构建，实现了跨任务的时空特征统一建模。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频分词器在同时学习空间和时间表示时存在训练效率低和性能受限的问题，需要一种能够解耦时空表示学习的方法。

Method: DeRA采用1D潜在空间，将视频编码分解为外观和运动两个流，分别与预训练的视觉基础模型对齐以捕获空间语义和时间动态。为解决异质监督带来的梯度冲突，提出了对称对齐-冲突投影（SACP）模块来主动重构梯度。

Result: DeRA在UCF-101数据集上的rFVD指标比之前的SOTA视频分词器LARP提高了25%，在自回归视频生成任务中，在UCF-101类条件生成和K600帧预测上都取得了新的SOTA结果。

Conclusion: DeRA通过解耦时空表示学习并解决梯度冲突问题，显著提升了视频分词器的性能和训练效率，为视频生成任务提供了更有效的表示学习框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在实时角色动画生成中存在两个主要问题：1）无法提供细粒度控制；2）长序列中运动质量会退化。这限制了它们在交互式应用中的使用。

Method: 提出COMET自回归框架，采用高效的Transformer条件VAE实现精确关节控制，引入参考引导反馈机制防止误差累积并确保长期稳定性，该机制还可作为即插即用风格化模块。

Result: COMET能够以实时速度稳健生成高质量运动，在复杂运动控制任务中显著优于现有方法，确认其适用于要求严格的交互应用。

Conclusion: COMET通过创新的参考引导反馈机制解决了实时角色动画中的控制精度和长期稳定性问题，为交互式应用提供了实用的解决方案。
...
TL;DR: UltraImage框架解决了图像扩散变换器在高分辨率生成中的内容重复和质量下降问题，通过频率分析和注意力优化实现了从1328p训练分辨率到6K*6K的极端外推能力。


<details>
  <summary>Details</summary>
Motivation: 当前图像扩散变换器在生成超出训练分辨率的高分辨率图像时，会出现内容重复和质量下降的问题，限制了其实际应用范围。

Method: 1. 通过位置嵌入的频率分析发现重复源于主导频率的周期性；2. 提出递归主导频率校正来约束外推后的频率周期；3. 提出熵引导的自适应注意力集中机制，通过调整关注因子来优化局部细节和全局结构。

Result: 在Qwen-Image和Flux（约4K分辨率）上优于现有方法，减少了内容重复并提高了视觉保真度。能够从1328p训练分辨率生成高达6K*6K的图像，展示了极端外推能力。

Conclusion: UltraImage通过频率校正和注意力优化，有效解决了高分辨率图像生成中的内容重复和质量下降问题，实现了从低分辨率训练到超高分辨率生成的稳定外推。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外基础模型InfMAE虽有效但仍存在局限性：1) 忽略信息丰富的tokens；2) 全局关联建模不足；3) 未考虑红外图像中常见的非均匀噪声。需要开发更强大的红外基础模型来提升红外图像解释任务的性能。

Method: 1) 基于token熵的确定性掩码策略，仅保留高熵tokens进行重建以增强信息性；2) 双域引导(DDG)模块，同时捕获全局token关系并自适应过滤红外图像中常见的非均匀背景噪声；3) 构建Inf-590K大规模红外图像数据集，包含多样化场景、目标类型和空间分辨率。

Result: 在Inf-590K数据集上预训练的DuGI-MAE在红外目标检测、语义分割和小目标检测等多个下游任务中展现出强大的泛化能力。实验结果表明，该方法在监督和自监督对比方法中均表现出优越性。

Conclusion: DuGI-MAE通过创新的掩码策略和双域引导模块，有效解决了红外基础模型的关键局限性，在多个红外图像处理任务中实现了卓越性能，为红外图像分析提供了强大的基础模型解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自回归模型在生成长的第一人称视角视频时存在内容漂移问题，物体身份和场景语义会随时间退化，这阻碍了生成连贯的、包含手-物交互和程序性任务的长视频。

Method: EgoLCD将长视频合成视为高效稳定的内存管理问题，结合了长期稀疏KV缓存用于稳定全局上下文、基于注意力的短期记忆（通过LoRA进行局部适配）、内存调节损失来强制一致的内存使用，以及结构化叙事提示提供显式时间指导。
...
TL;DR: BA-TTA-SAM：一个任务无关的测试时适应框架，通过测试时适应显著提升SAM在医学图像分割中的零样本性能，无需源域训练数据。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临标注数据稀缺和计算成本高的挑战。现有方法（包括全参数和参数高效微调）仍依赖下游任务的特定训练。零样本分割受到关注，但SAM在医学数据集上因领域偏移存在明显限制，需要高效的零样本增强方法。
...
Conclusion: BA-TTA-SAM框架通过测试时适应有效提升了SAM在医学图像分割中的零样本性能，无需任何源域训练数据，在公开医学数据集上表现出优越性。
...
TL;DR: Auto3R是一个数据驱动的不确定性量化模型，用于自动化3D扫描和重建过程，能够处理非朗伯体和镜面材质物体，通过预测扫描视点的不确定性分布来指导机器人系统进行高效扫描。


<details>
  <summary>Details</summary>
Motivation: 传统高质量的3D扫描和重建通常依赖人工规划扫描过程，随着无人机和机器人等具身系统的快速发展，需要实现完全自动化的精确3D扫描和重建，特别是针对非朗伯体和镜面材质物体。

Method: Auto3R采用数据驱动的不确定性量化模型，在迭代的3D重建和扫描过程中，无需知道真实几何和外观，就能高效准确地预测潜在扫描视点的不确定性分布。

Result: 通过大量实验，Auto3R实现了卓越性能，大幅超越现有最先进方法。在配备相机的机械臂上部署后，能够有效数字化真实世界的3D物体，生成可直接使用的逼真数字资产。

Conclusion: Auto3R为自动化3D扫描和重建提供了一种有效的解决方案，特别适用于处理复杂材质物体，推动了具身系统在3D数字化领域的应用。
...
Method: 提出PhyVLLM框架：1) 通过双分支编码器解耦视觉外观和物体运动；2) 使用神经ODE模块建模随时间变化的物理动力学，生成可微分的物理动态表示；3) 将运动感知表示投影到预训练LLM的token空间；4) 采用自监督方式建模物体运动的连续演化，避免需要显式物理标注。

Result: 实验结果表明，PhyVLLM在物理推理和通用视频理解任务上显著优于最先进的视频大语言模型，证明了显式物理建模的优势。

Conclusion: 通过显式地融入物理运动建模，PhyVLLM能够更好地理解视频中的物理动力学，提升视频理解和推理能力，同时保持模型原有的多模态能力。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像视频编辑方面取得显著进展，但物体重纹理任务尚未充分探索。现有方法如ControlNet存在两个主要问题：参考图像的原始条件会引入不需要的结构信息，以及无法有效分离源对象的视觉纹理和结构信息。

Method: 提出Refaçade方法，包含两个关键设计：1) 使用在成对纹理/无纹理3D网格渲染上训练的纹理移除器，去除外观信息同时保留源视频的几何和运动；2) 通过拼图排列破坏参考图像的全局布局，使模型专注于局部纹理统计而非对象的全局布局。

Result: 大量实验表明，该方法在视觉质量、精确编辑和可控性方面表现优异，在定量评估和人工评估中都优于强基线方法。

Conclusion: Refaçade方法能够实现精确可控的图像和视频纹理迁移，解决了现有方法在纹理与结构分离方面的局限性，为物体重纹理任务提供了有效的解决方案。
...
Method: 引入X-Humanoid方法，将强大的Wan 2.2模型适配为视频到视频结构，并针对人类到人形机器人的转换任务进行微调。设计了可扩展的数据创建流程，使用Unreal Engine将社区资产转化为17+小时的配对合成视频。

Result: 将训练好的模型应用于60小时的Ego-Exo4D视频，生成并发布了包含超过360万帧"机器人化"人形机器人视频的新大规模数据集。定量分析和用户研究证实了方法的优越性：69%用户认为其运动一致性最佳，62.1%认为具身正确性最佳。
...
TL;DR: VideoMem：通过自适应内存管理将超长视频理解建模为序列生成任务，使用PRPO算法训练，显著超越现有开源模型


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在超长视频理解上存在局限，受限于上下文长度和长期记忆保留效率。虽然已有工作尝试构建外部知识库和RAG系统，但存储和计算开销巨大。

Method: 提出VideoMem框架，将超长视频理解建模为序列生成任务，通过动态更新全局内存缓冲区自适应保留关键信息、丢弃冗余内容。采用PRPO算法训练，包含PSP模块自适应保留有效状态并传播到下一步，以及TCR模块缓解奖励稀疏性。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在新视角合成中表现出色，但表面重建质量有待提升。作者观察到良好重建的表面具有低构型熵特性，即主导基元清晰定义几何而冗余分量被抑制。
...
Conclusion: GEF框架能够在保持光度保真度的同时显著提升表面重建精度，验证了熵最小化方法在优化3D高斯泼溅几何质量方面的有效性。
...
TL;DR: 本文提出了一种针对文档视觉问答系统的新型对抗攻击方法，能够在视觉上难以察觉的情况下伪造文档内容，诱导模型产生特定或错误的答案。
...
Method: 开发了专门的攻击算法，能够生成对抗性伪造文档，针对不同攻击者目标（从针对性错误信息到系统性模型故障场景）。攻击方法旨在视觉上难以察觉但语义上有针对性。

Result: 成功攻击了两个最先进的端到端模型：Pix2Struct（联合处理图像和文本的视觉语言变换器）和Donut（直接从文档图像提取文本并回答问题的变换器模型）。

Conclusion: 研究结果突显了当前DocVQA系统的关键脆弱性，呼吁开发更强大的防御机制来应对此类对抗攻击。
...
Conclusion: 统一的MLLM可以通过学习生成辅助模态来内在增强空间感知，并通过自适应交错推理实现更强的空间智能，为多模态空间推理提供了有效的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在显微图像分析中展现出巨大潜力，但高质量大规模数据集的创建过程复杂且资源密集，受到时间限制、领域变异性和标注偏见等挑战的阻碍。需要系统指导来促进高质量数据集的开发。

Method: 提供数据集创建的关键步骤指南：1) 图像采集，2) 标注软件选择，3) 标注创建。强调应对领域偏移（如切片制备和数字化相关变异），提出标注质量的三个"C"标准（正确性、完整性和一致性），探索使用先进技术提高标注质量的方法，并提供标准操作程序作为补充材料。

Result: 通过系统指导数据集创建过程，强调开放数据集的重要性，提供实用建议和标准操作程序，旨在促进高质量大规模数据集的开发，推动病理学深度学习研究的创新和可重复性。

Conclusion: 这篇综述通过提供全面的数据集创建指南和实用建议，旨在推进高质量大规模数据集的创建和可用性，最终促进病理学应用中可泛化和稳健的深度学习模型的发展。
...
TL;DR: TARDis提出了一种物理感知的深度学习框架，将缺失的多期相CT扫描重新定义为连续时间-衰减曲线上的缺失采样点，通过解耦静态解剖和动态灌注特征来生成缺失的血流动力学信息。


<details>
  <summary>Details</summary>
Motivation: 临床CT扫描中，由于辐射剂量限制或扫描条件限制，常常无法获得完整的多期相序列，导致"模态缺失"问题。现有深度学习方法通常将缺失期相视为独立的缺失通道，忽略了血流动力学的固有时间连续性。

Method: 提出Time Attenuated Representation Disentanglement (TARDis)框架，将潜在特征空间解耦为时间不变的静态组件（解剖结构）和时间相关的动态组件（灌注）。采用双路径架构：基于量化的路径使用可学习的嵌入字典提取一致的解剖结构；概率路径使用条件变分自编码器建模基于估计扫描时间的动态增强特征。

Result: 在大型私有腹部CT数据集（2,282例）和两个公共数据集上的实验表明，TARDis显著优于现有最先进的不完整模态框架。即使在极端数据稀疏场景下，该方法仍能保持稳健的诊断性能。

Conclusion: TARDis通过物理感知的特征解耦方法，有效解决了多期相CT扫描中的模态缺失问题，在减少辐射暴露的同时保持了诊断精度，具有重要的临床应用潜力。
...
TL;DR: 提出了一种零样本恶意图像检测管道，能同时检测有害内容、识别关键元素并精确定位，通过多分割器集成提高对抗攻击鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有NSFW标记只能识别图像级别有害性，无法告诉审核人员具体哪些对象构成违法内容及其位置，需要更细粒度的恶意图像审核工具。

Method: 使用基础分割模型(SAM)生成候选对象掩码并细化为独立区域，通过视觉语言模型用开放词汇提示对每个区域进行恶意相关性评分，加权融合生成恶意对象地图，多分割器集成增强对抗攻击鲁棒性。
...
Method: 1) 分析VDiT内部表示，发现注意力头作为最小功能单元具有匹配、语义理解和位置编码等不同专门化功能；2) 发现VDiT特征中低频成分对建立对应关系至关重要，高频成分则引入噪声；3) 提出头部和频率感知的特征选择策略，联合选择最具信息量的注意力头和低频成分；4) 通过单步去噪提取判别性特征，应用特征选择，使用软argmax定位和前后一致性检查进行对应关系估计。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在处理视频时难以有效感知和利用丰富的时间信息，导致时间不一致或因果不合理的描述，产生严重幻觉问题。现有研究主要关注空间幻觉，而时间推理相对未被充分探索。

Method: 提出自诊断对比解码(SEASON)，这是一种无需训练的方法，通过动态诊断每个输出token的幻觉倾向，并针对其对应的时间和空间负样本应用自适应对比解码，从而增强每个输出token的时间和空间忠实性。
...
Method: 提出I2I-Bench基准测试，包含：(1) 10个任务类别，涵盖单图像和多图像编辑任务；(2) 30个解耦的细粒度评估维度；(3) 自动化混合评估方法，结合专业工具和大型多模态模型(LMMs)；(4) 严格的alignment验证，确保评估结果与人类偏好一致。

Result: 使用I2I-Bench对多个主流图像编辑模型进行基准测试，揭示了不同模型在各种维度上的差距和权衡。所有组件将开源以促进未来研究。

Conclusion: I2I-Bench提供了一个全面、可扩展的图像编辑模型评估框架，解决了现有基准测试的局限性，为未来图像编辑研究提供了标准化的评估工具。
...
TL;DR: Live Avatar：首个实现实时高保真虚拟人生成的算法-系统协同设计框架，通过时间步强制流水线并行打破自回归瓶颈，支持无限长度生成，在5个H800 GPU上达到20FPS
...
Result: 在5个H800 GPU上实现端到端20FPS生成，首次在140亿参数扩散模型规模上实现实用、实时、高保真的虚拟人生成，达到最先进性能
...
TL;DR: Reward Forcing框架通过EMA-Sink和Re-DMD两个创新设计，解决了现有流式视频生成方法中初始帧过度依赖和运动动态不足的问题，实现了高质量、高效率的流式视频生成。
...
Method: 提出Reward Forcing框架，包含两个关键设计：1) EMA-Sink：维护固定大小的token，从初始帧初始化，并通过指数移动平均融合被淘汰的token来持续更新，既捕获长期上下文又保持近期动态；2) Re-DMD（奖励分布匹配蒸馏）：通过视觉语言模型评估动态性，优先处理高奖励样本，使模型输出分布偏向高动态区域，提升运动质量同时保持数据保真度。

Result: Reward Forcing在标准基准测试中达到最先进性能，在单个H100 GPU上实现23.1 FPS的高质量流式视频生成，有效解决了初始帧复制问题并显著提升了运动动态质量。

Conclusion: Reward Forcing框架通过创新的EMA-Sink和Re-DMD设计，成功解决了流式视频生成中的关键挑战，实现了高质量、高效率的视频生成，为交互式和动态世界的模拟提供了有效解决方案。
...
TL;DR: 提出跨视角点对应任务和基准测试，构建大规模数据集，开发CroPond模型在跨视角点对应任务上显著超越现有模型


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在实现精确点级对应方面存在不足，这对于空间理解和具身AI中的精确交互至关重要

Method: 提出跨视角点对应任务，构建CrossPoint-Bench基准测试，创建CrossPoint-378K数据集，开发CroPond模型

Result: CroPond在CrossPoint-Bench上超越Gemini-2.5-Pro 39.7%准确率，但仍与人类存在54.65%差距

Conclusion: 跨视角点对应是重要挑战，提出的基准、数据集和模型为未来研究提供了基础
...
<details>
  <summary>Details</summary>
Motivation: 传统超分辨率方法只能处理固定尺度，而现有任意尺度超分辨率方法基于隐式神经表示，其回归驱动的特征提取和聚合限制了细节合成能力，导致真实感不足。虽然基于扩散模型的真实图像超分辨率在4x尺度上表现出色，但缺乏显式尺度控制，在超高尺度下会产生过度幻觉或模糊输出。

Method: 提出OmniScaleSR框架，包含：1）显式的扩散原生尺度控制机制，与隐式尺度适应协同工作，实现尺度感知和内容感知的扩散过程调制；2）多域保真度增强设计，进一步提高重建精度。

Result: 在双三次降质基准测试和真实世界数据集上的广泛实验表明，OmniScaleSR在保真度和感知真实感方面均优于现有最先进方法，在大放大倍数下表现尤为突出。

Conclusion: OmniScaleSR通过结合显式尺度控制和隐式尺度适应的扩散模型，成功实现了高质量、高真实感的任意尺度超分辨率，特别是在超高尺度下表现出色。
...
TL;DR: E3AD是一个情感感知的端到端自动驾驶框架，通过融合语言情感分析和双通路空间推理，实现了更人性化的驾驶决策。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统主要关注视觉-语言-动作模型，但忽视了乘客的情感状态，而情感状态对驾驶舒适度和自动驾驶接受度至关重要。

Method: 提出E3AD框架，包含两个认知启发组件：1) 连续VAD情感模型从语言中捕捉语气和紧迫性；2) 双通路空间推理模块融合自我中心和他者中心视角。采用一致性导向的训练方案，结合模态预训练和基于偏好的对齐。

Result: 在真实世界数据集上，E3AD提升了视觉基础和路径点规划能力，在情感估计方面达到了最先进的VAD相关性。

Conclusion: 将情感注入VLA风格驾驶能够产生更符合人类期望的基础、规划和以人为中心的反馈，展示了情感感知对自动驾驶系统的重要性。
...
TL;DR: 提出了一种实例感知的深度补全框架，通过整合二值实例掩码作为空间先验来优化深度预测，在Virtual KITTI 2数据集上取得了比U-Net基线和语义引导方法更低的RMSE。


<details>
  <summary>Details</summary>
Motivation: 现有深度补全方法通常依赖语义分割来指导，但忽视了对象级理解的优势。作者认为显式整合实例级信息可以更好地处理物体边界、遮挡和薄结构等挑战性区域。

Method: 提出四组件框架：1) 冻结的YOLO V11实例分割分支生成前景掩码；2) U-Net深度补全主干；3) 交叉注意力融合模块；4) 注意力引导预测头。通过交叉注意力让深度分支聚焦于物体中心区域进行细化。

Result: 在Virtual KITTI 2数据集上验证，相比U-Net基线和先前语义引导方法，取得了更低的RMSE，同时保持竞争力的MAE。定性和定量结果显示模型有效提升了物体边界、遮挡和薄结构附近的深度精度。

Conclusion: 整合实例感知线索为改进深度补全提供了有前景的方向，无需依赖密集语义标签。该方法在保持计算效率的同时，通过对象级理解增强了深度预测的准确性。
...
TL;DR: VRSketch2Shape：首个从顺序VR草图生成3D形状的框架，通过时序感知编码和扩散模型提升几何保真度，并发布了大规模多类别数据集。


<details>
  <summary>Details</summary>
Motivation: 现有草图到形状模型忽略了笔划的时间顺序，丢弃了关于结构和设计意图的关键线索。VR草图提供了更直观的3D设计方式，但需要更好地捕捉设计过程的时间信息。

Method: 提出VRSketch2Shape框架：1）从任意形状自动生成顺序VR草图的流水线；2）包含2万合成和900手绘草图-形状对的多类别数据集；3）时序感知草图编码器结合基于扩散的3D生成器。

Result: 相比先前工作具有更高的几何保真度，从合成草图到真实草图的泛化能力强且只需最小监督，即使在部分草图上也表现良好。

Conclusion: VRSketch2Shape是首个处理顺序VR草图的框架，通过利用时间顺序信息显著提升了3D形状生成质量，所有数据和模型将开源发布。
...
<details>
  <summary>Details</summary>
Motivation: 传统监督学习方法在保持图像一致性（如身份、风格、逻辑连贯性）方面面临挑战，主要因为缺乏大规模一致性数据集以及难以建模人类感知偏好。强化学习提供了一种数据自由的替代方案。

Method: 1. PaCo-Reward：基于自动化子图配对构建的大规模数据集训练的一致性评估器，采用生成式自回归评分机制，结合任务感知指令和思维链推理。2. PaCo-GRPO：采用分辨率解耦优化策略降低RL成本，配合对数平滑的多奖励聚合机制确保平衡稳定的奖励优化。

Result: PaCo-Reward显著提升了与人类视觉一致性感知的对齐度；PaCo-GRPO在一致性性能上达到最先进水平，同时提高了训练效率和稳定性。

Conclusion: PaCo-RL为一致图像生成提供了一个实用且可扩展的解决方案，展示了强化学习在复杂视觉一致性任务中的潜力。
...
TL;DR: LaFiTe是一个3D原生纹理生成框架，通过生成稀疏潜在颜色场实现高质量、无缝的3D表面纹理，在重建质量上超过现有方法10dB以上。


<details>
  <summary>Details</summary>
Motivation: 现有的3D原生纹理生成方法缺乏强大且通用的潜在表示，这严重限制了生成纹理的保真度和泛化能力。UV映射和多视图投影方法存在长期局限性，需要一种直接在3D表面上生成高质量无缝纹理的新方法。

Method: LaFiTe采用变分自编码器将复杂表面外观编码为稀疏结构化潜在空间，然后解码为连续颜色场。在此基础上，使用条件整流流模型合成高质量、连贯的纹理。

Result: LaFiTe在重建质量上超过现有最先进方法超过10dB PSNR，实现了前所未有的保真度，有效解耦了纹理外观与网格拓扑和UV参数化。能够生成多样风格和几何形状的高质量连贯纹理。
...
Method: 1. 延迟高斯增长策略：优先优化静态场景结构，再允许高斯分裂/克隆，减少早期优化中对瞬态物体的过拟合；2. 尺度级联掩码引导：先利用低分辨率特征相似性监督获得可靠的初始瞬态掩码估计，再逐步提升到高分辨率监督实现更精确的掩码预测；3. 将延迟增长策略和掩码引导与外观建模结合处理野外场景。

Result: 在多个具有挑战性的数据集上进行广泛实验，结果显示该方法优于现有方法，清晰证明了其鲁棒性和有效性。

Conclusion: RobustSplat++ 通过创新的延迟高斯增长和尺度级联掩码引导技术，有效解决了3D高斯溅射在野外场景中处理瞬态物体和光照变化的难题，显著提升了渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 流匹配（FM）在生成模型中取得了显著进展，展示了强大的生成能力，并作为能够学习精确数据密度的无模拟流基框架受到广泛关注。受这些进展启发，研究者希望将流匹配应用于医学图像分割领域。

Method: 首先设计两个变分自编码器（VAEs）将医学图像及其对应掩码编码到低维潜在空间。然后估计基于输入图像的条件速度场来引导流。通过采样多个潜在表示，该方法合成多样化的分割输出，其像素级方差可靠地捕捉底层数据分布。
...
TL;DR: FreeGen：用于自动驾驶自由视点场景合成的重建-生成协同训练框架，通过几何表示确保插值一致性，几何感知增强提升未见视角的真实感


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集和生成管道很少提供一致的偏离轨迹观测，限制了大规模评估和训练。虽然最近的生成模型展现了强大的视觉真实感，但难以同时实现插值一致性和外推真实感，且通常需要逐场景优化。

Method: 提出FreeGen框架，包含重建模型和生成模型协同训练。重建模型提供稳定的几何表示确保插值一致性，生成模型进行几何感知增强以提升未见视角的真实感。通过协同训练，生成先验被蒸馏到重建模型中改善偏离轨迹渲染，而精炼的几何又为生成提供更强的结构指导。

Result: 实验表明FreeGen在自由视点驾驶场景合成方面达到了最先进的性能。

Conclusion: FreeGen通过重建-生成协同训练框架，有效解决了自由视点驾驶场景合成中的插值一致性和外推真实感问题，为自动驾驶闭环仿真和大规模预训练提供了高质量的合成场景。
...
TL;DR: 本文提出了一种新的多域人脸伪造检测研究范式MID-FFD，并开发了DevDet框架来解决多域训练中域差异主导特征空间的问题，从而提升单图像真伪判断的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法追求通用性，但在训练数据有限的情况下难以覆盖真实世界中多样化的伪造变体。虽然引入大规模多域数据训练可行，但多域场景中域间差异会主导特征空间，导致检测器在域未指定条件下对单图像真伪判断准确率低。

Method: 首先定义了MID-FFD研究范式，包含足够多的真实-伪造域用于训练。然后提出了DevDet框架，包含Face Forgery Developer（FFDev）和Dose-Adaptive detector Fine-Tuning策略（DAFT），旨在放大真伪差异使其主导特征空间。

Result: 实验表明该方法在MID-FFD场景下能有效预测真伪，同时保持了原始模型对未见数据的泛化能力。

Conclusion: 提出的MID-FFD研究范式和DevDet框架解决了多域训练中域差异主导的问题，提升了深度伪造检测在真实世界应用中的单图像判断准确性。
...
TL;DR: LineAR是一种无需训练、基于渐进式KV缓存压缩的自回归图像生成方法，通过利用视觉注意力的内在特性，以行级管理缓存，显著减少内存占用并提高生成速度，同时保持甚至提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有自回归图像生成方法存在严重的内存瓶颈，需要在解码过程中缓存所有先前生成的视觉token，导致高存储需求和低吞吐量。为了解决这一问题，研究者提出了LineAR方法。

Method: LineAR采用训练免费的渐进式键值缓存压缩流程，通过2D视图在行级管理缓存，保留视觉依赖区域，同时逐步淘汰对后续行生成无害的信息较少token，通过行间注意力进行指导。

Result: 在六个自回归图像生成模型上的实验表明，LineAR在保持仅1/6 KV缓存的情况下，将ImageNet FID从2.77提升到2.68，COCO FID从23.85提升到22.86。同时实现了显著的内存和吞吐量提升，最高可达67.61%内存减少和7.57倍速度提升。

Conclusion: LineAR通过创新的缓存压缩方法，有效解决了自回归图像生成的内存瓶颈问题，在减少内存占用和提高生成速度的同时，保持甚至提升了生成质量，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 野外3D人体姿态捕捉对姿态估计和运动生成方法训练很有价值。现有视频估计方法在自接触场景（如手触脸）中经常失败，而生物阻抗传感能廉价、无干扰地测量真实皮肤接触

Method: BioTUCH方法：1) 使用现成姿态估计器初始化姿态；2) 在测量的自接触期间进行接触感知的姿态优化：最小化重投影误差和与输入估计的偏差，同时强制执行顶点接近约束

Result: 使用同步RGB视频、生物阻抗测量和3D动作捕捉的新数据集验证方法。测试三种输入姿态估计器，重建精度平均提升11.7%。还展示了微型可穿戴生物阻抗传感器

Conclusion: BioTUCH通过结合视觉姿态估计与生物阻抗传感，显著提升了自接触场景下的3D人体姿态捕捉精度，并为大规模接触感知训练数据收集提供了可行方案
...
TL;DR: SP-Det是一个自提示检测框架，通过自动生成丰富的文本上下文来指导胸部X光片中的多标签病变检测，无需专家标注。


<details>
  <summary>Details</summary>
Motivation: 现有的可提示检测框架虽然实现了高精度的目标定位，但通常依赖手动标注作为提示，这在临床应用中既费时又不切实际。需要一种能够自动生成提示的方法来消除对专家标注的依赖。

Method: 提出SP-Det框架，包含两个核心组件：1) 无需专家的双文本提示生成器(DTPG)，利用两种互补的文本模态：捕获全局病理模式的语义上下文提示和专注于疾病特定表现的特征信标提示；2) 双向特征增强器(BFE)，协同整合全面的诊断上下文与疾病特定嵌入，显著改善特征表示和检测精度。

Result: 在两个包含多种胸部疾病类别的胸部X光数据集上进行广泛实验，结果表明SP-Det框架在性能上优于最先进的检测方法，同时相比现有的可提示架构完全消除了对专家标注提示的依赖。

Conclusion: SP-Det通过自动生成丰富的文本上下文来指导多标签病变检测，为临床应用中自动化病变检测提供了更实用和高效的解决方案，消除了对专家标注的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 解决胎儿头部MRI扫描中头部运动的挑战性问题，实现自动自适应2D诊断MRI切片处方，支持6自由度头部姿态估计。现有方法由于解剖对称性引起的姿态模糊性以及低分辨率、噪声和伪影等问题，难以泛化到临床数据。

Method: E(3)-Pose方法通过构造捕获解剖对称性和刚性姿态等变性，联合且显式地建模旋转等变性和物体对称性。该方法利用快速获取的3D MRI体积来支持每个2D切片的姿态估计。

Result: 在公开可用的代表性临床胎儿MRI数据集上的实验表明，该方法在跨域情况下具有优越的鲁棒性和泛化能力。E(3)-Pose在临床MRI体积上实现了最先进的准确性，为临床转化铺平了道路。

Conclusion: E(3)-Pose通过显式建模对称性和等变性，能够稳健地估计胎儿头部姿态，在临床MRI数据上表现出色，有望推动临床应用的转化。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Flow Matching方法取得显著进展，但仍存在训练与推理阶段不一致导致的曝光偏差问题。本文旨在探究曝光偏差的根本原因，并提出有效的解决方案。

Method: 提出ReflexFlow方法，包含两个组件：1) 反漂移校正(ADR)，在训练时采样调度下通过重新设计损失函数来调整对偏差输入的预测目标；2) 频率补偿(FC)，通过重新加权损失来补偿早期去噪过程中缺失的低频内容。

Result: 在CIFAR-10、CelebA-64和ImageNet-256数据集上的实验表明，ReflexFlow在缓解曝光偏差方面优于先前方法，在CelebA-64上实现了35.65%的FID降低。

Conclusion: ReflexFlow是一种简单有效的Flow Matching学习目标改进方法，能够动态修正曝光偏差，模型无关且兼容所有Flow Matching框架，显著提升生成质量。
...
TL;DR: SFD是一种新的潜在扩散模型范式，通过异步去噪语义和纹理潜在表示，让语义形成优先于纹理生成，实现了更自然的粗到细生成过程。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型虽然遵循粗到细的生成过程，但语义和纹理同步去噪，忽略了语义形成略早于纹理的自然顺序。这限制了语义先验对纹理生成的指导作用。

Method: 提出语义优先扩散（SFD）范式：1）通过专门的语义VAE从预训练视觉编码器提取紧凑语义潜在表示；2）构建包含语义和纹理的复合潜在表示；3）使用分离的噪声调度异步去噪语义和纹理，语义比纹理提前一个时间偏移。

Result: 在ImageNet 256x256上，SFD达到FID 1.06（LightningDiT-XL）和FID 1.04（1.0B LightningDiT-XXL），比原始DiT收敛速度快100倍。同时能改进ReDi和VA-VAE等现有方法。

Conclusion: SFD通过显式优先处理语义形成，提供更清晰的高层语义指导，实现了更自然的粗到细生成，证明了异步、语义主导建模的有效性。
...
TL;DR: LiteVGGT通过几何感知缓存token合并策略，在保持VGGT核心性能的同时，实现10倍加速和显著内存减少，能够高效处理1000张图像的大规模场景。
...
Method: 提出几何感知缓存token合并策略：1) 分析token的几何重要性，优化锚点token选择以更好保留重建关键信息；2) 缓存并跨层重用合并索引，大幅减少延迟。支持高效微调和FP8量化。

Result: LiteVGGT实现高达10倍加速和显著内存减少，能够高效处理1000张图像的大规模场景，同时保持VGGT的核心性能。

Conclusion: LiteVGGT通过创新的几何感知缓存token合并策略，解决了3D视觉基础模型在大规模场景中的计算效率问题，具有良好的可扩展性和鲁棒性。
...
### [75] [GeoPE:A Unified Geometric Positional Embedding for Structured Tensors](https://arxiv.org/abs/2512.04963)
*Yupu Yao,Bowen Yang*

Main category: cs.CV

TL;DR: GeoPE是一种新的位置编码方法，通过四元数将旋转扩展到3D欧几里得空间，解决了传统ViT中2D空间拓扑被破坏的问题，能够更好地捕捉几何结构。


<details>
  <summary>Details</summary>
Motivation: 传统Vision Transformer将2D图像展平为1D序列，破坏了自然空间拓扑结构。现有的2D位置编码方法（如RoPE）通常独立处理空间轴，无法区分虚假的序列邻近性和真实的空间距离。

Method: 提出几何位置嵌入（GeoPE）框架，使用四元数将旋转扩展到3D欧几里得空间。通过计算李代数中的几何平均来构造统一的旋转算子，克服非交换性问题并确保对称性，创建几何耦合的编码来有效分离空间维度。

Result: 在图像分类、目标检测和3D语义分割等任务上的大量实验表明，GeoPE始终优于现有的2D RoPE变体，并显著增强了形状偏差，证实了其捕捉真实几何结构的能力。

Conclusion: GeoPE通过恢复2D空间流形，有效解决了传统位置编码中的几何结构破坏问题，为视觉Transformer提供了更准确的空间位置表示。
...
TL;DR: 提出针对视网膜疾病诊断的平衡少样本学习框架，通过平衡采样、针对性增强和ResNet-50编码器，在RFMiD数据集上显著提升少数类疾病诊断准确率。


<details>
  <summary>Details</summary>
Motivation: 视网膜疾病自动诊断需求增长，但传统深度学习方法需要大量标注数据且存在类别不平衡问题。少样本学习可以解决数据稀缺问题，但现有方法在视网膜疾病诊断中仍面临类别不平衡挑战。
...
Result: 在100个情景训练和1,000个测试情景评估中，框架显著提升准确率并减少对多数类的偏向，在代表性不足的疾病上表现尤为突出。
...
Conclusion: 该方法具有可扩展性和通用性，成功应用于其他预训练ViT模型如DINOv2，为AI生成图像检测提供了更有效的特征整合方案
...
TL;DR: 提出一种稳定的对比损失函数家族，用于学习同时捕获语义和几何信息的像素级表示，通过过完备描述符实现跨图像的精确点对应，无需基于动量的师生训练。


<details>
  <summary>Details</summary>
Motivation: 当前像素级表示学习方法通常需要复杂的师生训练架构，难以同时捕获语义和几何信息。需要开发一种更简单有效的方法来学习既具有视图不变性又语义有意义的像素表示。

Method: 提出稳定的对比损失函数家族，将图像中的每个像素映射到过完备描述符。该方法学习既具有视图不变性又语义有意义的表示，无需基于动量的师生训练架构。

Result: 在合成2D和3D环境中的两个实验验证了损失函数的特性，表明该方法能够学习到有效的过完备表示，实现跨图像的精确点对应。

Conclusion: 提出的稳定对比损失函数能够有效学习同时捕获语义和几何信息的像素级表示，为计算机视觉任务提供了一种无需复杂师生训练的新方法。
...
TL;DR: 研究发现基于大型视觉语言模型（LVLM）的文生图系统比非LVLM模型产生更多社会偏见，系统提示是偏见主要来源，提出了训练免费的元提示框架FairPro来减少偏见。


<details>
  <summary>Details</summary>
Motivation: 虽然基于大型视觉语言模型的文生图系统已成为图像生成的主流范式，但这些系统是否放大社会偏见尚未得到充分理解。研究者希望系统评估LVLM模型中的偏见问题，并探索其根源。

Method: 1. 创建包含1024个提示的基准测试，涵盖四个语言复杂度层次；2. 通过解码中间表示、标记概率诊断和嵌入关联分析揭示系统提示如何编码人口统计先验；3. 提出FairPro框架，让LVLM在测试时自我审计并构建公平感知的系统提示。

Result: 1. LVLM模型比非LVLM模型产生明显更多的社会偏见图像；2. 系统提示是偏见行为的主要驱动因素；3. FairPro在SANA和Qwen-Image两个LVLM模型上显著减少人口统计偏见，同时保持文本-图像对齐。

Conclusion: 系统提示在偏见传播中起核心作用，FairPro提供了一种实用、可部署的方法来构建更负责任的文生图系统，为理解LVLM中的偏见机制提供了深入见解。
...
TL;DR: 本文提出了一种针对VANICP点云配准算法的内存高效优化策略，使其能够在资源受限的嵌入式GPU上轻量级运行，内存消耗减少超过97%的同时保持原有性能。
...
Result: 实现了超过97%的内存消耗减少，同时保持了原有的性能水平，使算法能够在资源受限的嵌入式GPU上高效运行。

Conclusion: 通过提出的内存优化策略，成功解决了VANICP算法在嵌入式部署中的内存瓶颈问题，为点云配准算法在资源受限环境中的应用提供了有效解决方案。
...
TL;DR: 提出基于扩散变换器（DiT）的单图像反光去除框架，利用预训练基础扩散模型，通过物理渲染合成数据和LoRA高效适配，在多个基准测试中达到最佳性能


<details>
  <summary>Details</summary>
Motivation: 现有反光去除方法依赖特定任务架构，缺乏泛化能力。基础扩散模型在图像修复领域表现出强大泛化能力，但缺乏适用于反光去除的合适数据和适配方法

Method: 1. 构建基于扩散变换器（DiT）的框架，将预训练基础模型重新用于反光去除任务；2. 在Blender中开发基于物理的渲染（PBR）流水线，使用Principled BSDF合成逼真的玻璃材质和反光效果；3. 采用LoRA（低秩适配）高效调整基础模型；4. 通过条件输入引导模型从反光污染图像生成干净的透射层

Result: 在领域内和零样本基准测试中均达到最先进的性能，证明了预训练扩散变换器与物理基础数据合成和高效适配相结合的有效性

Conclusion: 预训练的扩散变换器配合物理基础数据合成和高效适配，为反光去除任务提供了可扩展、高保真的解决方案，展示了基础模型在图像修复任务中的强大潜力
...
Conclusion: 提出的自监督方法有效解决了透明物体深度感知中的标注数据需求问题，在减少标注成本的同时保持了性能。
...
TL;DR: GNVC-VD是首个基于DiT的生成式神经视频压缩框架，通过统一的流匹配潜在细化模块和视频扩散变换器，解决了现有感知编解码器缺乏时间建模导致闪烁的问题。


<details>
  <summary>Details</summary>
Motivation: 现有感知编解码器主要依赖预训练的图像生成先验来恢复高频细节，但其逐帧特性缺乏时间建模，不可避免地导致感知闪烁。需要将视频原生生成先验整合到神经编解码器中，实现时空一致的压缩。

Method: 1) 构建基于高级视频生成基础模型的DiT框架；2) 引入统一的流匹配潜在细化模块，利用视频扩散变换器通过序列级去噪联合增强帧内和帧间潜在表示；3) 从解码的时空潜在表示初始化细化过程，学习适应压缩诱导退化的校正项；4) 使用条件适配器向中间DiT层注入压缩感知线索。

Result: GNVC-VD在感知质量上超越了传统和学习的编解码器，显著减少了先前生成方法中持续的闪烁伪影，即使在低于0.01 bpp的极端比特率约束下也能保持时间一致性。

Conclusion: 将视频原生生成先验整合到神经编解码器中，为下一代感知视频压缩提供了有前景的方向，GNVC-VD展示了在保持时空一致性的同时实现高质量压缩的能力。
...
TL;DR: RAMEN是一个分辨率可调的多模态编码器，能够以传感器无关的方式学习地球观测数据的共享视觉表示，将空间分辨率作为可控输出参数，在统一潜在空间中实现跨模态的连贯分析。
...
Method: RAMEN将模态、空间和时间分辨率作为关键输入特征，将空间分辨率定义为可控输出参数，使用单一统一的Transformer编码器重建来自多样来源的掩码多模态地球观测数据。

Result: RAMEN在社区标准PANGAEA基准测试中表现优异，能够有效迁移到已知和未见过的传感器配置，并优于更大的最先进模型。

Conclusion: RAMEN提供了一种传感器无关的地球观测数据表示学习方法，通过将空间分辨率作为可控参数，实现了空间精度与计算成本之间的明确权衡，为跨模态分析提供了统一框架。
...
TL;DR: 提出了一种语义引导的分层合成架构，用于面部图像修复，通过语义布局生成和多模态纹理细化两阶段方法，有效处理大面积不规则掩码，在CelebA-HQ和FFHQ数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有面部图像修复方法在处理大面积不规则掩码时存在问题，包括边缘模糊纹理、语义不一致和面部结构不自然。这主要是由于直接像素级合成方法和对面部先验知识的有限利用造成的。

Method: 提出语义引导的分层合成架构：第一阶段结合CNN局部特征和Vision Transformer全局特征生成清晰语义布局；第二阶段使用多模态纹理生成器从不同尺度提取信息细化纹理。该架构通过动态注意力处理任意掩码配置，无需掩码特定训练。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，该方法在LPIPS、PSNR和SSIM等指标上优于现有最先进方法，特别是在具有挑战性的大面积修复情况下，能产生视觉上引人注目的结果，并具有更好的语义保持能力。

Conclusion: 提出的语义引导分层合成架构有效解决了面部图像修复中的关键挑战，通过语义布局生成和纹理细化的两阶段方法，实现了更好的结构一致性和视觉质量，特别是在处理大面积不规则掩码时表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D语义场构建方法主要依赖场景特定的高斯泼溅，需要逐场景优化，泛化能力有限，难以扩展到实际应用。需要一种能同时整合几何感知和语言对齐的统一框架。
...
Result: 模型在多样化的时间模式和相机轨迹上实现了稳健的4D控制，保持了高生成质量，并在可控性上优于先前工作
...
TL;DR: Deep Forcing：一种无需训练的长视频生成方法，通过Deep Sink和Participative Compression机制解决视频扩散模型中的时间重复、漂移和运动减速问题，实现12倍以上的外推生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视频扩散模型虽然能实现实时帧流式生成，但仍存在时间重复、漂移和运动减速问题。直接将StreamingLLM风格的注意力机制应用于视频扩散会导致保真度下降和运动停滞。

Method: 提出Deep Forcing方法，包含两个无需训练的机制：1) Deep Sink：将滑动窗口的一半分配给持久性sink token，并重新对齐其时间RoPE相位到当前时间线，稳定长序列生成中的全局上下文；2) Participative Compression：执行重要性感知的KV缓存剪枝，仅保留最近注意力中活跃参与的token，安全丢弃冗余和退化的历史信息。

Result: 实现了超过12倍的外推能力（如从5秒训练扩展到60秒以上生成），在图像质量上优于LongLive，美学质量优于RollingForcing，几乎保持整体一致性，动态程度显著提升，同时保持实时生成能力。
...
TL;DR: 论文提出视觉推理追踪器(VRT)任务，要求模型不仅定位目标物体，还要预测形成推理路径的中间物体，并创建了VRT-Bench基准、新评估指标和VRT-80k数据集。
...
Method: 引入视觉推理追踪器(VRT)任务，创建VRT-Bench评估基准、新的推理追踪质量评估指标，以及VRT-80k大规模数据集用于模型训练。

Result: 实验发现现有模型虽然能产生正确最终输出，但在中间推理的定位上表现不佳；而使用VRT-80k训练的模型在追踪推理路径方面取得了显著改进。

Conclusion: 通过VRT任务和相关资源，能够提高多模态模型的推理透明度，使其更接近人类的视觉推理过程，为可解释AI提供了重要工具。
...
TL;DR: 该论文提出了空间美学评估范式，构建了首个室内场景美学质量评估基准SA-BENCH，并开发了SA-IQA模型，通过多维融合方法显著提升了AI生成图像的美学评估性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估方法主要针对肖像和艺术图像，缺乏对室内场景的系统性美学评估，需要建立专门的空间美学评估框架。

Method: 提出空间美学评估范式，包含布局、和谐、照明和失真四个维度；构建SA-BENCH基准数据集（18,000张图像，50,000个标注）；通过MLLM微调和多维融合方法开发SA-IQA模型；将SA-IQA应用于GRPO强化学习和Best-of-N选择两个下游任务。

Result: SA-IQA在SA-BENCH基准上显著优于现有方法，为空间美学评估设立了新标准；成功应用于优化AIGC生成流程和筛选高质量图像。

Conclusion: 该研究填补了室内场景美学评估的空白，提出的空间美学评估范式和SA-IQA模型为AI生成图像的质量评估提供了有效工具，将开源代码和数据集以推动该领域的研究和应用。
...
Result: 在多个基准测试上的广泛实验表明，EvoIR优于现有的最先进全合一图像修复方法，FMM和EOS的协同作用带来了比单独使用任一组件更大的改进。

Conclusion: EvoIR通过进化频率调制和优化策略，有效解决了全合一图像修复中的异构退化问题，证明了显式频率建模和动态优化调度的重要性。
...
TL;DR: ShadowDraw是一个将普通3D对象转化为影子绘画艺术的框架，通过优化场景参数（物体姿态、光照）和生成部分线描，让投射的影子完成可识别图像的绘制。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是创建一个实用的计算视觉艺术创作流程，通过算法设计将3D对象转化为影子绘画艺术，拓展计算视觉艺术的设计空间，弥合算法设计与艺术叙事之间的鸿沟。

Method: 方法包括：1）优化场景配置以揭示有意义的影子；2）使用影子笔画指导线描生成；3）采用自动评估来确保影子-绘画的连贯性和视觉质量。系统预测场景参数（物体姿态、光照）和部分线描，使投射的影子完成可识别图像。

Result: 实验表明，ShadowDraw在多样化输入上都能产生令人信服的结果，包括真实世界扫描、精选数据集和生成资产，并能自然扩展到多对象场景、动画和物理部署。系统支持从单个3D对象创建完整的影子绘画艺术。

Conclusion: 该工作为创建影子绘画艺术提供了实用的流程，拓宽了计算视觉艺术的设计空间，在算法设计与艺术叙事之间架起了桥梁，展示了从3D对象到影子绘画艺术的有效转换。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在文本到图像生成方面存在局限性：要么将模型仅视为独立生成器，要么依赖抽象的文本规划。文本规划粒度较粗，难以处理罕见属性组合，需要更具体的视觉引导和验证机制。

Method: DraCo采用交错推理范式：1）首先生成低分辨率草稿图像作为视觉预览和结构化规划；2）利用模型内在理解能力验证草稿与输入提示之间的语义对齐；3）通过选择性修正和超分辨率进行细化。同时构建了DraCo-240K训练数据集，并开发了DraCo-CFG专门分类器自由引导策略。

Result: 在GenEval上提升8%，Imagine-Bench提升0.91分，GenEval++提升3%，显著优于直接生成和其他基于CoT的生成方法。解决了文本规划粒度粗和罕见属性组合生成困难两个基本挑战。

Conclusion: DraCo通过将视觉内容纳入思维链推理，实现了更具体的视觉规划和语义对齐验证，为文本到图像生成提供了更有效的交错推理范式，显著提升了生成质量和语义准确性。
...
TL;DR: 提出Splannequin方法，通过动态高斯泼溅技术从单目视频合成高质量的冻结3D场景，引入时间锚定机制解决稀疏时间监督导致的伪影问题


<details>
  <summary>Details</summary>
Motivation: 从单目Mannequin-Challenge视频合成高保真冻结3D场景是一个独特问题，不同于标准动态场景重建。目标是在创建冻结场景的同时策略性地保留细微动态，以实现用户控制的即时选择

Method: 引入动态高斯泼溅的新应用：场景被动态建模以保留附近时间变化，通过固定模型时间参数渲染静态场景。提出Splannequin架构无关的正则化方法，检测高斯基元的隐藏和缺陷状态，应用时间锚定机制。在主要前向相机运动下，隐藏状态锚定到最近观测良好的过去状态，缺陷状态锚定到有更强监督的未来状态

Result: 方法通过简单损失项集成到现有动态高斯管道，无需架构更改，零推理开销。显著改善视觉质量，实现高保真、用户可选择的冻结时间渲染，获得96%的用户偏好验证

Conclusion: Splannequin方法有效解决了从单目MC视频合成冻结3D场景时的伪影问题，通过时间锚定机制提升渲染质量，为高质量用户可控冻结场景渲染提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有光照控制方法在视频中面临光照保真度与时间一致性的权衡问题。真实场景的生成建模需要同时控制相机轨迹和光照，因为视觉动态本质上由几何和光照共同塑造。

Method: 1) 提出解耦设计：通过动态点云沿用户定义相机轨迹捕捉几何和运动，同时通过重光照帧提供一致的光照线索。2) 引入Light-Syn合成训练数据：通过基于退化的逆向映射流程从野外单目视频合成训练对，覆盖静态、动态和AI生成场景。

Result: 实验表明Light-X在联合相机-光照控制方面优于基线方法，在文本和背景条件设置下都超越了先前的视频重光照方法。

Conclusion: Light-X通过解耦几何与光照信号，结合创新的数据合成策略，实现了高质量的视频生成，支持同时控制相机视角和光照条件。
...
### [96] [CRAFT-E: A Neuro-Symbolic Framework for Embodied Affordance Grounding](https://arxiv.org/abs/2512.04231)
...
Result: 在静态场景、基于ImageNet的功能检索和涉及20个动词和39个物体的真实世界实验中取得竞争性性能，框架在感知噪声下保持鲁棒，并提供透明的组件级诊断。

Conclusion: 通过将符号推理与具身感知相结合，CRAFT-E为功能基础的物体选择提供了一个可解释且可定制的端到端模型替代方案，支持辅助机器人系统中的可信决策。
...
### [97] [Sliding Mode Control and Subspace Stabilization Methodology for the Orbital Stabilization of Periodic Trajectories](https://arxiv.org/abs/2512.04249)
...
<details>
  <summary>Details</summary>
Motivation: 针对具有一度欠驱动的机械系统，需要有效稳定周期轨迹的方法，同时避免传统方法中的计算复杂性和鲁棒性问题
...
Conclusion: 结合滑模控制与子空间稳定的方法为欠驱动机械系统的周期轨迹轨道稳定提供了一种计算高效且鲁棒的解决方案
...
### [98] [Driving Beyond Privilege: Distilling Dense-Reward Knowledge into Sparse-Reward Policies](https://arxiv.org/abs/2512.04279)
*Feeza Khan Khanzada,Jaerock Kwon*

Main category: cs.RO

TL;DR: 提出奖励特权世界模型蒸馏框架，通过教师模型利用密集特权奖励学习丰富动态模型，学生模型仅使用稀疏任务奖励但继承教师动态知识，在自动驾驶任务中实现更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶模拟器中密集特权奖励（如车道几何、违规、碰撞时间）可以加速强化学习，但直接基于这些信号训练的策略容易过拟合，在稀疏目标（如路线完成、无碰撞超车）上泛化能力差。需要利用密集奖励的优势同时避免其对部署指标的不对齐问题。

Method: 提出两阶段奖励特权世界模型蒸馏框架：1）教师DreamerV3智能体使用密集特权奖励训练；2）仅将教师的潜在动态蒸馏到学生模型中，学生仅使用稀疏任务奖励训练。师生共享相同的观测空间（语义鸟瞰图），特权信息仅通过教师奖励输入，学生不模仿教师的动作或价值估计，而是通过正则化使其世界模型匹配教师潜在动态，策略从头开始基于稀疏成功/失败信号学习。

Result: 在CARLA车道跟随和超车基准测试中，稀疏奖励学生模型优于密集奖励教师模型和从头开始训练的稀疏奖励基线。在未见过的车道跟随路线上，奖励特权蒸馏将成功率相对密集教师提高约23%，同时保持相当或更好的安全性。在超车任务中，学生在训练路线上保持接近完美的性能，在未见路线上成功率提高高达27倍，车道保持能力也有所改善。

Conclusion: 研究表明，密集奖励可用于学习更丰富的动态模型，同时保持部署策略严格针对稀疏、与部署对齐的目标进行优化。奖励特权蒸馏框架有效解决了密集奖励过拟合问题，实现了更好的泛化性能。
...
### [99] [Vertical Planetary Landing on Sloped Terrain Using Optical Flow Divergence Estimates](https://arxiv.org/abs/2512.04373)
*Hann Woei Ho,Ye Zhou*

Main category: cs.RO

TL;DR: 提出一种基于局部光流发散估计的非线性控制策略，用于小型航天器在斜坡地形上的自主着陆，通过调节平均发散实现平稳下降，利用发散差异实现姿态对齐。


<details>
  <summary>Details</summary>
Motivation: 小型轻量航天器（如旋翼机和着陆器）处理能力和有效载荷有限，难以采用先进的深度学习方法或重型传感器。受蜜蜂等飞行昆虫仅依赖光流实现卓越着陆的启发，需要开发一种资源需求低的斜坡着陆策略。

Method: 提出非线性控制策略，利用两个局部光流发散估计来调节推力和姿态。采用增量非线性动态反演处理非线性光流发散，推力控制通过保持局部发散估计的平均值恒定实现平稳垂直下降，姿态控制利用发散差异使飞行器在触地时与倾斜表面对齐。

Result: 数值模拟使用简化的2D航天器模型在不同坡度和发散设定点下进行评估。结果显示：调节平均发散可实现速度和高度指数衰减的稳定着陆；利用发散差异能有效实现与倾斜地形的对齐。

Conclusion: 该方法提供了一种鲁棒、低资源的着陆策略，增强了小型航天器自主行星任务的可行性，解决了传统控制器在斜坡着陆中因全局发散估计掩盖地形倾角和非线性控制不稳定性的问题。
...
### [100] [FALCON: Actively Decoupled Visuomotor Policies for Loco-Manipulation with Foundation-Model-Based Coordination](https://arxiv.org/abs/2512.04381)
*Chengyang He,Ge Sun,Yue Bai,Junkai Lu,Jiadong Zhao,Guillaume Sartoretti*

Main category: cs.RO

TL;DR: FALCON框架通过视觉语言基础模型协调解耦的移动和操作策略，提升机器人移动操作任务的性能与泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决单一策略融合异构移动和操作观测时性能下降的问题，通过解耦策略和基础模型协调来提升移动操作任务的性能

Method: 将移动和操作解耦为两个专门的视觉运动策略，使用视觉语言基础模型作为协调器编码全局观测和语言指令，引入阶段进度头和协调感知对比损失

Result: 在需要导航、精确末端执行器放置和紧密基座-手臂协调的移动操作任务上，超越集中式和去中心化基线，展现更好的鲁棒性和分布外泛化能力

Conclusion: FALCON框架通过解耦策略和基础模型协调，有效解决了移动操作中的观测融合问题，提升了任务性能和泛化能力
...
### [101] [Development of a 15-Degree-of-Freedom Bionic Hand with Cable-Driven Transmission and Distributed Actuation](https://arxiv.org/abs/2512.04399)
...
<details>
  <summary>Details</summary>
Motivation: 在机械手研究中，如何在保持人类手部尺寸和自由度的同时最小化驱动器数量是一个基本挑战。传统肌腱驱动系统需要大量电机，导致结构复杂、重量大。

Method: 从人类手部运动学配置和肌肉分布策略获得生物启发，提出新型肌腱驱动机制。在手臂中集成5个电机提供强力抓握，在手掌中安装10个小电机支持精细操作，并开发了相应的关节传感和电机驱动电气系统。

Result: 整个系统重量仅1.4kg，结合了轻量化和高性能特点。实验表明该仿生手展现出卓越的灵巧性和强大的抓取能力。

Conclusion: 这种设计显著减少了传统肌腱驱动系统所需的电机数量，同时提高了运动性能并简化了机械结构，在机器人操作任务中展现出巨大潜力。
...
### [102] [RoboBPP: Benchmarking Robotic Online Bin Packing with Physics-based Simulation](https://arxiv.org/abs/2512.04415)
*Zhoufeng Wang,Hang Zhao,Juzhan Xu,Shishun Zhang,Zeyu Xiong,Ruizhen Hu,Chenyang Zhu,Kai Xu*

Main category: cs.RO

TL;DR: RoboBPP是一个用于机器人在线三维装箱的基准测试系统，集成了物理模拟器来评估物理可行性，并提供真实工业数据集和扩展的评估指标。


<details>
  <summary>Details</summary>
Motivation: 当前三维装箱领域存在三个主要问题：1) 问题设置、测试数据集和评估指标不一致阻碍了领域进展；2) 缺乏全面的基准测试系统；3) 真实硬件测试成本高且模拟环境构建困难。此外，现有研究依赖与真实工业数据分布不同的合成数据集。

Method: 开发RoboBPP基准测试系统，集成基于物理的模拟器评估物理可行性。模拟环境包含真实尺度的机械臂和箱子，复制真实工业装箱流程。收集三个真实工业工作流程数据集（装配线生产、物流包装、家具制造）。设计三个测试设置，扩展现有评估指标，新增结构稳定性和操作安全性指标。设计评分系统并提供可视化工具和在线排行榜。

Result: RoboBPP提供了一个完全开源的基准测试平台，包含物理模拟器、真实工业数据集、扩展的评估指标和可视化工具。通过模拟真实工业应用条件，确保评估算法具有实际部署可行性。系统提供了可重现和可扩展的基础，支持未来研究和工业应用。

Conclusion: RoboBPP解决了三维装箱领域缺乏标准化基准测试的问题，通过物理模拟和真实工业数据集确保了算法的物理可行性和实际应用价值，为机器人在线装箱研究提供了全面的评估框架。
...
### [103] [Vision-Language-Action Models for Selective Robotic Disassembly: A Case Study on Critical Component Extraction from Desktops](https://arxiv.org/abs/2512.04446)
*Chang Liu,Sibo Tian,Sara Behdad,Xiao Liang,Minghui Zheng*

Main category: cs.RO

TL;DR: 研究探索了视觉语言动作模型在废旧台式机关键部件（RAM和CPU）自动拆卸中的应用，发现纯VLA模型能完成部分步骤但在关键子任务上失败，而VLA与规则控制器结合的混合策略能成功完成整个拆卸操作。


<details>
  <summary>Details</summary>
Motivation: 废旧台式机关键部件（如RAM模块、CPU和硬盘）的自动化拆卸面临产品变异性大、操作序列精确且需要灵巧性的挑战。现有机器人拆卸流程需要分阶段显式建模，限制了泛化能力。虽然视觉语言动作模型在简单机器人操作任务中表现出色，但将其应用于复杂拆卸任务的可行性尚未充分探索。

Method: 收集了机器人RAM和CPU拆卸的定制数据集，用于微调两种成熟的VLA方法（OpenVLA和OpenVLA-OFT）。将整个拆卸任务分解为多个小步骤，并测试了纯VLA模型和VLA与规则控制器结合的混合策略。

Result: 微调后的VLA模型能够忠实完成多个早期步骤，但在某些关键子任务上遇到困难，导致任务失败。然而，简单的混合策略（VLA与规则控制器结合）能够成功执行整个拆卸操作。

Conclusion: VLA模型在处理机器人废旧产品拆卸所需的灵巧性和精确性方面存在当前局限性。研究通过详细分析观察结果，为未来研究解决当前挑战、推进端到端机器人自动拆卸提供了见解。
...
### [104] [Open-Ended Goal Inference through Actions and Language for Human-Robot Collaboration](https://arxiv.org/abs/2512.04453)
*Debasmita Ghose,Oz Gitelson,Marynel Vazquez,Brian Scassellati*

Main category: cs.RO

TL;DR: BALI是一个双向动作-语言推理方法，通过整合自然语言偏好和观察到的动作来预测人类目标，在协作烹饪任务中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 机器人需要推断人类目标，但这些目标常常模糊、难以表达或不在固定集合中。现有方法要么局限于预定义目标集，要么只依赖观察动作或显式指令，在现实交互中表现脆弱。

Method: BALI方法整合自然语言偏好和观察到的动作，在滚动时域规划树中进行推理。它结合语言和动作线索，仅在预期信息增益超过中断成本时提出澄清问题，并选择与推断目标一致的支持性动作。

Result: 在协作烹饪任务中评估，BALI相比基线方法产生更稳定的目标预测和显著更少的错误。

Conclusion: BALI通过双向整合语言和动作信息，能够有效处理无界、新颖的目标推断问题，在真实世界协作任务中表现更稳健。
...
### [105] [One Ring to Rule Them All: Constrained Distributional Control for Massive-Scale Heterogeneous Robotic Ensemble Systems](https://arxiv.org/abs/2512.04502)
*Andres Arias,Wei Zhang,Haoyu Qian,Jr-Shin Li,Chuangchuang Sun*

Main category: cs.RO

TL;DR: 提出了一种用于参数化异构机器人系统的约束集成控制框架，通过矩核变换将参数化集成动力学映射到矩空间，实现安全的大规模集成控制


<details>
  <summary>Details</summary>
Motivation: 集成控制旨在使用共享控制输入来引导动态系统群体，但现有方法在处理参数化、异构机器人系统在状态和环境约束（如避障）下的操作方面存在不足

Method: 开发了矩核变换，将参数化集成动力学映射到核空间中的矩系统；将状态空间约束（如多面体路径点和避障）转换为矩空间；使用信号时序逻辑规范编码复杂的访问-避让任务；通过约束集成控制公式合成单一共享控制器

Result: 仿真和硬件实验证明了该方法在受限环境中安全高效地控制机器人集成系统的有效性

Conclusion: 提出的约束集成控制框架通过矩核变换实现了参数化异构机器人系统在约束环境下的安全大规模控制，为复杂访问-避让任务提供了统一的解决方案
...
### [106] [Bridging Simulation and Reality: Cross-Domain Transfer with Semantic 2D Gaussian Splatting](https://arxiv.org/abs/2512.04731)
*Jian Tang,Pu Pang,Haowen Sun,Chengzhong Ma,Xingyu Chen,Hua Huang,Xuguang Lan*

Main category: cs.RO

TL;DR: 提出S2GS方法，通过提取物体中心、领域不变的空间特征来桥接机器人操作中的仿真-现实领域差距，显著提升策略的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 机器人操作中的仿真到现实迁移存在显著的领域差距，现有方法如领域随机化、适应和仿真-现实校准需要大量调优或难以泛化到未见场景。如果能在仿真训练中使用领域不变特征，并在现实部署中提取相同特征作为策略输入，就能有效桥接领域差距。

Method: 提出语义2D高斯泼溅（S2GS）表示方法，构建多视图2D语义场并通过特征级高斯泼溅投影到统一3D空间。采用语义过滤机制去除无关背景内容，确保策略学习的输入干净一致。使用Diffusion Policy作为下游学习算法。

Result: 在ManiSkill仿真环境和现实部署中的实验表明，S2GS显著提升了仿真到现实的迁移能力，在现实场景中保持高且稳定的任务性能。

Conclusion: S2GS通过提取领域不变的空间特征有效桥接了仿真-现实领域差距，为机器人操作的跨域迁移提供了有效的解决方案。
...
### [107] [TEMPO-VINE: A Multi-Temporal Sensor Fusion Dataset for Localization and Mapping in Vineyards](https://arxiv.org/abs/2512.04772)
*Mauro Martini,Marco Ambrosio,Judith Vilella-Cantos,Alessandro Navone,Marcello Chiaberge*

Main category: cs.RO

TL;DR: TEMPO-VINE是首个针对葡萄园环境的大规模多模态多时序数据集，包含不同季节、植被生长阶段和天气条件下的异构传感器数据，用于评估传感器融合、SLAM和地点识别技术。


<details>
  <summary>Details</summary>
Motivation: 当前精准农业机器人研究多依赖受控模拟或孤立田间试验，缺乏真实复杂农业条件下的通用基准。葡萄园因其动态特性带来显著挑战，需要更真实的评估数据集来推动稳健自主系统的发展。

Method: 创建TEMPO-VINE数据集，整合不同价格水平的异构LiDAR、AHRS、RTK-GPS和相机数据，在真实棚架和棚架式葡萄园中采集，包含超过100米长的多行路径，涵盖不同季节、植被生长阶段、地形和天气条件。

Result: 提供了首个多模态公共数据集，包含多次运行和重访的序列路径，为农业领域的传感器融合、定位、建图和地点识别解决方案开发提供支持。

Conclusion: TEMPO-VINE填补了农业数据集的关键空白，为研究人员提供了全面的数据收集和地面真实轨迹，将促进农业领域自主导航技术的发展。
...
### [108] [Using Machine Learning to Take Stay-or-Go Decisions in Data-driven Drone Missions](https://arxiv.org/abs/2512.04773)
*Giorgos Polychronis,Foivos Pournaropoulos,Christos D. Antonopoulos,Spyros Lalis*

Main category: cs.RO

TL;DR: 无人机在数据驱动任务中面临实时决策困境：原地等待处理数据可能浪费时间，提前移动又可能错过需要后续行动的事件。论文提出基于分支预测和强化学习的机器学习方法来解决这一决策问题。


<details>
  <summary>Details</summary>
Motivation: 无人机在数据驱动任务中需要实时处理收集的数据以决定是否需要在当前位置采取额外行动。如果原地等待处理但最终不需要行动，则浪费了时间；如果提前移动到下一个兴趣点，但发现前一个点需要后续行动，则需要飞回并花费额外时间。现有方法无法有效解决这一决策困境。

Method: 提出了基于分支预测和强化学习的机器学习方法。这些方法旨在优化无人机在数据收集和处理过程中的决策策略，以平衡等待处理和移动之间的权衡。

Result: 提出的方法在事件发生概率随时间变化的各种场景中表现优异。相比文献中提出的基于回归的方法，新方法始终表现更好，最坏情况任务时间最多可改善4.1倍。同时，达到的中位任务时间非常接近具有完美事件概率知识的方法，仅高出最多2.7%。

Conclusion: 基于分支预测和强化学习的机器学习方法能有效解决无人机在数据驱动任务中的实时决策问题，显著改善任务执行效率，接近理想决策性能。
...
### [109] [MOVE: A Simple Motion-Based Data Collection Paradigm for Spatial Generalization in Robotic Manipulation](https://arxiv.org/abs/2512.04813)
*Huanqian Wang,Chi Bene Chen,Yang Yue,Danhua Tao,Tong Guo,Shaoxuan Xie,Denghang Huang,Shiji Song,Guocai Yao,Gao Huang*

Main category: cs.RO

TL;DR: MOVE提出了一种基于运动的数据收集范式，通过在演示过程中为可移动物体注入运动，从单条轨迹中生成密集多样的空间配置，显著提升了模仿学习在机器人操作中的空间泛化能力和数据效率。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人操作中展现出巨大潜力，但其实际部署受到数据稀缺的根本限制。现有的大规模数据集收集方法存在显著的空间泛化差距，主要原因是单个轨迹通常从环境的单一静态空间配置中收集，包括固定的物体和目标位置以及不变的相机视角，这严重限制了可用于学习的空间信息多样性。

Method: 提出了MOVE（基于运动的变异性增强）数据收集范式，核心贡献是一种增强策略：在每个演示过程中为环境中的任何可移动物体注入运动。这一过程隐式地在单个轨迹内生成密集多样的空间配置，从而从动态演示中获取更丰富的空间信息。

Result: 在仿真和真实环境中的广泛实验验证了该方法的有效性。在需要强空间泛化的仿真任务中，MOVE实现了39.1%的平均成功率，相对于静态数据收集范式（22.2%）有76.1%的相对提升，在某些任务上实现了2-5倍的数据效率增益。

Conclusion: MOVE通过简单的数据收集范式创新，显著提升了模仿学习在机器人操作中的空间泛化能力和数据效率，为解决模仿学习中的数据稀缺问题提供了有效解决方案。
...
### [110] [Hoi! - A Multimodal Dataset for Force-Grounded, Cross-View Articulated Manipulation](https://arxiv.org/abs/2512.04884)
*Tim Engelbracht,René Zurbrügg,Matteo Wohlrapp,Martin Büchner,Abhinav Valada,Marc Pollefeys,Hermann Blum,Zuria Bauer*

Main category: cs.RO

TL;DR: 提出了一个包含3048个序列、381个铰接物体、38个环境的力感知跨视角操作数据集，包含人类手部、手腕相机、手持UMI夹具和Hoi!夹具四种执行器，提供同步末端执行器力和触觉感知。


<details>
  <summary>Details</summary>
Motivation: 现有数据集通常只关注视觉信息，缺乏力感知和触觉信息的整合。为了全面理解交互过程，需要将视觉、动作和力感知相结合，特别是研究人类与机器人视角之间的转换，以及力感知和预测等未被充分探索的模态。

Method: 收集了3048个操作序列，涵盖381个铰接物体和38个环境。每个物体在四种执行器配置下操作：1) 人类手部；2) 人类手部+手腕相机；3) 手持UMI夹具；4) 定制Hoi!夹具。工具执行器提供同步的末端执行器力和触觉感知数据。

Result: 创建了一个全面的力感知跨视角铰接操作数据集，将视觉、动作和力感知信息耦合。该数据集支持从视频中理解交互过程，使研究人员能够评估方法在人类和机器人视角之间的迁移能力，并探索力感知和预测等模态。

Conclusion: 该数据集为交互理解提供了全面的多模态视角，特别关注力感知和跨视角转换，填补了现有数据集的空白，为研究人类与机器人操作之间的相似性和差异性提供了重要资源。
...
### [111] [On Disturbance-Aware Minimum-Time Trajectory Planning: Evidence from Tests on a Dynamic Driving Simulator](https://arxiv.org/abs/2512.04917)
...
TL;DR: 研究比较了三种扰动感知的鲁棒参考轨迹与自由驾驶基线，发现摩擦极限鲁棒轨迹(FLC)在圈时和转向努力之间取得了最佳平衡，显著降低转向努力而只轻微增加圈时。


<details>
  <summary>Details</summary>
Motivation: 研究扰动感知、鲁棒性嵌入的参考轨迹在实际驾驶中的表现，评估专业驾驶员在动态模拟器中执行这些轨迹时的驾驶性能，探索圈时与转向努力之间的权衡关系。

Method: 比较三种规划参考轨迹与自由驾驶基线：NOM（名义时间最优轨迹）、TLC（赛道极限鲁棒轨迹）、FLC（摩擦极限鲁棒轨迹）。所有轨迹基于作者提出的扰动感知最小圈时框架，通过传播最坏情况扰动增长来收紧轮胎摩擦和赛道限制约束。由两名专业驾驶员在虚拟赛道上使用高性能车辆进行评估。

Result: 结果显示帕累托式的圈时-转向努力权衡：NOM获得最短圈时但转向努力最高；TLC最小化转向努力但圈时较长；FLC接近效率前沿，相对于NOM显著降低转向努力而只轻微增加圈时。移除轨迹引导会增加圈时和转向努力，证实参考轨迹能提高速度和操控效率。

Conclusion: 基于参考轨迹和扰动感知的规划，特别是FLC轨迹，是实现快速稳定轨迹的有效工具，适用于驾驶训练和性能优化，在圈时和转向努力之间取得了良好平衡。
...
### [112] [Hybrid-Diffusion Models: Combining Open-loop Routines with Visuomotor Diffusion Policies](https://arxiv.org/abs/2512.04960)
*Jonne Van Haastregt,Bastian Orthmann,Michael C. Welle,Yuchong Zhang,Danica Kragic*

Main category: cs.RO

TL;DR: 提出Hybrid-Diffusion模型，结合开环例程与视觉运动扩散策略，通过Teleoperation Augmentation Primitives在演示中执行预定义例程，提高模仿学习在复杂操作任务中的精度和速度。


<details>
  <summary>Details</summary>
Motivation: 基于模仿学习的视觉运动策略在复杂操作任务中表现良好，但通常难以达到传统控制方法的精度和速度。为解决这一局限性，需要结合开环例程来提升性能。

Method: 提出Hybrid-Diffusion模型，结合开环例程与视觉运动扩散策略。开发Teleoperation Augmentation Primitives（TAPs），允许操作者在演示过程中无缝执行预定义例程（如锁定特定轴、移动到停靠点、触发任务特定例程）。该方法在推理过程中学习触发这些TAPs。

Result: 在三个具有挑战性的真实世界任务上验证了方法：Vial Aspiration（小瓶吸液）、Open-Container Liquid Transfer（开口容器液体转移）和容器拧开。所有实验视频可在项目网站上查看。

Conclusion: Hybrid-Diffusion模型通过结合开环例程与视觉运动扩散策略，有效提升了模仿学习在复杂操作任务中的精度和速度，在多个真实世界任务中展示了良好的性能。
...
### [113] [Preliminary Analysis and Simulation of a Compact Variable Stiffness Wrist](https://arxiv.org/abs/2512.04973)
*Giuseppe Milazzo,Manuel G. Catalano,Antonio Bicchi,Giorgio Grioli*

Main category: cs.RO

TL;DR: 本文提出了一种新型3自由度并联手腕，通过冗余弹性驱动实现变刚度，仅用4个电机实现紧凑轻量化设计，适用于假肢或人形机器人应用。


<details>
  <summary>Details</summary>
Motivation: 变刚度执行器在非结构化环境中对机器人应用具有重要价值，能促进安全交互并增强任务适应性。然而，传统变刚度执行器的机械设计通常导致结构更大更重，限制了其在紧凑型应用中的使用。

Method: 采用3自由度并联架构，通过冗余弹性驱动实现变刚度。仅使用4个电机，使设备紧凑轻便。建立了设备的理论模型，并提出了一种用于独立调节关节位置和刚度的复杂控制策略。

Result: 通过仿真验证了所提出的控制器，并对系统动力学进行了全面分析。结果表明，该设备能够在刚性配置下实现高精度和干扰抑制，同时在柔顺行为下最小化交互力。

Conclusion: 该3自由度并联手腕设计成功实现了紧凑轻量化的变刚度执行器，通过提出的控制策略能够独立调节位置和刚度，在刚性配置下具有高精度，在柔顺配置下能最小化交互力，特别适用于假肢和人形机器人应用。
...
### [114] [Introducing V-Soft Pro: a Modular Platform for a Transhumeral Prosthesis with Controllable Stiffness](https://arxiv.org/abs/2512.04998)
*Giuseppe Milazzo,Giorgio Grioli,Antonio Bicchi,Manuel G. Catalano*

Main category: cs.RO

TL;DR: 开发了一种采用可变刚度执行器的经肱骨假肢，通过弹性元件和模块化设计模拟生物关节的可控顺应性，提升假肢的自然运动和适应性


<details>
  <summary>Details</summary>
Motivation: 当前上肢假肢虽然具备基本运动功能，但无法复制人类手臂的自然运动和交互能力。人类肢体利用内在顺应性和主动调节关节刚度，能够适应不同任务、吸收冲击并在动态动作中高效传递能量。受此启发，需要开发能够模拟生物关节可控顺应性的假肢

Method: 开发了采用可变刚度执行器的经肱骨假肢，具有模块化设计，可适应不同残肢形状和多种独立控制信号。集成弹性元件被动支持更自然的运动，促进与环境的安全交互，并适应不同的任务需求

Result: 提出了一个具有可变刚度执行器的假肢平台，能够模拟生物关节的可控顺应性，支持自然运动、安全交互和任务适应性，在假肢领域具有潜在应用价值

Conclusion: 通过可变刚度执行器和弹性元件设计，开发了一种能够模拟生物关节顺应性的经肱骨假肢，为提升假肢的自然运动和适应性提供了新的技术方案
...
### [115] [Contact-Implicit Modeling and Simulation of a Snake Robot on Compliant and Granular Terrain](https://arxiv.org/abs/2512.05008)
*Haroon Hublikar*

Main category: cs.RO

TL;DR: 该论文提出了一个统一的建模与仿真框架，用于分析COBRA蛇形机器人在刚性、柔性和颗粒地形上的侧向滑行和翻滚运动，通过不同精度模型实现从实时控制到高保真物理仿真的全覆盖。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够准确模拟蛇形机器人在各种复杂地形（包括刚性、柔性和颗粒状表面）上运动的统一仿真框架，以解决现有模型在软质和动态环境中预测能力不足的问题。

Method: 1. 使用接触隐式公式化方法建模侧向滑行中的分布式摩擦相互作用；2. 集成Project Chrono的土壤接触模型（SCM）来捕捉地形变形效应；3. 使用Chrono DEM引擎模拟高能翻滚运动中的颗粒级相互作用；4. 建立分层仿真管道，涵盖从实时控制到高保真物理仿真的不同精度需求。

Result: 1. 刚性地面模型能够准确预测短期运动；2. 在软质和高度动态环境中，连续介质和基于颗粒的地形建模变得必要；3. 揭示了土壤失效、间歇性抬升和能量耗散等刚性模型无法捕捉的机制；4. 建立了能够预测滑移、沉降和载荷重新分布的分层仿真管道。

Conclusion: 该研究建立了一个分层仿真管道，通过整合不同精度的地形建模方法，显著提升了机器人在挑战性非结构化环境中进行地形感知运动的鲁棒性和可靠性，为复杂地形下的机器人运动控制提供了先进的仿真工具。
...
### [116] [From Generated Human Videos to Physically Plausible Robot Trajectories](https://arxiv.org/abs/2512.05094)
...
<details>
  <summary>Details</summary>
Motivation: 视频生成模型在合成人类动作方面进步迅速，有潜力作为机器人控制的高级规划器。但如何让类人机器人零样本执行生成视频中的人类动作仍是一个开放问题，因为生成视频通常有噪声且存在形态扭曲，难以直接模仿。

Method: 提出两阶段流程：1) 将视频像素提升为4D人体表示，然后重定向到类人机器人形态；2) 提出GenMimic——基于3D关键点的物理感知强化学习策略，使用对称正则化和关键点加权跟踪奖励进行训练。

Result: 创建了GenMimicBench合成人体运动数据集，使用两种视频生成模型生成多种动作和场景。实验表明在模拟中优于强基线，并在Unitree G1类人机器人上实现了连贯、物理稳定的运动跟踪，无需微调。

Conclusion: 这项工作为实现视频生成模型作为机器人控制高级策略的潜力提供了一条有前景的路径，能够模仿噪声生成视频中的人类动作。
...
### [117] [STARE-VLA: Progressive Stage-Aware Reinforcement for Fine-Tuning Vision-Language-Action Models](https://arxiv.org/abs/2512.05107)
*Feng Xu,Guangyao Zhai,Xin Kong,Tingzhong Fu,Daniel F. N. Gordon,Xueli An,Benjamin Busam*

Main category: cs.RO

TL;DR: 本文提出STARE模块，将长时程动作轨迹分解为语义阶段，提供密集、可解释的阶段对齐强化信号，结合TPO和PPO形成STA-TPO和STA-PPO，并提出了IPI串行微调流程，在机器人操作任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型将长时程动作视为语言序列，采用轨迹级优化方法（如TPO或PPO），导致信用分配粗糙和训练不稳定。与语言不同，动作轨迹通过因果链阶段进展，各阶段学习难度不同，这启发了渐进阶段优化的需求。

Method: 提出STARE模块，将长时程动作轨迹分解为语义上有意义的阶段，提供密集、可解释的阶段对齐强化信号。将STARE集成到TPO和PPO中，形成STA-TPO（用于离线阶段偏好）和STA-PPO（用于在线阶段内交互）。基于监督微调初始化，提出了Imitation -> Preference -> Interaction (IPI)串行微调流程。

Result: 在SimplerEnv和ManiSkill3上的实验显示显著提升，在SimplerEnv上达到98.0%的成功率，在ManiSkill3任务上达到96.4%的成功率，实现了最先进的性能。

Conclusion: STARE模块通过阶段分解和阶段对齐强化信号解决了长时程动作轨迹优化中的信用分配问题，STA-TPO和STA-PPO结合IPI串行微调流程显著提高了VLA模型的动作准确性，在机器人操作任务上取得了优异的性能。
