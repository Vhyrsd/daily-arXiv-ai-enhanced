...
TL;DR: 本文提出了一种全谱系人类上下文分类法，将人类中心智能组织为六个相互关联的层次（视觉外观、空间几何、运动动力学、交互建模、世界模拟和具身智能体），系统综述了相关方法、数据集、基准与评估指标，并探讨了开放挑战与未来方向，旨在弥合该领域不同任务、模态和研究社区之间的碎片化鸿沟。


<details>
  <summary>Details</summary>
Motivation: 人类中心智能在基础模型时代尚未像其他领域那样充分融入基础模型以实现可比进展；同时，该领域的最新进展在不同任务、模态和研究社区之间高度碎片化，内在的概念与方法论联系不清晰，亟需一个统一的框架来整合和梳理。

Method: 作者提出一个全谱系人类上下文分类法，从三个视角（可观察主体、动态行为者、情境化智能体）涵盖六个层次：视觉外观与空间几何、运动动力学与交互建模、世界模拟与具身智能。在此基础上，梳理人类中心数据家族、计算架构范式、训练与推理优化策略，并系统综述各层次的代表性方法、数据集、基准与评估指标。

Result: 论文提供了系统性的人类中心智能综述，覆盖六个层次的方法论基础、代表性方法、数据集与评估体系，并公开了持续更新的人类中心AI文献与资源项目页面。

Conclusion: 作者指出人类中心智能的发展方向应朝向可扩展、可信、具物理基础且可部署的智能系统，本文旨在为该领域提供一个连贯的框架和实用参考，以推动其与基础模型的深度融合。
...
TL;DR: 本文研究了在无分布假设条件下，为冻结的医学图像分割模型（nnU-Net，基于AMOS训练）添加器官特异性召回率保证的方法。研究发现该模型在AMOS上通过控制，但迁移到RAOS数据后7/12个器官超出α=0.10的误差阈值。通过对比RCPS和CRC两种风险控制方法，以及WSR投注界与Hoeffding-Bentkus界在本地重新认证中的效率，发现WSR仅需25例即可对六个一级器官重新认证，而CRC虽然样本需求更少（10-15例）但尾部风险更高。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型在临床应用前需要对其性能（尤其是召回率/漏检率）提供统计保证。传统方法依赖分布假设，当模型从训练域（AMOS）迁移到新域（RAOS）时，性能可能显著退化（如7/12个器官超出误差阈值），因此需要一种无分布假设的风险控制方法来校准每器官阈值，并对迁移后的模型进行本地重新认证，同时评估重新认证的样本成本。

Method: 采用无分布风险控制方法，为AMOS训练的nnU-Net校准每器官阈值；使用基于用例级体素假阴性率（FNR）的方法审计模型向RAOS数据集的迁移性能；对比三种方法：风险控制预测集（RCPS）提供高概率的总体均值风险控制，共形风险控制（CRC）提供较弱的期望控制，以及Waudby-Smith-Ramdas（WSR）投注界与Hoeffding-Bentkus（HB）界用于本地重新认证的样本量估计。

Result: AMOS上的控制通过，但迁移到RAOS后7/12个器官超出α=0.10阈值；较小校准集可能因保守或空洞阈值而掩盖违例；WSR投注界仅需25例本地病例即可重新认证六个一级器官（优于HB的30-40例），CRC仅需10-15例但个体案例尾部风险更重；二级器官均未能在25例下满足精度标准。

Conclusion: 冻结分割模型跨域迁移需要重新校准和本地认证；WSR投注界提供了比HB界更高效的重新认证方案，CRC样本效率最高但以个体尾部风险为代价；固定和全局阈值无法提供每器官级别的保证。
...
TL;DR: LumiTokens提出了一种全新的3D重光照框架，直接在潜在场景令牌（latent scene tokens）上进行变换，无需显式3D表示、渲染方程或物理分解。通过场景令牌编辑器联合处理场景令牌与光线令牌（如Plücker射线令牌），支持多种光照类型，并实现渐进式、可组合的光照编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的3D重光照方法要么依赖显式材质分解，要么使用基于扩散的视图空间生成，或两者结合，但每次更换光照条件都需要完整重新计算，效率低下且缺乏灵活性。作者观察到潜在场景表示编码了多视图信息且无固定物理语义，开辟了重光照的新设计空间。

Method: 提出了LumiTokens框架，核心创新包括：(1) Scene Token Editor，通过自注意力机制联合处理场景令牌与光线令牌，输出更新后的令牌可解码为多视图一致的重新光照图像；(2) 将所有光照信号（环境贴图、点光源、面光源）统一参数化为Plücker射线令牌，支持3D用户交互且无需显式空间结构；(3) 由于编辑器输出与输入在同一潜在空间，支持渐进式重光照——用户可以逐灯叠加，每次编辑在令牌空间内组合。

Result: 实验表明LumiTokens在重光照质量上与现有方法相当或更优，同时支持渐进式和可组合的光照编辑功能。

Conclusion: LumiTokens证明了直接在潜在场景令牌上进行重光照变换是可行且高效的，绕过了传统物理建模的复杂性，同时提供了更灵活、可交互、可渐进编辑的光照控制能力。
...
TL;DR: 本文提出了一种跨GelSight Mini传感器单元的3D力图估计泛化方法，通过两阶段流程（基于UniT的图像重构域自适应+基于U-Net的力图估计），分别实现了0.9338的SSIM和1.1294N的MAE。


<details>
  <summary>Details</summary>
Motivation: 触摸传感器虽然已快速工业化，但多数传感器在实验室中仍为手工制造，导致性能难以标准化，每个传感器单元都需要重复采集数据和训练模型，缺乏跨单元的泛化能力。

Method: 该方法包含两个阶段：第一阶段利用UniT-based模型进行域自适应，将输入触觉图像重构为通用触觉图像；第二阶段采用U-Net网络从重构图像中估计3D力图。

Result: 图像重构阶段SSIM达到0.9338±0.0358，力估计阶段MAE_F为1.1294±1.5934(N)。

Conclusion: 所提出的方法能够有效实现跨不同GelSight Mini传感器单元和版本的3D力图估计泛化，解决了传感器个体差异带来的标准化难题。
...
TL;DR: 本文提出一种单阶段端到端的野生动物重识别模型，在潜在空间中直接进行身份搜索，结合DINOv2和MegaDescriptor，并通过提示重识别特征增强潜在查询。初步结果显示mAP为30.584%，低于两阶段方法的44.89%，但定性结果展示了有效的边界框定位和身份识别能力。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的野生动物重识别方法采用检测与重识别两阶段流水线，该方案效率较低且复杂度高，论文旨在提出一种更简洁的单阶段端到端模型来解决细粒度野生动物重识别问题。

Method: 采用单阶段端到端架构，在潜在空间内完成身份搜索。使用DINOv2提取鲁棒的时空几何特征，借用MegaDescriptor进行野生动物重识别。通过提示重识别特征增强潜在查询，并由检测解码器在场景潜在空间中建立目标身份的对象边界。

Result: 初步实验显示，所提出方法的mAP为30.584%，与两阶段SOTA方法的44.89%相比具有竞争力。定性结果展示了有效的目标边界检测和动物身份识别效果。

Conclusion: 该单阶段方法简化了流水线并实现了端到端训练，尽管在精确度上略逊于两阶段方法，但仍展现出竞争性的性能，且定性验证了其有效性，为野生动物重识别提供了更高效的替代方案。
...
TL;DR: 本文研究了X射线荧光（XRF）显微镜成像与光学显微镜成像的配准问题，提出了一种基于采集几何约束的XRF瓦片组定位方法，通过GroupIoU量化评估定位精度，实验证明该方法显著优于独立瓦片定位方法。


<details>
  <summary>Details</summary>
Motivation: XRF显微镜可以映射元素分布，光学显微镜提供形态学背景信息，但两种模态在对比机制和分辨率上差异大，导致XRF视野在光学图像中难以定位。现有工作流大多独立放置每个XRF瓦片，忽略了采集元数据中已记录的相对扫描位置信息。

Method: 提出XRF瓦片组定位方法：为整个瓦片组估计一个光学帧放置，利用采集几何关系进行约束，并使用GroupIoU（组交并比）进行量化评估。在案例研究中比较了独立定位与组定位的差异，同时对比了NCC与互信息（MI）两种相似度指标，并引入多尺度粗扫描来提高定位精度。

Result: 在受控案例研究中，独立定位的GroupIoU为0.000（完全失败），而组定位达到0.931；用互信息替代NCC得到几乎相同的结果，说明结果不依赖于特定相似度指标；在多尺度案例中，利用粗XRF调查扫描连接细尺度瓦片组与光学图像，平均GroupIoU从0.694提升至0.856。

Conclusion: 案例研究支持在定位相关XRF瓦片时将采集几何作为显式约束，该方法能显著改善定位效果，且对相似度指标的选择不敏感。
...
TL;DR: 本文提出一种结合自由运行采集与分析-合成软件流水线的计算成像方法，用于缓解SPAD单光子相机在高光子通量条件下的堆积(pile-up)失真，同时解决数据带宽瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: SPAD单光子相机在3D感知中越来越受欢迎，但面临两大挑战：(i)高光子通量条件下存在非线性"堆积"失真；(ii)产生大量原始光子数据造成数据瓶颈。压缩采集虽能缓解数据传输问题，但会加剧死区时间失真，因为无法保留足够的光子检测历史信息以进行后处理堆积校正。

Method: 提出一种新的计算成像方法，将自由运行采集模式与分析-合成(analysis-by-synthesis)软件流水线相结合，以缓解堆积失真。该方法能够绕过压缩采集的信息损失限制，同时通过软件层面的校正来解决高光子通量下的非线性效应。

Result: 通过硬件仿真、全场景和单像素模拟实验，证明该方法能够在广泛的照明条件下可靠地捕获场景距离和反射率信息。实验表明该方法有效解决了压缩采集与堆积失真之间的矛盾。

Conclusion: 该工作将使带宽严重受限的高分辨率SPAD相机能够在现实世界的高光子通量场景中运行，推动SPAD相机在3D感知领域的实际应用。
...
TL;DR: 本文评估了免训练的视觉语言模型（VLM）在X射线荧光（XRF）与光学显微镜图像配准中的场域定位能力，提出了一种"提案-验证"工作流，在低对应性跨模态场景下能够恢复有效的定位结果。


<details>
  <summary>Details</summary>
Motivation: 不同显微镜模态的图像配准对关联同一标本的互补测量至关重要，但XRF图通常只覆盖光学图像的一小部分区域，且跨模态的外观和结构差异使得场域定位十分困难。

Method: 在两个数据集（同一切片高对应性和相邻切片低对应性）上评估免训练VLM定位，测试无约束和元数据约束搜索，并对比了几何控制、经典模板匹配、DINOv2和multiGradICON等替代方法，最终提出重复VLM预测作为候选、结合基于图像的相似性选择最终位置的提案-验证工作流。

Result: 直接VLM提示产生了内容相关的空间信号但单独使用不可靠；经典匹配在跨模态结构保持时最准确，但在低对应性数据集中失败；提案-验证工作流在低对应性场景下恢复了有用的定位能力。

Conclusion: 单一的VLM直接提示不足以保证可靠定位，但通过提案-验证框架将VLM与图像相似性结合，可以在跨模态结构差异较大的场景中提供有效的场域定位方案。
...
TL;DR: 本文提出FedCoRe（联邦跨模态表示补全）框架，解决联邦多模态学习中各医院模态缺失（EHR、胸片、心电图）的问题。FedCoRe不生成合成数据，而是学习表示或逻辑空间修正，利用配对样本进行监督，并通过验证集门控来决定是否部署补全模块。在MIMIC呼吸恶化任务中，该方法可恢复49%-52.8%因模态缺失导致的性能损失。


<details>
  <summary>Details</summary>
Motivation: 现实中的联邦多模态医疗模型假设所有站点拥有全部模态，但医院之间在EHR、胸片和心电图的可获得性上存在差异，导致部署时模态缺失会造成显著的性能下降。

Method: FedCoRe框架：(1) 学习表示空间或逻辑空间的修正而非生成合成ECG/CXR图像；(2) 客户端在同时拥有和缺失某模态时，对同一示例进行有无模态的评估以获取配对监督；(3) 仅拥有配对样本的客户端更新补全模块；(4) 验证集决定是否保留未更改的预测；(5) 冻结已训练的多模态预测器，确保测量差异仅来自补全模块。

Result: 在呼吸恶化任务中，隐藏ECG导致AUROC下降约0.085，配对示例FedAvg恢复0.0415（损失的49.0%）。对CXR隐藏场景，效果感知补全能恢复52.8%的性能损失。配对FedAvg可迁移部分效果，但验证集在输入缺少CXR的部署场景中保留无补全基线。CXR情况下补全行为更像是分类器逻辑修正而非真正的ECG/CXR恢复。

Conclusion: FedCoRe是一个验证门控的补全/修正框架：在支持的环境中可以恢复缺失模态的信号，但只有在配对样例和验证证据支持该模态时才应部署。其本质是任务特定的分类器修正，而非字面意义上的模态重建。
...
TL;DR: 本文提出了一种可配置的隐私保护MRI处理工作流，通过自适应解剖保留、交互式保留选择和集成质量控制扩展了基于深度学习的脑提取方法，在Renku可复现研究环境中实现，并在IXI数据集上验证了其可行性与实用性。


<details>
  <summary>Details</summary>
Motivation: 结构MRI体积可能保留面部和颅骨解剖信息，存在隐私泄露风险。现有的基于深度学习的脑提取方法通常只产生单一固定输出，限制了不同应用场景中对隐私保护和解剖保留之间不同平衡需求的灵活性。

Method: 工作流采用SynthStrip进行自动化脑提取，随后通过形态学掩膜扩展生成可配置的壳层式保留级别；引入交互式保留框架让用户比较和选择不同保留配置；集成质量控制框架提供多平面可视化和脑掩膜叠加验证。系统使用Python和开源神经影像库在Renku可复现研究环境中实现。

Result: 使用IXI数据集的T1加权MRI数据评估，实验结果表明脑提取在解剖学上合理，且能生成可配置的保留输出，并通过系统化的可视化验证获得支持。

Conclusion: 主要贡献是提供了一个模块化且可复现的MRI预处理框架，以可配置的解剖保留、交互式用户引导处理和集成质量控制增强了基于深度学习的脑提取方法，为隐私导向的神经影像研究和协作医学图像分析提供了实用基础。
...
TL;DR: 本文提出"Affordance Sheet"文档规范，用于标准化视觉可供性预测（affordance prediction）研究的任务定义、输入模态、模型架构、训练信息、数据集和实验协议，以解决该领域因问题表述不统一、注释不一致和实验报告不完整导致的评测困难问题。


<details>
  <summary>Details</summary>
Motivation: 视觉可供性预测领域存在异构的问题表述、不一致的数据集标注、不完整的实验协议报告以及部署条件信息匮乏等问题，这些因素严重阻碍了公平的基准测试和性能比较，亟需一套标准化的文档规范来促进透明性和可复现性。

Method: 提出"Affordance Sheet"文档规范，系统记录任务表述及其输入模态、模型架构与训练信息、数据集详情以及实验协议，作为可供性预测研究的标准文档框架。

Result: Affordance Sheet能够支持可复现的基准测试，并为真实应用场景下可供性模型的可靠评估提供保障，涵盖泛化到新条件及人类安全性等方面。

Conclusion: 通过引入Affordance Sheet文档规范，可以有效提升可供性预测研究的透明性和公平性，促进该领域可靠评估和可复现基准测试的发展。
...
TL;DR: 本文提出了一种名为\algname的视觉-语言模型测试时自适应方法，通过将推理和自适应的目标统一为Wasserstein最优传输（OT）框架，利用OT产生的稳健伪标签和软标签InfoNCE损失，在分布偏移场景下显著提升VLM的分类性能，最高超越现有最优方法7%，同时保持高效性。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM测试时自适应方法严重依赖从原始嵌入相似度直接预测的噪声伪标签，在分布偏移下不可靠并误导自适应过程；同时现有工作使用粗略的替代目标函数进行自适应，无法显式建模跨模态的样本级关系，导致自适应目标与推理目标不匹配，性能提升有限。

Method: 该方法包含两个关键部分：(1) 推理阶段：将零样本图像分类任务建模为通过Wasserstein OT编码的跨模态对齐问题，提供样本级稳健伪标签；(2) 自适应阶段：采用软标签InfoNCE损失基于OT诱导的伪标签自适应VLM，通过对比学习显式建模个体图像-文本对的细粒度关系。此外，理论上证明了InfoNCE损失可以重新表述为Wasserstein OT形式，从而统一推理和自适应的目标。

Result: 大量实验表明，该方法在效果和效率上均表现优越，相比最优方法性能最高提升7%，并达到了最先进的效率水平。

Conclusion: 通过将推理和目标统一为Wasserstein OT框架，本文成功解决了VLM测试时自适应中推理与自适应目标不一致的问题，实现了推理与自适应的相互促进，为VLM在分布偏移下的鲁棒部署提供了新的思路。
...
TL;DR: 本文提出TTSD-FAR框架，通过冻结教师模型引导自适应低秩学生模型的测试时自蒸馏，结合Fisher信息锚定恢复机制，解决大规模视频语言模型在模态缺失场景下的情感识别持续适应问题。


<details>
  <summary>Details</summary>
Motivation: 大型视频语言模型（LVLMs）在野外多模态情感识别中表现优异，但实际部署时模态可能缺失或含噪，导致分布偏移。现有的熵最小化或困惑度降低的TTA方法不适用于自回归LVLMs，而RAG在模态较弱时性能退化。此外，缺乏真实标签验证会导致适应过程中漂移累积，模型逐渐偏离可靠解。

Method: 提出测试时自蒸馏（TTSD）框架：冻结在完整模态上训练的教师模型，通过自蒸馏引导一个仅更新极少参数的低秩自适应学生模型。同时提出Fisher锚定恢复（FAR）机制，通过监测Fisher信息的稳定性来区分收敛与漂移，当检测到分布偏移时将学生模型恢复到教师锚点。

Result: 在MELD、DFEW和BAH三个数据集上，在0%-50%模态缺失条件下，TTSD-FAR统一的自适应-恢复设计在长时适应上持续优于基于熵的适应方法、RAG和基于困惑度的生成方法，而缺乏恢复机制的基线会逐步退化，TTSD-FAR则保持一致性能。

Conclusion: 统一的自蒸馏与锚定恢复设计能有效应对任意缺失模态模式，并在持续适应过程中保持稳定性，为LVLMs在部分观测条件下的多模态情感识别提供了可行的解决方案。
...
TL;DR: DAV4提出了一个基于单目视频的动态4D场景重建框架，核心创新是将黎曼流匹配（RFM）应用于4D高斯泼溅参数，在非欧几里得流形（尺度、旋转、不透明度）上定义概率路径。通过消融实验，RFM的独立贡献为F-score提升0.044（从0.762到0.806），预训练需360 GPU小时，在超过10,000个场景的大规模部署中可摊销。该框架在动态重建和新视角合成上优于先前的Depth Anything模型和逐场景4D-GS，且不使用人工标注深度作为训练损失。


<details>
  <summary>Details</summary>
Motivation: 动态4D场景重建面临的两个核心挑战：一是现有方法在非欧几里得流形上的概率建模不足，导致中间状态无效；二是需要明确量化各组件（RFM、测试时优化、预训练）的独立贡献，以及大规模部署的计算成本可负担性问题。

Method: 将黎曼流匹配（RFM）应用于4D高斯泼溅（4D-GS）的参数空间，在尺度、旋转、不透明度等非欧几里得流形上直接定义概率路径，确保所有中间状态有效。通过控制变量实验隔离RFM的贡献，并用负高斯对数似然和期望校准误差进行不确定性量化。

Result: RFM在相同数据、架构和TTO条件下取得F-score 0.806，相比确定性MLP基线的0.762提升了0.044，这是RFM的独立贡献。预训练成本为360 GPU小时，在超过10,000个场景的大规模部署中可摊销。DAV4在动态重建和新视角合成上优于先前模型，且无需人工标注深度。

Conclusion: DAV4证明了将流匹配扩展到非欧几里得流形对4D高斯泼溅参数建模的有效性，同时提供了更准确的计算成本分析和不确定性量化，为大规模动态场景重建提供了一种无需人工标注深度标签的可行方案。
...
TL;DR: 本研究通过实验证明，ViT-S学生模型从自监督教师模型蒸馏注意力图后，在分布外鲁棒性上仍落后于微调模型，但该差距主要源于训练不充分（"训练成熟度"伪影），而非注意力结构本身的缺陷。通过结构干预实验，作者推断鲁棒性差距 reside 在特征层面而非可见的注意力结构中。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，蒸馏教师注意力图的ViT能恢复大部分分布内精度，但在分布偏移下与微调仍有差距。但从未有人直接测量注意力结构中的转移质量并将其与鲁棒性联系起来。本文旨在构建这种测量工具，以定位差距来源。

Method: 在ImageNet-100上训练ViT-S学生模型蒸馏自监督教师的注意力图，设计了三组实验：(1)测量蒸馏学生与教师注意力结构的距离（与微调对比）并追踪训练过程中的漂移；(2)在更小规模（14倍更少参数、10倍更少数据）下检验分布外差距，比较完整训练与提前停止的结果；(3)人为干预降低跨行冗余度，在两种匹配精度的方式下观察鲁棒性是否变化。

Result: 三个关键发现：(1)注意力转移几乎完美且持久，蒸馏学生的注意力比微调更接近教师两个数量级，且不随额外训练漂移；(2)在小规模下鲁棒性差距真实存在但有时间轴，完整训练使2/3种子差距降至预注册阈值以下，说明终点差距主要是训练成熟度伪影；(3)强行降低结构分离度没有产生可检测的鲁棒性响应。结论：鲁棒性差距不在可见注意力结构中，而在特征层面。

Conclusion: 通过"排除+干预"的实验设计，研究证明在该测量尺度下，注意力图展示的是模型"看向哪里"，而非模型"知道什么"。鲁棒性缺口驻留在特征表示中，而不是注意力结构中，这为理解ViT蒸馏与鲁棒性关系提供了新的方法论视角。
...
TL;DR: 本文提出了JSL-DC，这是日本手语（JSL）领域最大的视频数据集，包含来自19位手语者的36.7K个视频、270个JSL词汇。数据集的构建过程以聋人为中心，由聋人语言学家和Coda语言学家共同设计词汇表，并由聋人语言学家参与两阶段审核。此外，论文还提供了语言学家推导的易混淆手势区分描述，并提出了受这些描述启发的新模型，在易混淆子集上比现有最优方法提升9.8%的准确率。


<details>
  <summary>Details</summary>
Motivation: 聋儿手语学习至关重要，但95%的聋儿出生于听力正常的父母家庭，这些父母通常不精通手语。手语识别技术可以为父母提供学习工具，帮助他们与孩子沟通。然而日本手语（JSL）缺乏大规模、多手语者的数据集，这阻碍了能泛化到新用户的模型开发。

Method: 1) 构建JSL-DC数据集：包含36.7K视频、19位手语者、270个手语词汇，为JSL领域最大的视频数据集；2) 采取以聋人为中心的数据构建流程：词汇表由聋人和Coda语言学家选定，参与者均为日常使用JSL的聋人，数据经过聋人语言学家两阶段审核；3) 提供语言学家推导的易混淆手势区分描述；4) 提出受这些语言描述启发的识别模型。

Result: 提出的基于语言描述启发的模型在易混淆手势子集上比最先进的手语识别方法提升了9.8%的准确率。

Conclusion: 数据集及其语言描述将以CC-BY 4.0许可证公开发布，以加速手语识别领域的研究，尤其是在模型泛化到新用户方面。
...
TL;DR: CoMVS-GS是一个结合多视图立体与3D高斯泼溅的表面重建框架，通过稠密MVS点初始化、PatchMatch-3DGS互监督以及Delaunay图割网格提取，在弱观测和遮挡区域实现更准确的几何重建，在多个数据集上达到具有竞争力的重建质量和渲染性能。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在高效新视角合成方面表现优异，但在弱观测和遮挡区域，高斯原语可能生长为不稳定或几何不一致的结构，导致网格重建精度不足。

Method: 1) 从稠密多视图立体点初始化高斯原语，采用预展平尺度和法线对齐方向；2) 引入PatchMatch-3DGS互监督机制，高斯渲染的深度和法线用于初始化PatchMatch细化，细化的PatchMatch深度反过来监督高斯优化；3) 用Delaunay图割网格管线替代TSDF体素融合进行表面提取。

Result: 在DTU、GauU-Scene V2和MatrixCity数据集上，CoMVS-GS在物体级重建上保持竞争力，在户外场景中提高了几何精度和网格紧凑性，同时保持高渲染质量。

Conclusion: CoMVS-GS通过将MVS几何先验融入高斯泼溅框架，有效提升了弱约束区域的网格重建质量，验证了多视图立体与高斯泼溅结合的有效性。
...
TL;DR: 本文提出COSTA框架，解决了航空点云语义分割在跨域场景下的开放集测试时适应问题。它通过将闭集点级适应转变为簇中心开放集语义传播，利用测试时适应后的特征分布提取可跨标签空间迁移的语义质心，并结合开放词汇视觉语言模型实现按需分割，在三个异质域基准上最高达到70.09% mIoU。


<details>
  <summary>Details</summary>
Motivation: 航空点云语义分割在跨域场景下面临严重的泛化危机。现有测试时适应方法受限于闭集标签假设和不可扩展的逐点分割流程，无法处理语义类别空间发生迁移的情况，即目标域可能包含源域标签空间之外的新类别。

Method: COSTA的核心是将闭集点级适应转变为簇中心开放集语义传播。具体流程：1）通过成熟的测试时适应技术弥合域差距；2）在适应后的特征空间中基于相似性分布将目标域点云聚为少量语义簇；3）利用开放词汇视觉语言模型获取高置信度伪标签，通过簇级投票传播到所有点。

Result: 以DALES为源域，COSTA在三个具有不同域和异质类别空间的航空点云基准上实现了按需分割，在新设置下最高达到70.09% mIoU。

Conclusion: 簇中心范式使航空点云在显著域差距和混合语义偏移条件下能够有效进行测试时适应，突破了闭集假设的局限，实现了超越源标签空间的开放集按需语义分割。
...
TL;DR: 本文提出SparsePR，一种视频Transformer的无训练块稀疏注意力加速方法。该方法通过响应耦合分区（Response-Coupled Partitioning）和探针拟合残差重建（Probe-Fitted Residual Reconstruction）的组合，在四种异构视频生成与世界模型上降低了注意力重建误差，在22.0-26.0%的执行对密度下保持生成质量，实现1.48x-2.61x端到端加速。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的块稀疏注意力虽然可以加速视频Transformer，但行级注意力集中本身并不足以确定一个可执行的稀疏算子。问题在于：共享块路由的查询可能支持集重叠很少，且仅保留注意力质量无法决定由跳过交互引起的softmax后误差。论文旨在解决这一几何与误差预测方面的根本缺陷。

Method: SparsePR包含两个核心组件。首先，响应耦合分区（Response-Coupled Partitioning）将采样的查询键响应形成配对的K/V组，利用质心诱导查询-响应对齐实现共享路由；其次，探针拟合残差重建（Probe-Fitted Residual Reconstruction）利用少量精确查询行，在探针残差所观察到的输出子空间内校准一个随调用变化的仿射修正。

Result: 在四种异构视频生成和世界模型上，SparsePR一致降低了注意力重建误差。消融实验表明探针拟合贡献了大部分误差降低，而响应耦合分区降低硬丢弃误差并在有限探针预算下改善重建。在22.0-26.0%实际执行对密度下保持生成质量，实现1.48x-2.61x端到端加速。

Conclusion: SparsePR证明了通过响应耦合分区和探针拟合残差重建可以显著改善训练无关稀疏注意力的重建精度，在降低计算成本的同时保持视频模型生成质量，为视频Transformer的稀疏化加速提供了新的实用方案。
...
TL;DR: 本文提出DyG²T，一个通过空间补全与时间判别关键点表征、并建模粒子图上多尺度交互来推断物体运动轨迹的动力学建模框架，在合成和真实数据集上取得了准确的结果并表现出良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的动力学建模方法将粒子表征压缩为稀疏关键点后用局部约束交互建模其演化，这丢失了细粒度局部细节，且在时空尺度上无法进行判别性交互建模，导致轨迹漂移和外观预测不准确。

Method: DyG²T包含三个核心模块：(1) 空间上，通过聚合邻近原始粒子位置补全关键点的局部细节，并编码关键点间相对偏移以增强几何结构感知；(2) 时间上，提出时间解耦网络(TDN)在潜在空间中识别主导的帧间变化并放大帧间差异，再通过时间注意力聚合帧间演化线索；(3) 采用粒子图Transformer通过全局注意力保留关键点间的判别性长程依赖关系，缓解局部约束建模导致的表征同质化。

Result: 在合成数据集和真实世界数据集上的实验表明，DyG²T能够实现准确的动力学建模与推理，并展现出强大的跨物体和真实世界泛化能力。

Conclusion: DyG²T通过空间补全与时间判别相结合的关键点表征学习，以及全局注意力驱动的粒子图交互建模，有效解决了现有方法在细粒度细节丢失和局部约束建模上的局限，为物体动力学建模提供了更稳健的方案。
...
TL;DR: 本文提出一种双分支框架，利用TVUS导出的原型库适配MedSAM3，实现经阴道超声与MRI两种模态下的卵巢分割，在子宫内膜异位症数据集上相比SOTA方法提升超5个百分点。


<details>
  <summary>Details</summary>
Motivation: TVUS和MRI对子宫内膜异位症影像分析提供互补信息，但现有研究多聚焦单模态分析或疾病分类，跨模态卵巢分割尚未被充分探索。MRI中卵巢目标小且与周围盆腔结构边界模糊，分割难度大，亟需跨模态分割方法。

Method: 提出双分支框架：将MedSAM3与TVUS生成的原型库（prototype bank）相结合，通过原型库对齐两种模态在解剖学上一致的特征表示，实现跨TVUS和MRI的卵巢分割。

Result: 在子宫内膜异位症相关的TVUS和MRI数据集上进行广泛实验，双分支方法相比多种SOTA方法在定量和定性指标上均提升超过5个百分点；消融研究验证了原型库各组件及源TVUS域热身预训练的重要性。

Conclusion: 所提出的基于原型库的双分支跨模态分割框架有效解决了MRI中卵巢分割困难的问题，为子宫内膜异位症影像分析中的多模态协同提供了可行方案。
...
TL;DR: 本文提出OptiModNet，一种轻量级混合架构模型，用于视盘和视杯的精确分割，在REFUGE2数据集上以仅3.73 GFLOPs和1.93M参数的极高效率超越现有方法2.5%以上。


<details>
  <summary>Details</summary>
Motivation: 青光眼的早期检测需要高精度的视盘和视杯分割，但现有深度学习方法（如UNet、ViT、Diffusion模型）虽性能优秀却计算开销巨大，难以在资源受限的临床环境和大规模筛查中部署。因此需要兼顾分割精度和计算效率的轻量级模型。

Method: OptiModNet是一个轻量级混合架构，在网络的多个阶段集成多种注意力机制以增强局部和全局特征表示；同时引入聚合金字塔损失（Aggregated Pyramid Loss）在不同解码器深度监督预测，促进梯度流动和结构一致性。

Result: 在REFUGE2数据集上对视盘和视杯分割任务进行评估，性能超越现有方法2.5%以上，达到state-of-the-art水平，同时仅需3.73 GFLOPs和1.93M参数，计算效率极高。

Conclusion: OptiModNet成功在分割性能与计算效率之间取得了良好平衡，为青光眼的大规模筛查和资源受限环境中的临床部署提供了可行方案。
...
TL;DR: 本文研究AI生成图像检测的迁移问题，提出基于先验条件的高斯判别阶梯（闭式解的分类头），在39个公开数据集（710万图像）上验证其性能可与已发布的检测器头相当甚至更优，揭示了训练先验敏感性和特征表示依赖性的关键洞见。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的合成图像已无处不在，但检测器在生成器、提示/风格和源域同时变化时往往失效。现有研究缺乏对"分类头训练在何种情况下能超越现代特征本身的可分性"的系统性理解，需要更严格的分析基线来评估AI生成图像检测的迁移性能。

Method: 将AI生成图像检测建模为一个由训练先验、冻结编码器特征空间和决策规则组成的迁移系统。提出先验条件的高斯判别阶梯——基于一阶和二阶特征统计量在嵌套协方差假设下构造的闭式解分类头。构建Percept-Lens统一协议，覆盖39个公开数据集共710万图像，对头部件与先验/编码器进行匹配比较。

Result: 在Percept-Lens协议下，高斯判别阶梯的最佳层级在与先验和编码器匹配时，经常与已发布的AI生成图像检测器头性能相当，有时甚至更优。同时量化了训练先验的强敏感性、基于矩的分类头的数据效率，以及高斯偏移度量对特征表示的依赖性。

Conclusion: 研究结果强调需要对AI生成图像检测迁移进行（先验、编码器、头）三级报告，并呼吁更强的分析性基线，以更准确地评估和比较不同检测方法在复杂迁移场景下的真实表现。
...
TL;DR: 本文提出StateTrace框架，赋予视频大语言模型（VideoLLMs）显式的"隐藏状态时空推理"能力，解决长视频中目标物体不可见时的状态推断问题，并构建了含1,427个样本的HSR-Bench基准，实验显示在多个VideoLLM上均取得显著性能提升（如VideoLLaMA3从39.6提升至64.2）。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在长视频时空推理中表现不佳，当目标物体在视频中不可见时，模型常常将"不可见"误判为"未知"，缺乏对物体在隐藏期间状态演化的推理能力。

Method: 提出StateTrace，一个以对象为中心的框架，包含三个关键组件：（1）可复用的时空状态记忆，组织对象轨迹、对象间关系和状态转换事件；（2）推理时检索与问题相关的状态演化轨迹并转换为紧凑的推理线索；（3）使模型能够显式推理物体为何消失、隐藏期间状态如何演化、查询时刻状态是否应持续。同时构建了HSR-Bench诊断基准，包含1,427个视频QA样本。

Result: 在多个VideoLLM和公开基准及HSR-Bench上的大量实验表明，StateTrace持续提升性能，例如将VideoLLaMA3在HSR-Bench上的得分从39.6提升至64.2，提升幅度显著。

Conclusion: StateTrace通过显式的对象中心状态记忆和检索机制，有效解决了长视频中隐藏状态推理的难题，为VideoLLMs提供了可靠的推理能力增强方案，且所构建的HSR-Bench为后续研究提供了标准化评估平台。
...
TL;DR: 本文提出EpigraphNet，一种基于分割引导的Transformer流水线，用于自动识别古代楔形文字。通过亮度自适应形态学预处理、SAM2-Large零样本分割生成干净的二进制符号掩码，再由带逆频率类别加权的微调ViT-B/16进行分类，在Persepolis Fortification Archive的132类基准上达到86.41%的top-1准确率，比最强CNN基线（ResNet-101）高出17.21个百分点。


<details>
  <summary>Details</summary>
Motivation: 古代楔形文字的自动识别面临复合信号退化问题：三维浮雕产生空间变化的照明和投影阴影、表面侵蚀引入与真实符号印象重叠的结构化噪声、以及141个符号类别间的严重类别不平衡破坏了分类器可靠性。现有方法难以同时应对这些挑战。

Method: 提出EpigraphNet流水线：(1) 对1,239张带注释的泥板图像进行亮度自适应形态学预处理；(2) 使用SAM2-Large零样本分割生成干净的二进制符号掩码；(3) 采用带逆频率类别权重的微调Vision Transformer (ViT-B/16)进行分类。整个流水线在NVIDIA A100 GPU上每个符号约需18毫秒。

Result: EpigraphNet在132类基准上达到86.41%的top-1准确率，相比最强CNN基线（ResNet-101，69.20%）提升17.21个百分点，相比四种现代骨干网络（DeiT-B/16、Swin-B、ConvNeXt-B、EfficientNet-B4）提升5.31-12.91%。频率与每类性能间的Spearman相关性更低，表明在常见和稀有类别上识别更均衡。

Conclusion: 分割引导的Transformer方法有效解决了楔形文字识别中的光照不均、噪声干扰和类别不平衡问题，在性能上显著优于CNN基线，且对稀有类别具有更均衡的识别能力。代码已开源。
...
TL;DR: 本文提出了一种名为PATE-Forensics的深度伪造取证方法，采用"感知即工具"(Perception-as-Tool)范式，将检测和定位从解释生成中解耦，利用基于DINOv3的取证感知工具进行多粒度检测和线索引导定位，再通过通用MLLM生成解释。在DDL-X Track 3上取得了0.89的官方最佳成绩，超过第二名0.19分。


<details>
  <summary>Details</summary>
Motivation: 现有的可解释深度伪造取证方法通常依赖任务适配的MLLM来同时处理检测、定位和解释三个任务，这在架构上存在耦合，限制了各任务的性能。作者受智能体工具使用范式启发，希望将检测定位与解释生成解耦，从而提升各环节的专业性和整体效果。

Method: 提出PATE-Forensics方法，包含三个核心部分：(1) 基于DINOv3的取证感知工具，将检测与定位紧密耦合；(2) 多粒度检测模块，整合全局、补丁级和分段级证据；(3) 线索引导的定位模块，通过将补丁级和分段级证据空间化为伪造得分图来引导密集掩码预测。原始图像和工具输出的取证感知结果构成结构化取证上下文，供通用MLLM在提示约束下生成解释，无需任务特定微调。

Result: 在DDL-X Track 3上，PATE-Forensics取得了0.89的最佳官方分数，比第二名高出0.19分，展示了该方法的显著优势。

Conclusion: PATE-Forensics通过"感知即工具"范式有效解耦了检测定位与解释生成任务，证明了利用通用MLLM配合专门的取证感知工具即可实现高性能的可解释深度伪造取证，无需任务特定的模型微调。
...
TL;DR: MR-IQA-2提出了一种面向盲图像质量评估的actor-editor-judge框架，通过解耦推理与评分的信用分配，并利用图像编辑-反射机制提供可验证的监督信号，从而提升MLLM在IQA中推理的忠实度与可靠性，同时在与人类评分对齐方面取得有竞争力的表现。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在IQA中虽然通过人类评分进行监督，但推理过程往往不忠实于图像质量。仅靠评分准确率无法保证推理可靠性，且共享奖励会混淆监督来源，甚至在评分正确时强化不忠实的推理，因此需要将推理与评分监督解耦并提供可验证的推理监督。

Method: 提出MR-IQA-2框架，基于演员-编辑-法官（actor-editor-judge）范式实现推理-编辑-反射闭环。演员生成图像质量推理，编辑者依据识别的质量因素对图像进行修改，冻结的法官比较原始与编辑后图像，为演员的推理提供反射式监督。同时采用细粒度信用分配，将推理监督（来自法官反馈）与评分监督（来自人类标注）解耦，并通过掩码token级更新区分两类信号，同时保持推理到评分的因果关系。

Result: 在多个IQA基准上，MR-IQA-2达到了与人类评分具有竞争力的对齐效果。此外，视觉反射机制使模型获得更丰富、更忠实的视觉理解能力，超越了单纯的评分任务，可用于图像质量优化及相关的下游任务。

Conclusion: 通过解耦推理与评分的监督来源，并引入基于图像编辑的可验证反射监督，MR-IQA-2有效提升了盲IQA中推理的忠实度和可靠性，兼具评分性能与更广泛的视觉理解潜力。
...
TL;DR: 本文提出Dynamic SpectraFormer，一种基于频域Transformer的水下图像增强方法，通过超高清稀疏频谱注意力模块和动态频谱权重生成层，同时处理高频和低频失真，在多个基准数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 水下图像因光在水中的折射和吸收，存在颜色失真、雾化和能见度差等问题。传统方法主要聚焦于空间域，难以同时修正混合失真（低频颜色亮度失真和高频边缘纹理失真），因此需要一种能够同时处理不同频率失真的新方法。

Method: 提出Dynamic SpectraFormer框架，包括两个关键组件：(1) 超高频稀疏频谱注意力模块（ultra-high-resolution sparse spectrum attention），在不损失通用逼近能力的前提下捕获长程依赖；(2) 动态频谱权重生成层（dynamic spectrum weight generation layer），作为自适应频谱波段选择器，增强关键频带并抑制无关频带。整体通过频域Transformer实现高、低频失真联合校正。

Result: 大量消融实验和对比评估表明，Dynamic SpectraFormer在多个水下图像增强基准数据集上均显著提升了图像质量，验证了该方法在处理高频与低频混合失真方面的有效性。

Conclusion: Dynamic SpectraFormer通过频域视角解决了传统空间域方法难以兼顾高低频失真校正的局限，为水下图像增强提供了有效的解决方案，且代码已开源。
...
TL;DR: SPARC是一种仅需k≤8张已验证正常图像即可完成少样本校准的工业异常检测方法。它通过在编码器与检测器之间拦截patch特征，利用闭式解、基于空间索引的子空间投影来消除部署时的干扰，无需梯度或权重更新，适用于多种检测器架构。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测器在校准时与部署环境之间存在分布偏移（如光照、夹具位置或传感器特性差异），导致检测性能严重下降。由于带标签的异常样本稀缺，自适应部署环境需要仅利用少量已验证正常图像进行校准。

Method: SPARC采用少样本校准策略：拦截编码器与检测器之间的patch特征，通过逐单元子空间投影移除闭式估计的部署期干扰向量。该方法利用代数饱和秩r=k-1在编码器原生patch网格上进行操作，无需反向传播或权重更新，可通用于记忆库型、密度型、原型型和互相关型检测器。

Result: 在偏移倾向基准上，SPARC对全部7种检测器的图像AUROC提升13.8个百分点，AU-PRO0.3提升3.5个百分点；在无工程偏移的基准上，变化较小且方向有正有负。对照实验表明增益来自逐单元子空间结构而非校准图像本身，进一步的消融实验支持饱和秩选择并表征了对骨干网络和校准条件的敏感性。

Conclusion: SPARC通过逐单元子空间投影的闭式校准方式，在极少量正常样本条件下有效缓解了工业异常检测的分布偏移问题，是一种轻量、通用且有效的校准方案。
...
TL;DR: 本文提出OmniHandwritingOCR，一个用于系统评估多模态大语言模型（MLLMs）手写OCR能力的诊断基准测试集，涵盖手写文本识别和手写数学公式识别两大任务，包含6个子任务、12个子集、共77.57K张标注图像，并评估了13个开源和闭源系统。


<details>
  <summary>Details</summary>
Motivation: 现有OCR基准测试主要聚焦印刷文本或干净的单行输入，对真实手写场景（如多语言手写、书写者错误、结构化复杂的数学表达式）覆盖不足，而多模态大语言模型在实际文档处理管线中越来越多被用作OCR系统，其真实手写读取能力尚未被充分探索。

Method: 构建了一个诊断性基准测试集OmniHandwritingOCR，包含手写文本识别和手写数学表达式识别两大任务，覆盖6个子任务和12个子集，总计77.57K张标注图像，数据来自公开数据集和新收集的学生手写内容；特别设计了一个难度分层的多行公式语料库以测试结构复杂性增加下的鲁棒性；在统一协议下使用5种互补指标评估13个开源和闭源系统。

Result: 当前系统在手写OCR方面远未达到忠实转录水平：在复杂的多行公式上性能急剧下降；模型排名在不同语言和公式设置下存在差异；多个生成式模型会产生看似合理但视觉上无依据的幻觉性修正。

Conclusion: OmniHandwritingOCR为诊断多模态模型在手写OCR场景中的语言、内容、结构和视觉锚定失败模式提供了一个具有挑战性的测试平台。
...
TL;DR: 本文提出FD-CanKD，一种面向紧凑目标检测器的频域解耦交叉注意力知识蒸馏框架，在预测级、关系级和频域级三个互补层面迁移教师知识。在COCO数据集上，固定50轮从头训练的实验表明其与代表性检测器蒸馏基线相当，额外20轮微调后达到48.87 mAP50:95/65.84 mAP50/53.40 mAP75，且蒸馏模块训练后全部移除，部署学生模型参数保持19.7M不变。


<details>
  <summary>Details</summary>
Motivation: 紧凑目标检测器适合资源受限的视觉感知场景，但其表征能力有限，与大型模型相比存在精度差距。现有的检测器蒸馏方法通常仅依赖预测级监督或单一特征对齐目标（如响应、分布、相关性或频域匹配），缺乏多层面互补的知识迁移机制，难以充分弥补小模型与教师模型之间的性能鸿沟。

Method: FD-CanKD在三个互补层面迁移教师知识：(1) 预测级（head-level）监督；(2) 关系级（relation-level）非局部上下文迁移——学生特征通过交叉注意力聚合教师侧空间上下文；(3) 频域级（frequency-level）组件选择性对齐——频域感知对齐保留互补的结构性和细节敏感线索。在YOLOv12教师-学生配置下进行受控实验验证。

Result: 在COCO数据集上，固定50轮从头训练的对比实验显示FD-CanKD与代表性检测器知识蒸馏基线相当。蒸馏后继续微调20轮可得到比仅检测器微调更强的学生模型，达到48.87 mAP50:95、65.84 mAP50和53.40 mAP75。所有蒸馏模块在训练后移除，部署学生模型参数仅19.7M。

Conclusion: FD-CanKD证明了多层面互补知识蒸馏（预测级、关系级、频域级）对紧凑检测器性能提升的有效性，且在不增加部署开销的前提下显著缩小了与大型模型的精度差距。频域解耦与交叉注意力机制的结合为检测器蒸馏提供了新的研究思路。
...
TL;DR: ReX-Shot是首个从单张图像联合控制视点、焦距和参数化摄影效果的统一生成框架，通过隐式变换的基座模型特征引导降低投影偏差，并将焦距放大建模为几何引导的超分辨率问题，实现近实时的交互式重摄影。


<details>
  <summary>Details</summary>
Motivation: 现有单图像重摄影方法将视点、焦距和摄影效果等内在耦合的成像因素分开处理，导致在联合控制下表现不佳：新视角合成在焦距变化时产生几何畸变，超分辨率和指令编辑局限于2D无法扩展到新视角。

Method: 1) 使用隐式变换的基座模型特征进行稳健的目标视角引导，减少几何误差带来的投影偏差；2) 将焦距放大建模为几何引导的超分辨率问题，利用生成式细节先验恢复稀疏3D重采样中丢失的细节；3) 基于3D感知生成主干，将摄影效果控制从2D滤波提升为3D感知外观编辑，保持跨视角和焦距的内容一致性。

Result: 实验表明ReX-Shot在视点、焦距和摄影效果三种控制维度上均优于代表性基线方法，并支持近实时的交互式重摄影。

Conclusion: ReX-Shot是首个统一控制视点、焦距和参数化摄影效果的单图像重摄影框架，通过几何与相机场景锚定的生成式方法解决了多因素联合控制下的畸变和细节丢失问题。
...
TL;DR: 本文提出了一种基于多模态大语言模型的交互式预测修正工作流Molmo2Fish，用于声纳鱼类追踪数据集的跟踪预测修正，通过引导式和未引导式任务评估其性能，结果显示鱼类追踪和轨迹修正表现良好，但自然语言引导的融入仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉在大规模生态数据集中的自动化识别任务日益普及，但多目标追踪等复杂任务仍具挑战。研究者探索通过人机协同流程利用不完美的预测结果，本文旨在提出一种新颖的交互式预测修正方法，解决生态学工作流中视觉模型预测不精准的问题。

Method: 提出Molmo2Fish系统，将交互式预测修正工作流设计为与多模态大语言模型的对话过程，并将其定制应用于声纳鱼类追踪数据集。在引导式（natural language guidance）和未引导式任务中评估系统对自身预测轨迹和外部轨迹的修正能力。

Result: Molmo2Fish在鱼类追踪和轨迹修正任务上取得了较高性能，但在结合自然语言引导方面仍有较大改进空间。

Conclusion: 该研究作为概念验证，展示了多模态大语言模型在生态数据集交互式追踪修正中的潜力，同时指出自然语言引导能力是需要进一步优化的方向。代码和数据已公开。
...
TL;DR: 本文提出了VA-Judger，一个用于联合视频-音频生成的链式思维全能奖励模型，并构建了VAPref-10K人类偏好数据集和VA-Judger-Bench基准，通过维度级强化学习实现更符合人类偏好的奖励信号，显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的联合视频-音频生成模型的强化学习后训练依赖将各质量维度指标简单组合作为奖励信号，但这些指标分别评估感知维度，无法捕捉文本提示、视频和音频之间整体的语义和时间连贯性。优化这些指标会导致奖励黑客问题，生成高分数但人类看来不连贯或不忠实的内容。

Method: 1) 构建大规模人类偏好数据集VAPref-10K（9K提示，10.3K细粒度成对比较）；2) 提出VA-Judger-Bench基准，包含域内和域外模型比较；3) 提出VA-Judger奖励模型，采用三阶段训练：先从质量差异明显的样本对学习结构化输出和粗粒度偏好判别，再通过拒绝采样蒸馏可靠偏好解释处理近似质量比较，最后进行维度级强化学习将人类反馈分解为各质量维度以获得更密集的奖励信号。

Result: 实验表明VA-Judger在域内和域外评估中均优于度量基线在预测人类偏好方面的表现，使用其人类对齐的奖励对音频-视频生成模型进行后训练也带来了生成质量的显著提升。

Conclusion: VA-Judger通过链式思维推理和维度级强化学习有效地解决了现有奖励信号无法捕捉人类对联合视频-音频生成偏好语义一致性的问题，为联合生成模型的后训练提供了更可靠的对齐奖励。
...
TL;DR: 本文提出CDGP（对比双高斯过程），一种弱监督异常分割框架，通过建模正常与异常诱导变量的预测分布，利用后验优势统计量标准化预测均值差与联合预测不确定性，实现缺陷定位与图像级置信度，在MVTec AD~2、KSDD2和VisA数据集上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 工业视觉检测需要同时判定缺陷并定位，但像素级标注成本高昂。现有异常分割方法仅从无缺陷图像学习并以偏差评分，但真实缺陷与异常但正常的区域都可能产生高偏差分数，导致误判，需要一种无需像素标注的弱监督方法加以区分。

Method: 提出对比双高斯过程（CDGP）框架，建模正常与异常两类诱导变量的预测分布；通过后验优势统计量，将预测均值差按联合预测不确定性标准化，提供空间证据和图像级置信度；该证据与分层正常重建残差互补，实现精细定位。所有校准仅使用训练数据，无需人工像素标注或测试时拟合。

Result: 在MVTec AD~2、KSDD2和VisA数据集上，CDGP在MVTec AD~2的所有定位指标中排名第一，在KSDD2和VisA上表现领先或具竞争力。通过分解和匹配线性头控制实验，界定了线性核高斯过程公式的贡献范围。

Conclusion: CDGP通过弱监督的双高斯过程建模有效解决工业异常分割中真实缺陷与正常变异难区分的问题，以低标注成本实现高水平定位精度，且无需测试时拟合，具有实际工业部署价值。
...
TL;DR: 本文提出SPARC流水线，一种结合物理信息切片到体素重建、深度学习分割与解剖重定向的全自动胎儿心脏MRI（fCMR）3D+时间电影重建方法，在121例临床队列中实现82.6%的完全自动处理率，平均运行时间7.1分钟，重建速度提升10倍，并已在临床部署。


<details>
  <summary>Details</summary>
Motivation: 胎儿心脏MRI动态电影成像是评估复杂先天性心脏病的重要工具，但胎儿运动不可预测且缺乏自动化、稳健的处理工具，使得3D+时间电影体素从2D+时间切片重建极具挑战，难以满足临床部署需求。

Method: SPARC流水线包含三个核心组件：1）基于多普勒超声门控的物理信息切片到体素重建（SVR）算法；2）深度学习模型进行胸部分割（采用集成聚合策略）；3）深度学习模型进行解剖重定向。整个流水线打包为Docker容器以实现临床部署。

Result: SVR重建时间从49.0±14.1分钟降至4.8±1.0分钟（p<0.0001）且质量更高；胸部分割Dice达84.7±3.9%，优于人工评分者间一致性（81.4±7.7%）；解剖重定向成功率为90.1%；在121例临床队列中82.6%的病例实现全自动处理，平均运行时间7.1±1.3分钟。

Conclusion: SPARC流水线在速度、质量和自动化程度上均优于现有方法，运行时间与临床工作流程兼容，已作为临床研究工具投入实际部署，有望推动胎儿心脏MRI的临床常规应用。
...
TL;DR: 本文提出PALATE框架，在不修改图像编辑器的情况下，通过共享奖励演化机制实现人像修图的个性化选择。它将用户奖励分解为全局骨干、类别级残差和轻量用户适配器三层，配合防坍缩正则化和循环双级蒸馏，使新用户仅需少量排序即可校准，在PPR10K上达到72.83%的偏好预测准确率，远超最强基线PickScore的58.06%。


<details>
  <summary>Details</summary>
Motivation: 人像修图具有主观性，同一肖像存在多个专业有效结果，用户偏好因人而异。现有方法要么优化群体审美标准无法捕捉个体品味，要么为每个用户微调独立模型导致训练、存储和数据成本过高。因此需要一种既能个性化又成本可控的解决方案。

Method: PALATE采用共享奖励演化框架：保持图像编辑器固定，仅个性化候选修图结果的筛选。核心创新包括：(1) 将用户奖励分解为全局骨干、类别级残差和轻量用户适配器三层，并用防坍缩正则化保持三层互补；(2) 循环双级蒸馏方案，先将用户级偏好蒸馏为类别奖励，再整合到全局骨干，循环迭代逐步改进共享初始化；(3) 新用户仅需少量偏好排序即可通过共享初始化进行校准。

Result: 在PPR10K专家修图候选上（含留出用户和留出图像），PALATE达到72.83%的成对偏好预测准确率，超过所有奖励、美学和图像质量基线，其中最强基线PickScore仅58.06%。每个新用户仅需512字节用户特定参数和毫秒级评分成本。

Conclusion: PALATE证明了在保持图像编辑器不变的前提下，通过共享奖励演化和层级化偏好建模，能够以极低存储与计算代价实现高精度的人像修图个性化偏好预测，为生成式编辑的个性化任务提供了高效可扩展的框架思路。
...
TL;DR: 本文评估了最新深度学习方法在无人机视觉里程计（VO）中的应用，使用朝下相机和合成数据集，发现RoMa匹配器表现最佳，但传统SIFT特征仍能超越部分最新方法。


<details>
  <summary>Details</summary>
Motivation: 无人机依赖GNSS导航，在定位信号不可用或受干扰时会面临灾难性故障，因此需要探索视觉里程计作为关键导航替代方案；同时最新深度学习方法尚未在完整的VO系统中得到验证。

Method: 在合成的无人机有向下相机数据集上，评估多种最新的图像匹配方法（包括RoMa等深度学习方法与SIFT传统特征）用于无人机位置跟踪的VO任务。

Result: RoMa匹配器生成最佳结果，但SIFT特征表现优于部分最新的深度学习方法。

Conclusion: 深度学习方法并非在所有场景下都优于传统方法，SIFT在VO任务中仍具竞争力，RoMa是当前表现最优的匹配器。
...
TL;DR: 本文提出了PCQA-R1，这是首个将强化学习与大型多模态模型（LMM）结合用于3D点云质量评估（PCQA）的方法。PCQA-R1基于组相对策略优化（GRPO）策略，通过构建链式思维数据集PCQA-CoT进行冷启动训练，并引入高斯邻近奖励来防止校准漂移。实验表明该方法在五个基准数据集上实现了最先进的跨数据集泛化性能，同时保持了具有竞争力的域内准确度。


<details>
  <summary>Details</summary>
Motivation: 现有基于LMM的PCQA方法主要依赖监督微调直接预测数值质量分数，这导致其在不同数据集的MOS尺度差异和标注有限的情况下泛化能力不足。绝对MOS回归在面对不同评分尺度和失真分布时表现脆弱，而相对质量排序则更为稳定，因此需要一种能够同时建模质量理解和评分的鲁棒方法。

Method: PCQA-R1采用强化学习框架，具体包括三个关键组件：1）基于GRPO策略进行强化学习训练；2）通过反向推理策略构建PCQA-CoT链式思维数据集作为冷启动训练数据，教导LMM生成推理过程；3）引入高斯邻近奖励机制，将分数预测锚定到源MOS范围以防止校准漂移。

Result: PCQA-R1在五个基准数据集上取得了最先进的跨数据集泛化性能，同时保持了有竞争力的域内准确度。消融研究验证了排序机制、高斯奖励和冷启动轨迹各自的重要作用。

Conclusion: 通过将强化学习引入LMM进行PCQA，PCQA-R1有效解决了跨数据集MOS尺度不一致导致的泛化瓶颈问题。相对排序结合高斯邻近奖励的策略验证了其在异构数据条件下的鲁棒性，为未来LMM在点云质量评估领域的应用提供了新范式。
...
TL;DR: 本文发现对齐后的视觉语言模型（VLM）在安全约束指令下会拒绝回答本可正确回答的问题，但内部视觉感知信息仍然保留，只是生成过程被重定向到拒绝行为。通过激活层干预抑制拒绝相关表征，可以无需重新训练即可恢复基于视觉证据的回答行为。


<details>
  <summary>Details</summary>
Motivation: 现有对齐VLM在安全指令下频繁拒绝回答本可回答的问题，但此前不清楚这是否意味着安全对齐抑制了感知能力本身，还是视觉证据仍然内部可用而仅生成被重定向。这一根本性问题缺乏系统研究。

Method: 跨多种架构和多模态基准测试，分析安全约束指令下的解码内部动态；比较不同架构中拒绝行为的表征组织；通过激活层干预实验验证抑制拒绝相关表征能否恢复基于视觉的回答行为。

Result: 1）拒绝性生成在解码过程中始终受视觉证据影响，感知基础基本保留；2）不同架构中拒绝的表征组织差异显著，但安全指令持续改变后期隐藏状态动态朝向拒绝；3）针对性激活干预可可靠恢复模型的基于视觉的回答行为。

Conclusion: 安全对齐可以在感知证据内部保留的情况下覆盖视觉表达，揭示了对齐VLM中一个此前未被充分探索的失败模式——安全对齐与感知基础的冲突。
...
TL;DR: SAM2Dual是一种无需训练的即插即用推理增强方法，通过双记忆设计和文本感知记忆机制提升SAM2在长视频目标分割中的鲁棒性，在MOSEv2和LVOSv2等基准上取得了稳定增益。


<details>
  <summary>Details</summary>
Motivation: 长视频目标分割面临长时间遮挡、物体重现和场景变化导致的误差累积问题。尽管SAM2具备强大的零样本性能，但其流式记忆机制在长时间跨度下会因为近期不可靠预测主导记忆状态而放大漂移，影响目标身份保持的稳定性。

Method: 提出SAM2Dual方法，包含两个核心组件：(1) 双记忆设计——将短期记忆与长期记忆分离，短期记忆用于快速局部自适应，长期记忆通过间隔采样构建以保留全局身份线索，并通过门控融合策略进行结合；(2) 文本感知记忆（TAM）——从早期帧提取紧凑的词级线索，利用文本嵌入根据语义兼容性重新加权记忆贡献，在视觉证据弱或模糊时支持身份保持。整个方法无需训练、不更新模型权重，为即插即用的推理时增强方案。

Result: 在长视频基准上，SAM2Dual一致提升了长时间视频的稳定性：在MOSEv2上将J&F从49.33提升至50.65，并在LVOSv2上取得一致的性能增益。

Conclusion: SAM2Dual通过双记忆架构和文本感知记忆机制有效缓解了SAM2在长视频中的漂移问题，以零训练开销实现了即插即用的鲁棒性提升，为长视频目标分割提供了一种实用且高效的增强方案。
...
TL;DR: 本文提出一种面向医学影像描述生成的临床结构化替代奖励框架，结合图像邻域对齐与临床图谱一致性两种结构化奖励，在ImageCLEFmedical标准与合成轨道及三个视觉-语言骨干网上，相比SFT基线在Overall、Relevance和Factuality上平均相对提升3.4%、2.1%和5.8%。


<details>
  <summary>Details</summary>
Motivation: 医学影像描述生成需要将异构视觉证据转化为简洁临床描述，其中发现、断言状态或解剖关系上的错误可能改变临床含义。现有的序列级策略优化虽能直接优化完整描述，但常用奖励依赖全局文本相似度、直接的图像-描述兼容性或无序概念重叠，未能充分体现视觉邻域和临床声明结构。

Method: 提出用于SFT后医学影像描述的临床结构化替代奖励框架。该框架结合生物医学语义和短距离词汇保真度，并引入两种结构化奖励：(1) 分布性图像邻域对齐，匹配参考与生成描述在医学图像库上的分布；(2) 临床图谱一致性，通过最大权重一对一匹配处理实体、断言状态和类型化关系。四个奖励在每个rollout组内独立归一化，以固定相对权重组合，并使用GDPO优化。

Result: 在组织者评估的ImageCLEFmedical Caption标准与合成轨道隐藏测试集上，以及三个视觉-语言骨干网的六种骨干-轨道组合中，该方法在所有组合上均优于匹配的SFT基线，Overall、Relevance和Factuality平均相对增益分别为3.4%、2.1%和5.8%。消融实验和配对诊断表明结构化奖励提供互补信号，减少了图像邻域发散并改善了实体-断言-关系一致性。

Conclusion: 结构化奖励在医学影像描述生成中提供互补且有效的信号，能够同时改善整体质量、相关性和事实准确性，验证了将临床知识结构融入奖励设计的有效性。
...
TL;DR: 本文提出Teeth2Point，一种基于点的高效Transformer框架，用于牙科CBCT语义分割。该方法通过卷积模型定位牙齿感兴趣区域（ROIs），然后使用自适应采样将ROIs转换为点令牌，由Transformer模型进行精确分割。结合DINO风格的SSL预训练与领域特定的数据增强策略，Teeth2Point在四个数据集上相比最强的两阶段基线在异常病例上平均提升1.44个DSC点，相比第一阶段nnU-Net提升1.9个点。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习架构在牙科CBCT分割中表现良好，但在牙齿缺失或错位等异常病例中的准确牙齿标记仍是关键挑战。基于Transformer的架构理论上能利用全局解剖上下文解决此类歧义，但CBCT体积高分辨率与牙齿宽空间分布导致稠密基于patch的体数据处理面临计算成本与上下文/细节之间的固有权衡，无法同时兼顾全局上下文和细粒度结构细节。

Method: Teeth2Point分为两阶段：1）使用卷积模型定位牙齿周围的体积感兴趣区域（ROIs）；2）通过自适应采样将ROIs转换为点令牌，利用Transformer模型预测精确分割。为了增强鲁棒性，Transformer先使用DINO风格的SSL自监督预训练（包含随机令牌掩码和领域特定增强策略），再进行有监督微调。这种方法避免了稠密patch处理中全局上下文与高分辨率之间的权衡。

Result: 在四个数据集上，Teeth2Point相比最强的两阶段基线在异常病例上平均提升1.44个DSC点，相比第一阶段nnU-Net提升1.9个点，证明了该方法在处理缺失或错位牙齿等异常病例时的有效性。

Conclusion: Teeth2Point通过点令牌表示和SSL预训练有效解决了CBCT分割中全局上下文与高分辨率不可兼得的难题，在复杂解剖变异的异常病例中取得了显著的性能提升，为牙科CBCT分割提供了一种高效且鲁棒的解决方案。
...
TL;DR: 本文对视觉-语言模型（VLMs）在自我中心视频理解中的应用进行了系统综述，涵盖了从传统识别架构到多模态基础模型和具身系统的演进过程，并指出了当前模型在理解交互、动作和用户意图方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 自我中心视频在可穿戴智能、辅助系统、人机交互和具身AI中日益重要，但面临自我运动、遮挡、小目标、视角依赖外观和长时序依赖等挑战。VLMs为应对这些挑战提供了有前景的基础，需要一个全面的综述来梳理该领域的研究进展和不足。

Method: 采用批判性综述方法，围绕任务、数据集、手-物交互理解、时序推理、帧与片段选择、多模态表示学习、提示、语义对齐和模型适应等维度组织文献，特别关注基于图和以对象为中心的推理机制，并探讨第一人称感知与多模态基础模型如何支持可穿戴辅助、机器人技能学习、人机迁移和具身决策。

Result: 综述揭示了当前模型普遍存在一个一致性的局限：相比对可见物体的识别，模型对演化中的交互、动作和用户意图的理解不够可靠，尤其是在长活动场景中。

Conclusion: 研究确定了时序接地推理、交互感知监督、高效长视频处理、多模态融合、图增强表示、跨域泛化、隐私和可信评估作为可部署具身智能的关键优先方向。
...
TL;DR: 提出了DynCur-Geo，一种动态好奇心框架，根据剩余目标距离调整预测误差内在奖励，在早期鼓励探索、接近目标时转向目标导向行为，并通过基于势能的奖励塑形提供密集进展引导，在多种场景下优于主动地理定位基线。


<details>
  <summary>Details</summary>
Motivation: 现有好奇心驱动方法在整个搜索过程中使用固定的内在奖励权重，在智能体接近目标后仍会持续奖励新颖性，导致绕路行为，难以平衡探索与目标收敛。

Method: 提出DynCur-Geo动态好奇心框架，核心是距离感知门控机制，根据剩余目标距离动态调整预测误差内在奖励：早期鼓励探索，接近目标时转向目标导向行为；同时采用基于势能的奖励塑形提供密集的进展信号。

Result: 在多模态、跨场景、灾后环境和长距离设置等实验中，DynCur-Geo相较于主动地理定位基线方法表现出一致的性能提升。

Conclusion: 动态调整内在奖励权重的策略能更好地平衡探索与目标收敛，为低空无人机主动地理定位提供了一种更有效的搜索框架。
...
TL;DR: FRAGMENT是一个将文档表示为类型化关系图并分解为p(structure)*p(content|structure)的生成框架，由Architect（生成图拓扑与空间关系的Transformer）和Builder（生成边界框、文本和视觉属性的GATv2图注意力网络）两个阶段组成，支持伪造检测和可控编辑。


<details>
  <summary>Details</summary>
Motivation: 结构化文档（如发票、表单、科学文章）的意义来源于空间布局、文本内容和逻辑结构之间的交互，而现有的像素级或token级生成模型难以有效捕获这些依赖关系。

Method: FRAGMENT框架将文档建模为类型化关系图，并分解其分布。第一阶段Architect使用类别条件化的因果掩码Transformer自回归生成图拓扑和类型化空间关系；第二阶段Builder使用基于GATv2的图注意力网络生成归一化边界框、文本和视觉风格属性。两个阶段均具有显式似然模型，可计算文档级似然用于伪造检测的异常分数。此外通过交叉注意力注入指令嵌入实现可控编辑。

Result: 在DocLayNet上进行训练，在FUNSD和SROIE上微调，并与自回归、仅布局和图基基线对比，提供了因子化图生成框架在特性与权衡上的实证分析。

Conclusion: FRAGMENT通过因子化图生成的方式，为结构化文档生成、伪造检测和可控编辑提供了一种统一的概率框架，实验评估展示了其与现有基线的性能对比和特性权衡。
...
TL;DR: 本文提出DocClaw，一个统一的智能文档处理智能体系统，将OCR、DocQA和KIE等多种任务形式化为智能体与文档之间的交互过程，通过结构化文档状态组织可复用知识，在多个基准上取得与专用方法竞争的成效。


<details>
  <summary>Details</summary>
Motivation: 智能文档处理中的多种任务（OCR、DocQA、KIE）虽有共同需求（感知文档内容、获取任务相关信息、逐步精化中间结果），但通常被当作独立的预测问题，由任务专用模型或流水线分别处理，缺乏统一的解决方案。

Method: DocClaw将不同IDP任务统一形式化为智能体与文档之间的交互过程。给定文档和任务查询，系统遵循合适的文档技能迭代识别所需信息，调用相关工具，并将观察结果整合为期望输出。过程中通过结构化文档状态组织可复用的文档知识和任务特定的交互上下文，使智能体能够积累、回溯并逐步精化信息。任务特定需求由智能体对查询目标和文档技能的解释捕获，而底层交互循环、工具空间和文档状态在任务间共享。

Result: 在多个智能文档处理基准上的大量实验表明，DocClaw能够在单一智能体框架内有效处理多种任务，并实现了与通用VLM和任务专用方法相当的竞争性能。

Conclusion: DocClaw验证了将多种IDP任务统一为智能体-文档交互过程的有效性，以共享的交互基础设施兼顾了任务灵活性与性能竞争力，为智能文档处理提供了统一的代理式解决范式。
...
TL;DR: 本文提出TDIR（时间可分解图像表示）算法，通过正交子空间将历史照片的嵌入表示分解为日期和内容两个独立分量，在保持检索性能的同时实现时间信息的可解释性和传递性操作。


<details>
  <summary>Details</summary>
Motivation: 神经嵌入空间通常是多维且混乱的，难以解释；若将嵌入压缩到单一时间维度又会牺牲下游任务性能。作者希望在不损失表达能力的前提下，学习既能保留时间结构又能用于图像和对象检索的表示。

Method: 提出TDIR算法，将历史照片分解为正交子空间中的日期分量和内容分量；定义了这种分解可实现的条件并证明其成立，同时刻画了条件部分满足时的误差界限；通过联合优化自然涌现正交性而非显式施加约束。

Result: TDIR支持跨图像的传递操作（将一张图像的时间信息提取并注入另一张图像的表示中，无需标签监督），并在历史照片的组成图像检索任务中验证了理论性质，在日期估计和对象检索上均取得有竞争力的性能。

Conclusion: TDIR为时间可分解的图像表示建立了数学基础，证明正交分解可以在联合优化中自然涌现，并提供了一种直观可解释的方式来导航摄影档案，同时保持检索性能。
...
TL;DR: 本文提出了一种针对历史手稿的传统OCR流水线，通过布局层面和外观层面的迭代微调来适应目标手稿的分布特征，从而减少人工标注成本。作者在三个复杂的梵文历史手稿上进行了数字化，并发布了带有精细布局级标注和Unicode标注（PAGE-XML格式）的数据集，同时基准测试了主流多模态大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 历史手稿的数字化对于文化遗产保存和学术研究至关重要，但历史手稿常因时代特定的书写风格、页面纹理、相机噪声等因素导致复杂异构布局和非标准外观，使传统OCR难以处理。而人工标注需要历史领域专业知识，昂贵且耗时，因此需要一种能减少人工干预的OCR方案。

Method: 作者提出一个本地传统OCR流水线，可在布局层面（layout-level）和外观层面（appearance-level）对目标手稿进行迭代微调。通过适配目标手稿的数据分布，流水线能在后续页面中做出更好的预测，从而迭代性地减少昂贵且耗时的人工标注工作量。

Result: 在三个复杂的梵文历史手稿上进行了数字化处理，发布了包含细粒度布局级标注和PAGE-XML格式Unicode标注的数据集。实验表明迭代微调带来了定量性能提升，并基准测试了主流多模态大语言模型在该数据集上的表现。代码和数据集已公开。

Conclusion: 提出的迭代微调传统OCR流水线能够有效适应历史手稿的特殊分布，在减少人工标注负担的同时提高数字化效果，并为历史文本数字化研究提供了带标注的公开数据集和基准结果。
...
TL;DR: 本文首次系统评估了不确定性量化（UQ）方法在语义分割基础模型上的应用。作者在SAM2编码器上微调轻量级DPT解码器作为基线，并对蒙特卡洛Dropout、深度子集成、测试时增强和证据深度学习四种UQ方法在Cityscapes、NYUv2及两个域外场景中进行了全面基准测试，揭示了分割精度、校准质量、不确定性与推理时间之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 基础模型虽然精度高且泛化能力强，但缺乏可解释性、容易过度自信且对真实世界域偏移敏感，这对安全关键应用构成严重威胁。不确定性量化是解决这些问题的原则性方法，但尚未被整合到分割基础模型中，亟需系统性的评估与探索。

Method: 在预训练的SAM2编码器之上微调轻量级DPT解码器构建基线模型，并基准测试四种代表性UQ方法：蒙特卡洛Dropout、深度子集成、测试时增强和证据深度学习。实验覆盖Cityscapes、NYUv2以及两个挑战性域外设置，从分割精度、校准质量、不确定性质量和推理时间四个维度进行比较分析。

Result: 四种UQ方法在分割精度、可靠性、校准和计算成本之间展现出明显的权衡。研究发现UQ方法能在一定程度上改善模型的可靠性和校准，但同时伴随计算开销或精度损失，目前尚无一种方法能在所有维度上同时达到最优。

Conclusion: 不确定性感知的基础模型展现了巨大潜力，但当前仍有局限性。未来工作需要在精度、鲁棒性和效率之间进行联合优化，以推动不确定性感知基础模型在真实世界中的部署。
...
TL;DR: 本文提出了CamWorldQA，这是首个针对摄像机控制的世界视频生成的感知质量评估基准，包含720个由6种生成方法产生的视频，并提出了CWQA无参考质量评估网络，通过空间、时间运动和光流三个互补分支联合预测质量分数，实验表明其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估（VQA）方法主要针对自然视频开发，无法捕捉摄像机控制生成视频的独特感知特征（如视点一致性、运动连贯性和内容保真度），亟需专门的基准和评估方法。

Method: 构建CamWorldQA基准数据集（6种生成方法×20个源视频×6条摄像机轨迹=720个视频，通过主观实验标注质量分数），并提出CWQA无参考质量评估网络，包含空间特征、时间运动特征和光流特征三个互补分支联合预测质量分数。

Result: 在CamWorldQA数据集上，CWQA相比现有质量评估方法取得了更优的性能表现。

Conclusion: CamWorldQA填补了摄像机控制生成视频质量评估基准的空白，CWQA通过多分支特征融合有效捕捉了此类视频的独特感知特性，为该领域提供了可靠的评估工具。
...
TL;DR: 本文提出EgoHRV，一种从第一人称（egocentric）头戴设备内置的注视摄像头视频中估计心率变异性（HRV）和心率（HR）的方法，通过3D骨干网络与新颖的低-高分解模块提取血容量脉搏信号，结合跨域预训练对齐频率域表征，实现了从极细微注视视频波动中精确恢复HRV，并将HRV估计与置信度集成到行为建模任务中提升17.8%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的第一人称视觉系统只能捕捉可见的人类行为线索，无法获取自主神经状态（如压力、参与度、注意力）的生理指标。HRV是评估自主神经调节的非侵入性金标准，但传统方法依赖接触式设备；而第一人称平台中强烈的运动和噪声会掩盖HRV所需的微小心跳间时间差异，使得HRV估计在此类平台上此前被认为不可行。

Method: EgoHRV采用3D骨干网络结合新颖的低-高分解模块，从注视摄像头视频中提取血容量脉搏（BVP）信号；通过跨域预训练将接触式传感器信号与摄像头派生信号的频域表征对齐，从而获得足够的时间精度以从注视视频的细微波动中恢复HRV。

Result: EgoHRV在从第一人称视频估计HR和HRV方面达到了最先进精度；将HRV估计和置信度集成到EgoExo4D的能力评估器中，准确率提升了17.8%。

Conclusion: EgoHRV证明了第一人称系统可以获得HRV级别的生理精度，且不确定性感知设计能显著改善下游行为建模；这一突破使第一人称系统扩展到压力感知和觉醒状态感知等任务，拓展了自主神经监测的应用边界。
...
TL;DR: 本文系统分析了CutMix数据增强策略对语义分割模型可靠性和鲁棒性的影响。研究发现CutMix对分割精度影响甚微，但能显著改善模型在校准和不确定性估计方面的可靠性，尤其在分布偏移场景下表现突出，这对安全关键应用（如自动驾驶）具有重要意义。


<details>
  <summary>Details</summary>
Motivation: 尽管CutMix在语义分割中被广泛使用，但其对模型可靠性和鲁棒性的影响尚未被系统探索。先前研究发现半监督分割方法中CutMix作为核心组件会严重降低模型可靠性，因此需要隔离分析CutMix对分割精度、校准和不确定性质量的独立影响。

Method: 采用隔离分析方法，单独评估CutMix对语义分割模型的影响。选取两种代表性架构——基于CNN的DeepLabV3+和基于Transformer的SegFormer，在域内和域外两种场景下，分别从分割精度、校准质量和不确定性三个维度进行系统评估。

Result: 实验结果表明：CutMix对分割精度影响较小，但持续改善模型的可靠性，尤其在分布偏移情况下提升更为显著。这说明CutMix主要增强模型校准和不确定性的可信度，而非原始分割预测本身的性能。

Conclusion: CutMix的核心价值在于提升语义分割模型的安全可信度（校准和不确定性质量）而非精度。在安全关键部署场景中，可靠的置信度估计与原始性能同等重要，因此这一发现对语义分割模型的实际部署具有重要指导意义。
...
TL;DR: 本研究通过LoRA适配MedSAM3医学基础模型，仅需10个标注病例即可在CT和MRI上实现与专业分割工具相媲美的腹部器官分割性能。五器官分割在AMOS22上表现优异，胆囊分割（Dice 0.68 CT/0.59 MRI）远超市面工具（Dice 0.0004），并在心脏分割外部验证中也取得竞争性结果，训练时间仅需3-5小时/器官。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割对临床工作流至关重要，但专业工具需要大量标注数据，而医学基础模型的零样本性能有限。关键问题在于：需要多少专家标注病例才能达到临床有用的分割性能？本研究旨在通过LoRA参数高效适配方式，探究最少标注数据下实现可靠分割的可能性。

Method: 采用MedSAM3作为基础模型，通过Low-Rank Adaptation（LoRA）进行参数高效微调。针对肝脏、肾脏、脾脏、胆囊和胰腺五个腹部器官，在CT和MRI上分别仅使用1、2、5和10个标注病例进行训练，并在AMOS22数据集上评估。随后在Whole Heart Segmentation数据集上进行心脏分割的外部验证。

Result: 仅用10个标注病例即可达到与专业系统竞争的性能：胆囊分割Dice为0.68（CT）和0.59（MRI），远超现有工具（Dice 0.0004）；肝脏、肾脏和脾脏性能仅比MRSegmentator低5-10%，但标注数据减少100倍以上。外部验证中左心室分割也取得竞争性结果。训练时间每器官仅3-5小时，比nnU-Net快2-3倍。

Conclusion: 十个标注病例足以实现临床有用的分割性能，有效缓解了医学图像标注和训练时间的瓶颈。该方法不仅适用于腹部器官，还能扩展到超出TotalSegmentator和MRSegmentator范围的心脏分割任务，展示了医学基础模型+LoRA在少样本场景下的巨大潜力。
...
TL;DR: 本文提出CL4D，首个直接操作动态点云的4D基础视觉编码器，通过对比学习对齐时空几何表征与自然语言描述；并在此基础上构建4DVLM，首个不依赖2D图像/视频或静态3D点云、直接处理4D点云的视觉语言模型。两者在DynAction4D数据集上训练，在多个4D人体动作基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编码器局限于静态2D图像、3D点云或2D视频，无法同时捕捉动态场景中的空间结构和运动演化，缺乏对动态物理环境的联合时空理解与推理能力。

Method: 1) 提出CL4D，基于对比学习目标将时空几何表征与自然语言描述对齐到共享嵌入空间；2) 提出4DVLM，直接以动态几何表征为条件生成语言；3) 构建新数据集DynAction4D，涵盖多样化人体动作、物体交互和场景环境。

Result: CL4D在多个4D人体动作基准上取得SOTA性能，比先前方法提升约16.75%；4DVLM在相同场景下优于Gemini和GPT-5等前沿视频VLM（即使后者使用RGB视频输入）。

Conclusion: CL4D和4DVLM验证了直接对4D点云进行视觉语言理解的可行性，为具身智能体在动态物理环境中的空间推理与运动理解提供了新的基础框架。
...
TL;DR: 本文提出了一种散射感知的共享-特定特征分解框架（Scattering-Aware Shared-Specific Feature Decomposition Framework），用于解决少样本SAR域自适应目标检测问题。该方法将检测特征分解为共享路径和多个软门控的散射特定专家路径，分别学习可迁移的目标结构信息和补偿异构SAR散射响应，并通过路由域辅助损失和专家平衡损失来增强适配效果。


<details>
  <summary>Details</summary>
Motivation: SAR目标检测在跨域场景下性能严重退化，因为频段、分辨率、背景杂波和目标散射响应的差异。现有域自适应方法主要追求域不变特征对齐，却抑制了对检测有用的传感器依赖散射特征。该问题在少样本场景下更加严峻，因为仅有少量带标注的目标域SAR图像可用。

Method: 提出散射感知的共享-特定特征分解框架：1) 将检测特征分解为共享路径和多个软门控散射特定专家路径；2) 共享路径学习可迁移的目标结构信息并用于非对称域对齐；3) 散射特定专家自适应补偿异构SAR响应；4) 引入路由域辅助损失以鼓励专家捕获传感器依赖的路由偏好；5) 引入专家平衡损失以防止路由坍缩。

Result: 在FARAD-X/FARAD-Ka和MiniSAR之间的四个双向异构SAR检测任务上，在不同少样本设置下进行了大量实验，结果表明该方法在正向和反向适配方向上均取得了优越性能。

Conclusion: 该框架通过显式建模和利用散射特性，克服了传统域自适应方法因抑制传感器依赖特征而导致检测性能下降的问题，为少样本SAR域自适应目标检测提供了有效的解决方案。
...
TL;DR: 本文提出MIFR框架，通过配对临床照片与皮肤镜图像，利用多目标损失函数实现模态不变的皮肤疾病分类，同时兼顾分类性能和跨肤色公平性。在内部数据集上取得了与基线相当的预测性能和公平性。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球重大公共卫生负担，但现有机器学习诊断工具存在两大关键局限：依赖单一模态进行诊断，以及不同肤色调之间存在系统性性能差异。已有研究分别解决这两个问题，但缺乏统一框架。

Method: 构建模态不变公平表示框架（MIFR），采用ViT编码器配对临床照片和皮肤镜图像，通过模态特定投影头将输入映射到高维嵌入空间。训练使用五组件多目标损失：加权交叉熵（分类）、混淆损失和肤型分类损失（公平性）、每模态监督对比损失（类别对齐）、模态不变损失（双模态对齐）。

Result: 在HIBA+Derm7pt配对内部数据集以及外部PAD-UFES-20和ISIC 2019数据集上的实验表明，模态不变表示学习在预测性能和内部数据集公平性方面均达到与相关基线模型相当的水平。t-SNE可视化证实同一疾病的临床与皮肤镜嵌入在几何上对齐。

Conclusion: 模态不变的表示学习能够统一解决单模态局限和跨肤色公平性两大挑战，为皮肤疾病诊断提供更全面、公平的机器学习方案。
...
TL;DR: 本文提出了EfficientSync，一个基于形变的实时口型同步框架，通过动态纹理混合器、时空自适应掩码和STAR采样三个组件保留参考纹理而非重新合成，在HDTF和VFHQ数据集上实现了166 FPS的实时性能，同时达到最先进的视觉质量和身份保持效果。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动口型同步方法虽然任务本质是局部编辑，但多数方法仍使用GAN或扩散模型重建整个下半脸，导致延迟高且容易产生牙齿、唇纹等口腔内部细节的幻觉，无法保留真实纹理。作者认为身份保持的瓶颈并非参考帧不足，而是缺乏能忠实传递参考帧中真实纹理的机制。

Method: EfficientSync包含三个核心组件：(1) 动态纹理混合器（Dynamic Texture Mixer），将多参考帧融合重构为通道级选择任务，在全局上下文中评估每个空间对齐的参考帧，通过通道级加权求和聚合以保留纹理完整性；(2) 时空移位自适应掩码（Spatio-Temporal Shifted Adaptive Masking），将源帧分解为唇部生成条件和独立背景先验，抑制下半脸信息泄漏的同时将合成嘴部无缝融合到背景中；(3) STAR采样（STAR Sampling），作为零开销预处理步骤，检索最锐利且拓扑多样性最高的参考帧。

Result: 在HDTF和VFHQ数据集上的实验表明，EfficientSync在视觉质量和身份保持方面达到了最先进的性能，并在单个GPU上实现了166 FPS的实时推理速度。

Conclusion: 该工作证明了保留参考帧真实纹理的形变方法比重建整个下半脸的生成方法更有效，既解决了延迟问题也避免了口腔内部细节的幻觉，为音频驱动口型同步任务提供了一种高性能的替代方案。
...
TL;DR: 本文提出EVADE（Evidence-Verified Agentic Diagnosis with Escape），一种无需训练的推理方法，通过在不同图像视图间验证一致性来增强冻结医学视觉语言模型的可靠性，同时改善校准质量和选择性风险。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型虽然准确率高但不可靠：系统性地过度自信，无法从测试时推理中获得收益，且缺乏对自身响应进行可靠信任校准的能力。现有的自我验证方法（如chain-of-thought、self-consistency）在单一模型自检中容易产生验证幻觉。

Method: EVADE采用推理式、无需训练的方法：模型先给出响应，当不确定时定位最具诊断相关性的区域，在放大视图上重新回答，只有当整图响应与放大视图响应一致时才做出承诺，否则弃权。核心思想是通过不同图像视图间的一致性门控来替代单一模型重读自身文本的验证方式。

Result: 在VQA-RAD、SLAKE和PathVQA数据集上使用Qwen2.5-VL-7B进行实验，EVADE是唯一同时改善校准质量和选择性风险并保持准确率的方法，将期望校准误差（ECE）相比零样本降低高达45%。链式推理、自我一致性和自我验证至少在一项指标上失败。接地分析表明自选区域在诊断结构定位上优于中心或随机裁剪，但7B模型无法利用该定位来修正答案。

Conclusion: EVADE的可靠性提升主要来源于一致性门控和校准弃权机制，而非模型的定位修正能力。这为部署冻结的医学视觉语言模型提供了更安全的推理框架。
...
TL;DR: 本文提出RVLoss，一种通过两阶段轮询投票机制（runoff vote）将运动刚性约束嵌入损失设计的自监督LiDAR场景流估计方法，在Argoverse2 2026挑战赛上超越基线20%，并在四个跨数据集上表现一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督LiDAR场景流方法通常最小化Chamfer损失（流补偿源点与目标点的最近邻距离），但最近邻搜索不强制运动刚性，导致物体实例内流场不一致。现有方法通过额外正则项缓解，但对大物体的流一致性仍有限。

Method: 核心观察到点级运动可通过投票聚成少量主导流候选（top-k投票）；将这些候选补偿源点后，最能代表刚体运动的流在第二次投票（top-1投票）中获得最高共识。基于此设计两阶段轮询投票（two-stage runoff vote）损失，生成簇级刚性流和自由流作为自监督伪标签，可直接集成到现有前馈网络中。

Result: 在Argoverse2 2026挑战赛中，使用RVLoss训练的模型在自监督方法中达到最先进性能，相比使用其他损失设计的基线模型提升20%；跨数据集评估在另外四个数据集上均表现出一致的性能提升。

Conclusion: RVLoss通过设计内嵌的运动刚性约束，显著提升了自监督LiDAR场景流估计中物体内部流场的一致性，且作为即插即用的损失模块，具有良好的泛化能力和实用性。
...
TL;DR: 本文探索将强化学习（特别是GRPO）应用于自回归感知模型的训练后对齐，直接优化与评估指标一致的奖励函数，在密集场景（多达500个物体）、指代表达分割等任务上取得显著提升，并消除了对NMS和坐标去重等后处理步骤的需求。


<details>
  <summary>Details</summary>
Motivation: 自回归感知模型在开放词汇定位任务中通常使用监督微调（SFT）和最大似然估计进行训练，但其优化的代理目标（逐token交叉熵）与感知评估指标（如精确率和召回率）根本不一致。作者希望探索通过强化学习直接对齐评估指标，克服SFT在感知任务上的局限性。

Method: 构建基于Falcon Perception的强化学习（RL）框架，采用GRPO算法，专门解决感知任务的挑战：（1）为集合结构输出设计奖励函数，简单地对假阴性（漏检）和假阳性（误检）进行惩罚；（2）实现多头采样控制。同时开发两种混合自标注流水线，分别针对困难指代表达和极密集场景进行RL训练。

Result: RL训练在极密集场景（高达500个物体/场景）中解锁了最先进的性能，而现有系统在该场景下性能急剧下降或崩溃；修复了掩码重复等常见问题，几乎完全消除对NMS和坐标去重的需求，提升性能和效率并省去超参数调优；在所有难度等级上的指代表达分割任务（PBench和SACO-Gold）均有改进；在没有负样本训练的情况下，保持了物体存在与否的判断能力（由MCC评估）。

Conclusion: 一个简单的惩罚假阴性和假阳性的奖励函数就足以驱动有效的RL训练，证明强化学习是一种可行且高效的后训练对齐方法，能够显著提升自回归感知模型在多类视觉任务上的表现。模型权重已以Falcon Perception修订版形式公开。
...
TL;DR: 提出了一种学习状态感知的动态生成式数据增强方法（LSADA），通过构建感知样本学习状态、映射增强强度，并采用解耦的数据增强与扩散融合策略，在保持类别语义的同时提升图像多样性，在九个公开数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成式数据增强方法依赖任务无关的增强策略，忽略了下游模型的需求；动态方法虽引入模型反馈，但难以可靠确定样本特定的增强强度，也难以在不同图像区域间调整增强策略，难以在图像多样性和类别语义之间取得平衡。

Method: LSADA首先基于每个样本的当前损失和损失下降率构建学习状态，并将其映射为样本特定的增强强度；其次引入解耦的数据增强与扩散融合策略，对类别相关区域施加强度可控的变换，同时生成多样化的类别无关区域，并通过渐进融合来提升图像多样性与保持类别语义。

Result: 在九个公开数据集上，LSADA在六个自然图像数据集上平均超越现有SOTA动态GDA方法4.5%，在三个医学图像数据集上平均超越2.5%。

Conclusion: LSADA通过引入学习状态感知机制和解耦的增强-扩散融合策略，有效解决了动态生成式数据增强中增强强度确定和区域适配的问题，展示了在自然图像和医学图像分类任务上的优越性能。
...
TL;DR: 本文提出Colorist，一种基于经典统计颜色匹配的高效数据增强策略，直接在RGB颜色空间中应用全局均值-标准差匹配，无需训练即可生成结构完整的域变化，在多个医学影像数据集上相较深度生成模型和现有正则化方法提升了分类鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 训练与部署之间的硬件变化、颜色差异和患者特征漂移会破坏医学图像分类器的性能。现有方案中，标准颜色抖动多样性不足，深度生成风格迁移会幻觉特征、破坏临床相关结构且计算开销巨大，亟需一种安全、高效且可解释的增强方法。

Method: 重新审视经典统计颜色匹配方法，将其改造为Colorist：直接在RGB颜色空间执行全局均值-标准差匹配，作为一种无需训练、完全可解释的数据增强策略，生成结构完整的域间变化。

Result: 在组织病理、外周血、皮肤科和视网膜四个分布外数据集上，Colorist相较最先进的域泛化正则化方法平均平衡准确率提升最高+9%，较无增强基线提升+13%；同时在结构保真度和颜色对齐方面优于深度生成模型。

Conclusion: 统计颜色匹配是一种安全、可解释却长期被忽视的医学影像鲁棒性增强替代方案，能保留解剖结构、减少碳足迹，并能无缝集成到标准数据加载器中。
...
TL;DR: 本文介绍了一个开源端到端流水线，用于从历史馆藏书籍中检测、分类、去重和生成视觉元素的描述，并发布了一个包含2260万视觉元素的数据集，这些元素来自哈佛图书馆数据集中的983,004本扫描书籍。


<details>
  <summary>Details</summary>
Motivation: 历史书籍中的视觉元素（如插图、照片、版画和装饰艺术）在大规模数字化项目中常被忽视，OCR技术仅标准化了文本提取，而视觉成分中包含的细微语境信息尚未被自动化文本提取工作流充分利用。

Method: 提出了一个开源的端到端流水线，涵盖视觉元素的检测、分类、去重和描述生成。同时发布了从哈佛图书馆983,004本扫描书籍中提取的2260万视觉元素的初始数据集。

Result: 发布了包含2260万视觉元素的数据集，来源于983,004本扫描的哈佛图书馆馆藏书籍，并提供配套的开源处理流水线。

Conclusion: 这项工作为社区范围内的努力做出贡献，通过计算访问方式为数字化图书馆馆藏开辟新用例，涵盖从AI模型训练到数字人文学研究的多个领域。
...
TL;DR: 本文提出TRAIL方法，利用冻结的DINO编码器对图像进行全局Haar扰动后，通过patch-token的余弦漂移来定位AI图像编辑区域，无需训练即可实现局部编辑定位。


<details>
  <summary>Details</summary>
Motivation: 局部图像编辑可能改变照片含义而大部分内容保持不变，法医分析需要精确定位编辑发生的位置，但现有方法通常需要掩码监督训练。

Method: 对冻结DINO编码器施加一次全局Haar扰动，计算扰动前后对应patch token之间的余弦漂移，将此漂移作为定位图。系统评估了16种DINO编码器的不同层块，发现最佳层块位于归一化深度0.80-0.94。

Result: 在80张源不相交的CocoGlide测试图像上，TRAIL达到0.903的patch AUROC（对比监督方法Detective SAM的0.912）；固定阈值Dice为0.619（对比0.709），使用oracle阈值可提升至0.790。迁移到泊松图像插值任务达到0.855 AUROC，无需生成器即可保持该线索。全局上下文关键：全局扰动AUROC为0.903，局部扰动降至0.857，独立裁剪编码降至0.735。

Conclusion: 冻结DINO patch token在深层包含强定位信号，其可见性依赖扰动方式与上下文保持，证明了无需训练即可实现图像编辑定位的可能性。
...
TL;DR: 该论文提出SiConMo，一个轻量级语义分割框架，核心设计原则是在极低计算预算下，bottleneck阶段是融合局部和全局上下文最有效的环节。SiConMo包含三个组件：Token金字塔提取模块、Transformer分支深度可分离卷积块和特征融合模块，在ADE20K等四个数据集上取得了SOTA的精度-效率平衡。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像的语义分割需要在精度、效率和可扩展性之间取得平衡。卷积网络善于建模局部模式但缺乏长距离依赖，而Vision Transformer虽能捕获全局上下文但计算成本高昂。现有研究大多聚焦于编码器设计，而bottleneck阶段作为上下文聚合和信息流的关键环节却被相对忽视。

Method: 提出SiConMo框架，包含两个变体：仅RGB模型（SiConMo）和GME增强变体（SiConMo†）。框架集成三个互补组件：(1) Token Pyramid Extraction Module——用于分层多尺度表示；(2) Transformer-Branched Depthwise Convolution block——用于bottleneck感知的上下文建模；(3) Feature Merging Module——在保持空间结构的同时增强语义一致性。

Result: 在ADE20K、PASCAL Context、Cityscapes和COCO-Stuff四个数据集上的大量实验表明，SiConMo在轻量级语义分割模型中达到了最先进的精度-效率权衡。

Conclusion: 该工作证明了一个关键设计原则：在极低计算预算下，bottleneck阶段是集成局部和全局上下文最有效的环节，简洁性本身就是一种强大的设计原则。
...
TL;DR: 本文研究了博物馆和档案数据集中的"时间纠缠"现象——预训练图像表示看似编码了艺术品的创作时间，实际上可能编码的是收藏、保存、编目和数字化等制度性条件。作者通过将艺术品年代测定建模为不确定性感知的回归任务，在时间受控的Wikidata艺术作品语料上评估多种预训练视觉模型，发现视觉语言模型优于纯视觉自监督基线，但定性分析表明这些时间知识受到多种偏见的影响。


<details>
  <summary>Details</summary>
Motivation: 博物馆和档案数据集并非历史艺术生产的真实镜像，而是反映了收集、保存、编目和数字化过程中的偶然历史。这导致预训练图像表示可能错误地将制度性条件编码为历史时间，即"时间纠缠"现象，需要对此进行系统研究。

Method: 将艺术品年代测定构建为不确定性感知的回归任务，使用冻结的图像嵌入，在时间受控的Wikidata艺术作品语料上评估多种预训练视觉模型（包括视觉语言模型和纯视觉自监督基线）。

Result: 预训练模型包含可用的时间信息，其中视觉语言模型（VLMs）优于纯视觉自监督基线；但定性分析显示，这些时间知识受到多种偏见的影响，可能反映了制度性条件而非真实的历史时间。

Conclusion: 时间纠缠是预训练视觉表示中的一个真实问题，视觉语言模型虽然在时间推断上表现更好，但其时间知识仍受制度性偏见塑造。这提醒我们对预训练表示中的时间信号应持批判态度。
...
TL;DR: 本文提出X-LMC，一种用于数字减影血管造影（DSA）中软脑膜侧支（LMC）自动评分的时空框架，通过DINOv2骨干网络编码空间特征、跨视角注意力融合双平面投影、递归网络建模造影剂动力学，在多中心134例M1段闭塞患者上取得优于基线方法的性能（QWK 0.398，宏F1 0.711）。


<details>
  <summary>Details</summary>
Motivation: 临床LMC分级依赖人工视觉检查ASITN/SIR量表，存在高度主观性和变异性；手动评估耗时且结果不一致，缺乏客观自动化的评分工具来支持卒中患者的治疗决策和预后研究。

Method: X-LMC采用三模块架构：1) DINOv2骨干提取空间帧表征；2) 令牌级跨视角注意力模块融合正交双平面投影；3) 递归神经网络建模造影剂团注的时间动力学特征。

Result: 在134例M1段闭塞患者的多中心数据集上，5折交叉验证中X-LMC取得QWK 0.398（优于基线0.322）和二分类宏F1 0.711（优于基线0.663），且与临床观察者间一致性（QWK 0.314）相当。

Conclusion: 本文作为首个尝试自动化LMC评分的DSA研究，证明多视角时序深度学习能够捕捉侧支特异的造影剂动力学特征，为客观血流动力学表型分析和卒中队列研究建立了可复现的基础。
...
TL;DR: 本文提出了ForeSightGuide，一个基于视觉语言模型（VLM）的预见性辅助导航框架，用于视障人士的独立出行。与传统的反应式系统不同，该框架利用VLM的推理能力预测障碍物运动，过滤非威胁性物体，减少误报冗余信息，同时保持低漏报率，实验证明其在动态真实交通场景中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 视障人士的独立出行高度依赖电子出行辅助设备。虽然视觉语言模型（VLM）能提供丰富的环境理解能力，但在动态场景中常常产生过多的误报（假阳性），导致认知过载，严重影响用户体验和安全性。现有的反应式系统缺乏对障碍物运动的预见性判断，无法有效区分真实威胁与非威胁物体。

Method: ForeSightGuide将语义场景理解与预测性危险评估相结合。与反应式系统不同，它利用VLM的推理能力来预测障碍物的运动轨迹，从而过滤掉无害物体，仅提供简洁、可操作的导航指引。此外，作者构建了一个新的数据集，在复杂动态的真实交通场景中采集，用于基准测试预测能力。

Result: 在公开基准和自建数据集上的大量实验表明，ForeSightGuide达到了最先进的性能。具体而言，它将冗余警报减少到每个导航输出仅0.299次，同时保持了0.112的低漏报率（missed-hazard rate），显著缓解了信息过载问题。

Conclusion: ForeSightGuide通过将VLM的预见性推理能力引入视障辅助导航，在减少干扰性警报和保持安全性的同时取得了优异表现，证明了预测性辅助框架在安全步行辅助中的有效性和实用价值。
...
TL;DR: 本文提出GrabVG，一个受人类视觉搜索启发的UAV图像视觉定位框架，通过"预注意假设搜索"和"图注意特征绑定"两阶段策略，在AerialVG和AerialSense数据集上分别达到67.31%和80.34%的Acc@0.5，超越基线10.55和8.76个百分点。


<details>
  <summary>Details</summary>
Motivation: UAV图像中目标物体数量多、尺寸小、分布密集且视觉相似，造成高视觉冗余和强拓扑歧义。现有方法侧重于视觉-语言特征对齐或密集上下文交互，难以区分细微的实例间差异并有效利用空间拓扑结构，导致在高度拥挤的场景中定位不准确。

Method: GrabVG将视觉定位显式分解为两个顺序阶段：(1)预注意假设搜索——通过蒸馏引导的提议归纳和文本感知假设过滤，生成紧凑可靠的目标假设集，减少背景干扰和语义不匹配；(2)图注意特征绑定——将假设组织成稀疏图，通过图注意力机制联合绑定和传播语言引导的实例内视觉线索与实例间拓扑关系，实现高效空间推理和精确目标定位。

Result: 在AerialVG和AerialSense数据集上，GrabVG分别达到67.31%和80.34%的Acc@0.5，相比对应基线分别提升10.55和8.76个百分点，实现了良好的精度-速度权衡。

Conclusion: GrabVG通过模拟人类视觉搜索的两阶段机制，有效解决了UAV高密度场景下视觉冗余和拓扑歧义问题，验证了"先筛选后绑定"策略在视觉定位任务中的有效性。
...
TL;DR: 本文提出Mise-en-Scène，一个两阶段框架，利用预训练图像编辑扩散Transformer的隐式布局生成能力，通过LoRA微调完成整体设计草稿，再通过确定性的匹配-放置步骤将原始高清素材精确放置到草稿位置，兼顾布局连贯性与素材保真度。


<details>
  <summary>Details</summary>
Motivation: 现有图形设计合成方法将布局预测（显式边界框坐标）与视觉合成分离，导致生成的构图僵化、素材缩放不当。作者希望让布局在预训练图像编辑扩散Transformer内部隐式涌现，实现布局规划与视觉渲染的联合生成。

Method: 提出两阶段框架：第一阶段，使用经knockout选择的少量LoRA适配的扩散Transformer，联合生成元素排布与渲染画布的完整设计草稿；第二阶段，通过确定性的匹配-放置步骤，将原始高清素材精确移动到草稿位置，保证素材保真度并生成可编辑的分层设计。

Result: 在PrismLayersPlus大规模基准上，Mise-en-Scène生成的图像在所有对比方法中与地面真实值的感知质量最接近，大幅超过LLM布局规划器和专用布局Transformer；匹配-放置阶段弥补了与真实合成图像的保真度差距。

Conclusion: 仅对预训练Transformer进行最小化适配即可实现高质量的隐式布局生成，无需专门为多元素生成设计的条件机制；匹配-放置阶段有效弥合了感知质量与素材保真度之间的差距，同时保留了对设计师友好的可编辑分层结构。
...
TL;DR: 本文是对自动驾驶中单阶段目标检测器的综合性综述，系统回顾了从YOLOv1到YOLOv10等主要单阶段检测架构的演进，涵盖设计选择、特征融合策略、损失函数、部署权衡及基准性能，并总结了常用数据集、评估指标、开放挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶依赖于快速可靠的目标检测系统，而单阶段检测器在速度与精度之间提供了关键平衡。作者旨在通过全面梳理该领域的发展脉络，帮助研究者理解现有架构的优劣，并揭示基准测试结果与真实自动驾驶场景之间的差距。

Method: 采用文献综述方法，系统回顾并比较了YOLOv1、SSD、RetinaNet、EfficientDet等经典单阶段检测器，以及FCOS、CenterNet等无锚框检测器和YOLOv10等最新实时模型，从架构设计、损失函数、特征融合和部署性能等多个维度进行对比分析。

Result: 综述表明单阶段检测器在速度、精度、效率和鲁棒性之间存在复杂权衡，不同架构各有优势；同时指出基准测试性能与真实世界自动驾驶可靠性之间仍存在显著差距。

Conclusion: 单阶段目标检测器已成为自动驾驶感知系统的核心组成部分，但要在真实场景中实现可靠的自动驾驶性能，仍需解决基准结果与现实差距、鲁棒性和边缘部署等开放挑战。
...
TL;DR: 本文提出用有限多项式逼近矩阵对数来替代谱分解方法，用于全局协方差池化（GCP）中的SPD流形归一化。通过均值特征值预归一化和标量后补偿，消除了特征分解，所有操作变为GEMM，梯度稳定且不会出现不稳定的1/(λi-λj)项。推荐使用8阶Chebyshev展开配合三项矩阵递推，在多个细粒度识别基准和ImageNet-1k上比谱对数及平方根近似更快更准。


<details>
  <summary>Details</summary>
Motivation: 全局协方差池化（GCP）通过捕获二阶特征统计量提升深度网络性能，尤其适合细粒度识别。协方差矩阵位于SPD流形上，需要归一化才能用于欧氏分类器。虽然矩阵对数（MLN-COV）是忠实的选择，但其基于特征分解的梯度数值不稳定，导致实践中被矩阵平方根取代。作者指出这种不稳定性是谱方法计算对数的伪影，而非对数本身的问题。

Method: 采用有限多项式逼近对数，消除前向和后向传播中的特征分解。关键创新是均值特征值预归一化，将谱中心移至1附近（远离log的奇点），并用标量后补偿以闭式返回log(A)的奇异部分。推荐使用8阶Chebyshev展开，通过三项矩阵递推进行求值，并配以匹配的逆向递推用于反向传播。作为对照，还研究了Legendre、Laguerre、Taylor和Pade展开以分离基函数和目标函数的作用。

Result: 在三个细粒度基准和ImageNet-1k上，无分解的对数方法在速度和精度上均优于谱对数和平方根近似方法。在匹配基和阶数条件下，对数目标优于平方根目标，证实性能提升来自忠实的Riemannian映射而非更好的多项式族。

Conclusion: 矩阵对数的数值不稳定性可以通过多项式逼近和使用GEMM操作来避免，无需特征分解。该方法不仅在速度和精度上超越现有方法，还验证了对数作为SPD流形映射的优越性来自其Riemannian几何保真性，而非多项式逼近技术本身。
...
TL;DR: 本文提出了一种基于对比分析（CA）的分类器无关的视觉反事实解释（VCE）方法，通过交换不同类别数据集中特有的生成因子来生成反事实图像，避免了对分类器决策边界的依赖，从而减少分类器偏差的影响。


<details>
  <summary>Details</summary>
Motivation: 现有VCE方法依赖于分类器，容易受到分类器偏差和失败模式（如捷径特征敏感性和校准误差）的影响，因此需要一种不依赖于分类器的反事实生成方法。

Method: 该方法基于对比分析框架，将两个不同类别数据集的生成因子分解为公共因子和各自特有的显著因子，通过仅交换显著因子来生成反事实图像。方法使用StyleGAN2中的特征空间F（而非常规的W空间）来改善细节保留，并提出了一个适配框架和损失函数，允许多个显著因子在每个数据集中存在。

Result: 在三个医学影像数据集上进行评估，该方法在反事实生成质量上优于现有方法。

Conclusion: 通过直接在数据分布上操作而非决策边界，所提出的方法提供了模型无关的VCE，对分类器偏差的敏感性更低，在医学影像领域展现了优越的反事实生成质量。
...
TL;DR: 本文提出USR-Drive，一个统一的条件生成框架，通过共享场景表征同时恢复稠密动态几何和实例级3D目标框，利用统一的多模态扩散Transformer和统一位置编码实现两者的相互约束与增强，在nuScenes和VKitti数据集上取得动态重建和3D检测的双重SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶空间表征学习中，动态重建和实例级感知（3D检测）被当作两个独立任务处理，尽管它们共享估计底层3D世界状态的共同目标。这导致动态重建缺乏约束，而3D检测缺乏几何基础。作者旨在弥合这一鸿沟，通过统一框架让两个模态相互增强。

Method: USR-Drive将稠密3D高斯基元和稀疏3D边界框表示为两个对齐的潜在token流，用统一的多模态扩散Transformer联合去噪。关键创新是统一位置编码（UPE），将异构token对齐到共享的度量时空坐标系中。不同于以往将box作为外部条件或分离模块预测的方法，USR-Drive将二者视为相互约束的状态变量。

Result: 在nuScenes和VKitti数据集上，USR-Drive在动态重建和3D检测两个任务上均取得了最先进的（SOTA）结果，验证了统一表征和生成框架的有效性。

Conclusion: 通过统一表征和生成框架，几何信息为box预测提供稠密度量证据，box为3D几何表征提供实例级结构先验以减少歧义和保持空间一致性，两个模态相互促进，证明了联合建模动态重建与实例级感知的可行性。
...
TL;DR: 本文提出一种基于扩散模型的音乐驱动角色动画生成框架，通过双目标损失函数解耦骨骼完整性与鼓槌精度，实现厘米级打击精度的同时保持自然身体动力学，并引入新数据集、数据增强策略及两种新评估指标（冲击-目标距离和音频-运动相关性分数），在真实世界音频上生成与真实表演难以区分的高质量鼓手动画。


<details>
  <summary>Details</summary>
Motivation: 音乐驱动的角色动画在娱乐和交互式教育中有广泛应用，但从音频合成逼真的击鼓动作极具挑战性，难点在于高加速度动力学与极端时空精度之间的内在矛盾。现有方法（如运动匹配或MIDI输入）难以泛化到多样化的真实世界音频，且缺乏能区分精确击鼓与噪声运动的标准评估指标。

Method: 提出生成式扩散框架，采用双目标损失函数将骨骼完整性与鼓槌精度解耦，实现厘米级鼓槌精度而不牺牲自然身体动态；使用自建数据集与数据增强策略使模型泛化到未经整理的野外音频；提出两种新评估指标：冲击-目标距离（量化空间精度）和音频-运动相关性分数（评估时间对齐）。

Result: 定量分析和用户研究表明，该系统的生成运动质量高，往往与真实地面真值表演难以区分。

Conclusion: 该框架成功解决了音乐驱动击鼓动画中精度与自然度难以兼得的难题，并通过新指标为领域提供更严格的评估标准，生成结果达到接近真实表演的质量水平。
...
TL;DR: 本文评估了用于前列腺癌全身PET/CT病灶分割的多模态融合策略，比较了早期融合（单编码器单/双解码器）和中间融合（双编码器交叉注意力U-Net）方法。结果显示单示踪剂基线模型表现最佳（PSMA Dice=0.93, FDG Dice=0.81），而融合模型在FDG分割上性能下降，未能在统一设置下超越单示踪剂基线。


<details>
  <summary>Details</summary>
Motivation: PSMA和FDG PET/CT提供互补的前列腺癌生物学信息，联合使用可捕捉单一示踪剂可能遗漏的异质性肿瘤表型，但目前缺乏理想的多模态深学习融合架构，需要系统评估不同融合策略的有效性。

Method: 使用公开DEEP-PSMA Challenge数据集，训练示踪剂特异性的3D nnU-Net基线，并比较三种融合架构：(i)早期融合-单编码器单解码器(OEOD)；(ii)早期融合-单编码器双解码器(OETD)；(iii)中间融合-双编码器交叉注意力U-Net(DECA-UNet)。

Result: 示踪剂特异性基线表现优异（PSMA Dice=0.93，FDG=0.81）。融合结果参差不齐：OEOD在非示踪剂特异性任务上达到组合Dice 0.90，但示踪剂特异性融合模型PSMA/FDG Dice分别为0.69/0.64（OETD）和0.76/0.57（DECA-UNet），FDG性能明显下降。

Conclusion: 在评估的设置下，示踪剂特异性模型仍是更强的基线方案；多模态融合要产生临床获益，需要能够更好地保留示踪剂特异性表征的架构设计。
...
TL;DR: 本文提出GS-VLA，一个轻量级即插即用框架，通过3D高斯新视角合成技术在观测空间层面进行视角归一化，在不重训练VLA策略的前提下显著提升其对相机视角偏移的鲁棒性。仅4M参数的3D高斯规范化器接在冻结策略前，在LIBERO基准上可将因视角偏移导致的成功率暴跌（90%→10%）大幅恢复，且跨策略架构、跨任务套件和跨扰动尺度均有效。


<details>
  <summary>Details</summary>
Motivation: VLA策略隐式假设训练与部署时相机配置一致，实际中微小的相机安装位移即可导致成功率从约90%暴跌至约10%。现有解决方案（大规模微调、生成式数据增强）计算成本高昂且存在灾难性遗忘风险，亟需一种无需策略重训练的轻量级视角鲁棒性提升方案。

Method: 将视角偏移重构为局部新视角合成问题。基于"局部性假设"（相机扰动相对于工作空间保持在有界小区域内），视角归一化可简化为场景无关且策略无关的去遮挡任务。具体实现为一个4M参数的3D高斯规范化器（3D-Gaussian canonicalizer），以即插即用方式前置冻结的VLA策略之前，不修改任何策略权重。

Result: GS-VLA在三个正交维度上均有效：不同策略架构、未见过的任务套件、不同扰动尺度。在LIBERO基准上能恢复因视角偏移损失的大部分性能，且无需策略重训练。

Conclusion: 实验表明轻量级视觉模块能够在无需策略重训练的情况下，恢复视角偏移下大部分丢失的性能，为VLA策略的视角鲁棒性提供了一种高效、通用且即插即用的解决方案。
...
TL;DR: 提出ReWEIGH，一种无需训练的解码干预方法，通过聚合视觉位置的词汇排名并与无标签图像估计的token特定参考比较，在推理时对低于参考的候选施加有界惩罚，从而减少LVLM的幻觉，最高降低21.3%的幻觉物体提及，额外延迟仅1.33%。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）经常产生与输入图像不符的幻觉内容。现有方法缺乏一种针对候选token的度量来评估图像对该token的支持程度，而视觉token状态提供的逐位置读出由于概率量级不可比且token间存在系统性差异，无法直接池化使用。

Method: 提出ReWEIGH框架：(1) 利用模型视觉token状态通过输出头投影得到逐位置的词汇排名作为尺度不变的池化基础；(2) 聚合视觉位置的排名并与从无标签图像估计的token特定参考进行比较；(3) 在推理预填充阶段缓存图像证据；(4) 仅对低于参考值的候选token施加有界惩罚。

Result: 在四个7B主干模型上，ReWEIGH将幻觉物体提及减少最高21.3%，同时基本保持或提升描述性及通用性能。证据缓存后每个token平均额外延迟仅1.33%，且效果可扩展至跨越六个架构族、参数量达32B的模型。

Conclusion: ReWEIGH通过token特定的参考校准解决了视觉证据在不同token间系统性差异的问题，是一种高效、可扩展且无需重新训练的幻觉缓解方案，在实际部署中具有低延迟优势。
...
TL;DR: 本文研究了三种参数高效适配方法（线性头、MLP、注意力池化）应用于冻结的Rad-DINO胸部X光编码器时，对病理分类性能和子组公平性的影响。研究发现注意力池化性能最佳但编码受保护属性最强，而更强的属性编码并不对应更大的子组差异，公平性影响与整体性能或编码强度无一致关系，需逐任务直接评估。


<details>
  <summary>Details</summary>
Motivation: 基础模型越来越多地被适配用于医学影像下游任务，但适配策略的选择如何影响子组公平性仍未被充分理解。作者希望通过研究不同适配方法对性能与公平性的影响，揭示二者之间的关系。

Method: 在MIMIC-CXR数据集上，对冻结的Rad-DINO胸部X光编码器应用三种参数高效适配技术（CLS token上的线性头、MLP、多层patch特征上的注意力池化模块），在保持患病率、人口统计均衡的测试集上评估八种病理在种族、性别和影像视图子组上的性能及子组差异，并探测各适配器对受保护属性的编码强度。

Result: 注意力池化取得最强整体鉴别性能以及对受保护属性（尤其种族）的最强编码，但整体性能提升并未持续减少子组差异。早期网络层对种族编码最弱却产生最大的子组性能差距；不同注意力池化层组合下，池化层、属性编码强度与子组公平性之间无一致关系。

Conclusion: 更丰富、更具表达力的表征可以提高准确率，但公平性影响是任务依赖且不可预测的，必须直接且逐任务地评估，而不能仅从属性编码强度或整体性能进行推断。
...
TL;DR: 本文提出SPK框架，通过显式挖掘预训练目标检测器中的潜在先验知识（包括部分级语义、几何和上下文先验），构建紧凑的五维表示用于越界（OoD）检测，在多种检测器架构和基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 目标检测器对训练类别之外的物体常产生过度自信的预测（OoD幻觉）。现有方法要么直接在检测器表示上构建评分函数，要么修改检测器本身来抑制幻觉，但检测器中隐式编码的潜在先验知识尚未被显式解码用于OoD检测，作者希望挖掘并利用这些先验。

Method: SPK框架使用分布内数据和幻觉诱导样本作为诊断监督，显式引出检测器决策背后的部分级语义概念（而非仅用于拒绝或检测器适配）；将这些语义先验与几何和上下文先验整合，形成紧凑的五维SPK表示用于OoD检测。

Result: 在多种目标检测器架构和多个OoD基准上的大量实验表明，SPK达到了最先进的OoD检测性能，证明预训练检测器编码了比通常被利用的更丰富的潜在知识。

Conclusion: 预训练检测器中隐含的潜在知识可以被显式引出并组织成紧凑、结构化、可解释的知识空间，用于预测可靠性分析；这为通过主动挖掘和利用潜在先验来提升检测器可靠性提供了一条有前景的路径。
...
TL;DR: DistScan提出了一种基于预NMS类别分布偏移的后门检测框架，无需模型权重访问、触发器知识或额外训练，即可有效检测目标检测模型中的后门攻击，平均检测准确率比现有最优方法提升27.32个百分点。


<details>
  <summary>Details</summary>
Motivation: 目标检测模型在安全关键应用中容易受到后门攻击，但现有检测方法依赖触发器反演或架构特定假设，无法可靠泛化到场景级攻击（单个触发器同时影响场景中所有物体）。

Method: 核心观测是后门注入会系统性地将模型预NMS预测类别分布偏离训练类别频率。DistScan在干净验证集上聚合中间类别预测，将得到的分布与训练类别频率进行比较，若显著偏离则标记为后门模型。该方法无需权重访问、无需触发知识、无需额外训练。

Result: 在MS-COCO和PASCAL VOC数据集上，跨两种架构和三种场景级攻击场景，DistScan显著优于现有方法，平均检测准确率比最佳适用基线提升27.32个百分点。

Conclusion: DistScan通过利用后门注入导致的类别分布偏移这一简单但未开发的现象，提供了一种高效、通用且无需触发器知识的场景级后门检测方案。
...
TL;DR: 本文提出一种利用正射RGB影像与3D探地雷达（GPR）数据对齐的标注流程，生成大规模真实道路3D GPR数据集，并设计了一种融合残差连接、混合卷积核及注意力机制的3D CNN模型，用于路面修补和裂缝缺陷的二元分类检测，实验表明该模型优于基线架构。


<details>
  <summary>Details</summary>
Motivation: GPR在路面自动检测中面临两大挑战：标注真实数据集稀缺，以及缺乏专为3D GPR数据特性设计的深度学习模型，制约了其大规模自动化应用。

Method: 首先构建低成本数据准备流程，将正射RGB影像与3D GPR扫描数据进行对齐，以路面表面图像为参考，将表面可见缺陷标签迁移至对应GPR分段，实现真实道路场景的大规模高效标注。其次提出专用3D CNN架构，包含残差连接、混合卷积核尺寸、深度注意力和通道注意力机制，以增强特征表示和缺陷分类能力。

Result: 模型在修补和裂缝缺陷的二元分类任务上优于各基线架构，消融实验验证了各设计组件的有效性。

Conclusion: 本文提供了一种可扩展且实用的真实世界数据集生成方法，并提出了新颖的深度学习框架，为3D GPR数据的大规模自动化路面检测提供了可行路径。
...
TL;DR: 本文提出了一种交织的调度与路由问题，其中新工作因现有工作的退化而出现，以“一针及时省九针”为核心理念，应用于空中森林消防场景，通过混合整数规划模型和基于动态约束生成的混合模型最大化价值保留。


<details>
  <summary>Details</summary>
Motivation: 在现实场景中，工作（如火灾区域）会随时间退化，不仅降低自身价值，还会触发新工作的出现（如火势蔓延至相邻区域）。这种交织的调度与路由问题具有实际意义，尤其在森林消防领域，及时干预至关重要。

Method: 提出了一个最大化剩余奖励的混合整数规划模型，并开发了一种基于动态约束生成的混合模型以增强可扩展性。

Result: 通过计算实验和案例研究评估了模型的性能与实用性，并公开了代码库以确保研究的可复现性。

Conclusion: 该问题模型在森林消防场景下能够有效最大化价值保留，且动态约束生成方法提升了模型的可扩展性，为实际应用和后续研究提供了基础。
...
TL;DR: 本文提出一种时空信任场框架，将车辆级别的微观信任聚合成道路上连续的空间-时间表示，并研究了从稀疏路侧单元测量中重建完整信任场的两种深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有信任评估方法主要在单个车辆层面操作，难以推理信任在道路区段上的演化过程，缺乏时空连续性的信任表示。

Method: 提出时空信任场框架，在道路区段上形式化定义信任场；通过受控条件下的合成轨迹进行仿真实验；比较两种重建方法：基于坐标的深度学习方法（通用信任场学习）和场信息引导的深度学习方法（将信任视为车辆携带的潜在量，通过聚合机制强制测量一致性）。

Result: 场信息引导的深度学习方法在恢复轨迹对齐的低信任模式方面更准确，重建误差更低。

Conclusion: 提出的时空信任场框架能够有效聚合车辆级信任为连续时空表示，且场信息引导的重建方法优于通用坐标基线方法。
...
TL;DR: 本文重新审视了Push-T基准任务，探索LLM编码智能体（Claude Code with Fable 5）能否在没有演示数据的情况下，通过Agentic Robotics的编码循环自主创建算法策略。结果显示，该编码智能体不仅以100%成功率完成了Push-T任务，且步数比最佳扩散策略少46%，还能自主扩展到Push-A到Push-Z全字母表任务及多机械臂3D仿真。


<details>
  <summary>Details</summary>
Motivation: Push-T是评估从人类演示中学习操作策略的标志性基准，需要机器人通过单点接触将T形块推至目标位姿。作者希望探究新兴的Agentic Robotics范式下，LLM编码智能体是否能在无需任何演示数据的情况下，仅通过编码循环自主解决该任务，挑战传统模仿学习的范式。

Method: 作者使用Claude Code（Fable 5）作为LLM编码智能体，通过代理编码循环提示其创建无需演示数据的算法解决方案。智能体需要自己检索（找到了2D gym仿真）、通过仿真实验学习推物运动学、迭代优化策略，并自主生成课程以扩展到更复杂的任务和跨具身仿真环境。

Result: 编码智能体实现了100%成功率，且使用比最佳扩散策略（基于200个人类演示训练）少46%的步数。此外，该智能体通过自生成课程将任务从T扩展到完整字母表（Push-A到Push-Z），并生成了Franka和UR5机械臂在3D跨具身仿真中带视觉反馈的仿真代码。

Conclusion: 研究表明，LLM编码智能体在没有人类演示数据的情况下，能够通过代理编码循环解决Push-T基准任务，且在效率和泛化能力上超越了基于大量演示训练的模仿学习策略。这为Agentic Robotics提供了新的可能性，即通过算法推理和仿真交互替代数据驱动学习。
...
TL;DR: 本文提出GigaBrain-WBC-0.5，首个用于人形机器人全身控制的"行为世界模型"（BWM），通过因果Transformer联合预测动作、状态和潜在行为命令分布，并利用自动地形标注管线实现可扩展的地形接触几何恢复，使策略能适应地形交互和合理命令，在四种场景下均超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有全身运动跟踪策略只在平地上训练，缺乏对地形和物体接触如何改变机器人动力学的理解；通过不断扩充参考运动数据来教会策略在任何命令下保持平衡的方法，在可行行为变得依赖环境时失效。

Method: 训练一个因果Transformer作为行为世界模型，联合预测下一动作、下一状态和下一潜在行为命令的分布；开发自动地形标注管线，从重定向运动中恢复完整3D接触几何，实现大规模地形标注；部署时利用预测的分布在线检测不合理命令并将其回撤到已学习行为上。

Result: 在地形交互上达到81.3%成功率（为最强基线的4.3倍），不合理的命令下83.1%，跌倒恢复99.3%（为最强基线的16.8倍），所有四项指标均在三大大规模跟踪基线中取得最高成功率。硬件实验中在缺失支撑和扰动下表现稳健，Unitree G1模型可简单微调迁移至Maker L01机器人。

Conclusion: 通过让"行动的同一网络同时建模环境如何塑造其下一步可能行为"，实现了统一策略，能够实时接受命令、与环境交互，并对不合理命令、跌倒和扰动保持稳健，证明行为世界模型范式在人形全身控制中的有效性。
...
TL;DR: 本文提出GAPL框架，将LLM的语义推理能力与仿真环境动力学结合，通过效果评估、仿真接地和策略优化三个模块形成闭环系统，在高速公路自动驾驶轨迹规划任务上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶轨迹规划需要高层语义推理与低层精确控制的结合。LLM虽具有丰富语义推理能力，但存在幻觉推理、环境动力学接地不足和数值精度有限等问题，需要统一框架来弥补这些缺陷。

Method: GAPL由三个模块组成：(1)基于LLM的效果评估器，用于结构化多维度动作效果估计；(2)基于仿真的效果接地器，从仿真器rollout中预测动力学一致的效果；(3)效果感知决策器，通过蒸馏器将LLM效果估计与仿真结果对接，引导基于PPO的策略学习。

Result: 在四个Highway-env场景中，GAPL一致优于基线方法，碰撞率平均降低0.76，平均位移误差(ADE)降低0.86，最终位移误差(FDE)降低2.00，平均奖励增益达1.44。

Conclusion: GAPL通过将LLM语义推理与仿真动力学接地结合，有效解决了LLM在自动驾驶规划中的幻觉和数值精度问题，证明了语义-物理混合方法在轨迹规划中的优越性。
...
TL;DR: 本文提出VERAGMIL框架，结合高保真模拟器与直观的VR界面，用于记录机器人辅助喂食（RAF）系统中颗粒状食物（如米饭、豆类）操作的演示数据。通过训练三种模仿学习模型（BC、BC-RNN、BCQ）评估框架效果，结果显示VR演示显著优于3D空间鼠标，BCQ模型整体表现最佳，尤其在减少食物洒落方面接近人类专家水平。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助喂食系统对行动不便人群至关重要，但操作颗粒状食物（如米、豆）因其动态物理特性极具挑战性。虽然从人类演示中学习是可行方案，但获取高质量的演示数据十分困难，亟需有效的演示采集与训练框架。

Method: VERAGMIL框架将高保真模拟器与直观的VR界面相结合，提供包含机器人、传感器及多种物理特性各异食物的真实环境，用于录制演示数据并支持不同模仿学习方法。框架在颗粒舀取和运送任务中，分别使用VR界面和3D空间鼠标采集演示，训练BC、BC-RNN和BCQ三种模仿学习模型，并与人类专家基线对比。

Result: 评估指标包括成功率、洒落量、对未见食物的泛化能力及任务完成时间。实验结果表明，VR演示数据显著优于3D空间鼠标数据；BCQ模型在整体性能上最佳，尤其在减少洒落方面表现突出，整体接近人类专家水平。

Conclusion: VERAGMIL框架能有效训练RAF系统进行颗粒材料操作，VR界面的演示采集方式更具优势，BCQ模型展现出最优的实用性能。该框架为机器人辅助喂食领域提供了可行的开源训练方案。
...
TL;DR: 本文提出一种基于深度图像的机器人手术工具-组织接触检测方法，通过提取工具边界周围的最小距离补丁并转为单个标量信号，结合隐马尔可夫模型进行分类，在跨任务和跨假体的泛化上显著优于基于RGB的方法（宏F1 0.927 vs 0.320）。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助手术中可靠的工具-组织接触检测对交互感知控制和力估计至关重要。现有方法大多基于RGB外观学习接触分类器，但泛化能力较差，因此需要更具可迁移性的信号来提升接触检测的跨场景鲁棒性。

Method: 利用立体视觉生成的深度图像，在每个深度帧中定位工具边界周围空间支持的最小距离补丁，将其降维为单个标量-log₁₀|d|作为接触信号。将该信号形式化为全监督的两状态隐马尔可夫模型，采用六折留一会话（LOSO）集成训练，在六个触诊会话上拟合，决策阈值从池化的折外预测中选取。

Result: 在四个留出会话的三个类别（同任务同假体、同任务不同假体、不同任务不同假体）上，所提模型达到宏F1为0.927、AUPRC为0.980。对比的RGB基线模型在同类任务上F1为0.965，但在其他两类上性能大幅下降，综合宏F1仅0.320。

Conclusion: 工具-组织间距离是接触检测中强有力且可迁移的线索，基于深度信息的接触检测方法在跨任务和跨假体泛化上显著优于RGB外观方法，为机器人辅助手术中的可靠接触检测提供了新的方向。
...
TL;DR: 本文提出GuideFetch，一个用于异构引导机器人与取物机器人团队并发执行导航和物体取回任务的协调框架。LLM根据自然语言指令实例化调度条件的四动作模式，在执行前进行别名规范化和动作验证。在90种场景组合的2×2对照实验（360次执行）中，并发执行将平均完成时间缩短41.3%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的机器人计划虽然语法上有效，但可能违反embodiment约束；同时控制器运动看似成功并不等同于任务实际完成。在引导机器人导航带路与取物机器人取咖啡并送达同一座位这种并发场景下，需要一种能够协调并发执行、验证计划有效性并区分"计划有效"与"任务完成"的框架。

Method: GuideFetch框架核心流程包括：(1) LLM从自然语言指令实例化一个调度条件的四动作模式；(2) 执行前对机器人、技能和目标别名进行规范化；(3) 根据注册目标、机器人能力和所选调度对提议动作进行验证；(4) 根据机器人和物体状态控制顺序与并行执行。

Result: 在90种场景与种子组合的2×2对照研究（共360次执行）中，所有180个在线LLM响应均可验证通过且与脚本化计划匹配；顺序与并行执行分别取得72/90和71/90的操作成功率；在两种调度均完成的56个案例中，并行执行平均缩短41.3%的完成时间。

Conclusion: 在受控环境下，角色分工与动作重叠能够缩短任务完成时间，而状态检查能够区分计划有效性与已验证的任务完成。这表明并发执行框架配合验证机制可以有效提升异构机器人团队的协作效率。
...
TL;DR: 本文提出一种基于逆模型控制策略的气动软体外骨骼控制方法，利用Hammerstein动态模型（包含Preisach磁滞模型和线性时不变滤波器）捕捉人机系统行为，仅依赖运动学传感实现任务无关的动态辅助，在腕部测试台上将交互扭矩降低73%，关键肌肉激活降低47%。


<details>
  <summary>Details</summary>
Motivation: 气动人工肌肉为上肢软体外骨骼提供了新机会，但其复杂动力学和有限带宽使得基于用户意图的响应式辅助长期面临挑战，需要一种无需生理或力传感器即可实现动态辅助的控制策略。

Method: 采用Hammerstein动态模型（Preisach磁滞模型+线性时不变滤波器）建模人机系统，利用140秒数据个性化模型，并在控制回路中近似逆模型。仅依赖运动学传感，实现任务无关的动态辅助。

Result: 在模拟腕部软性辅助外骨骼的测试台上，控制器在8°/s至120°/s的速度范围内，将交互扭矩降低高达73%，关键屈肌和伸肌激活降低高达47%。

Conclusion: 本工作提出了一种无需生理或力传感器即可解释用户意图的控制策略，能够为气动软体外骨骼提供任务无关的动态辅助，为日常生活中的康复和辅助应用提供了可行方案。
...
TL;DR: 本文提出"具身差距"（embodiment gap）概念，指机器人基础模型（如VLA策略）在跨机器人本体部署时，从可复用模型/表征/数据到目标机器人执行之间所需的工作量差异。作者构建了二维映射图、综述三类跨具身研究方向，并提出一个超越成功率单一指标的适应性工作报告框架。


<details>
  <summary>Details</summary>
Motivation: 现有机器人基础模型研究多从"扩展视角"出发（更多数据、更大模型、更广基准），忽视了模型泛化到特定机器人本体所需额外适配工作的差异。这种差异直接影响实际部署，但缺乏系统性分析和衡量框架。

Method: 提出"具身差距"概念；构建二维映射图（共享结构类型 × 执行阶段适配需求）；从三个交叉研究方向梳理近期工作：共享语义与感知、共享机器人数据与接口、跨具身对应关系学习；提出适应性工作报告框架以补充成功率指标的不足。

Result: 建立了具身差距的系统化分析框架，识别了跨具身学习比较中应检查的适配工作量，指出了新机器人上仍需完成的工作，并提出了未来研究方向。

Conclusion: 论文强调评估机器人基础模型时不应仅看成功率，还需报告跨本体部署所需的适配工作量。"具身差距"框架为跨具身学习的研究比较、报告规范和未来探索提供了统一视角。
...
TL;DR: 本文提出HarvestPoint-ACT，一种端到端模仿学习框架，通过实例分割和关键点分支显式预测每个可见果实的收获点，并结合调度器对候选果实进行遮挡和距离排序，实现机器人在遮挡环境下稳健的果实采摘。


<details>
  <summary>Details</summary>
Motivation: 端到端模仿学习虽然避免了手工设计机器人运动，但策略仍需自行决定采摘哪个果实以及在哪闭合夹爪。遮挡会导致策略丢失已选果实，且仅凭像素难以推断正确的闭合点。

Method: 采用带关键点分支的实例分割前端预测每个可见果实的掩膜和收获点；调度器按遮挡程度和旅行距离对检测候选排序并选择目标；每次尝试后重新检测和重新排序；将所选果实编码为八维状态（绝对收获点、夹爪到收获点向量、有效性标志、置信度分数）输入动作分块Transformer；当果实暂时不可见时保留最后收获点估计并标记为过时，持续丢失则中止尝试。

Result: 在冠层模拟装置上，HarvestPoint-ACT成功率达到88%，在重度遮挡下达到75%。

Conclusion: 通过将采摘决策（选择哪个果实和闭合点）显式化并在感知层面处理，结合遮挡感知的调度和状态编码策略，可显著提升端到端采摘在遮挡场景下的成功率。
...
TL;DR: 本文提出了一种名为Backward Layout Search (BLS)的机器人装配布局规划方法，通过逆装配顺序分配初始零件位姿，结合碰撞掩码和候选集过滤，在五个装配模型上验证了其生成无碰撞可执行布局的高效性。


<details>
  <summary>Details</summary>
Motivation: 机器人装配布局规划需要为每个零件确定装配站点和初始位姿，同时确保装配序列的无碰撞执行。由于每个装配步骤后障碍环境都会变化，且未装配零件可能阻挡机器人运动，该问题具有挑战性。

Method: BLS按逆装配顺序分配零件初始位姿，基于可行性依赖于当前及后续装配零件初始位姿的观察。每次扩展执行几何、运动学、抓取和规定运动检查，使用碰撞掩码和候选集过滤去除不可行的初始位姿候选，通过波束选择保留有前景的部分布局，最后以正向装配顺序进行完整运动规划验证。

Result: 在五个装配模型上的实验表明，BLS能够生成无碰撞的可执行布局，相比匹配的正向搜索方法，减少了步骤评估次数和搜索时间。

Conclusion: BLS通过利用装配步骤间的依赖关系和逆向搜索策略，有效解决了机器人装配布局规划中的碰撞规避问题，提升了搜索效率。
...
TL;DR: 本文提出DevGRU，一种基于深度图像和点目标条件的导航系统，通过动作预测器和碰撞预测器生成碰撞感知的未来轨迹，在复杂室内环境中显著优于ViNT、NoMaD和NavDP等基线方法，且模型参数更少、推理速度更快。


<details>
  <summary>Details</summary>
Motivation: 现有视觉导航基础模型虽然在多平台泛化上表现良好，但在复杂室内环境（尤其是结构化布局和狭窄通道）中容易发生碰撞，亟需解决导航安全性和鲁棒性问题。

Method: DevGRU系统包含两个核心组件：动作预测器（AP）生成碰撞感知的未来轨迹以避开即时障碍；碰撞预测器与AP协同工作，补偿目标位姿估计中的累积误差并主动缓解未来偏差。

Result: 在九个不同场景中进行实验，DevGRU在导航性能上大幅超越ViNT和NoMaD；模型参数量比NavDP小7倍，推理速度快17倍，且在所有基线中拥有最快的推理时间。

Conclusion: DevGRU通过深度图像与点目标条件融合、碰撞感知轨迹生成和误差补偿机制，在提升室内复杂环境导航安全性方面展现出显著优势，同时兼具轻量高效的特点。
...
TL;DR: 本文实验研究了旋翼无人机旋翼下洗气流对缆索驱动连续体机械臂末端位姿的影响，并提出了基于恒曲率引导的高斯过程回归残差模型来预测和补偿下洗引起的位姿偏差。


<details>
  <summary>Details</summary>
Motivation: 连续体机械臂空中操控系统利用软体机械臂的柔顺性和灵巧性在受限或危险环境中执行任务，但螺旋桨下洗气流会降低系统性能，尤其是在墙壁和地面附近。这一效应对连续体机械臂而言尚未被表征，因此需要系统性研究。

Method: 搭建了与多旋翼平台集成的缆索驱动连续体机械臂实验系统。在静止空气条件下将连续体机械臂与恒曲率模型对比；随后在自由空间中四种油门水平下，以及在最大油门靠近墙壁和地面条件下，量化下洗引起的末端位姿偏差；最后利用实验数据学习恒曲率引导的高斯过程回归残差模型。

Result: 垂直位置和偏航角表现出最大的下洗偏差，且地面效应会放大这些偏差。所提出的恒曲率引导高斯过程回归残差模型将正向位姿预测RMSE降低了89-95%（位置）和47-79%（姿态）。

Conclusion: 该研究为连续体机械臂空中操控系统提供了下洗感知建模方法，支撑下洗效应补偿，有助于提升此类系统在墙体和地面等复杂环境中的操控精度。
...
TL;DR: 本文提出了一种基于加速投影梯度的控制约束差分动态规划（ABC-DDP）框架，用于欠驱动腿式机器人的实时模型预测控制，首次实现了四足机器人在实时有限时域MPC下的静态双腿站立，并支持多种动态运动。


<details>
  <summary>Details</summary>
Motivation: 经典DDP难以处理控制约束，且欠驱动腿式机器人需要满足实时性要求，现有的方法在控制约束处理和计算效率方面存在局限，亟需一种能够高效求解控制约束下的DDP框架。

Method: 提出ABC-DDP算法，利用加速投影梯度法（APG）高效计算约束解并识别活动集，避免重复的KKT矩阵求逆；引入虚拟约束将控制约束整合到可行性驱动的多射击框架中，即使在动态不可行初始条件下也能稳定优化。

Result: 仿真实验展示了存在外部扰动下的静态双腿站立，以及在统一MPC框架下的慢速猫步、直立行走和高速奔跑等多种动态运动。这是首次通过实时有限时域MPC实现四足机器人静态双腿站立。

Conclusion: 所提出的ABC-DDP框架能够在强欠驱动条件下支持短时域的实时模型预测控制，并首次验证了四足机器人在实时有限时域MPC下实现静态双腿站立的可行性。
...
TL;DR: 本文提出了LabDex，一个面向化学实验室灵巧操作的大规模真实世界数据集和基准，首次统一了真实与仿真平台，并通过层次化任务分类体系（原子技能、组合任务、长时程实验）支持对机器人策略的系统训练和评估。


<details>
  <summary>Details</summary>
Motivation: 自主实验室加速科学发现的愿景需要机器人灵巧操作多样实验器具并执行长时程、状态依赖的实验流程，但现有基准未能同时涵盖灵巧手操作、真实实验室交互和多阶段实验流程，缺乏系统性训练与评估的支撑。

Method: LabDex采用层次化任务分类体系，将化学实验室操作分为三个互联层级：原子技能（基础灵巧操作能力）、组合技能、长时程实验室工作流。基准跨平台设计，首次在统一框架下融合真实世界与仿真平台，提供标准化的任务定义、演示数据和评估协议。

Result: 在真实与仿真环境中对代表性机器人学习方法进行了跨层级评估，实验结果验证了LabDex任务设计和演示数据的有效性，表明该基准能够支持现有机器人策略在不同层级的实验室灵巧操作任务上进行训练和系统评估。

Conclusion: LabDex为自主实验室机器人的研究与发展奠定了基础，既支持端到端任务性能评估，也支持分析基础灵巧技能如何组合并影响更复杂的实验室操作。
...
TL;DR: 本文对五种单目SLAM系统在无人机航拍视频场景下进行了全面基准测试，涵盖真实无人机飞行、合成城市场景和长距离航空序列。结果显示系统性能随环境和轨迹尺度变化显著：MASt3R-SLAM在五组DJI飞行中取得最低水平平均绝对误差（参考路径长度的0.53%），DROID-SLAM在全部完成的任务中综合表现最佳（平均2.88%），但垂直方向定位普遍较差，大范围轨迹即使有回环检测仍严重失真。结论是当前单目SLAM方法尚不足以支撑仅依赖视觉的可靠航空导航。


<details>
  <summary>Details</summary>
Motivation: 航空俯视视频存在弱几何约束和严重感知混淆的双重挑战，使得单目SLAM在该场景下的可靠性存疑。为了客观评估现有系统的能力边界，作者设计了无惯性/GNSS辅助的视觉纯定位基准测试，旨在探明当前单目SLAM在航空导航场景中的真实性能水平。

Method: 作者选取五种代表性单目SLAM系统，在三个层次的航空场景上进行基准测试：（1）五组真实DJI无人机本地飞行；（2）合成城市场景图像；（3）长距离航空序列（GES和ALTO数据集）。所有测试均去除惯性测量单元和全球导航卫星系统辅助，以隔离并评估纯视觉SLAM的定位能力。

Result: 结果表明：MASt3R-SLAM在五组DJI飞行中水平平均绝对误差最低（路径长度的0.53%）；DROID-SLAM综合表现最佳，在完成的任务中平均误差为参考路径长度的2.88%；但所有系统在长距离序列中都无法保持一致的整体轨迹形状，且垂直方向定位精度普遍较差，大范围轨迹即便具备回环检测能力仍严重扭曲。

Conclusion: 当前单目SLAM技术尚无法单独支撑可靠的视觉航空导航。尽管在局部短距飞行中某些系统表现良好，但在长距离和大规模场景中均存在显著的空间失真和垂直定位缺陷，需要进一步结合几何先验或其他传感器信息来弥补纯视觉方案的不足。
...
TL;DR: 本研究提出渐进式经验融合（PEF）方法训练多任务TD-MPC2控制器，用于自主血管内导航机械取栓术。PEF在已知解剖结构上成功率74%，优于SAC（37%）和基础TD-MPC2（65%）；在30个血管结构上训练的PEF控制器在10个未见血管上达到90%成功率，并通过微调成功迁移至体外卒中患者血管模型，路径比从63%提升至80%。


<details>
  <summary>Details</summary>
Motivation: 自主血管内导航有望将机械取栓术推广至医疗资源匮乏地区，但控制器需要在不同血管解剖结构中导航长距离、多阶段的路径，现有方法的泛化能力和患者特异性适应能力不足。

Method: 1) 提出渐进式经验融合（PEF）方法训练多任务TD-MPC2控制器；2) 利用残差动作序列离散度动态调整MPPI规划时域的自适应启发式方法；3) 在患者特异性模拟中进行微调以适应新解剖结构。

Result: 1) PEF在五个子任务、十个训练解剖结构上平均成功率达74%（SAC为37%，基础TD-MPC2为65%）；2) 在30个血管上训练的PEF + 自适应时域规划，在10个未见血管上成功率达90%；3) 在体外卒中患者血管模型（透视下）成功迁移，微调后路径比从63%提升至80%（p<0.001），仅需约107分钟临床转院时间对应的微调步数。

Conclusion: 本研究验证了多血管结构训练与患者特异性适应的概念可行性，为自主机械取栓导航的临床落地提供了重要基础，但仍需进一步验证方可进入临床部署阶段。
...
TL;DR: 本文提出不确定时变奖励的定向越野问题（OP-UTVR），允许智能体从观测中估计奖励动态并预测未来奖励，通过三个不同规划周期和在线自适应性的规划器来解决该问题，并在移动服务机器人基准上验证了长时域规划与在线自适应结合的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有定向越野问题（OP）大多假设奖励事先已知，但实际应用中奖励往往是不确定且随时间变化的（如配送代理面临的客户需求变化），需要放松这一假设以支持更现实的决策。

Method: 提出了OP-UTVR问题变体，设计了三个在规划时域和在线自适应性上有所不同的规划器，并为它们在奖励随机性下的性能导出了理论界。此外构建了一个移动服务机器人基准，机器人在室内环境中于行人之间导航。

Result: 实验揭示了规划时域与自适应性之间的权衡，并展示了长时域规划结合在线自适应的有效性。

Conclusion: 长时域规划与在线自适应的结合是应对不确定时变奖励的有效策略，为OP-UTVR问题提供了理论与实践方法。
...
TL;DR: SoftVTBench是一个面向物理交互感知的可变形物体操作的视觉-触觉数据集和闭环基准。该数据集包含4,000条专家演示和50多个资源，以20Hz同步多视角RGB、双指触觉RGB、标记运动、本体感觉、语言和动作，同时提供仅供评估使用的有限元（FEM）状态。基准提出了形变感知成功率（DSR），要求任务完成且峰值归一化形变在容差内才算成功。实验发现所有12种分布内配置都存在违反形变容差的成功轨迹，占各配置成功率的0.7%-24%，且在分布偏移下触觉变体在多数策略-套件对比中表现更好，但分布内收益不一。


<details>
  <summary>Details</summary>
Motivation: 现有可变形物体操作基准大多仅评估任务成功与否，忽视了物理交互质量（如滑移、过度压缩等）。缺乏将策略可见的接触观测与独立物理真值配对的全任务视觉-触觉数据集是主要瓶颈。

Method: 构建包含4,000条专家演示和50多个资源（含体积可变形物体及视觉匹配的刚性孪生体）的视觉-触觉数据集，以20Hz同步多模态数据，并引入仅评估者可用的FEM状态作为物理真值。在此基础上提出DSR指标，通过固定物体特定校准定义形变容差，评估任务完成与峰值形变是否均在容差内。

Result: 在Diffusion Policy、π0.5和FastWAM三种策略下，所有12个分布内配置都有违反形变容差的成功轨迹，占成功率的0.7%-24%。在分布偏移条件下，视觉-触觉变体在全部6个策略-套件对比中取得更高任务成功率，在5个对比中取得更高DSR；但分布内收益是混合的。

Conclusion: 视觉-触觉数据的可用性并不自动保证有效的多模态融合效果。SoftVTBench为研究策略是否成功、如何与可变形物体物理交互以及触觉何时能改善交互提供了公共资源。
...
TL;DR: Dream2Reward通过学习语言条件下的成功潜在状态转移场，从正样本演示中生成密集奖励，通过比较实际运动与预测的成功变化来惩罚错误方向、超调及停滞行为，无需失败标注即可提升机器人策略学习的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的基于进度的奖励在行为偏离成功示范后可能仍然保持较高值，无法有效区分成功与失败状态，尤其是在错误转换发生后仍可能给出误导性的高奖励，需要更精确的奖励信号。

Method: Dream2Reward从正样本演示中学习一个语言条件下的成功潜在状态转移场。给定转换起始前的视觉历史，模型预测成功执行对应的潜在位移，并通过带符号的方向一致性和对称的幅度一致性来评估观察到的位移。该方法不需要失败标注、进度标签或合成负样本，输出密集因果奖励。

Result: 在机制诊断和共享轨迹评估中，Dream2Reward相比基于进度的方法提供更强的成功-失败分离能力和更有信息量的低质量行为反馈。在在线和离线策略学习中，冻结的奖励模型减少了奖励黑客攻击并支持更强的下游性能，包括真实机器人操作任务。

Conclusion: 将实际运动与预测的成功变化进行比较，是有效将正样演示转化为机器人学习密集奖励的有效途径。
...
TL;DR: RoomWright是一个智能体驱动的、以使用场景为导向的3D场景生成框架，用代码表示场景，通过面向任务的对象推理、多部件交互编译和注释引导的方向优化，生成可执行、可编辑、可仿真的交互式环境。


<details>
  <summary>Details</summary>
Motivation: 现有的基于代码的场景生成方法虽然能生成可编辑和可扩展的环境，但主要关注视觉构建和对象级关节，忽视了场景的功能性使用（即对象如何被实际操作和交互），导致生成的环境缺乏交互能力。

Method: 1) 使用场景驱动的对象推理：将每个锚点视为任务中心，纳入任务所需对象及其功能属性；2) 代码智能体实现多部件交互：将每次交互编译为触发-条件-效果的规则，更新结构化对象状态并捕捉对象间因果依赖；3) 注释引导的使用场景驱动方向优化：通过注释信息缓解可操作对象方向模糊且难以从像素恢复的问题。

Result: 大量实验证明了方法的有效性。生成的场景是可执行的、可编辑的、可直接用于仿真的，为具身AI和策略学习提供了交互环境。

Conclusion: RoomWright成功弥合了场景视觉构建与功能使用之间的鸿沟，使得生成的3D场景不仅能看、能编辑，还能真正被智能体交互和使用，为具身AI和策略学习提供更实用的仿真环境。
...
TL;DR: RoboEdit是一个将人类操作视频转换为机器人视频的编辑套件，包含自动化数据管线RoboEdit-ADC（生成174K对齐视频对/1400万帧的RoboEdit-14M数据集，覆盖7种机器人形态）、核心编辑引擎RoboEdit-Trans（通过跨形态适应模块和3D机器人状态解码器实现时序一致性和结构化的运动监督），并支持真实世界下游机器人控制策略。


<details>
  <summary>Details</summary>
Motivation: 机器人手-物体交互数据的采集成本高昂且受限于具体形态，而大量可用的人类操作视频却无法直接用于机器人训练，亟需一种方法将丰富的人类视频转化为可用的机器人训练数据。

Method: 提出了双层方法：(1) RoboEdit-ADC自动数据管线，从RGB视频中重建3D交互并跨形态重定向，生成大规模对齐数据集；(2) RoboEdit-Trans编辑引擎，利用跨形态适应模块保持时间一致性并适配外观与运动，同时集成3D机器人状态解码器恢复逐帧手部状态以提供结构化运动监督。

Result: 实验表明RoboEdit在编辑质量上达到最先进水平，并能在真实世界操作任务中有效支持下游机器人控制策略。

Conclusion: RoboEdit套件释放了无标注人类视频的巨大潜力，为通用机器人学习提供了可扩展、高保真的视觉和3D运动监督。
...
TL;DR: 本文提出AgriNav，一个面向稻田定点杂草管理的集成自主拖拉机系统，通过四个ROS耦合模块解决田间部署中的三大挑战（GNSS退化下的行导航、水稻与杂草的实时视觉区分、误分类不可逆性），并引入零额外硬件成本的LiDAR-相机融合桥接机制。


<details>
  <summary>Details</summary>
Motivation: 稻田定点杂草管理相比传统喷洒可大幅减少除草剂用量，但受限于三个关键挑战：冠层下GNSS信号退化导致的行导航不稳定、水稻与形态多样杂草的实时区分困难、以及将水稻误判为杂草的不可逆代价。

Method: 构建由四个ROS耦合模块组成的AgriNav系统：自定义PyTorch实现的WeedDet水稻检测模型、带非对称类别权重的1.68M参数CNN-FPN变体、基于硬编码置信度门控否决的反向逻辑判别模块、以及融合GNSS/IMU/轮式里程计并具备三级中断桥接的6状态恒定速度转弯率扩展卡尔曼滤波器。核心贡献是四机制LiDAR-相机融合桥：利用导航LiDAR进行ROI约束、世界坐标投影、地面平面滤波和双向置信度融合。

Result: 仿真实验表明：系统可在20秒GNSS中断期间持续位置跟踪，作物行检测置信度全程保持0.9以上，水稻检测置信度在稻田、航拍和洪水后图像中达到0.32至0.95。LiDAR ROI约束将检测推理区域缩减约30%至50%。

Conclusion: AgriNav通过系统级集成与零额外硬件成本的传感器融合策略，有效解决了田间定点杂草管理的导航、判别和误分类三大难题，为实际农田部署提供了可行方案。
...
TL;DR: 本文提出LT-Mem，一个波动感知的记忆演化框架，通过多会话SLAM骨干、波动感知策略和Tri-Memory（Live、Delta、Meta）结构，在长期机器人运行中实现跨会话的对象级持久记忆，并引入LT-VQA数据集进行验证，在各项指标上优于基线方法且消耗少一个数量级的token。


<details>
  <summary>Details</summary>
Motivation: 现有系统要么覆盖历史以维护最新地图，要么存储语义快照但缺乏跨会话一致的对象身份，导致"时间失忆"——无法回答类似"绿色椅子在所有会话中出现在哪里"这样的历史查询问题。

Method: 三部分架构：(1) 多会话SLAM骨干提供跨会话空间对齐的单实例3D感知；(2) 推理层通过确定性证据评分保持跨会话身份，波动感知策略在覆盖、保持、多假设三种动作间选择；(3) Tri-Memory结构（Live、Delta、Meta）同时保留当前状态和事件历史。同时构建LT-VQA数据集用于评估。

Result: LT-Mem在所有评估指标上持续优于基线方法，且token消耗减少一个数量级；消融实验证实性能提升来自结构化记忆架构而非LLM容量。

Conclusion: 结构化记忆架构是解决长期机器人操作中对象级时间推理问题的关键，波动感知的记忆演化和多层记忆结构能够在不依赖更大模型的情况下实现高效、持久、可查询的对象历史理解。
...
TL;DR: 本文提出DA-WAM框架，将预测性表示学习、动作条件未来建模和轨迹评分统一到单一决策目标下，使未来预测直接服务于驾驶决策，在NAVSIM-v1和NAVSIM-v2上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶世界模型存在关键缺陷：未来建模仅具预测性而缺乏决策信息性，即预测的未来未能直接指导轨迹选择。已有方法要么将未来表示学习与规划优化解耦，要么在不同轨迹候选项间共享预测状态，从而稀释了应当指导选择的动作特定后果。

Method: DA-WAM包含三个核心组件：(1)通过在线编码器和稳定的动量目标在整个规划优化过程中维持预测监督，使未来表示与驾驶任务共同演化；(2)动作条件预测器为每条轨迹候选项生成独特的未来潜在状态；(3)未来潜在条件因子化评分器评估各轨迹。专家匹配轨迹的未来潜在表示由观测到的未来表示监督，同时安全关键硬负样本在规划边界附近提供额外监督。

Result: 在NAVSIM-v1和NAVSIM-v2基准上取得了最先进的性能，消融实验和诊断分析验证了各关键组件的有效性。

Conclusion: 通过将预测与决策统一到单一目标下，DA-WAM实现了真正决策信息性的未来建模，为安全自动驾驶提供了更有效的世界模型范式。
...
TL;DR: ADEPT是一个大规模强化学习框架，通过预训练通用物体重摆放策略作为先验，实现高自由度多指机器人的灵巧操作行为学习，并具备从原始视觉-触觉感知进行sim-to-real迁移的能力。该框架在23自由度Kuka-Allegro和29自由度Flexiv-Sharpa两种实体上实现了零样本迁移，能以人类级别的速度解决长时域任务。


<details>
  <summary>Details</summary>
Motivation: 多指机器人的高自由度灵巧操作任务通常难以从零开始学习，且每个新任务都需要重新学习相同的技能，缺乏可复用的行为先验。同时，现有的强化学习方法在sim-to-real迁移和长时域任务求解上存在挑战。

Method: ADEPT采用两阶段训练范式：首先在通用物体重摆放任务上预训练灵巧策略作为行为先验，然后通过结合行为克隆蒸馏、评论家预热和保守在线更新的稳定后训练配方来微调下游策略。同时引入关节空间几何织物（Geometric Fabric）作为RL策略与机器人之间的安全中介，最后将训练好的教师策略蒸馏为基于感知的学生策略。

Result: 预训练策略能零样本完成下游任务的重摆放阶段；通过稳定的后训练配方避免了朴素RL微调导致的性能退化。在23自由度Kuka-Allegro和29自由度Flexiv-Sharpa两个实体上实现了零样本sim-to-real迁移，能够从具有挑战性的初始状态解决长时域任务，并以人类级别的速度执行灵巧操作。

Conclusion: ADEPT证明了在大规模预训练范式下，灵巧操作技能可以被复用和迁移，显著降低了新任务的学习难度，并成功在真实机器人上实现了从视觉-触觉感知到高自由度灵巧操作的全链路sim-to-real迁移。
...
TL;DR: 本文提出PartialBiGrasp，一种基于部分点云观测的双臂机器人抓取框架，通过卷积占用网络隐式学习局部几何特征（可抓取性、无碰撞接触区域、物体厚度），生成力闭合抓取对并经采样优化精化，在仿真和真实实验中验证了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有双臂抓取方法假设可获得物体的完整点云，其包含抓取所需的几何信息，但在真实场景中往往只能获取部分观测数据，这限制了方法的实际应用。

Method: 提出PartialBiGrasp框架，直接在部分点云观测上工作。通过卷积占用网络隐式学习几何特征，实现对可抓取性、无碰撞接触区域和物体厚度的局部推理，生成力闭合抓取对，并使用基于采样的优化方法修正因几何不完整导致的歧义。

Result: 通过解析力闭合指标评估、大规模仿真实验以及在噪声部分点云上的真实机器人评估，验证了该方法能够生成鲁棒且物理稳定的双臂抓取。

Conclusion: 该研究解决了真实场景中因点云不完整导致的双臂抓取难题，证明在部分观测条件下也能实现可靠的力闭合抓取，具有实际应用价值。
