...
TL;DR: 本文开发了一个分析框架来研究AI系统在有限时间内能力无限增长的条件，提出了基于可观测指标（设施功率、IO带宽、训练吞吐量等）的决策规则，用于判断AI是否会失控增长或保持稳定。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统在什么可测量条件下能力会无限增长，以及在什么条件下可以排除这种可能性，用可测试的条件替代猜测。

Method: 开发递归自我改进的分析框架，将能力增长与资源建设和部署策略联系起来，利用物理和信息理论限制定义服务边界，建立内生增长模型耦合资本与计算、数据和能源。

Result: 推导出将可观测序列映射为失控与正常行为证书的决策规则，提供了基于改进加速速度相对于当前水平的可证伪测试，以及可直接实施的安全控制措施。

Conclusion: 该框架用可测试条件和可部署控制措施替代了猜测，为认证或排除AI奇点提供了理论基础和实践工具。
...
TL;DR: 提出一种可扩展的高效数据归因方法，通过将基于遗忘的慢速归因方法蒸馏到特征嵌入空间，实现高效检索高影响力训练图像，比现有方法快2500-400000倍。
...
Result: 在MSCOCO中等规模模型和LAION上的Stable Diffusion大规模模型上验证，能在几秒内达到更好或竞争性性能。
...
Method: 通过德尔菲法达成工作流程描述符共识，开发ColoWorkflow工具，并在多中心视频数据集上由独立评估者应用该工具，评估适用性和评估者间可靠性。
...
Method: 提出了一种新的基于Layer-wise Relevance Propagation(LRP)的剪枝方法，能够在少量数据情况下进行剪枝。

Result: 该方法相比现有方法实现了更高的剪枝率，同时更好地保持了模型精度。

Conclusion: 该方法在数据有限场景下实现了比现有方法更好的精度保持效果，具有更好的实用价值。
...
TL;DR: 提出短窗口滑动学习框架用于实时暴力检测，将视频分割为1-2秒片段，利用LLM自动标注构建细粒度数据集，在RWF-2000上达到95.25%准确率，在UCF-Crime长视频上提升至83.25%。


<details>
  <summary>Details</summary>
Motivation: 传统长视频训练方法无法有效捕捉快速暴力事件，需要开发能够实时检测并保持时间连续性的暴力识别方法。

Method: 采用短窗口滑动学习框架，将视频分割为1-2秒片段，使用大语言模型自动标注构建细粒度数据集，充分利用所有帧保持时间连续性。
...
TL;DR: 本文提出了一种竞争性强且计算效率高的概率性降雨临近预报方法，通过视频投影器和轻量级概率头结合预训练卫星视觉编码器，在Weather4Cast 2025基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一种既具有竞争力又计算效率高的概率性降雨临近预报方法，以改进传统的确定性预报方法。

Method: 使用视频投影器(V-JEPA Vision Transformer)和轻量级概率头连接到预训练的卫星视觉编码器(DINOv3-SAT493M)，将编码器标记映射到4小时累积降雨量的离散经验累积分布函数(eCDF)，并通过连续排序概率评分(CRPS)进行端到端优化。

Result: 在Weather4Cast 2025基准测试中，该方法取得了CRPS为3.5102的优异性能，相对于最佳3D-UNET基线模型实现了约26%的有效性提升。
...
TL;DR: PhaseWin是一种新颖的阶段窗口搜索算法，用于对象级基础模型的忠实区域归因，具有近线性复杂度，显著提高了计算效率。
...
Method: PhaseWin采用分阶段的粗到细搜索策略，结合自适应剪枝、窗口化细粒度选择和动态监督机制，替代传统的二次成本贪婪选择。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在OOD检测中使用的负提示往往包含广泛的非ID特征，可能捕获重叠或误导性信息，导致结果不理想。

Method: 使用LLM初始化类特定的正负提示，然后优化这些提示：正提示关注类内特征，负提示强调类别边界特征，并通过图架构将语义监督传播到视觉分支。
...
TL;DR: 本研究评估了YOLOv11n和YOLOv12n两种轻量级模型在面部表情识别中的性能，发现YOLOv12n在干净数据集上表现更好，而YOLOv11n在噪声环境中具有更高精度。
...
Result: YOLOv12n在干净KDEF数据集上获得95.6 mAP 0.5的最高性能，在FER2013数据集上mAP为63.8；YOLOv11n在FER2013上精确度达65.2，假阳性更少。
...
TL;DR: 本文提出了一种名为异构互补蒸馏（HCD）的框架，用于解决异构架构（如ViT到ResNet18）知识蒸馏中的特征表示差异问题。该方法通过整合教师和学生的互补特征，在共享logits中对齐表示，并通过子logit解耦蒸馏和正交性损失来促进多样化知识转移。


<details>
  <summary>Details</summary>
Motivation: 异构架构知识蒸馏面临空间特征表示差异的挑战，传统同构KD方法难以有效解决。现有异构KD方法存在计算成本高、设计复杂或过度依赖logit对齐的问题，无法充分利用互补特征。
...
TL;DR: FedBCS通过领域不变上下文原型对齐解决联邦学习中特征异质性问题，引入频域自适应风格重校准和上下文感知双级原型对齐方法，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中不同医疗机构的扫描设备或协议差异导致特征异质性，现有方法主要关注最终层特征而忽略多级线索，且中间层的领域特定偏差累积降低模型鲁棒性。

Method: 提出频域自适应风格重校准来解耦内容-风格表示并学习最优风格参数，构建更鲁棒的领域不变原型；设计上下文感知双级原型对齐方法，从编码器和解码器的不同层提取领域不变原型并与上下文信息融合。

Result: 在两个公共数据集上的广泛实验表明，该方法表现出显著性能提升。

Conclusion: FedBCS通过领域不变上下文原型对齐有效解决了联邦学习中的特征异质性问题，提高了分割任务的准确性和模型鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别对于推断真实情感至关重要。现有方法面临两个核心挑战：静态外观与动态运动线索的纠缠阻碍模型关注细微运动；现有MER数据集的文本标签与底层面部肌肉运动不完全对应，造成文本监督与物理运动之间的语义鸿沟。
...
Result: 在多个具有挑战性的MER基准测试中表现出最先进的性能，在局部面部运动的可解释建模方面具有特别优势。
...
TL;DR: ERMoE提出了一种新的稀疏MoE架构，通过将专家重新参数化为学习到的正交特征基，并用特征基得分（输入特征与专家基的余弦相似度）替代学习到的门控逻辑，从而稳定路由并促进可解释的专家专业化。


<details>
  <summary>Details</summary>
Motivation: 解决传统MoE架构中的两个核心挑战：路由器逻辑与专家内部结构不对齐导致的不稳定路由和专家利用率不足，以及负载不平衡造成的瓶颈问题。标准解决方案（如辅助负载平衡损失）会削弱专家专业化并损害下游性能。

Method: ERMoE将每个专家重新参数化为学习到的正交特征基，用特征基得分（输入特征与专家基的余弦相似度）替代学习到的门控逻辑，实现内容感知路由，将token分配直接与专家表示空间绑定。
...
Conclusion: ERMoE引入了一种新的稀疏专家模型架构原则，直接解决路由不稳定性问题，在保持稀疏性的同时实现可扩展、可解释的专家专业化和改进的性能。
...
TL;DR: DMC是一个用于CLIP模型类增量学习的两阶段框架，通过解耦视觉编码器和文本软提示的优化来缓解分类器偏差问题。DMC-OT是增强版本，引入最优传输校准策略来对齐记忆统计信息。
...
Method: 提出两阶段框架：第一阶段冻结文本提示优化视觉编码器，第二阶段冻结视觉编码器优化文本软提示。DMC-OT增加最优传输校准策略和任务特定提示设计。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101数据集上达到最先进性能，DMC-OT相比DMC平均提升1.80%准确率。

Conclusion: DMC框架通过解耦模态优化有效缓解了CLIP在类增量学习中的分类器偏差问题，DMC-OT进一步通过统计校准提升了性能。
...
TL;DR: 本文提出Phase Aggregated Smoothing (PAS)方法，通过多相位平均来平滑视频LLMs中的时间核，解决时间不一致性问题，提高模型对微小时间偏移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小偏移会导致注意力翻转并抑制相关帧。这种不稳定性源于多模态RoPE扩展导致的逆傅里叶时间核中的帧级波纹。

Method: 提出PAS方法，在多个注意力头中应用小的相反相位偏移，然后聚合它们的输出。这种方法保持每个头的频谱幅度，同时通过聚合有效平滑时间核并降低相位敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现成视觉语言模型在零样本视觉任务中直接回答开放性问题时准确率不高的问题，提出一种推理时设计而非任务特定训练的方法。

Method: 工作流包含两个步骤：(1) 量化：将开放性问题转化为具有明确候选答案的多项选择题；(2) 二值化：对每个候选答案提出真/假问题，通过确定性逻辑进行选择。

Result: 在指代表达式定位、空间推理和BLINK-Jigsaw等任务上，量化到多项选择题带来显著提升，真/假二值化进一步提供一致性的性能提升。

Conclusion: 该工作流提供了一个简单统一的推理时设计方法，无需特定任务训练即可显著提升现成视觉语言模型在零样本视觉任务中的性能。
...
TL;DR: 提出了Phys-Liquid物理信息数据集，包含97,200个模拟图像和3D网格，用于透明变形液体的几何和体积属性估计。通过四阶段重建和估计流程，在液体几何和体积重建方面实现了更高的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 自主机器人执行精确液体操作任务时，容器移动会引起液体表面动态变形，使得准确评估液体状态变得复杂。现有数据集缺乏代表不同动态场景下真实液体行为的物理信息模拟数据。
...
TL;DR: CLUE是一个基于Stable Diffusion的生成模型框架，通过固定格式提示实现多样且稳定的图像生成，无需额外数据。在医学图像数据集上显著提升了生成质量和分类器性能。


<details>
  <summary>Details</summary>
Motivation: 解决专业领域（如医学）中数据有限且类型单一的问题，现有方法在通用领域表现良好但在专业领域受限。

Method: 基于Stable Diffusion架构，引入Style Encoder处理图像和提示生成风格嵌入，通过新的第二注意力层和KL散度实现潜在空间的连续高斯表示。

Result: 在中耳炎数据集上，FID从46.81降至9.30，召回率从49.60%提升至70.29%。仅使用合成数据训练的分类器F1得分达83.21%，合成与真实数据结合时F1得分达94.76%。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在模态缺失时性能显著下降，主要原因是完整模态数据与不完整模态场景之间的表示学习不一致。现有方法通过简单生成方式处理缺失模态，但未能充分保持跨模态一致性。
...
Conclusion: PROMISE通过创新的提示注意力机制和分层对比学习，有效弥合完整数据与不完整数据之间的表示差距，显著提升了模态缺失情况下的跨模态表示鲁棒性。
...
TL;DR: EmoVid是首个专门为创意媒体设计的多模态情感标注视频数据集，包含卡通动画、电影片段和动画表情包，每段视频都有情感标签、视觉属性和文本描述。通过分析发现视觉特征与情感感知之间的时空模式，并基于此开发了情感条件视频生成技术，显著提升了文本到视频和图像到视频任务的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成系统主要关注低层次视觉指标而忽视情感维度，视频领域缺乏专门的资源来连接情感理解与生成任务，特别是在风格化和非现实场景中。

Method: 引入EmoVid数据集，通过系统分析揭示视觉特征与情感感知的时空模式，并基于Wan2.1模型微调开发情感条件视频生成技术。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型取得了显著成功，但在复杂视觉任务中经常受到"视觉处理瓶颈"的阻碍：在长时间生成过程中容易失去视觉证据的根基，并表现出情境化视觉经验的缺乏。

Method: 受人类认知记忆理论启发，提出VisMem框架，为VLMs配备动态潜在视觉记忆，包括用于细粒度感知保留的短时模块和用于抽象语义巩固的长时模块。这些记忆在推理过程中无缝调用，使VLMs能够在思考和生成过程中保持感知保真度和语义一致性。

Result: 在多样化的视觉理解、推理和生成基准测试中，VisMem相比原始模型实现了11.8%的平均性能提升，并优于所有对比方法。

Conclusion: VisMem为潜在空间记忆增强建立了新范式，显著提升了视觉语言模型在复杂视觉任务中的性能表现。
...
TL;DR: SP-Guard是一种针对扩散式文本到图像模型的安全保护方法，通过估计提示词危害性并应用选择性引导掩码，仅对图像中的不安全区域进行引导，在提升安全性的同时最小化对内容的意外修改。
...
Method: SP-Guard通过估计提示词危害性并应用选择性引导掩码，仅对图像中被识别为不安全的区域进行引导，而不是对整个图像施加统一的引导强度。
...
TL;DR: 提出SUPER解码器块，利用小波完美重建特性防止信息丢失，选择性抑制冗余特征，作为即插即用模块提升U-Net变体在逆问题中的高频细节恢复能力。
...
Result: 在CrackVision12K数据集上显著提升细裂纹（宽度小于4像素）分割性能；在SIDD智能手机图像去噪中PSNR适度提升；验证了其在不同频率域中的鲁棒性。
...
TL;DR: AirCopBench是首个专门评估多模态大语言模型在具身空中协作感知中的基准，包含14.6k+问题，涵盖4个关键任务维度，在挑战性感知条件下测试模型性能。
...
Method: 使用模拟器和真实世界数据构建基准，包含14.6k+问题，涵盖4个任务维度和14种任务类型，通过模型、规则和人工方法生成问题并实施严格质量控制。

Result: 对40个MLLMs的评估显示在协作感知任务中存在显著性能差距，最佳模型平均落后人类24.38%，且在不同任务中表现不一致。

Conclusion: 微调实验证实了空中协作感知和推理中模拟到真实迁移的可行性，基准揭示了MLLMs在复杂协作场景中的局限性。
...
TL;DR: EmbryoDiff是一个基于扩散模型的两阶段框架，用于体外受精胚胎发育阶段的细粒度识别，通过多焦点特征融合和混合语义-边界条件注入，在细胞遮挡情况下实现准确分类。
...
Method: 提出两阶段扩散框架：第一阶段训练冻结帧级编码器提取鲁棒多焦点特征；第二阶段引入多焦点特征融合策略构建3D感知形态表示，并设计混合语义-边界条件块注入扩散去噪过程。

Result: 在两个基准数据集上达到最先进结果，仅用单次去噪步骤就获得最佳平均测试性能，准确率分别达到82.8%和81.3%。
...
<details>
  <summary>Details</summary>
Motivation: 解决可控生成模型在处理控制条件和内容生成时的计算需求问题，提高生成效率。

Method: 使用块级粗粒度缓存基于特征重用来动态绕过模型推理各步骤间编码器-解码器块的冗余计算；设计提示级细粒度缓存，在模块内重用连续推理步骤间的交叉注意力图，并将其扩展到相邻步骤的相应模块计算。

Result: 在COCO-Stuff分割基准测试中，HGC将计算成本（MACs）显著降低了63%（从18.22T降至6.70T），同时将语义保真度损失控制在1.5%以内。

Conclusion: HGC方法在平衡生成效率和视觉质量方面具有优势，能够有效降低可控生成模型的计算开销。
...
TL;DR: CrossMed是一个评估医学多模态大语言模型组合泛化能力的基准，使用模态-解剖-任务(MAT)框架，将4个公共数据集转换为统一的视觉问答格式，包含20,200个多选题实例。
...
Method: 将CheXpert、SIIM-ACR、BraTS 2020和MosMedData四个数据集重新格式化为统一的视觉问答格式，构建相关和不相关的MAT分割，以及零重叠设置。评估LLaVA-Vicuna-7B和Qwen2-VL-7B模型。

Result: 在相关分割上模型达到83.2%分类准确率和0.75分割cIoU，但在不相关和零重叠条件下性能显著下降。跨任务迁移显示分割性能提升7% cIoU。多模态LLM在组合泛化方面表现优异。
...
TL;DR: 该论文提出了一种多参考图像生成式图像合成方法，通过校准前景参考图像的全局和局部特征，使其与背景信息兼容，从而实现细节保持和前景姿态/视角调整的平衡。
...
Result: 在MVImgNet和MureCom数据集上的广泛实验表明，生成模型能够从校准的参考特征中显著受益。

Conclusion: 通过多参考图像和特征校准，生成式图像合成模型能够更好地平衡细节保持和前景调整的需求。
...
TL;DR: LiteAttention通过利用扩散注意力中稀疏模式的时序一致性，在去噪过程中实现进化计算跳过，显著加速视频扩散变换器而保持质量不变。


<details>
  <summary>Details</summary>
Motivation: 现有扩散变换器在视频生成中面临二次注意力复杂度的瓶颈，现有加速方法存在动态稀疏模式计算开销大和静态模式次优的权衡问题。

Method: 利用扩散注意力稀疏模式在去噪步骤间的时序一致性，通过早期标记非必要tile并向前传播跳过决策，实现进化计算跳过，无需重复分析开销。

Result: 在FlashAttention基础上实现高度优化的LiteAttention内核，在工业生产视频扩散模型上实现显著加速，且无质量损失。
...
TL;DR: 这篇论文系统综述了2016-2025年间糖尿病视网膜病变（DR）的深度学习研究进展，整合了50多项研究和20多个数据集，重点关注方法创新、评估标准和临床应用挑战。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球可预防性失明的主要原因，早期检测对减少视力丧失至关重要。深度学习在过去十年中彻底改变了DR筛查，但需要系统梳理技术进展和临床转化障碍。

Method: 采用系统综述方法，分析自监督学习、半监督学习、领域泛化、联邦训练和混合神经符号模型等技术进展，同时考察评估协议、报告标准和可重复性挑战。

Result: 建立了跨数据集的基准性能表，识别了多中心验证和临床信任方面的开放差距，为可重复、隐私保护和临床可部署的DR AI提供了实用议程。

Conclusion: 该研究不仅为DR筛查提供了技术路线图，其调查的创新方法也广泛适用于规模化医学影像分析，连接了技术进步与转化障碍。
...
TL;DR: 本文提出了S2D-Align方法，通过从粗到深的策略建立解剖学基础的跨模态对齐，利用不同粒度的辅助信号来改进放射学报告生成的质量。
...
Method: 提出S2D-Align范式，采用从浅到深的策略：从粗粒度的放射图像-报告配对开始，引入参考报告进行实例级指导，最后使用关键短语将生成过程锚定在特定解剖细节上，并通过基于记忆的适配器实现特征共享。
...
TL;DR: 本文比较了去噪扩散概率模型和自回归因果变换器在3D形状生成和补全任务中的表现，发现扩散模型在连续潜在空间中表现最佳，而自回归模型在相同离散潜在空间中也能达到或超越扩散模型的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管生成模型已在多种数据模态中广泛应用，但对于哪种模型最适合特定任务尚未达成共识。特别是条件信息（如文本和图像）被广泛使用，而部分3D数据等条件信息尚未得到充分评估。

Method: 本研究比较了两种最有前景的生成模型——去噪扩散概率模型和自回归因果变换器，将它们适配用于生成形状建模和补全任务。进行了全面的定量评估和比较，包括基线判别模型和广泛的消融研究。

Result: 结果显示：(1) 具有连续潜在空间的扩散模型在从单个噪声深度图像进行多模态形状补全的任务中，优于判别模型和自回归方法，达到了最先进的性能；(2) 在相同的离散潜在空间中，自回归模型能够匹配或超越扩散模型在这些任务上的表现。

Conclusion: 扩散模型在连续潜在空间中的表现优于其他方法，而自回归模型在离散潜在空间中具有竞争力，表明模型选择应根据具体任务和潜在空间表示来决定。
...
TL;DR: SaTformer是一个基于全时空注意力的视频变换器，能够从卫星辐射数据中准确预测极端降水，并在NeurIPS Weather4Cast 2025累积降雨挑战赛中夺冠。
...
Conclusion: SaTformer通过视频变换器架构和创新的数据处理技术，成功解决了极端降水预测问题，为AI天气预测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 将强化微调(RFT)框架扩展到大型视频语言模型(LVLMs)面临挑战，需要专门针对视频推理特点设计新的方法。

Method: 提出两阶段框架：SFT阶段使用三步流程生成高质量过程感知链式思维数据集VideoP2R-CoT-162K；RL阶段引入过程感知组相对策略优化(PA-GRPO)算法，为感知和推理提供独立奖励。

Result: 在7个视频推理和理解基准测试中的6个达到最先进性能，消融研究证实了过程感知建模和PA-GRPO的有效性。

Conclusion: VideoP2R成功将RFT框架扩展到视频领域，过程感知建模显著提升了视频推理能力，且模型感知输出对下游推理具有充分信息量。
...
<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型等AI技术的快速发展，生成逼真合成媒体的能力大幅提升，导致难以区分真实与合成内容。深度伪造等技术的滥用可能传播错误信息、侵犯隐私和进行欺诈，因此需要有效的检测方法。
...
Conclusion: 建议未来研究聚焦于多模态深度学习模型，这种模型有潜力提供更鲁棒和通用的检测能力，为构建更强防御体系提供明确起点。
...
TL;DR: 本文提出了一种大型向量化字形模型(LVGM)，通过预测下一个笔画来生成向量化中文字形，并发布了一个包含907,267个样本的大规模中文SVG数据集。


<details>
  <summary>Details</summary>
Motivation: 向量化字形因其可扩展性和灵活性在多个领域广泛应用，基于大语言模型的标记序列预测能力，可以通过笔画建模实现向量化字符生成。

Method: 首先将笔画编码为离散潜在变量（笔画嵌入），然后通过微调DeepSeek LLM来预测下一个笔画嵌入，训练LVGM模型。

Result: 实验结果显示模型在数据规模上具有缩放行为，生成的向量化字形已通过专家和相关人员验证，能够基于有限笔画生成完整字符、语义优美的词语甚至未见过的诗句。

Conclusion: LVGM模型成功实现了通过笔画预测生成向量化中文字形，为向量化字形生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的KBVQA方法要么通过上下文学习利用MLLMs中的隐式知识，要么通过检索增强生成利用显式知识，但它们的推理过程仍然不明确，缺乏从MLLMs中获得的明确多步轨迹。

Method: 首先通过提示冻结的7B大小MLLM完成问题与其真实答案之间的推理过程，构建Hindsight-Zero训练数据；然后将Hindsight-Zero自蒸馏为思维链生成器和知识生成器；最后通过KEPO优化知识生成器，偏好低置信度但有用的知识。

Result: 在OK-VQA和A-OKVQA上的实验验证了HinD的有效性，表明使用7B大小MLLM激发推理的HinD在不使用商业模型API或外部知识的情况下实现了优越性能。

Conclusion: HinD框架成功激发了MLLMs的内部知识推理能力，通过后见蒸馏和知识鼓励偏好优化解决了KBVQA中的推理监督和知识置信度错位问题。
...
TL;DR: OT-ALD是一种基于最优传输理论的图像到图像翻译框架，解决了DDIB方法在翻译效率和潜在分布不匹配方面的挑战，通过OT映射优化翻译轨迹，在保持图像质量的同时显著提升效率。
...
Result: 在三个高分辨率数据集的四个翻译任务上，OT-ALD相比最佳基线模型平均提升20.29%的采样效率，FID分数降低2.6。

Conclusion: OT-ALD通过最优传输理论有效消除了潜在分布不匹配问题，在图像翻译效率和图像质量之间实现了良好平衡。
...
TL;DR: 本文提出了一种基于混响变换的轨迹预测模型Rev，通过显式学习和预测智能体对轨迹变化事件的响应延迟，实现了可控的轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法难以显式学习和预测智能体对轨迹变化事件的响应延迟（latency），这会影响预测系统的因果连续性并导致不合理的轨迹。不同智能体对特定事件可能有不同的延迟偏好。

Method: 受声学中混响曲线的启发，提出混响变换和Rev轨迹预测模型，使用两个显式可学习的混响核来模拟和预测每个智能体的延迟偏好及其随机性。
...
TL;DR: MultiTypeFCDD是一个轻量级卷积框架，用于可解释的多类型异常检测，能够区分不同类型的异常，同时减少计算资源和内存需求。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法通常只能识别异常但无法区分异常类型，且需要为每个对象类别训练和维护单独的模型，这成本高昂。同时，现有的大规模视觉语言模型计算密集，不适合实时或嵌入式系统。

Method: MultiTypeFCDD使用简单的卷积框架，仅需图像级标签来学习并生成多通道热图，每个通道对应特定的异常类型。该模型作为统一框架，能够跨多个对象类别区分异常类型。

Result: 在Real-IAD数据集上的评估表明，该方法在显著减少参数负载和推理时间的同时，实现了与最先进复杂模型相竞争的结果。
...
TL;DR: CATS-V2V是首个面向复杂恶劣交通场景的V2V协同感知真实世界数据集，包含10种天气光照条件下的100个片段，提供LiDAR点云、多视角相机图像、GNSS/IMU数据及3D标注，支持4D BEV表示。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注普通交通场景，限制了V2V协同感知在复杂恶劣交通场景中的性能提升，需要专门数据集来克服这一限制。

Method: 使用两辆硬件时间同步的车辆采集数据，覆盖10种天气光照条件和10个不同地点，提供多模态传感器数据和时间一致的3D标注，并提出基于目标的时序对齐方法。

Result: 构建了包含60K帧LiDAR点云、1.26M相机图像、750K GNSS/IMU记录的100片段数据集，是目前同类数据集中规模最大、支持最全面、质量最高的。

Conclusion: CATS-V2V数据集将为自动驾驶社区在相关任务中提供重要支持，促进V2V协同感知在复杂恶劣场景下的发展。
...
Method: 采用粗到精的对齐模块，首先确定粗略的帧级偏移，然后细化到亚帧精度，该策略可作为模块集成到现有4DGS框架中。

Result: 实验表明该方法能有效处理时序错位视频，并显著提升基线方法的性能。

Conclusion: 提出的时序对齐策略增强了4DGS框架处理异步数据的鲁棒性，为真实场景中的多视角视频重建提供了有效解决方案。
...
TL;DR: Viper-F1是一个混合状态空间视觉语言模型，使用液体状态空间动力学替代注意力机制，通过令牌-网格相关模块增强视觉定位，实现高效、细粒度的视觉语言理解。
...
Method: 使用液体状态空间动力学替代Transformer的交叉注意力机制，提出令牌-网格相关模块计算文本令牌与图像块之间的轻量级相关性，并通过FiLM条件调节状态空间动力学。

Result: 在多个基准测试中，Viper-F1实现了准确、细粒度的理解，同时显著提高了效率。

Conclusion: Viper-F1通过状态空间模型和视觉定位增强，在保持线性时间推理的同时，实现了高效的细粒度视觉语言理解。
...
TL;DR: 提出一个高效框架用于印度次大陆及周边地区PM1、PM2.5和PM10的6小时预报，使用CAMS全球大气成分预报数据，通过轻量级参数特定架构提升精度并减少系统偏差。


<details>
  <summary>Details</summary>
Motivation: 为Weather4Cast 2025污染任务开发专门解决方案，针对印度地区的颗粒物污染提供准确、快速的短期预报，解决现有模型在特定区域预报精度不足的问题。

Method: 使用CAMS全球大气成分预报的0.4度分辨率分析场作为输入，训练三个轻量级参数特定架构模型，采用256x256空间区域输入，预测中央128x128区域，覆盖印度核心区域。

Result: 在2024年独立测试数据上评估显示，相比Aurora基础模型，在RMSE、MAE、偏差和SSIM指标上均有显著性能提升，证明了紧凑专业化深度学习模型在有限空间域短期预报中的有效性。

Conclusion: 轻量级专业化深度学习模型在特定区域颗粒物污染短期预报中表现出色，能够提供准确、快速且偏差较小的预测，为区域空气质量预报提供了有效解决方案。
...
TL;DR: 提出基于ConvGRU的迁移学习框架，用于Weather4Cast 2025竞赛中的短期降雨预测，使用单通道SEVIRI红外数据作为输入，采用两阶段训练策略，在累计降雨任务中获得第二名。
...
Result: 在累计降雨任务中获得第二名；在事件预测任务中与基线模型表现相当。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉问答模型在处理复杂地理空间查询时缺乏结构化推理能力，而地理空间思维链推理对于气候变化相关应用至关重要。

Method: 集成思维链推理与直接偏好优化，生成中间推理过程来更好地处理检测、分类、空间关系和比较分析等任务。

Result: 思维链监督比直接基线准确率提升34.9%，直接偏好优化进一步提高了准确率和推理质量。

Conclusion: 该框架通过实现更丰富的地理空间推理，推进了多光谱地球观测的视觉问答能力，为气候变化应用提供更有效的支持。
...
TL;DR: 该研究首次系统性地评估了视觉语言模型对良性视觉和文本扰动的鲁棒性，发现即使是最先进的模型也对像素级偏移、几何变换、重述等微小变化高度敏感，稳定性与正确性密切相关。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型取得了显著进展，但其在保持语义不变的小幅输入变化下的可靠性尚未得到充分研究，需要系统评估模型对良性扰动的鲁棒性。

Method: 通过大规模实验，测试多种VLMs对像素级偏移、几何变换、缩放填充、文本重述和多语言改写等语义保持扰动的稳定性，分析不同扰动类型、问题类别和模型间的差异。

Result: 现代VLMs对微小扰动高度敏感，大量样本在至少一种视觉或文本修改下会改变预测答案；稳定性强的样本正确率显著更高；小模型稳定性模式可预测大模型正确性。
...
TL;DR: 本文提出了首个3D视觉中的一对多后门攻击框架，使用可配置的球形触发器，能够将单个触发器设计编码为多个目标类别。
...
Method: 基于球形触发器的空间特性作为参数空间，构建可配置的球形触发器，使单个触发器设计能够编码多个目标类别，并建立了一对多后门攻击的理论基础。

Result: 在多个数据集和模型架构上的实验验证了该方法的有效性，攻击成功率高达100%，同时保持了对干净数据的准确性。

Conclusion: 这项工作为3D视觉中的多目标威胁建立了重要基准，为保护未来3D驱动的智能系统提供了基础理解。
...
TL;DR: MAFM^3框架通过轻量级模块化组件，使单个基础模型能够扩展到不同医学影像领域、任务和模态，实现高效的多任务和多模态适应。
...
Method: 提出MAFM^3框架，使用轻量级模块化组件作为专门技能集，根据输入类型或临床目标灵活激活相应能力。

Result: 将胸部CT基础模型适应到预后和分割模块，在两个任务上都表现出改进性能；整合PET扫描后，Dice分数比基线提高5%。
...
TL;DR: 提出了一种名为Guidance Score Distillation (GSD)的框架，利用预训练的视频扩散模型(VDM)来增强3D高斯溅射在稀疏训练视图下的性能，通过统一的引导形式纠正VDM的噪声预测结果。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射(3DGS)在稀疏训练视图下容易过拟合，主要缺乏中间视图的监督，需要从预训练视频扩散模型中提取丰富的多视图一致性先验。

Method: 基于Score Distillation Sampling (SDS)的GSD框架，监督多个相邻视图的渲染图像，通过深度扭曲引导和语义图像特征引导来纠正VDM的噪声预测，确保得分更新方向与正确相机姿态和准确几何对齐。

Result: 实验结果表明，该方法在多个数据集上优于现有方法。

Conclusion: GSD框架有效解决了3DGS在稀疏视图下的过拟合问题，通过VDM的引导实现了更好的3D场景表示性能。
...
TL;DR: 本文研究了多模态表示模型中的位置偏见问题，特别是在图像-文本检索任务中。研究发现文本编码器倾向于偏向输入开头，而图像编码器在开头和结尾都表现出偏见。
...
Method: 通过区分上下文重要性和位置偏见，评估了不同模型和数据集中的位置偏见存在程度和范围。实验分析了文本编码器和图像编码器在位置偏见上的不同表现。

Result: 实验证明位置偏见在多模态模型中普遍存在，但不同模态表现不同：文本编码器偏向输入开头，图像编码器在开头和结尾都表现出偏见。这种偏见源于位置编码方案、训练损失、上下文重要性以及多模态训练中图像-文本对的使用方式等因素的组合。

Conclusion: 多模态表示模型中存在显著的位置偏见，且在不同模态中表现出不同的模式。这种偏见由多种因素共同导致，需要在模型设计和训练中加以考虑。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射模型在渲染细微连续视线转移时存在困难，需要更高保真度的视线重定向方法来生成增强数据以提升视线估计器的泛化能力。

Method: 结合扩散变换器(DiT)、跨视线角度的弱监督策略和正交约束损失，DiT实现高保真图像合成，弱监督使用合成生成的中间视线角度提供平滑视线方向流形，正交约束损失数学上强制视线、头部姿态和表情的内部表示解耦。

Result: DiT-Gaze在感知质量和重定向精度上均达到新SOTA，将最先进的视线误差降低了4.1%至6.353度，为创建合成训练数据提供了更优方法。

Conclusion: DiT-Gaze框架通过扩散变换器、弱监督和正交约束损失的创新组合，显著提升了视线重定向的保真度和准确性，为视线估计研究提供了更好的合成数据生成工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D深度学习在多领域泛化受限，主要由于数据集规模有限和多源点云的高异构性。不同传感器采集的点云在密度和噪声分布上存在显著差异，导致多领域融合时出现负迁移。现有方法往往只关注领域感知或领域通用特征，忽略了二者之间的协同潜力。

Method: 提出DoReMi框架，包含领域感知专家分支和统一表示分支。通过领域引导空间路由动态激活专家分支进行上下文感知的专家选择，使用熵控制动态分配实现稳定高效的专家利用。统一表示分支通过鲁棒的多属性自监督学习预训练并保持冻结状态。
...
TL;DR: 本文提出了一种新颖的少样本风格编辑框架，通过参数高效的MoE LoRA方法，使用风格特定和风格共享路由机制来联合微调多种风格，在有限配对数据下实现高质量图像编辑。
...
Method: 提出MoE LoRA框架，包含风格特定路由和风格共享路由机制；使用度量引导方法自动确定各层最优秩；探索DiT模型中LoRA的最佳插入位置；集成对抗学习和流匹配指导扩散训练过程。

Result: 实验结果表明，该方法在显著减少LoRA参数量的情况下，优于现有最先进方法。

Conclusion: 所提出的少样本风格编辑框架能够有效解决新风格适应问题，在参数效率和编辑质量方面均表现出色。
...
TL;DR: GEODE是一个解决视觉语言模型3D空间理解瓶颈的新架构，通过解耦3D推理和数值生成，使用两个可插拔模块实现高效的空间推理。
...
Method: 引入GEODE架构，包含两个模块：解耦推理模块(DRM)作为空间协处理器，通过交叉注意力对齐3D数据和2D视觉特征；直接回归头(DRH)采用'嵌入即值'范式，使用轻量MLP进行精确连续回归。

Result: 仅1.5B参数的模型实现了最先进的空间推理性能，可与7B+模型相媲美。

Conclusion: GEODE通过解耦3D推理和数值生成的双模块设计，有效解决了视觉语言模型在3D空间理解方面的根本瓶颈。
...
TL;DR: 本文提出了一种用于自闭症儿童注视目标检测的新AI框架SACF，通过利用社交上下文来解决数据集类别不平衡问题，显著提升了面部注视这一关键少数类别的检测性能。


<details>
  <summary>Details</summary>
Motivation: 自闭症儿童注视目标自动检测对改善生活质量有重要影响，特别是对于缺乏专业医疗资源的儿童。该任务对于构建测量共同注意力的自动化系统至关重要。
...
Conclusion: SACF框架通过显式利用场景的社交上下文，有效克服了自闭症数据集中的类别不平衡问题，为自闭症儿童的注视目标检测提供了有效的解决方案。
...
TL;DR: CountSteer是一种无需训练的方法，通过引导扩散模型的交叉注意力隐藏状态来改进文本到图像生成中的对象计数准确性，提升约4%的计数准确率而不影响视觉质量。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成逼真连贯的图像，但经常无法遵循文本中的数值指令，这揭示了语言和视觉表示之间的差距。研究发现模型实际上对数字有隐含感知，能够识别自身计数准确性，这表明模型已经编码了数值正确性的潜在概念。

Method: 基于模型内部信号随计数准确性变化而一致变化的观察，提出了CountSteer方法。这是一种无需训练的方法，在推理过程中通过引导模型的交叉注意力隐藏状态来改进指定对象数量的生成。

Result: 实验表明，CountSteer将对象计数准确率提高了约4%，同时没有损害视觉质量。

Conclusion: CountSteer展示了向更可控和语义可靠的文本到图像生成迈出的简单而有效的步骤，通过利用模型已有的数值正确性潜在概念来精确指导生成过程。
...
TL;DR: 该论文提出了一种通过将标题标记分组来捕获语言细粒度表示的模型，旨在提升视觉语言模型对真实世界的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注将图像块与语言标记对齐，但图像块对人类无意义，单个标记也不一定携带可接地信息。需要将标记分组来描述场景的不同方面，以获得更好的细粒度知识理解。

Method: 提出一种在架构中自动将标题标记分组的模型，使语言表示达到图像中对象的粒度级别，并与经过训练以发现对象的图像编码器输出对齐。

Result: 通过学习标记分组，视觉语言模型在视觉和语言理解方面获得了更好的细粒度能力。模型发现的标记组在定性和定量上都与文本中的可接地短语高度相似。

Conclusion: 通过自动分组语言标记来学习细粒度表示，能够显著提升视觉语言模型的理解能力，且发现的标记组与真实语义单元高度一致。
...
TL;DR: 提出了一种模型无关的方法，通过交通场景图来增强基于语言的驾驶模型，在LangAuto基准测试中显著提升了驾驶性能（LMDrive提升15.6%，BEVDriver提升17.5%）。
...
TL;DR: CORAL提出了一种协调顺序关系解剖学习方法，通过对比排名目标和顺序目标来捕捉体积医学图像中的局部和全局结构，在有限标注设置下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用硬二元阈值定义正负样本，丢弃了有价值的连续解剖相似性信息，并且忽略了全局解剖进展的方向一致性，导致特征空间扭曲无法捕捉跨患者的规范解剖流形。

Method: CORAL采用对比排名目标来利用连续解剖相似性，确保切片间特征距离与其解剖位置差异成比例；同时引入顺序目标来强制全局方向一致性，使学习到的特征分布与跨患者的规范解剖进展对齐。

Result: CORAL在有限标注设置下的基准数据集上实现了最先进的性能，同时学习了具有有意义的解剖结构的表示。

Conclusion: 通过协调学习框架，CORAL能够学习解剖感知的表示，这些表示有利于下游分割任务，同时捕捉体积图像中的局部和全局结构。
...
### [61] [SimuFreeMark: A Noise-Simulation-Free Robust Watermarking Against Image Editing](https://arxiv.org/abs/2511.11295)
*Yichao Tang,Mingyang Li,Di Miao,Sheng Li,Zhenxing Qian,Xinpeng Zhang*

Main category: cs.CV

TL;DR: SimuFreeMark是一个免噪声模拟的水印框架，通过利用图像低频分量的固有稳定性来嵌入水印，无需训练过程中的噪声模拟，在各种攻击下表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的水印方法依赖手工噪声模拟层进行训练，这限制了它们对未知失真的泛化能力。需要开发能够同时抵抗传统信号处理和新型语义编辑攻击的鲁棒图像水印技术。

Method: 利用图像低频分量对多种攻击的显著鲁棒性，将水印直接嵌入到低频分量的深度特征空间中，使用预训练的变分自编码器将水印与结构稳定的图像表示绑定，完全消除训练过程中的噪声模拟需求。

Result: 广泛的实验表明，SimuFreeMark在广泛的传统和语义攻击范围内优于最先进的方法，同时保持优异的视觉质量。

Conclusion: SimuFreeMark通过利用低频分量的固有稳定性，成功实现了免噪声模拟的水印嵌入，在各种攻击下表现出卓越的性能和泛化能力。
...
### [62] [AUVIC: Adversarial Unlearning of Visual Concepts for Multi-modal Large Language Models](https://arxiv.org/abs/2511.11299)
*Haokun Chen,Jianing Li,Yao Zhang,Jinhe Bi,Yan Xia,Jindong Gu,Volker Tresp*

Main category: cs.CV

TL;DR: AUVIC是一个针对多模态大语言模型的视觉概念遗忘框架，通过对抗扰动实现精确的目标概念移除，同时避免影响相关实体性能。
...
Method: 提出AUVIC框架，采用对抗扰动技术实现精确的视觉概念遗忘，并构建首个视觉概念遗忘基准VCUBench进行评估。

Result: 实验表明AUVIC在目标遗忘率方面达到最先进水平，同时对非目标概念的性能影响最小。

Conclusion: AUVIC框架为MLLMs中的视觉概念遗忘提供了有效解决方案，在保护数据隐私的同时维持了模型性能。
...
Method: 采用分层多模态压缩器联合编码每页的视觉、文本和布局信息为固定长度序列，并引入基于熵的不确定性校准器的流式弃权机制进行可扩展处理。

Result: 在多个长多模态文档基准测试中，DocSLM匹配或超越最先进方法，同时减少82%视觉标记、75%参数和71%延迟。

Conclusion: DocSLM能够在轻量级边缘设备上提供可靠的多模态文档理解，显著降低了资源需求。
...
### [64] [YCB-Ev SD: Synthetic event-vision dataset for 6DoF object pose estimation](https://arxiv.org/abs/2511.11344)
*Pavel Rojtberg,Julius Kühn*

Main category: cs.CV

TL;DR: YCB-Ev SD是一个用于6自由度物体姿态估计的标准分辨率事件相机合成数据集，包含50,000个事件序列，填补了事件视觉领域缺乏综合数据资源的空白。


<details>
  <summary>Details</summary>
Motivation: 解决事件视觉领域缺乏与帧基计算机视觉相当的全面合成数据资源的问题，为6DoF物体姿态估计提供高质量的事件相机数据。

Method: 基于物理渲染(PBR)场景合成事件序列，采用模拟线性相机运动确保完整场景覆盖，包括背景活动，遵循BOP方法学。

Result: 通过系统评估发现，具有线性衰减和双通道极性编码的时间表面在CNN推理中实现最佳姿态估计性能，显著优于指数衰减和单通道替代方案。

Conclusion: 极性信息对性能提升贡献最大，线性时间编码比指数衰减更有效地保留关键运动信息，数据集以结构化格式提供原始事件流和预计算最优表示。
...
### [65] [Free3D: 3D Human Motion Emerges from Single-View 2D Supervision](https://arxiv.org/abs/2511.11368)
...
### [66] [Unsupervised Segmentation of Micro-CT Scans of Polyurethane Structures By Combining Hidden-Markov-Random Fields and a U-Net](https://arxiv.org/abs/2511.11378)
...
<details>
  <summary>Details</summary>
Motivation: 现有分割方法要么需要大量标注数据（监督学习），要么分割速度慢且精度差（无监督学习），需要结合两者优势的方法。

Method: 集成HMRF理论和CNN分割，利用HMRF的无监督学习特性和CNN的快速分割能力，研究不同邻域项和组件对无监督HMRF损失的影响。

Result: 在聚氨酯泡沫结构的μCT图像数据集上，HMRF-UNet实现了无需真实标注的高分割精度。

Conclusion: 提出的预训练策略显著减少了训练分割模型所需的标注数据量，证明了HMRF-UNet在材料表征中的有效性。
...
TL;DR: 提出了基于低秩稀疏原则的LSEF框架，通过分层建模将情感动态分解为长期情感基调和短期情感波动，解决了视频情感计算中的模型不稳定和表示退化问题。


<details>
  <summary>Details</summary>
Motivation: 视频情感计算面临模型不稳定和表示退化问题，主要原因是缺乏分层结构机制来解耦不同的情感成分（长期情感基调和短期情感波动）。

Method: 提出LSEF框架，包含三个即插即用模块：稳定性编码模块捕获低秩情感基调，动态解耦模块分离稀疏瞬态信号，一致性集成模块重建多尺度稳定性与反应性一致性，采用秩感知优化策略自适应平衡梯度平滑性和敏感性。

Result: 在多个数据集上的广泛实验证实，LSEF显著增强了鲁棒性和动态辨别能力，验证了分层低秩稀疏建模在理解情感动态方面的有效性和通用性。

Conclusion: 分层低秩稀疏建模是理解情感动态的有效方法，LSEF框架通过解耦情感成分显著提升了视频情感计算的性能。
...
### [68] [MicroVQA++: High-Quality Microscopy Reasoning Dataset with Weakly Supervised Graphs for Multimodal Large Language Model](https://arxiv.org/abs/2511.11407)
*Manyu Li,Ruian He,Chenxi Ma,Weimin Tan,Bo Yan*

Main category: cs.CV

TL;DR: MicroVQA++是一个三阶段构建的大规模高质量显微镜视觉问答数据集，通过专家验证的图-标题对、异构图一致性过滤和MLLM生成多选题结合人工筛选的方式，显著提升了显微镜推理任务的性能。
...
Method: 采用三阶段构建方法：1) 从专家验证的图-标题对中引导监督；2) 使用HiCQA-Graph异构图融合NLI文本蕴含、CLIP视觉语言对齐和智能体信号来过滤不一致样本；3) 使用MLLM生成多选题并进行人工筛选。

Result: 构建的数据集在Bloom认知水平难样本分布上超过MicroVQA基准，使4B规模的MLLM达到与GPT-5竞争的显微镜推理性能，并在开源MLLM中实现最先进性能。

Conclusion: 通过精心设计的数据构建方法，结合专家文献、图基过滤和人工精炼，能够显著提升小规模MLLM在显微镜推理任务上的性能表现。
...
### [69] [Q-Doc: Benchmarking Document Image Quality Assessment Capabilities in Multi-modal Large Language Models](https://arxiv.org/abs/2511.11410)
...
Method: 提出三层次评估框架：粗粒度质量评分、中粒度失真类型识别（单选和多选测试）、细粒度失真严重程度分类。使用思维链提示来提升性能。
...
### [70] [BOFA: Bridge-Layer Orthogonal Low-Rank Fusion for CLIP-Based Class-Incremental Learning](https://arxiv.org/abs/2511.11421)
*Lan Li,Tao Hu,Da-Wei Zhou,Han-Jia Ye,De-Chuan Zhan*

Main category: cs.CV

TL;DR: BOFA是一个用于类增量学习的新框架，通过在CLIP的跨模态桥接层进行正交低秩融合，实现无额外参数的高效知识积累，结合跨模态混合原型提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中CLIP模型的两个主要挑战：(1)适应下游任务需要额外可学习模块，增加模型复杂度和遗忘风险；(2)现有方法未能充分利用多模态表示的互补优势。

Method: 提出BOFA框架，将模型适应限制在CLIP的跨模态桥接层，使用正交低秩融合机制将参数更新约束到与过去任务特征正交的低秩安全子空间，结合跨模态混合原型。

Result: 在标准基准测试中，BOFA相比现有方法实现了更优的准确性和效率。

Conclusion: BOFA通过桥接层正交融合策略，在无需数据回放的情况下实现了稳定高效的知识积累，显著提升了类增量学习的性能。
...
### [71] [Shrinking the Teacher: An Adaptive Teaching Paradigm for Asymmetric EEG-Vision Alignment](https://arxiv.org/abs/2511.11422)
*Lukun Wu,Jie Li,Ziqi Ren,Kaifan Zhang,Xinbo Gao*

Main category: cs.CV

TL;DR: 本文提出自适应教学范式来解决视觉-脑电信号不对称对齐问题，通过让视觉模态动态调整知识结构来匹配脑电模态的能力，在零样本脑到图像检索任务中达到60.2%的top-1准确率，比之前方法提升9.8%。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了视觉和脑电信号之间的本质不对称性，包括保真度差距（脑电信号噪声大）和语义差距（脑电语义表示浅层），强行将两种模态作为平等伙伴对齐导致泛化能力差。

Method: 提出自适应教学范式，让"教师"模态（视觉）在任务指导下动态缩小和调整其知识结构，使其语义密集特征匹配"学生"模态（脑电）的能力。具体实现为ShrinkAdapter模块，采用无残差设计和瓶颈结构。
...
TL;DR: 本文研究了文本到图像扩散模型中泛化与记忆之间的模糊界限，特别关注多模态标志性现象——即图像和文本唤起文化共享关联的情况。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型中文化参考识别与再现之间的平衡问题，探索模型如何记忆和转化文化知识，而不仅仅是遗忘。

Method: 提出了一个评估框架，将识别（模型是否识别参考）与实现（如何通过复制或重新诠释来描绘）分开，使用基于Wikidata的767个文化参考进行评估，并进行提示扰动实验。

Result: 框架比现有基于相似性的方法更有效地区分复制与转化；模型即使文本线索改变也常重现标志性视觉结构；文化对齐与训练数据频率、文本独特性、参考流行度和创建日期相关。

Conclusion: 扩散模型的价值不仅在于复制，更在于转化和重新语境化文化知识，推动评估从简单的文本-图像匹配转向更丰富的语境理解。
...
### [74] [Hi-DREAM: Brain Inspired Hierarchical Diffusion for fMRI Reconstruction via ROI Encoder and visuAl Mapping](https://arxiv.org/abs/2511.11437)
*Guowei Zhang,Yun Zhao,Moein Khajehnejad,Adeel Razi,Levin Kuhlmann*

Main category: cs.CV

TL;DR: Hi-DREAM是一种受大脑启发的条件扩散框架，通过将fMRI特征按视觉皮层层次结构分组并转换为多尺度皮层金字塔，在去噪过程中注入尺度特定提示，实现了高效的图像重建和视觉区域功能贡献分析。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的解码器直接基于fMRI特征而不分析视觉信息在皮层中的组织方式，忽略了大脑的层次处理结构，模糊了早期、中期和晚期视觉区域的作用。
...
### [75] [VP-Bench: A Comprehensive Benchmark for Visual Prompting in Multimodal Large Language Models](https://arxiv.org/abs/2511.11438)
...
TL;DR: VP-Bench是一个评估多模态大语言模型视觉提示感知能力的基准，包含两阶段评估框架：第一阶段测试模型在自然场景中感知视觉提示的能力，第二阶段评估视觉提示对下游任务的影响。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统评估MLLMs解释视觉提示能力的基准，无法确定模型是否能有效识别人类直观的提示方法并用于解决问题。

Method: 采用两阶段评估框架：第一阶段使用30k可视化提示（8种形状和355种属性组合）测试模型感知能力；第二阶段评估视觉提示在实际问题解决场景中的效果。

Result: 评估了28个MLLMs（包括GPT-4o、InternVL3和Qwen2.5-VL等），全面分析了影响视觉提示理解的因素，如提示属性变化、问题安排和模型规模等。
...
### [76] [From Synthetic Scenes to Real Performance: Enhancing Spatial Reasoning in VLMs](https://arxiv.org/abs/2511.11440)
*Massimo Rizzoli,Simone Alghisi,Seyed Mahed Mousavi,Giuseppe Riccardi*

Main category: cs.CV

TL;DR: 本文提出了一种通过控制合成数据生成来解决视觉语言模型微调中偏见和分布不平衡问题的方法，在平衡合成数据上微调能提升模型在真实数据上的性能表现。
...
Method: 重新设计微调过程：1) 控制数据生成和标注，确保无偏见、分布平衡且无标注错误；2) 通过全面采样物体属性自动构建数据集；3) 使用该数据集微调最先进的视觉语言模型。

Result: 实验发现：1) 在平衡合成数据上微调能在整个视觉场景中产生均匀性能并减轻常见偏见；2) 在合成刺激上微调显著提升了在真实数据上的性能，优于匹配设置下的微调模型。

Conclusion: 通过控制合成数据生成过程可以有效解决视觉语言模型微调中的偏见和分布不平衡问题，提升模型在真实世界数据上的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够理解自由形式文本描述并执行3D医学图像分割的模型，以支持从简单词汇到完整临床句子的多样化输入。

Method: 使用多阶段视觉语言融合在解码器层中，在多个尺度上对齐文本和视觉特征，在62K+ CT、MRI和PET体积数据上训练，涵盖1K+解剖和病理类别。
...
TL;DR: 提出了一种名为MaMOL的框架，通过将模态缺失重新表述为多任务学习问题，使用双路由机制（任务导向动态路由器和模态特定共享静态路由器）来处理遥感中的多模态分类问题，在计算开销最小的情况下实现了优越的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决遥感多模态分类中因环境干扰、传感器故障或大气效应导致的模态缺失问题，现有两阶段适应方法计算成本高且假设训练期间具有完整多模态数据，限制了其在现实世界不完整性中的泛化能力。

Method: 提出Missing-aware Mixture-of-Loras (MaMOL)框架，采用双路由机制：任务导向动态路由器自适应激活不同缺失模式的专家，模态特定共享静态路由器保持稳定的跨模态知识共享；通过轻量级专家更新和共享专家重用实现参数高效适应。

Result: 在多个遥感基准测试中，在不同缺失率下表现出优越的鲁棒性和泛化能力，计算开销最小；在自然图像数据集上的迁移实验验证了其可扩展性和跨领域适用性。

Conclusion: MaMOL是不完整多模态学习的一种通用且高效的解决方案，通过多任务学习框架和双路由机制有效处理模态缺失问题，具有优越的性能和广泛的应用潜力。
...
Result: 在肺鳞状细胞癌数据集上，平均Dice分数为0.805，平均IoU为0.709，性能与现有基线方法相当，并提供像素级不确定性图。
...
TL;DR: 本文提出了Prelim Attention Score (PAS)方法，通过分析LVLM对先前生成标记的注意力权重来检测对象幻觉，无需额外训练即可实现最先进的幻觉检测效果。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)存在对象幻觉问题，研究发现许多幻觉预测中模型实际上忽略了图像信息，而是依赖先前生成的标记来推断新对象。

Method: 引入Prelim Attention Score (PAS)，这是一种轻量级、无需训练的信号，通过计算对先前生成标记(prelim tokens)的注意力权重来检测对象幻觉。

Result: PAS在多个模型和数据集上实现了最先进的对象幻觉检测性能，能够实时进行过滤和干预，且无需额外前向传播计算。

Conclusion: PAS通过利用先前被忽视的注意力信号，为LVLM的对象幻觉检测提供了一种高效实用的解决方案。
...
TL;DR: BRIDGE提出了一种轻量级的跨模态融合模块，通过在编码器顶部添加少量双向交叉注意力层，在共享空间中对齐视觉和文本隐藏状态，同时保持编码器的高效性和解码器的解耦。
...
Method: 在视觉和文本编码器顶部附近放置少量仅交叉的双向注意力层，将隐藏状态投影到共享空间进行跨模态注意力计算，通过门控残差更新返回，并使用简单稳定器改善对齐。

Result: 在标准检索、VQA和视觉推理基准测试中，BRIDGE优于可比较的视觉语言模型，同时保持了对比模型的双编码器效率。

Conclusion: BRIDGE提供了一种高效且有效的跨模态融合方法，在不牺牲编码器理解能力的情况下实现更好的模态对齐。
...
### [82] [Attentive Feature Aggregation or: How Policies Learn to Stop Worrying about Robustness and Attend to Task-Relevant Visual Cues](https://arxiv.org/abs/2511.10762)
...
TL;DR: 本文提出了一种名为注意力特征聚合（AFA）的轻量级可训练池化机制，用于提高预训练视觉表示在视觉扰动场景下的鲁棒性，无需昂贵的数据增强或微调。
...
Method: 通过注意力特征聚合（AFA）机制，学习自然地关注任务相关的视觉线索，忽略语义丰富的场景干扰物。
...
### [83] [From Framework to Reliable Practice: End-User Perspectives on Social Robots in Public Spaces](https://arxiv.org/abs/2511.10770)
*Samson Oruma,Ricardo Colomo-Palacios,Vasileios Gkioulos*

Main category: cs.RO

TL;DR: 本文报告了ARI社交机器人在大学接待员角色的试点部署，基于SecuRoPS框架进行伦理安全设计，通过35名用户反馈评估了安全性、隐私、可用性等方面，结果显示正面评价但存在可访问性和互动性挑战。


<details>
  <summary>Details</summary>
Motivation: 随着社交机器人进入公共环境，其接受度不仅依赖技术可靠性，还取决于伦理完整性、可访问性和用户信任。研究旨在验证伦理安全框架在实际部署中的可行性。

Method: 采用ARI社交机器人作为大学接待员进行试点部署，基于SecuRoPS框架设计，收集35名学生和员工的反馈，评估安全、隐私、可用性、可访问性和透明度等方面。

Result: 结果显示用户对物理安全、数据保护和伦理行为普遍持正面看法，但可访问性、包容性和动态互动方面存在挑战。研究提供了可复用的GitHub模板资源。

Conclusion: 研究证明了伦理安全设计框架可在现实环境中实施，结合用户视角和技术资源，有助于开发可信赖、包容和负责任的公共空间社交机器人。
...
### [84] [$\rm{A}^{\rm{SAR}}$: $\varepsilon$-Optimal Graph Search for Minimum Expected-Detection-Time Paths with Path Budget Constraints for Search and Rescue](https://arxiv.org/abs/2511.10792)
*Eric Mugford,Jonathan D. Gammell*

Main category: cs.RO

TL;DR: 本文提出了A^SAR算法，一种ε最优搜索算法，用于搜索与救援规划。该算法通过计算启发式边界搜索空间，并使用图搜索方法找到在用户指定因子ε内最优解的正式保证解。


<details>
  <summary>Details</summary>
Motivation: 在搜索与救援场景中，由于信息不确定、观察者不完美和搜索区域大，搜索复杂。特别是在海上救援等场景中，预期生存时间短，最优搜索可提高成功可能性。现有随机优化方法缺乏对有限时间内解质量的正式保证。

Method: 提出A^SAR算法，计算启发式来边界搜索空间，使用图搜索方法寻找在用户指定因子ε内最优解的形式保证解。

Result: 在操作模拟中比现有优化方法更快找到更好解。在加拿大安大略湖的实际现场试验中，仅用150秒定位了漂流的人体模型。

Conclusion: A^SAR算法提供形式保证的ε最优解，在搜索与救援规划中优于现有方法，并在实际应用中验证了有效性。
...
### [85] [An Investigation into Dynamically Extensible and Retractable Robotic Leg Linkages for Multi-task Execution in Search and Rescue Scenarios](https://arxiv.org/abs/2511.10816)
*William Harris,Lucas Yager,Syler Sylvester,Elizabeth Peiros,Micheal C. Yip*

Main category: cs.RO

TL;DR: 本文提出了一种新颖的动态可伸缩机器人腿部设计，通过五连杆机构实现高度优势和力优势配置之间的机械切换，旨在解决搜救机器人既需要地形适应性又需要高力输出的双重需求。


<details>
  <summary>Details</summary>
Motivation: 搜救机器人需要快速穿越地形并执行高力救援任务，但现有平台难以同时满足地形适应性和高力输出需求。腿式机器人具有地形适应性优势，但通常无法提供可变高力输出，而轮式系统则缺乏地形适应性。

Method: 采用动态可伸缩五连杆机构设计，通过几何变换在高度优势配置和力优势配置之间进行机械切换。通过测试台评估不同连杆几何形状和操作模式下的腿部性能，并进行步长、力输出和稳定性的经验与分析研究。

Result: 研究结果表明，变形腿部为搜救机器人提供了一条有前景的技术路径，能够同时实现快速地形导航和有效救援任务执行。

Conclusion: 动态可伸缩腿部设计为解决搜救机器人地形适应性和高力输出的双重需求提供了可行的解决方案，通过机械配置切换实现了两种优势模式的灵活应用。
...
### [86] [MIGHTY: Hermite Spline-based Efficient Trajectory Planning](https://arxiv.org/abs/2511.10822)
*Kota Kondo,Yuwei Wu,Vijay Kumar,Jonathan P. How*

Main category: cs.RO

TL;DR: MIGHTY是一种基于Hermite样条的轨迹规划器，通过时空联合优化在连续搜索空间中实现高效规划，相比现有方法在计算时间和行程时间上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有软约束方法要么解耦时空优化，要么限制搜索空间，存在局限性。需要克服这些限制，在连续搜索空间中实现高效的时空联合优化。

Method: 提出MIGHTY规划器，基于Hermite样条进行时空优化，充分利用样条的连续搜索空间特性。

Result: 仿真中比最先进基线计算时间减少9.3%，行程时间减少13.1%，成功率100%。硬件实验中在杂乱静态环境中完成最高6.7m/s的高速飞行，并能处理动态添加的障碍物。

Conclusion: MIGHTY在连续搜索空间中实现了高效的时空联合优化，在计算效率和轨迹质量方面均优于现有方法。
...
### [87] [Decentralized Swarm Control via SO(3) Embeddings for 3D Trajectories](https://arxiv.org/abs/2511.10858)
*Dimitria Silveria,Kleber Cabral,Peter Jardine,Sidney Givigi*

Main category: cs.RO

TL;DR: 提出了一种基于SO(3)李群的新型去中心化多智能体系统方法，无需速度输入即可产生稳定周期性轨迹，并确保智能体均匀分布。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成周期性轨迹方面存在限制，且需要速度输入。本文旨在开发一种更通用的方法，仅需位置输入即可实现稳定的周期性行为。

Method: 利用SO(3)李群进行几何嵌入，稳定系统产生周期性轨迹；提出新型相位控制器确保智能体均匀分布；提供形式化稳定性证明。

Result: 该方法比现有四元数方法产生更广泛的周期性曲线，无需速度输入，能够适应复杂的底层动力学和干扰。

Conclusion: 所提出的基于SO(3)李群的方法为多智能体系统提供了一种高效、稳定的周期性轨迹生成方案，具有更好的适应性和实用性。
...
### [88] [WetExplorer: Automating Wetland Greenhouse-Gas Surveys with an Autonomous Mobile Robot](https://arxiv.org/abs/2511.10864)
*Jose Vasquez,Xuping Zhang*

Main category: cs.RO

TL;DR: WetExplorer是一个自主履带式机器人，用于自动化湿地温室气体采样工作流程，通过集成低接地压力移动、厘米级精确升降放置、双RTK传感器融合、避障规划和深度学习感知等技术，实现无人干预的高频多点温室气体测量。


<details>
  <summary>Details</summary>
Motivation: 湿地温室气体量化对气候建模和恢复评估至关重要，但手动采样劳动密集且耗时，需要自动化解决方案来提高采样效率和频率。

Method: 开发了集成低接地压力移动系统、厘米级精确升降机构、双RTK传感器融合定位、避障运动规划和深度学习视觉感知的机器人系统，采用容器化ROS2软件架构。

Result: 室外试验验证传感器融合定位平均误差1.71厘米，视觉模块物体姿态估计精度7毫米平移和3度旋转；室内试验显示全运动规划管道可将采样室定位在70毫米全局容差内并避开障碍物。

Conclusion: WetExplorer通过消除人工瓶颈，能够在饱和湿地地形中实现高频、多点温室气体测量，为密集、长期数据集收集开辟了新途径。
...
### [89] [Collaborative Multi-Robot Non-Prehensile Manipulation via Flow-Matching Co-Generation](https://arxiv.org/abs/2511.10874)
*Yorai Shaoul,Zhe Chen,Mohamed Naveed Gul Mohamed,Federico Pecora,Maxim Likhachev,Jiaoyang Li*

Main category: cs.RO

TL;DR: 提出一个统一框架，用于协作式多机器人、多物体非抓取操作，结合流匹配协同生成和匿名多机器人运动规划，在复杂环境中实现高效物体重新定位。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人在杂乱环境中协同重新定位多个物体的挑战，现有方法要么需要学习整个任务，要么依赖特权信息和手动设计的规划器，难以处理多样化物体和长时程任务。

Method: 集成流匹配协同生成与匿名多机器人运动规划的统一框架，生成模型从视觉观察中协同生成接触形态和操作轨迹，新型运动规划器实现大规模机器人协调。

Result: 在具有挑战性的模拟环境中实验表明，该方法在运动规划和操作任务方面均优于基线方法。

Conclusion: 生成式协同设计和集成规划有助于将协作操作扩展到复杂的多智能体、多物体场景。
...
### [90] [Terradynamics and design of tip-extending robotic anchors](https://arxiv.org/abs/2511.10901)
...
TL;DR: 本文研究了尖端延伸锚固机制与传统桩式锚固的差异，开发了一种轻量级软体机器人锚固装置，能够在松散火星土壤模拟物中实现40:1的锚固重量比。


<details>
  <summary>Details</summary>
Motivation: 传统桩式锚固需要较大的插入力但锚固力有限，不适合难以到达或外星环境。树根通过尖端延伸机制实现了插入力小而锚固力大的特性，这启发了新型锚固设计。

Method: 研究尖端延伸锚固的土壤动力学，与传统桩式侵入器对比，开发设计原则并应用于可部署的机器人锚固装置。

Result: 开发出300克重的软体机器人锚固装置，在松散火星土壤模拟物中插入深度达45厘米，平均锚固力120N，锚固重量比达到40:1。

Conclusion: 通过尖端延伸机制、毛发状突起、近垂直延伸和多锚固点等设计原则，可以实现插入力小而锚固力大的高效锚固系统，特别适合外星环境应用。
...
### [91] [Dexterous Manipulation Transfer via Progressive Kinematic-Dynamic Alignment](https://arxiv.org/abs/2511.10987)
*Wenbin Bai,Qiyu Chen,Xiangbo Lin,Jianwen Li,Quancheng Li,Hejiang Pan,Yi Sun*

Main category: cs.RO

TL;DR: 提出了一种手部无关的操纵转移系统，能够从人类手部操纵视频中高效生成高质量的多指灵巧手操纵轨迹，无需大量训练数据，解决了灵巧手操纵数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 多指机器人手硬件平台收集操纵数据存在固有困难和可扩展性限制，导致数据严重稀缺，阻碍了数据驱动的灵巧操纵策略学习研究。

Method: 设计了渐进式转移框架：首先基于运动学匹配建立灵巧手的主要控制信号；然后通过动作空间重缩放和拇指引导初始化训练残差策略，在统一奖励下动态优化接触交互；最后计算手腕控制轨迹以保持操作语义。

Result: 实验结果表明，该框架能自动生成平滑且语义正确的灵巧手操纵，忠实再现人类意图，平均转移成功率达到73%，具有高效率和强泛化能力。

Conclusion: 该系统为收集机器人灵巧操纵数据提供了一种易于实现和可扩展的方法，能够自动配置不同任务的系统参数，平衡灵巧手、物体类别和任务之间的运动学匹配和动态优化。
...
### [92] [Dynamic Reconfiguration of Robotic Swarms: Coordination and Control for Precise Shape Formation](https://arxiv.org/abs/2511.10989)
*Prab Prasertying,Paulo Garcia,Warisa Sritriratanarak*

Main category: cs.RO

TL;DR: 本文提出了一种机器人群体协调算法，能够实现从一种配置到另一种配置的无缝转换，利用几何公式通过控制、定位和映射技术映射到物理领域。


<details>
  <summary>Details</summary>
Motivation: 机器人群体运动与配置协调是一个具有挑战性的问题，涉及计算复杂性、物理系统固有的测量误差和控制动力学等困难，如何确定每个机器人的最优路径并执行相应运动仍是一个开放性问题。

Method: 采用几何公式，通过适当的控制、定位和映射技术将几何公式映射到物理领域，实现机器人群体配置的无缝转换。
...
### [93] [Latent-Space Autoregressive World Model for Efficient and Robust Image-Goal Navigation](https://arxiv.org/abs/2511.11011)
*Zhiwei Zhang,Hui Zhang,Xieyuanli Chen,Kaihong Huang,Chenghao Shi,Huimin Lu*

Main category: cs.RO

TL;DR: LS-NWM是一种轻量级潜在空间导航世界模型，完全在潜在空间中进行训练和操作，相比现有方法显著降低了计算成本，同时提升了导航性能。


<details>
  <summary>Details</summary>
Motivation: 传统导航方法依赖精确定位和建图，而世界模型虽然提供了新视角但计算成本高昂。本文旨在解决世界模型在训练和推理中的高计算成本问题。

Method: 提出LS-NWM模型，在潜在空间中预测未来状态并执行路径规划，采用自回归多帧预测策略捕捉长期时空依赖关系。

Result: 相比基线方法，训练时间减少约3.2倍，规划时间减少约447倍，同时导航性能提升35%的SR和11%的SPL。

Conclusion: 该方法在保持最先进导航性能的同时，相比现有方法具有显著效率优势，证明了像素级环境预测对导航任务并非必要。
...
### [94] [Miniature Testbed for Validating Multi-Agent Cooperative Autonomous Driving](https://arxiv.org/abs/2511.11022)
...
Conclusion: CIVAT测试平台为协同自动驾驶的验证提供了有效的解决方案，填补了现有测试平台的空白。
...
### [95] [Humanoid Whole-Body Badminton via Multi-Stage Reinforcement Learning](https://arxiv.org/abs/2511.11218)
*Chenhao Liu,Leyun Jiang,Yibo Wang,Kairan Yao,Jinchen Fu,Xiaoyu Ren*

Main category: cs.RO

TL;DR: 本文提出了一个基于强化学习的训练框架，为人形机器人开发统一的全身控制器，用于羽毛球动态交互任务。该框架通过三阶段课程学习训练机器人协调下肢步法和上肢击球动作，无需运动先验或专家演示。


<details>
  <summary>Details</summary>
Motivation: 现实世界是动态的，准静态交互不足以应对各种环境条件。为了向更动态的交互场景迈进，需要开发能够在动态环境中进行精确目标击打的人形机器人控制器。

Method: 采用基于强化学习的训练管道，包含三阶段课程学习：步法获取、精确引导的球拍挥动生成、任务聚焦细化。部署时结合扩展卡尔曼滤波器估计羽毛球轨迹，还引入了无需预测的变体。

Result: 在仿真中，两个机器人连续击球21次；无需预测的变体性能与目标已知策略相当。真实世界测试中，控制器模块精度高，击球速度达10m/s，平均回球落点距离3.5米。

Conclusion: 该人形机器人能够在羽毛球中实现高度动态且精确的目标击打，并可适应更多动态关键领域。
...
### [96] [Sashimi-Bot: Autonomous Tri-manual Advanced Manipulation and Cutting of Deformable Objects](https://arxiv.org/abs/2511.11223)
...
TL;DR: 本文介绍了Sashimi-Bot系统，这是一个用于处理三文鱼等可变形物体的多机器人系统，能够进行高级的切割操作。


<details>
  <summary>Details</summary>
Motivation: 处理可变形、易碎、形状多变的物体是机器人操作中的重大挑战，特别是像三文鱼这样的自然物体具有弹性参数不明确、表面光滑难以抓握等特点。

Method: 系统使用三个机器人分别负责拉直鱼块、抓握刀具、切割操作，并结合深度强化学习、视觉和触觉反馈来实现鲁棒性。

Result: 成功实现了对三文鱼的自动化切割操作，能够在切割过程中协同稳定鱼块，并拾取薄片。

Conclusion: 这项工作代表了可变形物体机器人操作的重要里程碑，为其他实际应用提供了启发和可能性。
...
### [97] [Experiences from Benchmarking Vision-Language-Action Models for Robotic Manipulation](https://arxiv.org/abs/2511.11298)
*Yihao Zhang,Yuankai Qi,Xi Zheng*

Main category: cs.RO

TL;DR: 本文通过建立标准化评估框架，对四种代表性视觉-语言-动作模型（ACT、OpenVLA-OFT、RDT-1B、π₀）在模拟和真实机器人平台上的性能进行实证比较，重点关注准确性、适应性和指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 当前机器人领域的基础模型特别是VLA模型在实现通用操作方面具有巨大潜力，但缺乏系统的真实世界评估和跨模型比较，需要建立标准化评估框架来指导模型选择和应用。

Method: 建立标准化评估框架，在模拟和ALOHA Mobile平台上对四种VLA模型进行测试，从三个维度评估：准确性和效率（成功率、成功时间）、适应性（分布内、空间分布外、实例+空间分布外）、语言指令跟随准确性。

Result: π₀在分布外场景中表现出更好的适应性，ACT在分布内场景中稳定性最高。分析还揭示了计算需求、数据扩展行为和常见失败模式（如接近抓取失败、过早释放、长时状态漂移）的差异。
...
### [98] [Simulating an Autonomous System in CARLA using ROS 2](https://arxiv.org/abs/2511.11310)
*Joseph Abdo,Aditya Shibu,Moaiz Saeed,Abdul Maajid Aga,Apsara Sivaprazad,Mohamed Al-Musleh*

Main category: cs.RO

TL;DR: 本文提出了一种在CARLA模拟器中设计和评估自动驾驶赛车软件栈的方法，针对FS-AI 2025竞赛，使用多传感器融合和ROS 2系统实现赛道边界检测和优化轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 为FS-AI 2025竞赛开发高性能自动驾驶赛车软件栈，在高速和不确定性条件下测试感知、规划和控制系统。

Method: 使用360°激光雷达、立体相机、GNSS和IMU传感器通过ROS 2系统，检测赛道边界锥桶（最远35米），考虑车辆动力学和环境因素计算优化轨迹。

Result: 系统在CARLA模拟器中可靠检测赛道边界锥桶，并在专用车辆上验证后移植到实际硬件平台。
...
### [99] [A Comparative Evaluation of Prominent Methods in Autonomous Vehicle Certification](https://arxiv.org/abs/2511.11484)
*Mustafa Erdem Kırmızıgül,Hasan Feyzi Doğruyol,Haluk Bayram*

Main category: cs.RO

TL;DR: 本文比较评估了自动驾驶车辆认证过程中的主要方法，开发了认证流程管道，确定了阶段、参与者和方法应用领域。


<details>
  <summary>Details</summary>
Motivation: 瑞典议会1997年提出的"零愿景"政策旨在消除交通事故导致的死亡和重伤，需要确定自动驾驶车辆的基本安全要求验证和认证方法。

Method: 比较评估认证过程中的主要方法，开发自动驾驶车辆认证流程管道，确定认证阶段、参与者和方法应用领域。

Result: 建立了自动驾驶车辆认证流程的系统框架，明确了不同方法在认证过程中的适用位置和作用。

Conclusion: 为自动驾驶车辆的认证提供了系统化的方法框架，支持"零愿景"政策的实施。
...
### [100] [Collaborative Representation Learning for Alignment of Tactile, Language, and Vision Modalities](https://arxiv.org/abs/2511.11512)
*Yiyun Zhou,Mingjing Xu,Jingwei Shi,Quanjiang Li,Jingyuan Chen*

Main category: cs.RO

TL;DR: TLV-CoRe是一种基于CLIP的触觉-语言-视觉协同表示学习方法，通过传感器感知调制器和触觉无关解耦学习统一不同传感器的触觉特征，并使用统一桥接适配器增强三模态交互，显著提升了传感器无关表示学习和跨模态对齐能力。


<details>
  <summary>Details</summary>
Motivation: 现有触觉传感器缺乏标准化导致冗余特征，阻碍跨传感器泛化；现有方法未能充分整合触觉、语言和视觉模态之间的中间通信。
...
Conclusion: TLV-CoRe通过传感器感知调制、触觉无关解耦学习和统一桥接适配器，有效解决了触觉传感器标准化和多模态交互的挑战，推动了多模态触觉表示学习的发展。
...
### [101] [Scalable Coverage Trajectory Synthesis on GPUs as Statistical Inference](https://arxiv.org/abs/2511.11514)
*Max M. Sun,Jueun Kwon,Todd Murphey*

Main category: cs.RO

TL;DR: 该论文将覆盖运动规划问题重新表述为基于流匹配的统计推断问题，统一了常用的统计差异度量与线性二次调节器问题，并通过GPU并行化实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 传统运动规划方法在计算效率和并行化方面存在局限，无法有效处理覆盖运动规划中对整个轨迹空间分布进行推理的需求。

Method: 采用流匹配的生成建模技术，将覆盖运动规划问题表述为统计推断问题，解耦轨迹梯度生成与非线性系统动力学下的控制合成。

Result: 该方法能够显著提高计算效率，特别适合在现代GPU架构上进行并行化处理，相比基于航点跟踪的传统方法具有计算优势。

Conclusion: 提出的统计推断框架为覆盖运动规划提供了一种可扩展的解决方案，通过并行化实现了计算效率的显著提升。
...
### [102] [Scalable Policy Evaluation with Video World Models](https://arxiv.org/abs/2511.11520)
...
<details>
  <summary>Details</summary>
Motivation: 评估通用机器人操作策略在真实世界中存在成本高、耗时长、安全风险等问题，而传统仿真环境存在工程量大和模拟到真实差距大的挑战。

Method: 将动作条件融入预训练的视频生成模型，利用互联网规模的野外在线视频进行预训练，避免收集昂贵的配对视频-动作数据。

Result: 实验表明，在策略排序和实际策略值与预测策略值相关性等多种指标上，该方法为无需真实世界交互的策略评估提供了有前景的解决方案。

Conclusion: 动作条件视频生成模型为机器人操作策略评估提供了一种可扩展且有效的方法，能够利用大规模在线视频数据，减少对真实世界交互的依赖。
