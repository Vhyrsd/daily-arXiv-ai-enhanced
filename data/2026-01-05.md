...
Method: 提出生成-重建-引导范式：生成的视频流被连续重建为动态4D时空表示，该表示反过来引导后续生成以保持一致性。采用增强的自回归扩散视频模型，结合宏观-微观规划（MMPL）和高效分布匹配蒸馏（DMD），实现低延迟的长期生成。
...
TL;DR: 本文提出通过噪声优化解决文本到图像模型的模式崩溃问题，相比传统方法，该方法能提升生成多样性和质量


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型存在严重的模式崩溃问题，即相同文本提示下生成的图像缺乏多样性。传统方法通过引导机制或候选池筛选来解决，但本文探索了不同的噪声优化方向

Method: 提出简单的噪声优化目标来缓解模式崩溃，同时保持基础模型的保真度。分析噪声的频率特性，探索不同频率分布的替代噪声初始化方法以改善优化和搜索效果

Result: 实验表明噪声优化在生成质量和多样性方面都取得了优越的结果，能够有效缓解模式崩溃问题

Conclusion: 噪声优化是一种有效解决文本到图像模型模式崩溃问题的方法，通过优化噪声初始化可以显著提升生成多样性和质量
...
TL;DR: 本文介绍了Spatial4D-Bench，一个用于评估多模态大语言模型4D空间智能的大规模基准测试，包含约40,000个问题-答案对，涵盖18个任务和6个认知类别。


<details>
  <summary>Details</summary>
Motivation: 人类天生具备4D空间智能（感知物体随时间的变化），但现有空间智能基准测试要么规模小，要么多样性不足。本文旨在评估多模态大语言模型是否能达到人类水平的4D空间智能，并为此创建了一个全面、结构化的评估基准。

Method: 开发了Spatial4D-Bench基准测试，包含约40,000个问题-答案对，覆盖18个明确定义的任务。这些任务被系统性地组织到6个认知类别中：物体理解、场景理解、空间关系理解、时空关系理解、空间推理和时空推理。

Result: 在Spatial4D-Bench上评估了各种最先进的开源和专有多模态大语言模型，发现它们在多种4D空间推理方面存在显著局限性，如路径规划、动作识别和物理合理性推理等。

Conclusion: Spatial4D-Bench为评估MLLMs的空间认知能力提供了一个结构化、全面的基准，揭示了当前模型在4D空间智能方面的不足，希望该基准能促进开发更接近人类水平4D空间智能的MLLMs。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶视觉系统通常假设每次都是首次访问新地点，忽略了历史遍历数据中蕴含的宝贵空间先验信息。人类驾驶员很少去完全陌生的地方，大多数道路已被多次遍历，这些历史数据应被有效利用。

Method: 提出压缩地图先验（CMP）框架，从历史遍历数据中学习空间先验。使用二值化哈希映射存储地图先验，存储密度仅为32KB/km²。该方法可轻松集成到主流3D感知系统中，几乎不增加额外计算成本。

Result: 在nuScenes数据集上，CMP框架显著且一致地提升了多种架构的3D目标检测性能。压缩存储比密集存储减少了20倍空间需求，同时保持了高效集成能力。

Conclusion: 利用历史遍历数据构建压缩地图先验是提升自动驾驶视觉系统性能的有效方法，能以极低的存储和计算成本显著改善3D感知能力。
...
TL;DR: FCMBench-V1.0是一个专门为金融信贷领域设计的大规模多模态基准测试，包含4,043张隐私合规图像和8,446个QA样本，用于评估视觉语言模型在信贷风险评估和文档审查中的实际应用能力。


<details>
  <summary>Details</summary>
Motivation: 随着多模态AI在信贷风险评估和文档审查中的广泛应用，迫切需要专门针对金融信贷领域的基准测试，该基准需要：(1)反映金融信贷申请特有的文档和工作流程，(2)包含信贷特定的理解和现实世界鲁棒性，(3)在不牺牲实用性的前提下保持隐私合规性。

Method: 通过封闭式合成-捕获流水线构建所有样本：手动合成带有虚拟内容的文档模板，并在内部捕获场景感知图像。评估框架包括三个维度：感知、推理和鲁棒性，涵盖3个基础感知任务、4个信贷特定推理任务和10种现实世界采集伪影类型。

Result: 在评估的23个最先进视觉语言模型中，Gemini 3 Pro作为商业模型获得最佳F1分数(64.61%)，Qwen3-VL-235B作为开源基线获得最佳分数(57.27%)，而专门针对金融信贷的模型Qfin-VL-Instruct获得最高总体分数(64.92%)。鲁棒性评估显示，即使表现最佳的模型在采集伪影下也会出现明显的性能下降。

Conclusion: FCMBench-V1.0能够有效区分现代视觉语言模型的性能差异和鲁棒性，为金融信贷领域的多模态AI评估提供了专门、实用且隐私合规的基准测试框架。
...
<details>
  <summary>Details</summary>
Motivation: 面部分析中一个未被充分探索的问题：为任意选定面部区域生成和识别多属性自然语言描述，包含面部动作单元、情绪状态和年龄估计。系统关注个体面部区域的能力能带来更好的理解和控制

Method: 1) 构建新的多属性描述数据集，为任意选定面部区域提供丰富的区域级标注和自然语言描述；2) 提出基于Qwen2.5-VL微调的视觉语言模型Focal-RegionFace，通过多个渐进微调阶段逐步细化对局部面部特征的关注

Result: Focal-RegionFace在新基准测试中，在传统广泛使用的指标和新提出的指标上都取得了最佳性能，充分验证了其在细粒度多属性面部区域聚焦分析场景中的有效性和多功能性

Conclusion: 该研究成功解决了面部分析中的FaceFocalDesc问题，通过构建新数据集和开发Focal-RegionFace模型，实现了对任意选定面部区域的细粒度多属性分析，为面部状态分析提供了新的解决方案
...
TL;DR: MorphAny3D是一个无需训练的三维变形框架，利用结构化潜在表示（SLAT）实现高质量的三维变形，通过创新的注意力机制融合源和目标特征，支持跨类别变形。


<details>
  <summary>Details</summary>
Motivation: 三维变形面临语义一致性和时间平滑性的挑战，特别是在跨类别变形时更加困难。现有方法难以生成高质量、连贯的变形序列。

Method: 提出MorphAny3D框架：1) 使用Morphing Cross-Attention（MCA）融合源和目标SLAT特征以保持结构一致性；2) 引入Temporal-Fused Self-Attention（TFSA）通过整合前一帧特征增强时间一致性；3) 采用方向校正策略缓解变形过程中的姿态模糊问题。

Result: 实验表明该方法能生成最先进的变形序列，即使在具有挑战性的跨类别情况下也能表现优异。进一步支持解耦变形和三维风格迁移等高级应用，并可推广到其他基于SLAT的生成模型。

Conclusion: MorphAny3D通过创新的注意力机制有效解决了三维变形的语义一致性和时间平滑性问题，为高质量三维变形提供了有效的训练免费解决方案，具有广泛的应用潜力。
...
TL;DR: 提出基于3D实例分割的作物计数框架，利用多视角2D图像和神经辐射场进行精确计数，消除对作物特定参数调整的依赖


<details>
  <summary>Details</summary>
Motivation: 室外田间环境中，部分遮挡和作物聚集造成的模糊性给基于图像的分割方法带来巨大挑战，需要精确的作物计数方法

Method: 使用多视角2D图像，结合神经辐射场视图合成，引入作物可见性和掩码一致性评分，实现3D实例分割

Result: 在棉花、苹果、梨三个农业数据集上验证，展示一致的计数性能，与现有技术相比具有优越性能

Conclusion: 提出的3D实例分割框架能有效解决作物计数问题，不受作物颜色、形状、大小变化影响，并贡献了棉花植物数据集
...
Method: 提出基于示例的风格合成方法IntraStyler，使用示例图像引导风格合成，使输出风格匹配示例风格。引入风格编码器，基于对比学习提取纯风格特征。

Result: 在最大的跨模态域适应公开数据集CrossMoDA 2023上评估，实验证明该方法在可控风格合成方面的有效性，以及多样化合成数据对下游分割任务的益处。

Conclusion: IntraStyler能够无需先验知识捕捉多样化的域内风格，通过示例引导的风格合成方法有效提升了跨模态域适应的性能，代码已开源。
...
TL;DR: 该论文提出使用强化学习来提升多模态大语言模型的视觉推理能力，通过设计针对不同推理方面的奖励函数，激励模型生成更长、结构化的视觉推理过程，从而解决视觉信息整合不足的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在生成推理链时缺乏对视觉信息的有效整合，这限制了它们解决需要准确视觉感知的任务（如视觉谜题）的能力。研究表明，将图像转换为文本描述能显著提升性能，但需要一种无需昂贵监督的方法来解锁模型的长期视觉推理能力。

Method: 采用奖励驱动的强化学习方法，设计了六个针对不同推理方面的奖励函数，包括图像理解、思维步骤和答案准确性。使用组相对策略优化（GRPO）来明确激励更长、结构化的推理，并缓解视觉信息被绕过的问题。

Result: 在Qwen-2.5-VL-7B模型上实现了5.56%的性能提升，在领域内和领域外设置中都取得了持续性的改进。实验还显示，将图像转换为文本描述能为Claude 3.5和Claude 3.7分别带来26.7%和23.6%的性能增益。

Conclusion: 强化学习是解锁开源多模态大语言模型长期视觉推理能力的有效机制，无需昂贵的监督数据。通过设计针对性的奖励函数和优化策略，可以显著提升模型在需要准确视觉感知的任务上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 随着数据和模型复杂度不断增加，现有向量量化方法面临高容量与紧凑性之间的矛盾。需要一种既能保持高性能又能显著减小码本规模的向量量化技术。

Method: LooC采用低维码本组合量化：1) 将码向量重构为特征向量中的低维组合单元，通过组合而非单独匹配来扩展解空间；2) 引入参数无关的外推-插值机制，在量化过程中增强和平滑特征；3) 实现全码本利用，避免码本崩溃问题。

Result: 在不同任务、数据集和架构上的广泛评估表明，LooC在显著减小码本规模的同时，性能优于现有向量量化方法，达到了最先进的性能水平。

Conclusion: LooC成功解决了向量量化中高容量与紧凑性的矛盾，提供了一种高效、紧凑且可即插即用的量化解决方案，在各种下游任务中展现出优越性能。
...
TL;DR: SynDR-IQA通过重塑合成数据分布来解决BIQA中合成数据泛化能力不足的问题，提出分布感知多样性内容上采样和密度感知冗余聚类下采样两种策略，显著提升了跨数据集评估性能。


<details>
  <summary>Details</summary>
Motivation: 盲图像质量评估(BIQA)面临大规模标注数据稀缺的挑战，合成数据虽然提供了解决方案，但现有合成数据集训练的模型泛化能力有限。研究发现合成数据集学习到的表征呈现离散聚类模式，阻碍了回归性能。

Method: 基于样本多样性和冗余对泛化误差影响的理论推导，提出SynDR-IQA框架：1) 分布感知多样性内容上采样，在保持内容分布的同时增强视觉多样性；2) 密度感知冗余聚类下采样，通过减少密集聚类区域的样本密度来平衡样本分布。

Result: 在三种跨数据集设置（合成到真实、合成到算法、合成到合成）上的广泛实验证明了该方法的有效性，显著提升了BIQA模型的泛化能力。

Conclusion: 合成数据分布问题是限制BIQA泛化能力的关键因素，SynDR-IQA通过重塑合成数据分布有效解决了这一问题，为基于合成数据的BIQA研究提供了新思路。
...
TL;DR: 提出跨模态数据增强框架，结合CycleGAN和YOLOv8解决PCB红外缺陷检测数据稀缺问题，通过可见光图像生成伪红外样本增强检测性能


<details>
  <summary>Details</summary>
Motivation: 红外(IR)数据稀缺是PCB缺陷检测的关键瓶颈，传统方法依赖成对监督数据，实际应用中红外数据获取成本高且数量有限

Method: 使用CycleGAN进行无配对图像转换，将丰富的可见光PCB图像映射到红外域，生成高质量伪红外样本；构建异构训练策略，融合伪红外数据和有限真实红外样本训练轻量级YOLOv8检测器

Result: 该方法有效增强了低数据条件下的特征学习，增强后的检测器显著优于仅使用有限真实数据训练的模型，性能接近完全监督训练的基准水平
...
### [14] [Efficient Prediction of Dense Visual Embeddings via Distillation and RGB-D Transformers](https://arxiv.org/abs/2601.00359)
...
Method: 提出DVEFormer - 基于RGB-D Transformer的高效方法，通过知识蒸馏从Alpha-CLIP获取教师嵌入，指导学生模型学习细粒度像素级嵌入。支持传统语义分割（如线性探测），同时支持灵活的基于文本的查询。

Result: 在常见室内数据集上评估显示，该方法在满足实时性要求的同时达到竞争性性能：完整模型26.3 FPS，小型变体77.0 FPS（NVIDIA Jetson AGX Orin）。定性结果展示了在实际应用中的有效性和可能用例。

Conclusion: 该方法可作为传统分割方法的即插即用替代方案，同时支持灵活的自然语言查询和无缝集成到移动机器人3D建图流程中。
...
TL;DR: 本文提出了一种轻量级害虫检测与农药推荐框架，适用于智能手机和无人机等低资源设备，帮助小农户实现精准农业。


<details>
  <summary>Details</summary>
Motivation: 传统害虫管理方法依赖人工田间检查和化学农药，成本高、耗时长、劳动密集且对环境有害。需要为资源有限的小农户开发高效、环保的解决方案。

Method: 框架包含两个模块：1) 害虫检测模块使用轻量级CNN结合原型元学习，在少量训练样本下准确识别害虫；2) 农药推荐模块结合作物类型和生长阶段等环境因素，推荐安全环保的农药。

Result: 实验表明，轻量级CNN在保持高精度的同时显著降低计算复杂度，性能可与最先进模型媲美。决策支持系统减少了对传统化学农药的依赖，促进了可持续农业实践。
...
### [16] [RGS-SLAM: Robust Gaussian Splatting SLAM with One-Shot Dense Initialization](https://arxiv.org/abs/2601.00705)
*Wei-Tse Cheng,Yen-Jen Chiou,Yuan-Fu Yang*

Main category: cs.CV

TL;DR: RGS-SLAM是一种鲁棒的高斯溅射SLAM框架，通过训练无关的对应到高斯初始化取代了GS-SLAM的残差驱动致密化阶段，使用DINOv3描述符和置信感知内点分类器进行多视角对应三角化，实现更稳定、更快的SLAM性能。


<details>
  <summary>Details</summary>
Motivation: 传统GS-SLAM使用残差驱动致密化逐步添加高斯分布，这种方法在早期映射阶段不稳定且收敛慢。RGS-SLAM旨在通过一次性初始化提供结构感知的高斯种子先验，改善早期映射稳定性并加速收敛。

Method: RGS-SLAM采用训练无关的对应到高斯初始化方法：1）使用DINOv3描述符获取密集多视角对应；2）通过置信感知内点分类器进行细化；3）进行一次性三角化生成结构感知的高斯种子；4）在优化前提供良好分布的高斯先验。

Result: 在TUM RGB-D和Replica数据集上评估，RGS-SLAM相比最先进的高斯和基于点的SLAM系统，实现了竞争性或更优的定位和重建精度。收敛速度提升约20%，在纹理丰富和杂乱场景中渲染保真度更高，保持实时映射性能达925 FPS。

Conclusion: RGS-SLAM通过训练无关的对应到高斯初始化方法，显著改善了高斯溅射SLAM的早期映射稳定性和收敛速度，同时保持与现有GS-SLAM管道的完全兼容性，在实时性能和渲染质量方面表现出色。
...
### [17] [TotalFM: An Organ-Separated Framework for 3D-CT Vision Foundation Models](https://arxiv.org/abs/2601.00260)
...
<details>
  <summary>Details</summary>
Motivation: 放射学基础模型在处理3D-CT体积数据时面临计算成本限制的挑战，需要一种既能保持计算效率又能具备良好表征能力的方法。

Method: 基于器官分离概念，利用14万系列大规模数据集，通过分割技术和基于LLM的放射学报告处理自动化创建器官体积-发现句子对，结合VideoMAE自监督预训练和体积-文本对的对比学习。

Result: 在零样本器官病变分类任务中，相比CT-CLIP在83%器官上获得更高F1分数，相比Merlin在64%器官上表现更好；在零样本发现病变分类任务中，相比Merlin在83%发现类别上获得更高AUROC；在放射学报告生成任务中达到与现有VLM相当的性能。
...
### [18] [S1-MMAlign: A Large-Scale, Multi-Disciplinary Dataset for Scientific Figure-Text Understanding](https://arxiv.org/abs/2601.00264)
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习在通用领域取得了革命性进展，但在科学发现中的应用受到复杂科学图像与稀疏文本描述之间深刻语义鸿沟的阻碍。

Method: 从250万篇开放获取科学论文中提取1550万图像-文本对，涵盖物理、生物、工程等多学科；引入AI就绪的语义增强管道，利用Qwen-VL多模态大模型系列，通过合成论文摘要和引用上下文来重新描述图像。

Result: 增强显著提升了数据质量：基于SciBERT的伪困惑度指标显示语义模糊性降低，CLIP分数表明图像-文本对齐提高了18.21%。
...
### [19] [ActErase: A Training-Free Paradigm for Precise Concept Erasure via Activation Patching](https://arxiv.org/abs/2601.00267)
*Yi Sun,Xinhao Zhong,Hongyan Li,Yimin Zhou,Junhao Li,Bin Chen,Xuan Wang*

Main category: cs.CV

TL;DR: 提出ActErase：一种无需训练的概念擦除方法，通过激活差异区域识别和动态替换实现高效概念移除，在多个任务上达到SOTA性能
...
Method: 基于模型激活主要由通用概念组成、仅极小部分能表示目标概念的观察，通过提示对分析识别激活差异区域，提取目标激活并在前向传播中动态替换输入激活

Result: 在三个关键擦除任务（裸露内容、艺术风格、对象移除）上实现SOTA擦除性能，有效保留模型整体生成能力，对对抗攻击具有强鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的动态目标重建面临观测稀疏的挑战。传统方法需要密集的多视角视频，但在实际场景（如监控摄像头）中，观测在时间和空间上都很稀疏，导致重建问题高度不适定。

Method: SV-GS框架同时估计变形模型和物体运动。利用粗略骨架图和初始静态重建作为输入引导运动估计。方法优化骨架驱动的变形场，包含粗粒度骨架关节姿态估计器和细粒度变形模块。仅使关节姿态估计器随时间变化，实现平滑运动插值并保留几何细节。

Result: 在合成数据集上，方法在稀疏观测下比现有方法PSNR提升高达34%。在真实数据集上，使用显著更少的帧数就能达到与密集单目视频方法相当的性能。还证明初始静态重建可被基于扩散的生成先验替代，提高了实用性。

Conclusion: SV-GS能够在稀疏观测下有效重建动态目标，通过骨架驱动变形场实现高质量运动估计，为现实世界动态重建提供了实用解决方案。
...
TL;DR: 基于Swin Transformer的深度学习模型在ISIC2019数据集上对8种皮肤病变类别达到87.71%的预测准确率，可作为临床诊断支持工具和患者自评辅助。
...
Method: 利用公开皮肤疾病图像数据集进行预训练，开发基于Swin Transformer的深度学习模型，优化模型架构、数据预处理流程，并应用针对性数据增强技术。
...
TL;DR: TimeColor是一个基于草图的视频着色模型，支持使用异构、可变数量的参考图像，通过每参考区域分配和时空对应掩码注意力来提升着色质量。
...
Method: 1. 将参考图像编码为额外的潜在帧，在时间维度上拼接，使模型能在每个扩散步骤中同时处理参考图像，同时保持参数数量不变。2. 使用每参考区域分配机制。3. 采用时空对应掩码注意力来强制主体-参考绑定，结合模态分离的RoPE索引，防止捷径学习和跨身份调色板泄漏。

Result: 在SAKUGA-42M数据集上的实验表明，TimeColor在单参考和多参考协议下，相比先前基线方法，在颜色保真度、身份一致性和时间稳定性方面都有显著提升。

Conclusion: TimeColor通过支持异构、可变数量的参考图像，结合创新的区域分配和注意力机制，有效提升了视频着色的质量和一致性，为视频着色任务提供了更灵活的解决方案。
...
TL;DR: VisNet是一个高效的人体重识别模型，通过多尺度特征融合、语义聚类、动态权重平均和FIDI损失函数等技术，在保持高精度的同时大幅降低计算成本，适用于监控和移动应用。


<details>
  <summary>Details</summary>
Motivation: 当前人体重识别方法虽然准确率高但计算成本大，难以在计算资源有限的实时监控和移动应用中部署。需要开发既高效又准确的模型来满足实际应用需求。

Method: 1. 多尺度特征融合：融合ResNet50第1-4阶段特征，不使用并行路径；2. 语义聚类：通过基于规则的伪标签进行解剖学身体分区；3. 动态权重平均：平衡分类语义正则化；4. FIDI损失函数：改进度量学习任务。

Result: 在Market-1501数据集上达到87.05% Rank-1准确率和77.65% mAP，模型参数量为32.41M，计算量为4.601 GFLOPs，显著降低了计算成本。

Conclusion: VisNet提出了一种实用的人体重识别方法，在保持高精度的同时大幅降低计算复杂度，适合在计算资源有限的监控和移动应用中实时部署。
...
TL;DR: OmniVaT框架首次成功解决了单域泛化多模态视觉-触觉学习任务，通过多模态分数傅里叶适配器和离散树生成模块，有效缓解模态差异并增强对未见域的适应性。


<details>
  <summary>Details</summary>
Motivation: 视觉-触觉学习面临模态差异和域差距的双重挑战：VIS和TAC图像之间存在模态差异，同时非标准化的触觉传感器和不一致的数据收集程序导致域差距。这些挑战被形式化为单域泛化多模态VTL任务。

Method: 提出OmniVaT框架，包含两个核心组件：1）多模态分数傅里叶适配器（MFFA），将VIS和TAC嵌入映射到统一的嵌入-频率空间，无需多域训练数据或复杂的跨模态融合策略；2）离散树生成（DTG）模块，通过分层树结构获得多样且可靠的多模态分数表示，增强对未见域波动的适应性。

Result: 大量实验证明OmniVaT在SDG-VTL任务上具有优越的跨域泛化性能，成功解决了单域泛化多模态视觉-触觉学习任务。

Conclusion: OmniVaT框架首次成功解决了单域泛化多模态视觉-触觉学习任务，通过创新的MFFA和DTG模块有效缓解了模态差异和域差距问题，为具身智能体感知物理世界提供了有效解决方案。
...
TL;DR: 提出概率双流框架统一可靠性建模和多模态集成，解决骨架动作识别中忽略手部细微动作的问题，通过校准自由预处理、概率融合和跨模态集成提升细粒度识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有骨架动作识别方法主要关注身体大尺度运动，忽略了对手部细微动作的识别，而手部动作对于细粒度动作识别至关重要。同时，现有方法在多模态融合和可靠性建模方面存在不足。

Method: 提出概率双流框架，包含三个核心组件：1) 校准自由预处理管道，直接从原生坐标学习；2) 概率Noisy-OR融合，无需显式置信度监督；3) 骨架到RGB的跨模态集成，耦合四种骨架模态。

Result: 在多个基准数据集（NTU RGB+D 60/120、PKU-MMD、N-UCLA）和新定义的手部中心基准上均表现出持续改进和鲁棒性，特别是在噪声和异构条件下。

Conclusion: 该概率双流框架通过统一可靠性建模和多模态集成，有效解决了骨架动作识别中手部细微动作的识别问题，在多种条件下展现出优越性能，为细粒度动作识别提供了新思路。
...
TL;DR: NeoVerse是一个多功能4D世界模型，能够进行4D重建、新轨迹视频生成和丰富的下游应用，通过创新的可扩展设计解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前4D世界建模方法存在可扩展性限制，主要源于昂贵的多视角4D数据或繁琐的训练预处理，限制了在多样化单目视频中的应用。

Method: NeoVerse采用无姿态前馈4D重建、在线单目退化模式模拟等技术，使整个流程能够扩展到多样化的野外单目视频。

Result: NeoVerse在标准重建和生成基准测试中达到了最先进的性能，并展现出对多个领域的泛化能力。

Conclusion: NeoVerse通过创新的可扩展设计，成功构建了一个多功能4D世界模型，为4D重建和生成任务提供了有效的解决方案。
...
TL;DR: RoLID-11K是首个用于车载摄像头路边垃圾检测的大规模数据集，包含超过11,000张标注图像，专注于极端小目标检测，并评估了多种现代检测器在该挑战性任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前路边垃圾监测依赖劳动密集型调查和公众报告，空间覆盖有限。现有的视觉垃圾检测数据集主要关注街景、航拍或水环境，无法反映车载摄像头视频中垃圾目标极小、稀疏且嵌入杂乱路侧背景的独特特征。

Method: 研究人员创建了RoLID-11K数据集，包含超过11,000张标注图像，涵盖英国多样驾驶条件，呈现明显的长尾分布和小目标分布特征。他们基准测试了广泛的现代检测器，包括精度导向的Transformer架构和实时YOLO模型。

Result: 实验结果显示，CO-DETR及相关Transformer架构实现了最佳定位精度，而实时模型受限于粗糙的特征层次结构。该数据集为动态驾驶场景中的极端小目标检测建立了具有挑战性的基准。

Conclusion: RoLID-11K数据集旨在支持开发可扩展、低成本的路边垃圾监测系统，为计算机视觉社区提供了首个专门针对车载摄像头路边垃圾检测的大规模基准数据集。
...
TL;DR: CPPO是一种用于微调视觉语言模型的对比感知策略优化方法，通过检测扰动图像下的熵变化来识别感知标记，并引入对比感知损失来增强感知一致性。
...
TL;DR: MotionPhysics是一个端到端可微分框架，通过自然语言提示为3D场景推断合理的物理参数，无需真实轨迹或标注视频指导，利用多模态大语言模型和运动蒸馏损失实现逼真的动态模拟。


<details>
  <summary>Details</summary>
Motivation: 传统3D物体和材料模拟需要专家知识和耗时的物理参数调整才能达到期望的动态效果。现有方法通常需要真实轨迹或标注视频作为指导，限制了其应用范围。

Method: 1. 使用多模态大语言模型估计材料参数值，并约束在合理范围内；2. 提出可学习的运动蒸馏损失，从预训练视频扩散模型中提取鲁棒的运动先验，同时最小化外观和几何归纳偏差来指导模拟。

Result: 在30多个场景中评估，包括真实世界、人工设计和AI生成的3D物体，涵盖弹性固体、金属、泡沫、沙子、牛顿和非牛顿流体等多种材料。MotionPhysics能生成视觉逼真的动态模拟，超越现有技术水平，同时自动确定物理合理的参数。

Conclusion: MotionPhysics通过自然语言提示为3D场景推断物理参数，无需真实轨迹指导，在多种材料和场景中都能产生逼真的动态模拟，为物理模拟提供了更便捷和自动化的解决方案。
...
TL;DR: FreeText是一个无需训练、即插即用的框架，通过利用扩散Transformer模型的内部机制来改进文本渲染，解决了多行布局、密集排版和中文等长尾脚本的文本渲染问题。
...
Method: FreeText将问题分解为"在哪里写"和"写什么"。对于"在哪里写"，通过读取token级别的空间归因来定位书写区域，使用类似汇点的token作为稳定的空间锚点，并通过拓扑感知细化产生高置信度掩码。对于"写什么"，引入频谱调制字形注入(SGMI)，注入与噪声对齐的字形先验，通过频域带通调制来增强字形结构并抑制语义泄漏。

Result: 在Qwen-Image、FLUX.1-dev和SD3变体上的大量实验表明，在长文本基准测试、CVTG和作者提出的CLT-Bench上，文本可读性持续提升，同时很大程度上保持了语义对齐和美学质量，推理开销适中。

Conclusion: FreeText是一个无需训练、即插即用的框架，通过利用扩散Transformer模型的内部机制，有效解决了文本到图像模型中的文本渲染问题，特别是在多行布局、密集排版和中文等长尾脚本方面表现优异。
...
TL;DR: VNS-SAM通过增强SAM在视觉非显著场景下的分割能力，同时保持其零样本泛化性，解决了SAM在低对比度前景背景场景中的性能瓶颈问题。
...
Method: 提出VNS-SAM，通过Mask-Edge Token Interactive解码器和Non-Salient Feature Mining模块有效利用SAM的低层特征，仅需少量参数增加和计算成本。同时构建了包含35K+图像的VNS-SEG统一数据集。

Result: VNS-SAM在各种VNS分割任务中表现出优越性能，特别是在零样本设置下，额外参数可在4小时内优化完成，证明了其实用性和可行性。
...
Method: 提出DynaDrag方法，采用"预测-移动"框架，迭代执行运动预测和运动监督：运动预测预测处理点应移动的位置，运动监督据此拖动处理点，并动态调整有效处理点

Result: 在人脸和人体数据集上的实验表明，该方法在性能上优于先前的工作

Conclusion: DynaDrag通过创新的"预测-移动"框架有效解决了传统拖拽式图像编辑方法的问题，实现了更好的像素级图像操控性能
...
TL;DR: SlingBAG Pro是一种基于点云迭代概念的光声成像重建算法，专门针对不规则几何换能器阵列设计，在保持高质量重建的同时显著提升计算效率。
...
Conclusion: SlingBAG Pro算法能够在不规则阵列配置下实现高质量三维光声成像重建，显著减少所需换能器数量并大幅缩短重建时间，具有临床应用潜力。
...
TL;DR: MS COCOAI是一个新的AI生成图像检测数据集，包含96000个真实和合成数据点，基于MS COCO数据集构建，使用五种生成器创建合成图像，并提出两个检测任务。


<details>
  <summary>Details</summary>
Motivation: 随着多模态生成AI系统（如Stable Diffusion、DALL-E、MidJourney）的发展，合成图像越来越难以与真实照片区分，这导致了误导性内容、虚假信息和操纵媒体的传播，因此检测AI生成图像变得迫切重要。

Method: 基于MS COCO数据集构建MS COCOAI数据集，包含96000个真实和合成数据点。使用五种生成器创建合成图像：Stable Diffusion 3、Stable Diffusion 2.1、SDXL、DALL-E 3和MidJourney v6。提出两个任务：1）分类图像为真实或生成；2）识别合成图像由哪个模型生成。

Result: 发布了MS COCOAI数据集，该数据集已在Hugging Face上公开可用，为AI生成图像检测研究提供了标准化的基准数据集。

Conclusion: MS COCOAI数据集为解决AI生成图像检测问题提供了重要资源，有助于开发更有效的检测方法，应对生成AI技术带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在评估统一多模态模型的世界知识应用能力方面存在不足，仅提供孤立的单任务评估且诊断能力有限，需要更全面的多任务评估框架。

Method: 提出了AEGIS基准，涵盖视觉理解、生成、编辑和交错生成任务，包含1050个挑战性问题，涉及21个主题和6种推理类型。同时提出了确定性检查表评估协议，用原子化的"是/否"判断替代模糊的提示评分。

Result: 实验表明大多数统一多模态模型存在严重的世界知识缺陷，复杂推理任务下性能显著下降。简单的插件推理模块可以部分缓解这些弱点。

Conclusion: 世界知识推理是统一多模态模型发展的关键前沿，AEGIS基准和DCE评估协议为系统评估模型的世界知识应用能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 零样本视频时刻检索面临的主要挑战是文本查询与视觉内容之间的语义粒度不匹配。现有方法虽然利用了预训练知识，但未能平衡不同模态间的语义粒度，导致检索不准确。

Method: 提出无训练框架GranAlign，包含两个互补技术：1) 基于粒度的查询重写，生成不同语义粒度的查询；2) 查询感知字幕生成，将查询意图嵌入视频内容。通过将多粒度查询与查询无关和查询感知字幕配对，有效解决语义不匹配问题。
...
Conclusion: GranAlign框架通过粒度感知对齐有效解决了零样本视频时刻检索中的语义粒度不匹配问题，无需额外训练即可显著提升检索性能。
...
TL;DR: SafeMo是一个可信赖的运动生成框架，通过最小化运动遗忘策略在连续空间中实现安全的人体运动生成，避免了离散码本替换方法的缺陷，并提供了首个安全文本到运动数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到运动生成方法存在安全隐患，离散码本替换方法会导致良性任务性能下降并产生伪影，且现有数据集包含不安全内容，不适合安全驱动的机器学习。

Method: 提出SafeMo框架，集成最小化运动遗忘策略，这是一种两阶段的机器学习遗忘方法，在连续空间中实现安全人体运动生成，避免了码本损失，并创建了SafeMoVAE-29K安全数据集。

Result: 实验显示SafeMo在HumanML3D和Motion-X数据集上分别达到2.5倍和14.4倍更高的遗忘集FID，相比之前的SOTA方法LCR，在安全提示上的良性性能相当或更好。

Conclusion: SafeMo框架通过连续空间中的最小化运动遗忘策略，有效解决了文本到运动生成的安全问题，在保持良性性能的同时实现了更强的安全-效用权衡。
...
TL;DR: 本文提出了一种模态主导指数（MDI）来量化RGB-IR多模态感知中的优化偏差，并开发了模态主导感知跨模态学习（MDACL）框架，通过分层跨模态引导和对抗均衡正则化来平衡优化动态，在三个基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: RGB-红外多模态感知对于复杂物理环境中的嵌入式多媒体系统至关重要。尽管现有的跨模态融合方法有所进展，但由于模态特征不对称导致的优化动态问题仍未得到充分研究。实践中，信息密度和特征质量的差异会引入持续的优化偏差，导致训练过度强调主导模态，阻碍有效的融合。

Method: 提出了模态主导指数（MDI），通过联合建模特征熵和梯度贡献来量化模态主导程度。基于MDI，开发了模态主导感知跨模态学习（MDACL）框架，包含：1）分层跨模态引导（HCG）增强特征对齐；2）对抗均衡正则化（AER）平衡融合过程中的优化动态。

Result: 在三个RGB-IR基准测试上进行了广泛实验，结果表明MDACL能有效缓解优化偏差，并实现了最先进的性能。

Conclusion: 该研究通过量化模态主导现象并提出相应的平衡优化框架，有效解决了RGB-IR多模态感知中的优化偏差问题，为复杂环境中的嵌入式多媒体系统提供了更可靠的感知解决方案。
...
TL;DR: TOLF框架通过归一化流建模预测误差分布和不确定性引导优化，解决小目标检测中标注噪声导致的过拟合问题


<details>
  <summary>Details</summary>
Motivation: 小目标检测性能显著低于正常尺度目标，且对标注噪声高度敏感，优化严格定位目标容易导致噪声过拟合

Method: 提出TOLF框架：1）使用归一化流进行灵活误差建模，捕捉复杂非高斯预测分布；2）不确定性感知梯度调制机制，抑制高不确定性噪声样本的学习

Result: 在三个数据集上的实验验证了方法的有效性，特别是在AI-TOD数据集上将DINO基线提升了1.2% AP

Conclusion: TOLF通过流基误差建模和不确定性引导优化，有效解决了小目标检测中的噪声鲁棒性问题，提升了定位性能
...
Result: 实现了康复训练中的实时3D人体姿态估计与运动分析，能够实时监测和评估患者动作，提供即时反馈和指导，跟踪速度快（<1ms/帧），姿态估计误差小，运动状态恢复真实平滑。

Conclusion: RePose方法能够有效支持康复训练，通过实时姿态估计和运动分析帮助患者正确执行康复动作，加速康复进程，具有临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 室管膜瘤术前预后对治疗规划至关重要，但MRI缺乏术后手术报告中的语义信息。现有多模态方法在推理时无法利用这些特权文本数据，需要解决术前数据与术后知识之间的鸿沟。

Method: 提出HyperPriv-EPN框架，采用Severed Graph策略：共享编码器处理教师图（含术后特权信息）和学生图（仅术前数据）。通过双流蒸馏，学生图学习从视觉特征中"幻觉"语义社区结构。

Result: 在311名患者的多中心队列验证中，HyperPriv-EPN实现了最先进的诊断准确性和生存分层性能，有效将专家知识转移到术前场景。

Conclusion: 该框架解锁了历史术后数据的价值，可在无需推理时文本的情况下指导新患者诊断，实现了从术后特权信息到术前预测的有效知识迁移。
...
TL;DR: 基于图像的深度学习为马铃薯储存期间的质量监测提供了非侵入式、可扩展的解决方案，通过预训练模型实现了发芽检测、重量损失估计和货架期预测，其中DenseNet在发芽检测中达到98.03%准确率。
...
Method: 在200天控制温湿度条件下收集图像和重量数据，利用ResNet、VGG、DenseNet和Vision Transformer等预训练架构，设计了两个专门模型：高精度二分类发芽检测器和多类别重量损失/货架期预测器。

Result: DenseNet在发芽检测中表现最佳，准确率达98.03%；货架期预测在粗粒度分类（2-5类）时准确率超过89.83%，细粒度分类（6-8类）时准确率下降；证明了图像模型集成到自动化系统的可行性。

Conclusion: 基于图像的深度学习为马铃薯质量评估提供了经济有效、非破坏性的方法，支持储存和分销的效率与可持续性；未来应开发适应不同品种和储存条件的通用模型，增强适应性和可扩展性。
...
Conclusion: CRoPS通过更全面地建模幻觉来源，有效缓解了LVLM的幻觉问题，提高了模型在实际应用中的可靠性。
...
TL;DR: 提出一种新颖的单图像到视频生成框架，通过构建3D高斯场景表示并在单次前向传播中采样合理的物体运动，实现快速、相机引导的视频生成，无需迭代去噪过程。


<details>
  <summary>Details</summary>
Motivation: 现有单图像条件视频生成方法虽然改进了时间一致性和3D一致性，但缺乏鲁棒的用户可控性（如修改相机路径），且大多数相机控制的图像到视频模型在准确建模相机运动、保持时间一致性和保持几何完整性方面存在困难。

Method: 提出一个新颖框架，从单张图像构建3D高斯场景表示，并在单次前向传播中采样合理的物体运动。这种方法避免了传统两步法（先渲染场景再添加物体运动）的问题，实现了快速、相机引导的视频生成，无需迭代去噪过程。

Result: 在KITTI、Waymo、RealEstate10K和DL3DV-10K数据集上的广泛实验表明，该方法在视频质量和推理效率方面达到了最先进的水平。

Conclusion: 该方法通过构建3D高斯场景表示并在单次前向传播中采样物体运动，实现了快速、相机引导的视频生成，在保持时间一致性和几何完整性的同时提供了精确的相机控制能力。
...
TL;DR: 该研究针对医学视觉语言模型在部署后可能因数据分布变化导致性能下降的问题，提出结合输入数据偏移检测和输出置信度指标的双重监控框架，以提高模型在数字病理学中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学图像分析和疾病诊断中表现出强大潜力，但部署后当输入数据分布发生变化时，模型性能可能下降。检测这种性能退化对于临床可靠性至关重要，但对于大型预训练VLM来说，在没有标注数据的情况下进行检测仍然具有挑战性。

Method: 1. 开发DomainSAT工具箱，集成代表性偏移检测算法，提供图形界面用于系统分析输入数据偏移；2. 研究输出级预测行为监控，引入无标签、基于置信度的退化指标；3. 在肿瘤分类的大规模病理数据集上进行实验，结合输入数据偏移检测和输出置信度指标。

Result: 分析表明：输入数据偏移检测能有效识别分布变化并提供早期诊断信号，但并不总是与实际性能退化相对应；基于置信度的退化指标与性能退化密切相关，可作为输入偏移检测的有效补充；结合两种方法能够更可靠地检测和解释数据偏移下VLM的性能退化。

Conclusion: 该研究为数字病理学中基础模型的可靠性监控提供了一个实用且互补的框架，通过结合输入数据偏移检测和输出置信度指标，能够更有效地检测和解释视觉语言模型在数据分布变化下的性能退化问题。
...
<details>
  <summary>Details</summary>
Motivation: 手写STEM考试能捕捉开放式推理和图表，但人工批改速度慢且难以扩展。需要一种能保持标准考试流程（A4纸、无约束手写）的自动批改解决方案。

Method: 端到端工作流：1) 讲师提供手写参考答案和评分规则；2) 参考答案转换为纯文本摘要作为评分条件；3) 多阶段设计：格式/存在性检查、独立评分器集成、监督器聚合、刚性模板验证；4) 使用GPT-5.2和Gemini-3 Pro作为后端模型。

Result: 在斯洛文尼亚语真实课程测验（含手绘电路图）中，完整流程与讲师评分相比平均绝对差异约8分，偏差低，手动审查触发率约17%（D_max=40）。消融实验显示简单提示和移除参考答案会显著降低准确性并导致系统性评分过高。

Conclusion: 结构化提示和参考答案基础是确保自动评分准确性的关键，提出的多阶段工作流能有效批改手写STEM考试，为教育评估提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测方法在跨数据集泛化方面存在局限，本文旨在探索自监督学习作为辅助任务如何优化主要检测任务，提升模型的泛化能力。

Method: 研究了自监督学习与主要任务的不同训练方案组合，提出融合自监督辅助任务的特征表示方法，利用自监督任务和主要任务的独特表示来增强检测能力。

Result: 在DF40、FaceForensics++、Celeb-DF、DFD、FaceShifter、UADFV等多个数据集上实验，结果显示该方法在跨数据集评估中相比当前最先进的检测器具有更好的泛化性能。

Conclusion: 自监督学习作为辅助任务能够有效优化深度伪造检测，融合自监督任务的特征表示是一种强大的特征表示方法，能够提升主要任务的性能并增强跨数据集泛化能力。
...
TL;DR: 本文提出了LNU-Net和IBU-Net两种深度学习架构用于心脏MRI图像的左心室分割，通过改进归一化方法提升分割性能。
...
Method: 提出了两种基于U-Net的改进架构：LNU-Net在卷积块中使用层归一化，IBU-Net在第一个卷积块中结合实例和批量归一化。使用仿射变换和弹性变形进行数据增强，在包含45名患者805张MRI图像的数据集上进行评估。

Result: 实验结果表明，提出的LNU-Net和IBU-Net方法在Dice系数和平均垂直距离指标上优于原始U-Net和其他最先进方法。

Conclusion: 通过改进归一化策略的深度学习架构能够有效提升左心室分割的准确性和鲁棒性，为临床诊断提供更可靠的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态3D场景重建方法存在两个主要问题：1）使用单高斯基元具有低通滤波特性，无法捕捉高频细节，而标准Gabor函数存在能量不稳定问题；2）缺乏时序连续性约束，导致插值时出现运动伪影。
...
Conclusion: AdaGaR通过统一解决频率自适应性和时序连续性，显著提升了动态3D场景重建的质量，在细节捕捉和运动平滑性方面优于现有方法，具有广泛的应用潜力。
...
TL;DR: 提出基于自动机的强化学习框架，在MDP和POMDP中合成满足MITL时序逻辑规范的政策，通过时间限确定广义Büchi自动机转换和产品模型构建，在随机动态和部分可观测环境下实现时间约束任务规划。


<details>
  <summary>Details</summary>
Motivation: 动态不确定环境中的机器人系统需要满足复杂任务序列和严格时间约束的规划器。MITL提供了表达时间约束需求的正式框架，但将其与强化学习结合仍面临随机动态和部分可观测性的挑战。

Method: 将MITL公式转换为时间限确定广义Büchi自动机(Timed-LDGBA)，与底层决策过程同步构建产品时间模型，采用简单而表达力强的奖励结构，通过Q-learning学习满足时间约束的政策。

Result: 在三个仿真研究中验证：5×5网格世界MDP、10×10网格世界POMDP和办公室服务机器人场景。结果表明框架能一致学习满足严格时间约束的政策，适应随机转移，扩展到更大状态空间，在部分可观测环境中保持有效。

Conclusion: 提出的统一框架能够在随机动态和部分可观测环境下合成满足MITL时序逻辑规范的政策，展示了在时间关键和不确定环境中可靠机器人规划的潜力。
...
TL;DR: CDGS通过将搜索嵌入扩散去噪过程，解决组合生成模型中的模式平均问题，实现多模态局部分布下的全局一致规划


<details>
  <summary>Details</summary>
Motivation: 组合生成模型在处理长时程任务分布时面临模式平均问题，当局部分布为多模态时，现有组合方法会平均不兼容的模式，导致生成的规划既不可行也不一致

Method: 提出组合扩散引导搜索(CDGS)，将搜索直接嵌入扩散去噪过程，通过基于种群的采样探索局部模式的多样化组合，使用基于似然的过滤修剪不可行候选，并通过重叠段之间的迭代重采样强制执行全局一致性

Result: 在7个机器人操作任务上匹配了oracle性能，优于缺乏组合性或需要长时程训练数据的基线方法；方法可跨领域泛化，通过有效的局部到全局消息传递实现连贯的文本引导全景图像和长视频生成

Conclusion: CDGS通过将搜索与扩散去噪相结合，有效解决了组合生成模型中的模式平均问题，实现了局部可行且全局一致的规划，展示了在机器人操作、图像合成和视频生成等多个领域的通用性
...
TL;DR: SLAP系统为无人机在垂直树干上实现轻柔停栖和失败恢复，通过视觉检测、IMU故障检测、姿态控制、光学近距离检测和弹性抓取器，在1.2kg四旋翼上实现75%停栖成功率和100%失败恢复率。


<details>
  <summary>Details</summary>
Motivation: 无人机在垂直表面停栖可降低能耗、进行表面采样或稳定观测，但现有方案多为轻量化机械设计，系统集成度低，且需要高速危险的着陆操作，不适合携带敏感电子设备的大型调查无人机。

Method: 开发SLAP系统，包含：视觉停栖点检测器、IMU停栖故障检测器、轻柔停栖姿态控制器、光学近距离检测系统、以及由商用拍手带制成的带微刺快速主动弹性抓取器。在1.2kg商用四旋翼上验证。

Result: 室内自主飞行实验在真实橡树段上进行20次飞行，达到75%停栖成功率；在2次诱导故障飞行中实现100%停栖故障恢复率。

Conclusion: SLAP系统为大型无人机提供了在垂直树干上轻柔停栖和故障恢复的可行方案，通过系统级集成解决了传统方案中高速危险着陆的问题。
...
TL;DR: 本文提出了一种分层优化方法，用于自动化车辆喷漆过程中多机械臂的路径规划，解决了传统手工设计耗时且难以直接应用现有机器人路径规划技术的问题。


<details>
  <summary>Details</summary>
Motivation: 车辆生产工厂中，喷漆过程需要多个机械臂同时对传送线上的车身进行喷漆。目前喷漆路径设计（包括为机械臂分配车身区域和确定每个机械臂的喷漆顺序）仍然是工程师耗时的手动任务，需要自动化和减少设计时间。喷漆过程的独特约束阻碍了传统机器人路径规划技术（如焊接中使用的技术）的直接应用。

Method: 将喷漆路径设计表述为分层优化问题：上层子问题类似于车辆路径问题（VRP），下层子问题涉及详细的路径规划。这种方法允许在不同层次使用不同的优化算法，并通过设计变量表示、约束、修复算子和初始化过程，灵活处理车辆喷漆过程的特定约束。

Result: 通过对三种商用车辆模型的实验证明，所提出的方法能够自动设计出满足所有车辆喷漆约束的路径，其质量与工程师手动创建的路径相当。

Conclusion: 分层优化方法成功解决了车辆喷漆路径规划的自动化问题，能够生成满足所有工艺约束的高质量路径，显著减少了设计时间，为车辆生产工厂的喷漆过程自动化提供了有效解决方案。
...
TL;DR: WiCHINS：一种结合轮载和底盘惯性传感器的轮式底盘惯性导航系统，用于在GNSS受限或光照条件差的环境下实现精确的纯惯性导航。


<details>
  <summary>Details</summary>
Motivation: 在GNSS信号受限或光照条件恶劣的实际场景中，自主车辆和轮式机器人的导航解决方案可能只能依赖惯性传感器，但惯性测量误差会导致时间漂移问题。

Method: 提出WiCHINS系统，将轮载惯性传感器与底盘惯性传感器结合，采用三阶段框架，每个阶段配备专门的扩展卡尔曼滤波器，充分利用轮子和车身不同位置的测量优势。

Result: 使用5个惯性测量单元，总记录时间228.6分钟的数据集进行测试，与4个惯性基线方法比较，平均位置误差为11.4米，占平均行驶距离的2.4%。
...
TL;DR: 提出可更换夹爪系统，通过专用附件处理杂乱食品的重量分拣，针对鲑鱼子和意大利面设计专用夹头，实现80%和95%以上的重量分拣精度


<details>
  <summary>Details</summary>
Motivation: 食品包装行业面临快速变化的食品种类和重量处理挑战，需要处理从单件到柔性、长条状、杂乱堆积的各种食品，特别是重量分拣的难题

Method: 设计可更换夹爪系统，配备专用食品附件（夹头）增强抓取能力，采用皮带更换系统实现不同食品间的快速切换，针对鲑鱼子（粘性颗粒食品）和意大利面（长条状粘性杂乱食品）设计专用夹头

Result: 夹爪成功抓取鲑鱼子和意大利面，重量分拣精度分别超过95%和80%，系统展示不同夹头间的快速切换能力，可处理多种食品

Conclusion: 可更换夹爪系统能有效处理杂乱食品的重量分拣问题，通过专用附件设计实现高精度重量控制，为食品包装自动化提供灵活解决方案
...
### [56] [LLM-Based Agentic Exploration for Robot Navigation & Manipulation with Skill Orchestration](https://arxiv.org/abs/2601.00555)
*Abu Hanif Muhammad Syarubany,Farhan Zaki Rahmani,Trio Widianto*

Main category: cs.RO

TL;DR: 本文提出了一种基于LLM的端到端智能探索系统，用于室内购物任务，在Gazebo仿真和真实走廊环境中进行了评估。系统通过检测路口标志牌增量构建轻量级语义地图，利用AprilTags作为可重复的锚点，通过LLM生成决策，ROS有限状态机控制器执行模块化运动原语。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在复杂室内环境中执行购物任务的智能机器人系统，需要解决语义理解、环境探索、导航决策和任务执行等多个挑战。传统方法通常将这些功能分离，而本文旨在构建一个端到端的集成系统，利用LLM的高级语义理解能力，结合模块化的运动控制，实现从用户指令到多店铺导航和物品抓取的全流程自动化。

Method: 系统采用增量式轻量级语义地图构建方法，在路口检测标志牌并存储方向到兴趣点的关系以及估计的路口姿态。AprilTags作为可重复的锚点用于接近和对齐。对于自然语言购物请求，LLM在每个路口生成受限的离散动作（方向和是否进入商店），ROS有限状态主控制器通过门控模块化运动原语来执行决策，包括基于局部代价地图的障碍物避让、AprilTag接近、商店进入和抓取。

Result: 定性结果表明，集成系统能够从用户指令到多店铺导航和物体检索执行端到端任务。系统保持了模块化和可调试性，通过其基于文本的地图和记录的决策历史，便于系统分析和故障排除。

Conclusion: 本文成功展示了一个基于LLM的端到端智能探索系统，能够在室内购物场景中有效工作。系统结合了高级语义理解与模块化运动控制，在保持可调试性的同时实现了完整的任务执行流程。这种方法为复杂环境中的机器人任务执行提供了一种有前景的解决方案。
...
### [57] [Priority-Aware Multi-Robot Coverage Path Planning](https://arxiv.org/abs/2601.00580)
...
<details>
  <summary>Details</summary>
Motivation: 传统多机器人覆盖路径规划（MCPP）方法假设所有区域具有相同重要性，但在实际应用中某些区域需要优先覆盖，现有方法无法有效处理这种优先级需求

Method: 提出可扩展的两阶段框架：1）贪心区域分配结合局部搜索和基于生成树的路径规划；2）斯坦纳树引导的剩余覆盖

Result: 实验表明该方法相比标准MCPP基线显著降低了优先级加权延迟，同时保持了有竞争力的总完成时间；敏感性分析显示方法能良好扩展到多机器人场景，且通过调整优先级权重可有效控制区域覆盖行为

Conclusion: PA-MCPP框架成功解决了优先级感知的覆盖路径规划问题，在保证整体效率的同时优先覆盖重要区域，为实际应用提供了更灵活的解决方案
...
### [58] [Vision-based Goal-Reaching Control for Mobile Robots Using a Hierarchical Learning Framework](https://arxiv.org/abs/2601.00610)
*Mehdi Heydari Shahna,Pauli Mustalahti,Jouni Mattila*

Main category: cs.RO

TL;DR: 本文提出了一种用于大型机器人的安全目标到达控制框架，通过模块化分解系统，结合强化学习规划、深度学习建模和鲁棒自适应控制，确保在不稳定地形上的安全操作。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人应用中虽然有效，但需要大量探索状态-动作空间，期间行为可能不安全，这限制了其在具有复杂执行机构的大型机器人上的应用，特别是在不稳定地形上。

Method: 将系统分解为五个紧密耦合的功能模块：1) 实时视觉姿态估计提供准确机器人状态；2) RL运动规划器生成平滑运动指令；3) 监督深度学习模型捕获机器人复杂动力学；4) 基于模型的鲁棒自适应控制器确保轮子跟踪RL指令；5) 数学安全监督器监控机器人安全。

Result: 该框架保证了执行系统的均匀指数稳定性和整个操作的安全性。在6000公斤机器人上的不同场景实验证实了该框架的有效性。

Conclusion: 提出的模块化控制框架成功解决了大型机器人在不稳定地形上的安全目标到达问题，通过结合RL规划、深度学习建模和鲁棒控制，实现了安全可靠的自主操作。
...
### [59] [From 2D to 3D terrain-following area coverage path planning](https://arxiv.org/abs/2601.00614)
*Mogens Plessen*

Main category: cs.RO

TL;DR: 提出了一种三维地形跟随区域覆盖路径规划算法，能够生成多条相邻路径，这些路径在水平方向上保持与机械工作宽度相等的间距，同时在垂直方向上保持特定工作高度跟随地形起伏。


<details>
  <summary>Details</summary>
Motivation: 在农业等实际应用中，需要机械在复杂三维地形上进行高效的区域覆盖作业。传统的二维路径规划方法无法适应地形起伏，而现有的三维路径规划方法在保持路径间距和跟随地形方面存在挑战。

Method: 采用逆距离加权方法生成均匀间距的高程数据，结合局部搜索算法，生成多条相邻路径。这些路径在水平方向保持机械工作宽度的间距，在垂直方向保持特定工作高度跟随地形起伏。

Result: 算法在实际农业三维数据上进行了验证，成功生成了适应复杂地形的覆盖路径。与二维等效算法相比，该算法能够更好地处理地形起伏带来的复杂性。

Conclusion: 提出的三维地形跟随区域覆盖路径规划算法能够有效解决复杂地形下的机械作业路径规划问题，为农业等领域的自动化作业提供了实用的解决方案。
...
### [60] [RoboReward: General-Purpose Vision-Language Reward Models for Robotics](https://arxiv.org/abs/2601.00675)
...
Method: 1) 构建RoboReward数据集和基准，基于Open X-Embodiment和RoboArena的真实机器人数据；2) 提出负样本数据增强管道，通过反事实重标注和时间裁剪生成校准的负样本和接近成功样本；3) 训练4B和8B参数的视觉语言奖励模型。

Result: 1) 评估发现现有视觉语言模型在所有任务中表现不均，有改进空间；2) 训练的4B/8B参数模型在短视界机器人任务中优于更大的视觉语言模型；3) 8B参数模型在真实机器人强化学习中大幅优于Gemini Robotics-ER 1.5，接近人类提供奖励的效果。

Conclusion: 视觉语言模型可以作为有效的机器人奖励模型，通过系统化的数据集构建和模型训练，能够显著提升强化学习策略学习效果，缩小与人类提供奖励的差距。
...
### [61] [DefVINS: Visual-Inertial Odometry for Deformable Scenes](https://arxiv.org/abs/2601.00702)
*Samuel Cerezo,Javier Civera*

Main category: cs.RO

TL;DR: DefVINS是一个视觉惯性里程计框架，通过显式分离刚性IMU锚定状态和非刚性变形图，在可变形场景中实现鲁棒姿态估计。
...
Method: 提出DefVINS框架，将刚性IMU锚定状态与非刚性变形图分离表示。系统先通过标准VIO初始化固定重力、速度和IMU偏差，然后根据可观测性分析逐步激活非刚性自由度。采用IMU锚定和基于条件的激活策略防止不良激励下的病态更新。

Result: 消融研究表明，结合惯性约束和可观测性感知的变形激活策略，在非刚性环境中提高了系统的鲁棒性。

Conclusion: DefVINS通过显式分离刚性和非刚性分量，结合惯性约束和可观测性分析，有效解决了可变形场景中的视觉惯性里程计问题，提高了在非刚性环境中的鲁棒性。
...
### [62] [Calling for Backup: How Children Navigate Successive Robot Communication Failures](https://arxiv.org/abs/2601.00754)
*Maria Teresa Parreira,Isabel Neto,Filipa Rocha,Wendy Ju*

Main category: cs.RO

TL;DR: 研究探索儿童对机器人重复错误的反应，发现儿童与成人既有相似调整行为，也有更多脱离互动行为，但错误不影响对机器人的感知。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注成人对机器人连续错误的反应，但儿童对此的反应尚未充分探索。本研究旨在填补这一空白，探究儿童对机器人社交错误和性能错误的反应，特别是重复对话错误。

Method: 采用Liu等人的连续机器人失败范式，招募59名8-10岁儿童参与实验。儿童与机器人互动，机器人连续三次未能理解他们的提示。研究通过视频记录和分析参与者的行为反应。

Result: 儿童反应与成人既有相似也有差异：相似之处包括调整提示、改变语气、情绪化非语言反应增加；差异在于儿童表现出更多脱离行为（暂时忽略机器人或寻求成人帮助）。错误不影响儿童对机器人的感知，表明儿童对对话期望更灵活。

Conclusion: 儿童对机器人重复错误的反应模式与成人不同，表现出更强的脱离倾向但更灵活的期望。这些发现有助于为年轻用户设计更有效、更适合发展阶段的机器人交互系统。
