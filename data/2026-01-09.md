...
TL;DR: 该论文提出了一个两阶段对齐框架，通过构建层次化细粒度评估标准和复杂偏好优化（CPO）方法，解决了扩散模型与复杂人类专业知识对齐的问题。
...
Method: 1. 与领域专家合作构建层次化细粒度评估标准，将图像质量分解为多个正负属性并组织成树状结构；2. 提出两阶段对齐框架：首先通过监督微调将领域知识注入辅助扩散模型，然后引入复杂偏好优化（CPO），将DPO扩展到非二元层次化标准对齐。
...
TL;DR: 提出了一种基于RGB空间五进制像素强度组合的文本嵌入图像新方法，相比传统LSB、变换域和深度学习等方法，能在单个像素中编码完整文本符号，计算效率更高且图像失真小。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入图像方法主要依赖LSB、MSB、像素值差分、变换域、量化、边缘区域检测以及深度学习和生成式AI等技术，这些方法大多需要多个像素进行强度翻转或变换系数操作，计算量大且容易产生噪声。现有方法的编码解码通常是确定性的，深度学习等方法计算开销大。

Method: 在RGB空间中采用五进制像素强度组合方法，每个R、G、B通道使用五种受控的不同像素强度变化，形成125种不同的像素强度组合。这些组合被映射到文本符号（大小写字母、数字、空格和常用特殊字符），实现单个RGB像素编码完整文本符号。

Result: 通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热图分析等多种指标评估，原始图像和编码图像之间没有显著失真。该方法相比LSB、MSB等需要多个像素或多步处理的方法，以及变换域和学习方法的高计算开销，实现了更高的嵌入效率。

Conclusion: 提出的五进制像素强度组合方法为文本嵌入图像提供了一种高效且低失真的解决方案，能够在单个像素中编码完整文本符号，相比现有方法具有更好的嵌入效率和计算性能。
...
TL;DR: 该研究探索通过后训练实现完全统一的文本-图像生成，使模型能在单一推理过程中自主从文本推理过渡到视觉合成，提升跨模态耦合效果。
...
Result: 通过针对特定限制的有针对性数据集，相比广泛的图像-标题语料库或基准对齐数据，取得了更优的结果。在四个不同的T2I基准测试中，多模态图像生成均得到改进。

Conclusion: 研究表明，对两种模态进行奖励加权以及策略性设计的后训练数据是有效的，能够实现完全统一的文本-图像生成，提升跨模态耦合和自动化生成能力。
...
TL;DR: ReHyAt提出了一种循环混合注意力机制，将softmax注意力的保真度与线性注意力的效率结合，实现分块循环重构和恒定内存使用，将注意力成本从二次降低到线性，同时保持视频生成质量。
...
Method: 提出ReHyAt循环混合注意力机制，结合softmax注意力的保真度和线性注意力的效率，支持分块循环重构和恒定内存使用。采用轻量级蒸馏和微调流程，可从现有的基于softmax的模型中高效蒸馏，大幅降低训练成本。

Result: 在VBench和VBench-2.0上的实验以及人类偏好研究表明，ReHyAt实现了最先进的视频质量，同时将注意力成本从二次降低到线性。训练成本降低两个数量级至约160 GPU小时，为长时长和端侧视频生成提供了实际可扩展性。

Conclusion: ReHyAt通过混合注意力设计在保持视频生成质量的同时显著提高了效率，为未来基于双向softmax的最先进模型提供了可应用的轻量级蒸馏和微调方案，解锁了长视频生成的实际可扩展性。
...
TL;DR: 提出了一种基于残差向量量化的渐进式3D高斯泼溅压缩方法，使用自回归熵模型和多分辨率哈希网格来提高压缩效率


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅模型在大型和中型场景中存储需求大，阻碍了云端和流媒体服务的部署。现有的渐进压缩技术主要使用标量量化，可能无法最优地捕捉高维特征向量的相关性，限制了率失真性能

Method: 提出了一种新颖的渐进式编解码器，用更强大的残差向量量化方法替代传统方法来压缩原始特征。核心贡献是自回归熵模型，由多分辨率哈希网格引导，准确预测每个连续传输索引的条件概率

Result: 该方法允许粗糙层和细化层以高效率压缩，提高了3D高斯泼溅模型的压缩性能

Conclusion: 通过残差向量量化和自回归熵模型，实现了对3D高斯泼溅模型更高效的渐进式压缩，解决了现有标量量化方法的局限性
...
TL;DR: 该研究比较了自定义CNN与预训练模型（ResNet-18和VGG-16）在孟加拉国五个图像分类数据集上的表现，发现微调迁移学习效果最佳，准确率提升3%-76%，ResNet-18在道路损伤数据集上达到100%准确率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估不同深度学习策略在孟加拉国特定图像分类任务中的效果，为实践者提供基于数据集特性、计算资源和性能要求的选择指导。

Method: 研究方法包括：1）构建自定义CNN；2）使用ResNet-18和VGG-16进行特征提取；3）使用相同预训练模型进行迁移学习微调；4）在五个孟加拉国图像数据集（人行道视觉、三轮车检测、芒果分类、水稻品种识别、道路损伤检测）上进行评估比较。

Result: 实验结果显示：1）微调迁移学习始终优于自定义CNN和特征提取方法；2）准确率提升幅度为3%-76%；3）ResNet-18微调在道路损伤数据集上达到100%准确率；4）自定义CNN在模型大小（340万参数）和简单任务训练效率上有优势；5）预训练模型在数据有限的复杂任务中表现更优。

Conclusion: 结论是迁移学习微调在大多数情况下性能最优，特别是对复杂分类任务。自定义CNN适合计算资源有限或简单任务。研究为实践者提供了基于具体需求选择深度学习方法的实用指导。
...
TL;DR: PackCache：一种训练无关的KV缓存管理方法，通过动态压缩KV缓存来加速统一自回归视频生成，利用文本和图像令牌的语义锚定特性以及跨帧注意力衰减规律，实现1.7-2.2倍加速。


<details>
  <summary>Details</summary>
Motivation: 统一自回归模型虽然能处理多模态任务，但其KV缓存机制存在瓶颈：缓存大小随生成令牌数量线性增长，限制了推理效率和生成长度。视频生成任务尤其受此限制，需要更高效的缓存管理方案。

Method: PackCache包含三个协调机制：1) 条件锚定：保留文本和条件图像等语义参考令牌；2) 跨帧衰减建模：根据时间距离分配缓存预算；3) 空间保持位置嵌入：在缓存移除时保持连贯的3D结构。该方法无需额外训练。

Result: 在48帧长序列上，PackCache将端到端生成加速1.7-2.2倍。对于受KV缓存扩展影响最大的最后四帧（视频中最昂贵的部分），在A40和H200上分别实现2.6倍和3.7倍加速。

Conclusion: PackCache通过利用KV缓存令牌的时空特性，有效解决了统一自回归视频生成中的缓存瓶颈问题，显著提升推理效率，为更长序列的视频生成提供了可行方案。
...
TL;DR: 该研究提出了一种基于3D面部几何和生理信号的多模态压力识别方法，通过Transformer框架和跨模态注意力机制，在分心驾驶场景中实现了高精度的压力检测。


<details>
  <summary>Details</summary>
Motivation: 当前基于面部动作单元的压力识别方法存在局限性，因为压力具有主观性且面部表情可被自主控制。3D面部几何在压力识别中的作用尚未得到充分探索，特别是在分心驾驶等实际场景中。

Method: 研究使用EMOCA提取的3D表情和姿态系数分析分心驾驶中的压力反应。通过配对假设检验识别压力响应特征，并提出了基于Transformer的时间建模框架，评估了单模态、早期融合和跨模态注意力三种融合策略。

Result: 56个EMOCA系数中有41个在压力和基线阶段表现出显著差异。跨模态注意力融合EMOCA和生理信号的方法取得了最佳性能（AUROC 92%，准确率86.7%），EMOCA与注视融合也表现良好（AUROC 91.8%）。

Conclusion: 3D面部几何特征能有效反映压力状态，时间建模和跨模态注意力机制显著提升了压力识别性能，为可靠的压力检测提供了新方法。
...
TL;DR: 研究探索了如何利用少量配对数据，通过LoRA微调RGB预训练的基础模型，实现跨光谱图像转换（RGB到红外/合成孔径雷达），并将生成的合成数据用于提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型主要基于RGB数据训练，但许多安全关键应用依赖红外和合成孔径雷达等非可见光模态。研究旨在探索是否能用少量配对数据将RGB预训练模型重新用于跨光谱翻译，并验证合成数据是否能增强下游检测任务。

Method: 从FLUX.1 Kontext模型出发，插入低秩适应模块，仅使用每个领域100张配对图像进行微调。研究两个设置：KAIST数据集上的RGB到红外转换，以及M4-SAR数据集上的RGB到SAR转换。通过LPIPS指标评估翻译质量，并验证其与下游检测性能的相关性。

Result: LPIPS指标能有效预测下游性能：较低的LPIPS值一致对应较高的mAP。使用最佳LPIPS选择的LoRA适配器，从外部RGB数据集生成的合成红外图像能提升KAIST红外行人检测性能，合成SAR数据与有限真实SAR数据结合能显著提升M4-SAR基础设施检测。

Conclusion: 少量样本的LoRA适配流匹配基础模型是实现非可见光模态基础模型支持的有前景路径，能够有效利用RGB预训练知识进行跨光谱翻译并提升下游检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决实际应用中一个重要问题：在图像分类任务中，是应该从头设计自定义CNN，还是使用已有的预训练架构。需要明确不同方法在性能、复杂度和计算效率方面的权衡。

Method: 研究方法包括：1）开发并从头训练一个自定义CNN；2）使用迁移学习方法应用VGG-16、ResNet-50和MobileNet等流行架构；3）在相同实验设置下评估所有模型；4）使用准确率、精确率、召回率和F1分数等标准性能指标进行比较。

Result: 实验结果显示：1）预训练CNN架构在分类准确率和收敛速度方面始终优于自定义CNN，特别是在训练数据有限的情况下；2）自定义CNN在参数数量显著减少和计算复杂度降低的情况下仍表现出竞争力。

Conclusion: 研究结论强调了模型复杂性、性能和计算效率之间的权衡，为图像分类问题中选择合适的CNN架构提供了实用见解。预训练模型在性能上更优，而自定义模型在资源受限场景下更具优势。
...
TL;DR: Tri MARF是一个新颖的三模态框架，通过整合2D多视角图像、文本描述和3D点云，采用多智能体协作架构来提升大规模3D标注效果。
...
Conclusion: Tri MARF通过三模态输入和多智能体协作架构有效解决了3D标注的挑战，在大规模3D标注任务中表现出卓越性能。
...
<details>
  <summary>Details</summary>
Motivation: 人工耳蜗手术中乳突切除术是关键步骤，准确预测切除区域能改善术前规划、降低风险。但由于获取真实标注困难，现有深度学习研究有限，需要无标注或弱标注的解决方案。

Method: 提出混合自监督与弱监督学习框架，直接从术前完整乳突的CT扫描预测乳突切除术区域。结合自监督学习从无标注数据中学习特征，弱监督学习利用有限标注信息，并采用3D T分布损失函数。

Result: 在预测复杂且无明确边界的乳突切除术形状时，平均Dice分数达到0.72，超越了现有最优方法，表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在心脏MRI重建中表现出色，但现有方法泛化能力有限。心脏MRI扫描存在图像对比度、采样模式、扫描仪厂商、解剖结构和疾病类型等多种变化，现有模型只能处理单一或狭窄的变化子集，面对分布偏移时性能下降。因此需要开发能够泛化到多样化心脏MRI场景的统一模型。

Method: 提出CRUNet-MR-Univ基础模型，该模型利用时空相关性和提示先验来有效处理心脏MRI扫描的完整多样性。通过提示机制整合先验知识，增强模型对不同变化场景的适应能力。
...
TL;DR: GPRO提出了一种元推理控制器，通过动态路由计算路径来解决LVLM中的过度思考问题，特别关注视觉感知失败这一根本瓶颈，在提高准确性的同时显著减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型通过思维链机制展现强大推理能力，但慢思考方法常导致过度思考问题：即使简单查询也会产生冗长响应，导致测试效率低下甚至准确性下降。先前工作尝试通过自适应推理策略缓解此问题，但大多忽略了根本瓶颈：视觉感知失败。作者认为稳定推理关键依赖于低层次视觉基础，推理错误往往源于不完美的感知而非不足的深思熟虑。

Method: 提出Gated Perception-Reasoning Optimization (GPRO)，一种元推理控制器，在每个生成步骤动态路由计算到三条决策路径：1）轻量级快速路径；2）慢感知路径用于重新检查视觉输入；3）慢推理路径用于内部自我反思。从约79万样本中获取大规模失败归因监督，使用教师模型区分感知幻觉和推理错误。通过多目标强化学习训练控制器，在不确定性下优化任务准确性和计算成本之间的权衡。

Result: 在五个基准测试上的实验表明，GPRO显著提高了准确性和效率，优于最近的慢思考方法，同时生成显著更短的响应。

Conclusion: GPRO通过动态路由计算路径有效解决了LVLM中的过度思考问题，特别强调了视觉感知失败这一关键瓶颈。该方法在保持推理质量的同时显著提高了效率，为视觉语言模型的优化提供了新思路。
...
TL;DR: UniDrive-WM是一个统一的视觉语言模型世界模型，将自动驾驶中的场景理解、轨迹规划和未来图像生成整合到单一架构中，通过轨迹条件图像生成提供额外监督信号，在Bench2Drive基准上显著提升了规划性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统通常将感知、预测和规划作为独立模块处理，缺乏统一的整合。现有基于视觉语言模型的规划方法也存在类似问题。作者提出需要将驾驶场景理解、轨迹规划和轨迹条件未来图像生成统一到单一架构中，以提升自动驾驶系统的整体性能。

Method: 提出UniDrive-WM统一世界模型，包含：1）轨迹规划器预测未来轨迹；2）基于VLM的图像生成器根据轨迹条件生成合理的未来帧；3）这些预测提供额外的监督信号，增强场景理解并迭代优化轨迹生成；4）比较离散和连续输出表示对未来图像预测的影响。

Result: 在Bench2Drive基准测试中，UniDrive-WM能够生成高保真度的未来图像，规划性能相比之前最佳方法提升了5.9%的L2轨迹误差和9.2%的碰撞率，证明了紧密整合VLM驱动推理、规划和生成式世界建模的优势。

Conclusion: UniDrive-WM展示了将视觉语言模型驱动的推理、规划和生成式世界建模紧密整合的优越性，为自动驾驶系统提供了统一的端到端解决方案，显著提升了规划性能和安全性。
...
<details>
  <summary>Details</summary>
Motivation: 三维医学图像分割计算需求大，存在体素处理立方增长和同质区域冗余计算的问题，需要开发更高效的分割方法。

Method: 1) 多尺度分层编码器在四个分辨率级别提取400个候选令牌；2) 边界感知分词器结合VQ-VAE量化和重要性评分选择100个关键令牌；3) 稀疏到密集解码器通过令牌重投影、渐进上采样和跳跃连接重建完整分辨率掩码。

Result: 在960例3D乳腺DCE-MRI数据集上达到94.49% Dice和89.61% IoU的SOTA性能，GPU内存减少64%，推理延迟降低68%。在MSD心脏和脑MRI基准数据集上也表现出优异的泛化能力。

Conclusion: TokenSeg通过解剖学指导的稀疏表示实现了准确高效的3D医学图像分割，证明了边界感知稀疏令牌表示框架的有效性。
...
TL;DR: 使用3D条件生成模型（Pix2Pix GAN、SPADE-GAN、SPADE-LDM）从语义标签图合成LGE MRI图像，以增强左心房分割的训练数据，SPADE-LDM表现最佳，显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 左心房壁和心内膜的准确分割对于量化心房颤动患者的心房纤维化至关重要，但基于机器学习的分割模型开发面临数据稀缺和解剖结构复杂性的挑战。

Method: 开发了一个管道，从结合解剖专家标注和无监督组织聚类的复合语义标签图合成高保真3D LGE MRI体积，使用了三种3D条件生成器（Pix2Pix GAN、SPADE-GAN和SPADE-LDM）。评估合成图像的真实性及其对下游左心房分割的影响。

Result: SPADE-LDM生成最真实和结构准确的图像，FID为4.063，优于Pix2Pix GAN（40.821）和SPADE-GAN（7.652）。使用合成LGE图像增强后，3D U-Net模型的左心房腔分割Dice分数从0.908提升到0.936，具有统计学显著改善（p < 0.05）。

Conclusion: 标签条件的3D合成在增强代表性不足的心脏结构分割方面具有潜力，SPADE-LDM在生成真实LGE MRI图像方面表现最佳，能显著改善左心房分割性能。
...
TL;DR: 该论文提出了MiLDEAgent框架，用于处理多层设计文档（如海报）的自然语言编辑任务，通过结合RL训练的多模态推理器和图像编辑器实现层感知编辑，并建立了包含20K+设计文档的MiLDEBench基准测试。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的设计文档（如海报）本质上是多层的，包含装饰、文本和图像等元素。现有的编辑方法主要关注单层图像编辑或多层生成，忽略了多层设计文档编辑所需的细粒度、层感知推理能力，无法确定需要修改哪些层以及如何协调修改。

Method: 提出了MiLDEAgent框架，结合了RL训练的多模态推理器（用于层级理解）和图像编辑器（用于针对性修改）。同时建立了MiLDEBench基准测试（包含20K+设计文档和多样化编辑指令）和MiLDEEval评估协议（涵盖指令遵循、布局一致性、美学和文本渲染四个维度）。

Result: 在14个开源模型和2个闭源模型的广泛实验中，现有方法普遍无法泛化：开源模型通常无法完成多层文档编辑任务，而闭源模型存在格式违规问题。相比之下，MiLDEAgent实现了强大的层感知推理和精确编辑，显著优于所有开源基线，性能与闭源模型相当，为多层文档编辑建立了首个强基线。

Conclusion: MiLDEAgent框架成功解决了多层设计文档编辑中的层感知推理问题，通过结合多模态推理和针对性编辑，在新建的MiLDEBench基准测试上表现出色，为这一重要但被忽视的研究方向建立了首个有效的解决方案。
...
TL;DR: 本文讨论AI赋能人机交互系统的安全挑战，提出评估框架，并以糖尿病血糖控制为例展示检测技术


<details>
  <summary>Details</summary>
Motivation: AI赋能的人机交互系统在医疗监控、自动驾驶等场景中面临操作不确定性，可能导致安全要求被违反，需要系统化解决方案

Method: 首先讨论操作偏差问题，然后创建评估框架分析不同安全策略，最后以个性化图像技术检测糖尿病患者未报告进餐为例进行演示

Result: 提出了一个评估AI赋能人机交互系统安全策略的框架，并展示了针对1型糖尿病患者血糖控制中未报告进餐的检测技术

Conclusion: AI赋能人机交互系统在不确定条件下的安全挑战需要系统化评估框架，个性化检测技术能够有效应对特定场景的安全风险
...
Method: 提出高不确定性区域引导的多架构协同学习（HUR-MACL）模型：1）使用CNN自适应识别高不确定性区域；2）在高不确定性区域同时使用Vision Mamba和Deformable CNN联合提升分割精度；3）提出异质特征蒸馏损失促进两个架构在高不确定性区域的协同学习。
...
### [21] [Driving on Registers](https://arxiv.org/abs/2601.05083)
...
TL;DR: DrivoR是一个基于Transformer的端到端自动驾驶架构，使用预训练的Vision Transformers和相机感知寄存器令牌来压缩多摄像头特征，通过两个轻量级Transformer解码器生成并评分候选轨迹，在多个基准测试中表现优异。
...
Method: 基于预训练的Vision Transformers，引入相机感知寄存器令牌来压缩多摄像头特征，使用两个轻量级Transformer解码器：一个生成候选轨迹，另一个评分轨迹并预测可解释的子分数（安全性、舒适性、效率等）。

Result: DrivoR在NAVSIM-v1、NAVSIM-v2和HUGSIM基准测试中优于或匹配现有强基线，展示了纯Transformer架构结合令牌压缩能够实现准确、高效和自适应的端到端驾驶。

Conclusion: 纯Transformer架构结合有针对性的令牌压缩足以实现准确、高效和自适应的端到端自动驾驶，DrivoR的简单设计在多个基准测试中表现出色。
...
Conclusion: HyperAlign框架通过引入双曲几何建模和自适应机制，有效解决了现有文本-图像对齐评估方法的局限性，为准确评估生成图像与文本提示的对齐性提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于硬件和物理设置限制，收集大规模真实世界操作数据在不同环境中难以扩展。现有基于文本提示的图像扩散方法往往忽略多视角和时间一致性的实际需求，且文本提示无法可靠指定场景设置。
...
TL;DR: HATIR提出了一种基于热感知扩散的红外湍流视频超分辨率方法，通过注入热感知变形先验来联合建模湍流退化和结构细节损失的反过程，在首个湍流红外VSR数据集FLIR-IVSR上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 红外视频在恶劣环境下具有重要应用价值，但常受严重大气湍流和压缩退化影响。现有VSR方法要么忽略红外与可见光图像间的模态差异，要么无法恢复湍流引起的失真。将湍流缓解算法与VSR方法级联会导致误差传播和累积，因为湍流和分辨率退化是解耦建模的。

Method: HATIR通过将热感知变形先验注入扩散采样路径，联合建模湍流退化和结构细节损失的反过程。具体包括：1）基于热活跃区域随时间呈现一致相量响应的物理原理，构建相量引导流估计器，生成可靠的湍流感知流来指导反向扩散过程；2）提出湍流感知解码器，通过湍流门控和结构感知注意力，选择性抑制不稳定时间线索并增强边缘感知特征聚合。

Result: 构建了首个湍流红外视频超分辨率数据集FLIR-IVSR，包含来自FLIR T1050sc相机（1024×768分辨率）的640个不同场景的成对LR-HR序列，涵盖不同相机和物体运动条件。该方法在恢复湍流失真和提升分辨率方面表现出色。

Conclusion: HATIR通过热感知扩散框架有效解决了红外湍流视频超分辨率问题，联合建模湍流退化和分辨率损失，为红外VSR研究提供了新思路和首个基准数据集，推动了该领域的发展。
...
TL;DR: HuForDet是一个用于人类图像伪造检测的整体框架，采用双分支架构分别处理面部伪造和全身语义一致性检测，在统一的人类图像伪造数据集上实现了最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: AI生成内容（AIGC）的快速发展加剧了深度伪造的威胁，从面部操作到全身逼真人体合成。现有检测方法存在碎片化问题，要么专门处理面部区域伪造，要么专门处理全身合成图像，无法泛化到整个人类图像操纵的完整谱系。

Method: 提出HuForDet整体框架，采用双分支架构：1）面部伪造检测分支，在RGB和频域使用异构专家，包括自适应拉普拉斯-高斯（LoG）模块，用于捕捉从细粒度融合边界到粗尺度纹理不规则的各种伪影；2）上下文伪造检测分支，利用多模态大语言模型（MLLM）分析全身语义一致性，并配备置信度估计机制，在特征融合时动态加权其贡献。

Result: 构建了统一的人类图像伪造（HuFor）数据集，整合现有面部伪造数据和新创建的全身合成人体语料库。大量实验表明，HuForDet在各种人类图像伪造检测中实现了最先进的性能，并表现出卓越的鲁棒性。

Conclusion: HuForDet通过整体框架解决了人类图像伪造检测的碎片化问题，双分支架构结合面部细节分析和全身语义一致性检测，在统一数据集上验证了其优越性能，为全面的人类图像伪造检测提供了有效解决方案。
...
TL;DR: 该研究评估了CNN架构在五个异构数据集上的表现，比较了自定义轻量CNN与ResNet-18、VGG-16等经典架构，分析了架构复杂性、模型深度和迁移学习在不同数据条件下的影响。


<details>
  <summary>Details</summary>
Motivation: 深度学习特别是CNN在视觉数据分析中表现出色，但现有研究较少系统评估不同CNN架构在异构数据集上的表现。本研究旨在探索CNN架构在农业和城市领域的五个不同数据集上的有效性，这些数据集存在光照、分辨率、环境复杂性和类别不平衡等挑战。

Method: 研究评估了自定义轻量CNN与ResNet-18、VGG-16等经典架构，采用从头训练和迁移学习两种方式。通过系统预处理、数据增强和受控实验，分析架构复杂性、模型深度和预训练对模型收敛、泛化和性能的影响。

Result: 开发了一个高效的自定义CNN，在多个应用领域达到竞争性性能；通过比较分析发现，在数据受限环境中，迁移学习和深度架构能提供显著优势；为资源有限但高影响的现实世界视觉分类任务提供了实用见解。

Conclusion: 该研究为部署深度学习模型在资源有限的实际视觉分类任务中提供了实用指导，展示了自定义轻量CNN的竞争力，并明确了迁移学习和深度架构在数据受限环境中的优势。
...
Result: AIVD显著降低资源消耗，提高MLLM分类性能和语义生成质量，调度策略在多样化场景中实现更高吞吐量和更低延迟。
...
TL;DR: 该论文提出了一种利用骨架化技术的新型对抗攻击方法，专门针对包含文本（特别是数学公式）的图像，有效缩小搜索空间，评估了基础模型的视觉能力和局限性。


<details>
  <summary>Details</summary>
Motivation: 探索基础模型的视觉能力和局限性，特别是针对包含文本（尤其是数学公式）图像的对抗攻击挑战，这些图像由于LaTeX转换和复杂结构而更具挑战性。

Method: 提出了一种新颖的对抗攻击方法，利用骨架化技术有效减少搜索空间，专门针对文本图像（特别是数学公式图像），并详细评估字符和语义层面的变化。

Result: 该方法在ChatGPT等模型上展示了实际应用效果，揭示了模型在视觉解释和推理能力方面的局限性，特别是在处理复杂文本结构时的脆弱性。

Conclusion: 骨架化对抗攻击方法有效揭示了基础模型在视觉理解方面的局限性，特别是在处理数学公式等复杂文本结构时，为评估和改进模型的视觉能力提供了新视角。
...
TL;DR: ProFuse是一个高效的开源词汇3D场景理解框架，基于3D高斯泼溅技术，通过上下文感知方法增强跨视图一致性和掩码内聚性，无需渲染监督微调，语义标注速度比现有最佳方法快2倍。


<details>
  <summary>Details</summary>
Motivation: 现有的3D场景理解方法通常需要预训练的3DGS场景或复杂的优化过程，存在计算开销大、跨视图语义不一致的问题。需要一种高效、一致的开源词汇3D场景理解方法。

Method: 1. 引入密集对应引导的预注册阶段，初始化具有准确几何的高斯分布；2. 通过跨视图聚类联合构建3D上下文提议；3. 每个提议通过加权聚合成员嵌入获得全局特征；4. 在直接注册过程中将特征融合到高斯分布上，保持跨视图的每个基元的语言一致性；5. 无需标准重建之外的额外优化。

Result: ProFuse实现了强大的开源词汇3DGS理解能力，每个场景的语义标注仅需约5分钟，比现有最佳方法快2倍，同时保持了几何细化而无需密集化。

Conclusion: ProFuse提供了一个高效、上下文感知的3D场景理解框架，通过预建立的关联和特征融合机制，在保持几何质量的同时实现了快速、一致的语义标注，为开源词汇3D场景理解提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多图像视觉定位方法受限于单目标定位和有限的任务类型，缺乏对广义定位任务的统一建模。为了解决这一问题，需要开发能够处理多样化多图像定位任务的通用模型。

Method: 1. 系统分类和组织多图像定位任务；2. 构建MG-Data-240K数据集解决现有数据集在目标数量和图像关系方面的限制；3. 提出混合强化微调策略，结合思维链推理和直接回答，采用R1-like算法和基于规则的奖励机制。

Result: 在多图像定位方面，在MIG-Bench和MC-Bench上分别比先前领先的MLLMs提升2.0%和9.7%；在单图像定位方面，在ODINW上比基础模型提升9.1%；同时保持了强大的通用多图像理解能力。

Conclusion: GeM-VG通过统一建模广义多图像视觉定位任务，结合系统化的任务分类、大规模数据集和创新的训练策略，显著提升了多模态大语言模型在多样化定位任务中的性能，展示了强大的泛化能力。
...
TL;DR: 该论文提出了一个可扩展的反事实视频生成框架CounterVid，用于缓解视频语言模型的幻觉问题，特别是针对动作识别和时间推理任务。通过生成语义硬负样本和混合偏好优化方法，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型在多模态理解方面表现出色，但在推理动作和时间顺序时容易产生幻觉。现有的缓解策略（如文本过滤或随机视频扰动）未能解决根本问题：过度依赖语言先验而非细粒度视觉动态。

Method: 提出了一个可扩展的反事实视频生成框架，结合多模态LLM进行动作提议和编辑指导，使用基于扩散的图像和视频模型生成语义硬负样本。构建了CounterVid数据集（约26k偏好对），并引入了MixDPO方法，统一利用文本和视觉偏好进行直接偏好优化。

Result: 使用MixDPO微调Qwen2.5-VL模型取得了持续改进，特别是在时间排序方面表现显著提升，并能有效迁移到标准视频幻觉基准测试中。

Conclusion: 提出的反事实视频生成框架和MixDPO方法能有效缓解视频语言模型的幻觉问题，特别是在动作识别和时间推理任务上。代码和模型将公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 从2D图像准确估计深度是3D恢复领域的基本挑战。散焦提供了重要的深度信息，但现有方法面临空间变化散焦模糊的欠定问题。高斯模型因其数学简单性和计算效率成为实时应用的最优选择，但需要验证其在真实成像设备中的适用性。

Method: 在几何光学框架内进行散焦分析，通过衍射极限光学中的散焦像差理论来评估实际模型与其高斯近似的拟合精度。研究为传统成像设备设置了确保散焦算子遵循高斯模型的参数条件。

Result: 对于典型的聚焦深度范围（1-100米）且最大深度变化为聚焦深度的10%时，高斯模型在大多数成像设备中适用。结果显示最大平均绝对误差(MAE)小于1%，证明了模型的准确性和可靠性。

Conclusion: 高斯模型作为散焦算子在大多数成像设备中具有适用性，能够准确拟合实际散焦模型，为基于散焦的深度估计提供了可靠的理论基础。
...
TL;DR: 该研究提出了一种增强的Pix2Pix框架，通过集成SEResNet和U-Net++来改进医学图像翻译质量，在少样本条件下（少于500张图像）实现了更好的结构保真度和图像质量。
...
Method: 提出增强的Pix2Pix框架，集成SEResNet（通过通道注意力加强关键特征表示）和U-Net++（增强多尺度特征融合），并使用简化的PatchGAN鉴别器来稳定训练和细化局部解剖真实性。

Result: 在少于500张图像的少样本条件下，该方法在多个模态内MRI翻译任务中实现了一致的结构保真度和优越的图像质量，表现出强大的泛化能力。

Conclusion: 该方法为医学图像翻译提供了Pix2Pix的有效扩展，能够解决MRI临床应用中的限制问题，在少样本条件下仍能保持高质量的图像翻译效果。
...
TL;DR: 本文提出MCLC方法，通过测量一致的Langevin更新来稳定基于潜在扩散模型的零样本逆问题求解器，解决现有方法的不稳定性和伪影问题。
...
Method: 提出测量一致的Langevin校正器（MCLC），这是一个理论基础的即插即用校正模块，通过测量一致的Langevin更新来修正基于LDM的逆问题求解器。与依赖线性流形假设的先前方法不同，MCLC无需此假设，在潜在空间中更稳定可靠。

Result: 实验证明MCLC的有效性及其与现有求解器在各种图像恢复任务中的兼容性。作者还分析了斑点伪影并提供了对其根本原因的见解。

Conclusion: MCLC是迈向更稳健零样本逆问题求解器的关键一步，通过减少求解器与真实反向扩散动态之间的差异来稳定求解过程。
...
<details>
  <summary>Details</summary>
Motivation: 现有的开源金字塔视频模型需要从头训练，且在视觉逼真度上落后于最先进系统。需要一种方法能够利用现有预训练模型，通过低成本微调将其转换为金字塔模型，避免从头训练的开销和质量损失。

Method: 开发了一个将预训练扩散模型转换为金字塔模型的流程，通过低成本微调实现转换而不降低输出视频质量。同时研究和比较了金字塔模型中的各种步数蒸馏策略。

Result: 成功将预训练扩散模型转换为金字塔模型，在保持输出视频质量的同时显著降低了推理计算成本。通过步数蒸馏策略进一步提升了推理效率。

Conclusion: 提出了一种有效的金字塔模型转换方法，能够利用现有预训练模型资源，通过低成本微调实现高效的金字塔视频生成，为多步去噪模型提供了更实用的解决方案。
...
TL;DR: 本文首次系统评估了SAMURAI基础模型在无人机跟踪中的应用，并提出了检测器增强的扩展方法，显著提升了城市监控环境中长期无人机跟踪的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 无人机威胁日益增加，需要鲁棒的长期跟踪系统。现有基于检测器的方法存在时间不一致性和检测丢失问题，而基于RGB的无人机跟踪研究有限且依赖传统运动模型。SAMURAI等基础模型在其他领域表现出强大的类别无关跟踪性能，但在无人机特定场景中的应用尚未研究。

Method: 提出了检测器增强的SAMURAI扩展方法，通过结合检测器线索来减轻对边界框初始化和序列长度的敏感性，提高复杂城市环境中的跟踪鲁棒性。

Result: 提出的扩展方法显著提升了长期序列中的鲁棒性，特别是在无人机离开-重新进入场景中。在多个数据集和指标上，相比SAMURAI的零样本性能，成功率提升最高达+0.393，误报率降低最高达-0.475。

Conclusion: 这是首次系统评估SAMURAI在无人机跟踪中的应用，提出的检测器增强扩展方法有效解决了现有方法的局限性，为城市监控环境中的鲁棒长期无人机跟踪提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 古代文字图像常因背景噪声、低对比度以及老化和环境影响而严重退化，前景文字和背景常具有相似的视觉特征，导致铭文难以辨认，因此需要有效的图像增强方法来提高可读性。

Method: 采用基于二值化和互补预处理技术的图像增强方法，包括去除污渍和增强模糊古代文字的处理步骤，并在石头、金属板和历史文献等多种古代文字材料上进行评估。

Result: 使用K-NN分类器在石头、金属板和文献文字上分别获得55.7%、62%和65.6%的分类准确率；使用SVM分类器分别获得53.2%、59.5%和67.8%的准确率，证明了增强方法的有效性。

Conclusion: 所提出的图像增强方法能有效提高古代马拉地语铭文图像的可读性，为古代文字识别和保存提供了实用的技术方案。
...
TL;DR: SOVABench：基于监控视频的车辆行为检索基准，评估动作区分能力；提出基于MLLM的免训练框架生成可解释嵌入
...
Method: 1) 构建SOVABench监控视频基准，定义两种评估协议（跨动作区分和时间方向理解）；2) 提出基于多模态大语言模型的免训练框架，通过MLLM生成描述来产生可解释嵌入

Result: SOVABench显示现有先进视觉和多模态模型在动作区分上仍有挑战；提出的MLLM框架在SOVABench及空间和计数基准上表现良好，优于对比式视觉语言模型

Conclusion: SOVABench填补了监控视频动作评估的空白；基于MLLM的免训练框架为视频理解提供了可解释且有效的解决方案，在动作区分任务上展现出优势
...
<details>
  <summary>Details</summary>
Motivation: 古文书学中识别抄写员对确定手稿年代和理解书写演变至关重要。传统模板匹配方法需要手动设置阈值且存在局限性，需要更先进的自动检测技术来提高识别准确性和可靠性。

Method: 使用YOLOv5目标检测模型替代先前工作中的模板匹配和CNN方法，自动检测页面中的字母"a"实例，然后进行抄写员分类。YOLO的置信度分数为拒绝阈值提供了基础。

Result: YOLO能够提取更多字母实例，实现更准确的第二阶段分类。置信度分数支持开发拒绝阈值系统，即使在未见手稿中也能实现可靠的抄写员识别。

Conclusion: YOLO目标检测模型在古文书学抄写员识别中优于传统模板匹配方法，提供了更准确、可靠的识别系统，为处理未见手稿奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF分割方法多为优化型，需要缓慢的逐场景训练，牺牲了2D基础模型的零样本能力，需要一种无需优化的快速交互式解决方案

Method: 提出DivAS框架：1) 基于GUI工作流，用户点提示生成2D SAM掩码；2) 利用NeRF深度先验精炼掩码，提升几何精度和前景-背景分离；3) 开发自定义CUDA内核，在200ms内将多视角掩码聚合到统一的3D体素网格中

Result: 在Mip-NeRF 360°和LLFF数据集上，DivAS达到与优化方法相当的分割质量，端到端速度快2-2.5倍，排除用户提示时间后快一个数量级

Conclusion: DivAS提供了一种无需优化的完全交互式NeRF分割框架，在保持分割质量的同时显著提升速度，实现了实时视觉反馈
...
TL;DR: 本文提出一个工业级GenAI框架，用于处理大规模多模态数据（20万+PDF、2.5万+视频、888多语言音频），分析40+视觉语言模型在长视频理解中的实际限制与效率权衡，为工业部署提供实用指导。
...
Method: 提出工业级大规模多模态推理架构，在医药领域处理超20万PDF、2.5万视频和888多语言音频文件；对40+VLMs在两个主流基准和专有数据集上进行实证分析；研究长视频推理中的多模态作用、注意力机制权衡、时序推理限制和GPU约束下的视频分割挑战。

Result: 在商用GPU上使用SDPA注意力获得3-8倍效率提升；多模态在8/12任务领域（特别是长度依赖任务）提升性能；开源和闭源VLMs在时序对齐和关键帧检测方面存在明显瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动评估指标在文本到图像模型评估中广泛使用，但存在一个关键问题：这些指标是否真正评估语义正确性，还是仅仅偏好从有偏数据分布中学到的视觉和社会原型图像。作者识别并研究了原型性偏差这一系统性的多模态评估失败模式。

Method: 作者提出了ProtoBias基准，涵盖动物、物体和人口统计学图像，通过配对语义正确但非原型的图像与语义错误但原型的对抗性对应图像，进行定向评估。基于研究发现，作者开发了ProtoScore这一7B参数的鲁棒评估指标。

Result: 实验显示，广泛使用的评估指标（如CLIPScore、PickScore和基于VQA的分数）经常错误排序这些配对，甚至LLM-as-Judge系统在社会相关案例中也表现出不均匀的鲁棒性。相比之下，人类评估始终更偏好语义正确性且决策边界更大。ProtoScore显著降低了失败率，抑制了错误排序，运行速度比GPT-5推理时间快几个数量级，接近更大规模闭源评估者的鲁棒性。

Conclusion: 多模态评估指标存在显著的原型性偏差问题，倾向于偏好视觉原型而非语义正确性。ProtoScore作为新的评估指标，在保持高效的同时显著提高了评估的鲁棒性，为解决这一偏差问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有卫星图像时间序列分割方法在固定序列长度下表现良好，但忽视了模型在不同时间长度场景下的泛化能力，导致在变长序列情况下分割效果显著下降

Method: 提出TEA方法，采用教师-学生框架：教师模型封装全局序列知识，指导学生模型适应不同时间输入长度；通过中间嵌入、原型和软标签三个视角进行知识传递；动态聚合学生模型缓解知识遗忘；引入全序列重建作为辅助任务提升表征质量

Result: 在常见基准测试中，该方法在不同时间长度输入下都带来了显著改进

Conclusion: TEA方法有效增强了卫星图像时间序列分割模型在变长序列场景下的鲁棒性和泛化能力
...
TL;DR: SparseLaneSTP：一种集成车道几何特性和时序信息的稀疏车道Transformer方法，在3D车道检测任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D车道检测方法存在以下问题：1）传统BEV方法由于错误的特征变换导致特征表示与真实3D路面不对齐；2）稀疏车道检测器虽然超越了密集BEV方法，但完全忽略了有价值的车道先验知识；3）现有方法未能利用历史车道观测信息，而这些信息在能见度差的情况下有助于解决歧义。

Method: 提出了SparseLaneSTP方法，包括：1）新的车道特定时空注意力机制；2）为稀疏架构设计的连续车道表示；3）时序正则化。同时还提出了一个精确一致的3D车道数据集，采用简单有效的自动标注策略。

Result: 在所有检测和误差指标上，在现有3D车道检测基准和作者提出的新数据集上都实现了最先进的性能。

Conclusion: SparseLaneSTP通过集成车道几何特性和时序信息，有效解决了现有3D车道检测方法的局限性，同时提出的新数据集弥补了现有数据集的不足。
...
<details>
  <summary>Details</summary>
Motivation: 水下场景由于光学退化（如散射、吸收）导致多视图不一致，传统3D重建方法难以准确表示几何结构，需要专门解决水下环境特有的挑战。

Method: 1) 三目视图一致性：通过水平和垂直平移相机视图并利用逆变形对齐；2) 合成极线深度先验：通过三角测量获得自监督深度正则化；3) 深度感知alpha调整：基于z分量和视角方向在早期训练中调制3D高斯的不透明度。

Result: 在真实水下和模拟场景的实验中，OceanSplat在场景重建和散射介质恢复方面显著优于现有方法，能够将3D高斯与散射介质解耦，减少漂浮伪影。

Conclusion: OceanSplat通过几何约束和深度感知调整有效解决了水下3D重建的挑战，实现了对物体几何的鲁棒表示，为水下场景重建提供了有效的解决方案。
...
TL;DR: 论文研究高阶对抗攻击对目标检测器的影响，通过连续训练攻击模式和对抗训练来强化YOLOv10检测器，发现高阶对抗补丁具有更强的泛化能力，仅靠对抗训练不足以有效防御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 研究高阶对抗攻击（类似猫鼠游戏中的持续追逐）对目标检测器的影响，探索攻击模式与对抗训练的循环训练效果，了解高阶对抗攻击的威胁程度。

Method: 选择YOLOv10目标检测器作为代表，使用对抗补丁进行规避攻击，通过连续训练攻击模式和对抗训练来研究高阶对抗攻击的影响。

Result: 高阶对抗补丁不仅影响直接训练的目标检测器，相比低阶对抗补丁具有更强的泛化能力；仅靠对抗训练不足以有效强化目标检测器抵御此类攻击。

Conclusion: 高阶对抗攻击对目标检测器构成严重威胁，需要更全面的防御策略，仅依赖对抗训练是不够的，需要开发更有效的防御机制来应对高阶对抗攻击。
...
TL;DR: PolyFit是一种基于面片拟合的曲面表示方法，通过局部拟合jet函数实现高效学习，可用于多种曲面变形任务，显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统曲面变形方法需要优化每个顶点的自由度，计算成本高。需要一种更紧凑、高效的曲面表示方法，能够快速适应计算机视觉和图形学中的各种下游任务。

Method: 提出PolyFit表示方法，通过在曲面面片上局部拟合jet函数来学习曲面表示。该方法可以从解析函数和真实数据中监督学习，学习后可泛化到多种曲面类型。通过更新紧凑的jet系数集而非优化每个顶点自由度来实现曲面变形。

Result: 在形状模板任务中，PolyFit在测试时优化实现了竞争性精度，比离线物理求解器显著更快，在精度上优于最近的物理引导神经模拟器。在服装悬垂任务中，训练了自监督、网格和服装无关的模型，在分辨率和服装类型上具有良好泛化能力，推理速度比强基线快一个数量级。

Conclusion: PolyFit提供了一种高效、紧凑的曲面表示方法，能够显著加速计算机视觉和图形学中的曲面变形任务，在保持精度的同时大幅提升计算效率。
...
TL;DR: 提出一个针对制药行业的视频剪辑生成框架，结合音频语言模型和视觉语言模型，实现高效、个性化的视频摘要生成，相比现有方法显著提升效率和降低成本。
...
Method: 提出领域适应的视频到视频剪辑生成框架，整合音频语言模型和视觉语言模型，包含三个核心贡献：1) 可复现的剪切合并算法，带淡入淡出和时间戳标准化；2) 基于角色定义和提示注入的个性化机制；3) 成本高效的端到端管道策略。

Result: 在Video MME基准测试（900个视频）和16,159个制药视频数据集上评估，显示3-4倍速度提升，4倍成本降低，剪辑连贯性得分0.348和信息性得分0.721，优于Gemini 2.5 Pro等先进基线。

Conclusion: 该框架为生命科学领域提供了透明、可定制且符合合规要求的视频摘要解决方案，显著提升了制药行业多模态内容处理的效率和效果。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用中未标注的LiDAR数据包含丰富的3D几何信息，但由于缺乏人工标注而难以利用，这构成了自动驾驶感知研究的主要成本障碍。作者旨在解决这一瓶颈问题。

Method: 提出无监督多模态伪标签方法：1）利用从时间累积的LiDAR地图中学习的强几何先验；2）引入新颖的迭代更新规则，强制执行联合几何-语义一致性；3）通过不一致性检测移动物体；4）将文本和2D视觉基础模型的线索直接提升到3D空间。

Result: 方法在三个数据集上表现出稳健的泛化能力，相比现有需要额外人工监督的语义分割和物体检测伪标签方法更具优势。实验证实，即使使用一小部分几何一致的密集化LiDAR数据，也能在80-150米和150-250米范围内分别将深度预测的MAE提高51.5%和22.0%。

Conclusion: 该方法成功解决了自动驾驶感知研究中LiDAR数据标注成本高的瓶颈问题，通过无监督方式利用时间几何一致性生成高质量的3D伪标签，显著提升了深度预测性能，为自动驾驶研究提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有前馈视图合成模型使用Plücker射线图编码相机参数，这种方法将预测与任意世界坐标系绑定，使其对微小相机变换敏感，从而破坏了几何一致性。本文旨在寻找能够实现稳健和一致视图合成的最佳输入条件。
...
Conclusion: 投影条件化方法通过将视图合成任务重构为更稳定的目标视图图像到图像转换问题，显著提高了几何一致性和合成质量，同时支持大规模未标定数据的预训练。
...
TL;DR: VERSE是一种分析和改进视觉语言模型在文档理解中应用的方法，通过探索视觉嵌入空间来可视化潜在表示、识别问题区域并生成合成数据以提高性能。
...
Method: VERSE方法通过探索视觉嵌入空间来可视化模型的潜在表示，识别错误集中的问题区域，并指导生成包含这些视觉特征的合成数据来增强模型训练。

Result: 在MERIT数据集上的验证显示，VERSE能有效发现与错误相关的视觉特征，使用包含这些特征的样本重新训练显著提升了F1分数而不损害泛化能力。优化后的本地模型（如Donut和Idefics2）性能达到甚至超过SaaS解决方案（如GPT-4和Pixtral）。

Conclusion: VERSE为视觉语言模型的诊断和改进提供了系统化框架，通过视觉嵌入空间分析能够针对性地提升模型在文档理解任务中的性能，使本地模型能够与商业SaaS解决方案竞争。
...
TL;DR: VerseCrafter是一个4D感知的视频世界模型，通过4D几何控制表示实现对相机和多物体运动的统一精确控制，解决了现有方法在投影2D图像平面上操作动态的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型难以提供对相机和多物体运动的统一精确控制，因为视频本质上是投影到2D图像平面上的动态。需要一种能够桥接这一差距的方法，实现对动态环境的统一4D几何控制。

Method: 提出4D几何控制表示，通过静态背景点云和每个物体的3D高斯轨迹编码世界状态，捕捉物体路径和随时间变化的概率性3D占用。这些4D控制被渲染为预训练视频扩散模型的调节信号。为解决4D标注数据稀缺问题，开发了自动数据引擎从野外视频中提取所需4D控制。

Result: 能够生成高保真、视角一致的视频，精确遵循指定的动态控制。通过自动数据引擎在大规模多样化数据集上训练模型。

Conclusion: VerseCrafter通过4D几何控制表示实现了对相机和物体动态的统一精确控制，解决了视频世界模型中的关键挑战，为动态环境模拟提供了新的解决方案。
...
TL;DR: 提出轻量级视觉语言框架用于作物病害识别，结合Swin Transformer视觉编码器和序列到序列语言解码器，在减少参数的同时超越大型基线模型性能


<details>
  <summary>Details</summary>
Motivation: 作物病害视觉问答需要准确的视觉理解和可靠的语言生成，现有大型视觉语言模型参数过多，需要开发轻量级但高性能的解决方案

Method: 采用Swin Transformer视觉编码器与序列到序列语言解码器结合的两阶段训练策略，通过任务特定的视觉预训练提升视觉表示学习和跨模态对齐

Result: 在大规模作物病害数据集上评估，在作物和病害识别方面均取得高准确率，在BLEU、ROUGE和BERTScore指标上表现优异，参数显著少于大型基线模型但性能更优

Conclusion: 任务特定的视觉预训练对作物病害视觉问答非常有效，提出的轻量级框架在减少参数的同时实现了强大的性能，并通过Grad-CAM和token级归因验证了可解释性
...
Method: 开发了Atlas 2、Atlas 2-B和Atlas 2-S三个病理视觉基础模型，使用来自柏林夏里特医学院、慕尼黑大学和梅奥诊所的550万张病理切片图像进行训练，这是迄今为止最大的病理基础模型数据集

Result: 在80个公开基准测试中展示了最先进的预测性能、鲁棒性和资源效率，综合评估表现优异

Conclusion: Atlas 2系列模型通过大规模数据集训练，成功解决了现有病理基础模型在性能、鲁棒性和计算效率方面的权衡问题，为临床部署提供了更好的解决方案
...
TL;DR: MuLo-SD提出了一种多尺度局部推测解码框架，通过多分辨率草稿生成和空间感知验证来加速自回归图像生成，实现1.7倍加速同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像合成中表现出色，但其顺序性导致显著的延迟问题。现有推测解码方法受限于令牌级模糊性和缺乏空间感知能力，需要更高效的加速方案。

Method: 提出多尺度局部推测解码框架：1）使用低分辨率草稿器和学习上采样器生成候选图像令牌；2）通过高分辨率目标模型并行验证；3）引入局部拒绝和重采样机制，在空间邻域内纠正草稿错误而非全图重采样。

Result: 在MS-COCO 5k验证集上，MuLo-SD实现最高1.7倍加速，超越EAGLE-2和LANTERN等基线方法，同时在语义对齐和感知质量方面保持可比性能，在GenEval、DPG-Bench和FID/HPSv2指标上验证有效。

Conclusion: MuLo-SD为图像合成的推测解码设立了新的技术标准，通过多分辨率草稿和局部纠正机制，在效率和保真度之间取得了良好平衡，为自回归图像生成加速提供了有效解决方案。
...
TL;DR: VLI是一个无需训练的多模态大语言模型推理框架，通过模拟元认知自我修正过程来减少物体幻觉，在MMHal-Bench上将物体幻觉率降低12.67%，在POPE上提升准确率5.8%


<details>
  <summary>Details</summary>
Motivation: 物体幻觉严重损害多模态大语言模型的可靠性，源于模型认知内省的失败——盲目信任语言先验而非具体视觉证据。现有缓解方法有限：对比解码方法操作表面化，未能纠正内部语义错位；而当前潜在引导方法依赖静态向量，缺乏实例特异性精度。

Method: VLI包含两个核心组件：1) 归因内省：通过概率冲突检测诊断幻觉风险并定位因果视觉锚点；2) 可解释双因果引导：主动调制推理过程，动态隔离视觉证据与背景噪声，同时通过自适应校准消除盲目置信。

Result: VLI在先进模型上实现最先进性能，在MMHal-Bench上将物体幻觉率降低12.67%，在POPE上提升准确率5.8%。

Conclusion: VLI通过模拟元认知自我修正过程，有效减少多模态大语言模型的物体幻觉问题，提供了一种无需训练的高效推理框架。
...
TL;DR: VideoAuto-R1框架采用"必要时推理"策略，在视频理解任务中实现高效推理，减少响应长度约3.3倍，同时保持最先进准确率。


<details>
  <summary>Details</summary>
Motivation: 研究发现对于RL训练的视频模型，直接回答通常匹配甚至优于链式思维推理性能，但链式思维需要更高计算成本。因此需要探索何时需要显式推理，何时可以直接回答。

Method: 提出VideoAuto-R1框架，采用"思考一次，回答两次"训练范式：模型首先生成初始答案，然后进行推理，最后输出审查后的答案。两个答案都通过可验证奖励进行监督。推理时使用初始答案的置信度来决定是否进行推理。

Result: 在视频QA和grounding基准测试中，VideoAuto-R1达到最先进准确率，同时显著提高效率，平均响应长度减少约3.3倍（从149个token减少到44个token）。感知导向任务中思维模式激活率低，而推理密集型任务中激活率高。

Conclusion: 显式语言推理通常有益但并非总是必要。VideoAuto-R1的"必要时推理"策略能有效平衡准确性和效率，为视频理解任务提供更高效的解决方案。
...
TL;DR: 论文研究大型视觉语言模型中的提示诱导幻觉现象，发现在物体计数任务中，当文本提示高估物体数量时，模型会倾向于服从文本提示而忽略视觉证据，尤其是在物体数量较多时。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然功能强大，但经常出现幻觉现象，倾向于优先考虑文本提示而忽略视觉证据。研究者希望深入理解这种失败模式，特别是在物体计数任务中，当文本提示与视觉证据不一致时模型的行为。

Method: 研究采用受控的物体计数设置，让文本提示高估图像中的物体数量。通过机制分析三个不同的视觉语言模型，识别出一小部分注意力头，这些头在消融后能显著减少提示诱导幻觉。研究还分析了不同模型中这些注意力头如何以模型特定的方式介导提示复制行为。

Result: 研究发现：1）在低物体数量时，模型通常会纠正高估；2）随着物体数量增加，模型越来越倾向于服从文本提示而忽略视觉证据；3）识别出的注意力头消融后能将提示诱导幻觉减少至少40%，且无需额外训练；4）不同模型中，这些注意力头以模型特定的方式介导提示复制行为；5）消融这些头能增加模型向视觉证据的纠正。

Conclusion: 研究揭示了视觉语言模型中提示诱导幻觉的内部机制，发现了不同模型在实现这些行为时的特异性差异。通过识别和消融特定的注意力头，可以在不进行额外训练的情况下显著减少幻觉现象，为理解和改进视觉语言模型的可靠性提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D MRI数据集存在人口统计学偏差，限制了脑年龄预测模型的公平性和泛化能力。获取新数据成本高且受伦理约束，而当前生成方法（如潜在扩散模型）推理速度慢、可能引入伪影且很少考虑年龄条件化

Method: FlowLet框架在可逆3D小波域中利用流匹配技术合成年龄条件化的3D MRI图像，避免重建伪影并降低计算需求

Result: FlowLet能以较少采样步骤生成高保真度的脑部体积图像；使用FlowLet生成的数据训练脑年龄预测模型能改善对代表性不足年龄组的性能；区域分析证实了解剖结构的保留

Conclusion: FlowLet通过可逆3D小波域中的流匹配技术，提供了一种高效的条件生成方法，能够改善脑年龄预测模型的公平性和性能，同时保持解剖结构的完整性
...
TL;DR: ObjectForesight：一种从短时第一人称视频预测刚体物体6自由度位姿和轨迹的3D物体中心动力学模型


<details>
  <summary>Details</summary>
Motivation: 人类能够轻松预测物体如何通过交互移动或变化，我们希望赋予计算系统类似能力，直接从被动视觉观察中预测合理的未来物体运动

Method: 提出ObjectForesight模型，在物体层面显式表示3D世界，利用分割、网格重建和3D位姿估计技术构建包含200多万短片段伪真值3D物体轨迹的数据集进行训练

Result: ObjectForesight在准确性、几何一致性和对未见物体场景的泛化能力方面取得显著提升，建立了从观察中学习物理基础物体中心动力学模型的可扩展框架

Conclusion: 该研究展示了直接从视觉观察学习几何基础和时序一致的物体运动预测的可行性，为物体中心动力学建模提供了可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有指代表达式分割(RES)、理解(REC)和生成(REG)任务通常只支持单目标表达式，即一个表达式只指代一个对象，忽略了多目标和无目标表达式的情况，这大大限制了REx在实际应用中的能力。

Method: 提出了ReLA方法，通过自适应地将图像划分为包含子实例线索的区域，并显式建模区域-区域和区域-语言之间的依赖关系，以应对GRES/GREC任务中复杂关系建模的挑战。
...
Method: 1. 基于像素空间扩散变换器(DiT)构建PPD模型；2. 提出语义提示DiT，利用视觉基础模型的语义表示指导扩散过程；3. 采用级联DiT架构逐步增加图像token数量；4. 对于视频扩展PPVD，引入语义一致DiT，从多视角几何基础模型提取时间一致的语义信息；5. 在DiT内进行参考引导的token传播以保持时间一致性。

Result: 模型在所有生成式单目和视频深度估计模型中表现最佳，产生的点云比其他所有模型都显著更干净。

Conclusion: 通过像素空间生成建模和创新的DiT设计，实现了高质量、无漂浮像素的几何恢复，为机器人和增强现实应用提供了更好的几何基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 夜间色彩恒常性由于低光噪声和复杂光照条件而具有挑战性，现有方法在夜间场景中表现不佳，需要更有效的解决方案。

Method: 结合统计方法与深度强化学习：1）针对夜间场景的统计算法，集成显著灰度像素检测与新颖光照估计；2）首个基于深度强化学习的色彩恒常性方法，以统计算法为核心，模仿专业AWB调优专家动态优化每张图像参数。

Result: 方法在低光和正常光照图像上表现出优越的泛化能力，通过引入首个多传感器夜间数据集进行跨传感器评估验证了有效性。

Conclusion: RL-AWB框架成功解决了夜间白平衡问题，结合统计方法与深度强化学习的方法在复杂光照条件下表现出色，为夜间计算摄影提供了有效解决方案。
...
Method: 提出QNeRF模型，利用参数化量子电路通过量子叠加和纠缠编码空间和视角相关信息。设计了两种架构变体：Full QNeRF最大化利用所有量子振幅增强表示能力；Dual-Branch QNeRF引入任务感知归纳偏置，通过分支处理空间和视角相关的量子态准备，大幅降低操作复杂度并确保可扩展性和硬件兼容性。

Result: 在中等分辨率图像训练时，QNeRF匹配或优于经典NeRF基线，同时使用不到一半的参数数量。这表明量子机器学习可以作为计算机视觉中等任务中连续信号表示的竞争性替代方案。

Conclusion: QNeRF成功将量子视觉场方法扩展到新视角合成任务，证明了量子机器学习在3D表示学习等计算机视觉任务中的潜力，为更紧凑高效的模型设计提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单目视频中重建动态物体完整3D形状和运动的问题，传统方法需要骨骼信息或逐帧处理，效率较低且依赖额外信息

Method: 1）构建紧凑潜在空间编码整个动画序列；2）使用自编码器学习潜在空间，训练时利用骨骼结构提供变形先验；3）编码器采用时空注意力机制；4）基于潜在扩散模型，以输入视频和首帧重建网格为条件，一次性预测完整动画

Result: 在重建和新视角合成基准测试中优于现有方法，能够准确恢复3D形状和变形，推理时无需骨骼信息

Conclusion: Mesh4D通过紧凑潜在空间和潜在扩散模型实现了高效的单目4D网格重建，在保持准确性的同时减少了推理时的依赖条件
...
TL;DR: 该论文提出了一种基于学习的制导控制方法，将具备推理能力的大型语言模型与组相对策略优化相结合，通过两阶段训练在多种动态系统中实现稳定控制。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够结合大型语言模型的推理能力和强化学习优化方法的控制框架，以解决从简单线性系统到复杂非线性系统的控制问题，同时提供人类可理解的决策解释。

Method: 采用两阶段训练方法：首先通过监督微调学习格式化和控制原语，然后使用组相对策略优化进行交互驱动的策略改进。该方法在四个不同动态复杂度的控制问题上进行了验证。

Result: 结果表明，通过GRPO优化的具备显式推理能力的LLM能够在一致的训练设置下，在线性和非线性系统中合成可行的稳定策略，并能生成控制序列并提供人类可读的决策过程解释。
...
TL;DR: UNIC是一个无需先验知识或相机标定的统一多模态框架，用于估计抓取物体与环境之间的外部接触，通过视觉、本体感觉和触觉模态的数据驱动融合实现可靠接触估计。


<details>
  <summary>Details</summary>
Motivation: 现有接触估计方法依赖预定义接触类型、固定抓取配置或相机标定等限制性假设，阻碍了在新物体和非结构化环境中的泛化应用。需要一种无需先验知识、能适应新场景的通用接触估计方法。

Method: UNIC直接编码相机坐标系中的视觉观测，与本体感觉和触觉模态以数据驱动方式集成。采用基于场景可供性地图的统一接触表示，捕捉多样接触形态，并通过随机掩码的多模态融合机制实现鲁棒的多模态表示学习。

Result: 在未见过的接触位置实现9.6毫米的平均Chamfer距离误差，在未见物体上表现良好，对缺失模态保持鲁棒性，并能适应动态相机视角变化。

Conclusion: UNIC将外部接触估计确立为接触丰富操作中实用且多功能的能⼒，为规划、控制和策略学习提供了可靠的上下文信息。
...
TL;DR: 提出一个连续体机器人状态估计的通用框架，结合时空域联合估计，能够处理驱动输入、外部载荷和不确定性，应用于肌腱驱动机器人和手术同心管机器人。


<details>
  <summary>Details</summary>
Motivation: 现有的连续体机器人状态估计方法通常采用简化的Cosserat杆模型，无法直接考虑驱动输入或外部载荷，需要更全面的框架来整合各种不确定性因素和测量信息。

Method: 提出一个通用框架，整合驱动不确定性（如肌腱张力）、施加的力和力矩、过程噪声、边界条件和任意背部测量。通过添加时间先验，实现空间（弧长）和时间域的联合估计。将弧长域离散化得到因子图表示，利用SLAM风格的稀疏非线性优化进行快速批量计算。

Result: 在仿真中展示了肌腱驱动机器人的实时运动学（含不确定性）、基于位置反馈的末端力感知、以及基于背部应变的分布式载荷估计。在实验中验证了手术同心管机器人的精确运动学和末端力估计，展示了在手术触诊中的潜力。

Conclusion: 该框架为连续体机器人提供了通用的时空状态估计方法，能够整合多种不确定性因素和测量信息，在仿真和实验中均表现出良好性能，具有广泛的应用前景。
...
TL;DR: 开发了名为Moonbots的4自由度机器人肢体模块，可与轮式模块连接形成多种配置，用于月球探索和建设的模块化机器人系统
...
Method: 采用统一的驱动器设计（高扭矩-速度比），开发硬件实现、机械设计和软件架构，支持模块间的多种连接配置

Result: 展示了9种功能配置：4自由度肢体、8自由度肢体、车辆、龙形、最小化、四足、货物、最小货物和自行车配置，验证了系统的适应性

Conclusion: 模块化机器人系统为可重构机器人研究提供了实用基础，统一的驱动器设计简化了开发和维护，适合空间任务应用
...
TL;DR: 开发了一个结合高视觉保真度和真实地形交互的月球表面模拟器，使用数据驱动的回归模型来准确模拟轮-土相互作用，支持实时应用。


<details>
  <summary>Details</summary>
Motivation: 当前月球表面模拟器要么注重视觉真实性，要么注重物理准确性，无法全面复现月球条件。需要结合两者来提供更真实的月球车操作环境。

Method: 采用数据驱动方法，使用从全车和单轮实验及模拟中收集的数据构建回归模型，预测滑移和沉陷。改进了地形变形和车轮痕迹可视化的真实感。

Result: 回归模型在平坦地形和高达20度的斜坡上准确复现了稳态和动态滑移以及沉陷行为，并通过了现场测试验证。实现了物理合理的地形响应和高视觉保真度的结合。

Conclusion: 该方法成功填补了现有模拟器的空白，为需要物理合理地形响应和高视觉保真度的实时应用提供了有效的解决方案。
...
TL;DR: 提出一种基于离散傅里叶变换的点云地图压缩方法，针对渐进地形（如行星、沙漠）通过去除高频分量实现高效压缩


<details>
  <summary>Details</summary>
Motivation: 在机器人探索任务中，SLAM系统面临计算能力和通信带宽限制，而点云数据量庞大，需要有效的压缩方法

Method: 将数字高程模型转换为频域二维图像，去除高频分量，专注于渐进地形（如行星、沙漠）的压缩

Result: 使用两种不同高程剖面的地形相机序列进行评估，在压缩率和精度方面表现良好

Conclusion: 该方法能有效压缩点云数据大小而不会显著降低质量，特别适用于渐进地形环境
...
TL;DR: UniBiDex是一个统一的机器人双手灵巧操作遥操作框架，支持VR和领导者-跟随者两种输入模式，通过集成异构输入设备和共享控制栈实现实时接触丰富的双臂遥操作。


<details>
  <summary>Details</summary>
Motivation: 为了降低收集大规模高质量人类示范数据集的障碍，并加速机器人学习进展，需要一种能够支持多种输入模式、保证安全性的统一遥操作框架。

Method: 框架将异构输入设备集成到共享控制栈中，采用一致的动力学处理和安全性保证，使用零空间控制优化双手配置，确保平滑、无碰撞、感知奇异点的运动。

Result: 在厨房整理任务（包含五个顺序操作子任务）上验证，相比强基线方法，UniBiDex显示出更高的任务成功率、更平滑的轨迹和更好的鲁棒性。

Conclusion: 通过开源所有硬件和软件组件，UniBiDex旨在降低收集高质量人类示范数据集的障碍，加速机器人学习领域的进展。
...
TL;DR: 研究构建了一个考虑他人沟通意愿的计算空间交互模型，通过VR实验验证了模型在机器人社交互动中的有效性。


<details>
  <summary>Details</summary>
Motivation: 公共空间中的沟通机器人需要在发起对话和不打扰行人之间取得平衡，这需要机器人能够基于他人行为估计其沟通意愿，并相应调整自身行为。

Method: 构建了一个计算空间交互模型，将"考虑他人"量化为参数：根据估计的他人内部状态调整自身内部状态的程度。通过VR环境进行人机交互实验验证模型。

Result: 当参与者向目标移动时，低考虑值的虚拟机器人会抑制参与者移动，而高考虑值的机器人则不会。当参与者接近机器人时，无论考虑值高低，机器人都会表现出接近行为，从而减少参与者的移动。

Conclusion: 实验结果验证了所提模型能够阐明考虑他人的交互行为，为公共空间中沟通机器人的行为设计提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统网格路径规划方法（如A*和Dijkstra算法）在动态农业环境中存在局限性，需要更自适应、能处理连续动作空间的智能路径规划方法，以适应复杂的农业场景。

Method: 研究采用渐进式方法：首先回顾传统网格方法，然后探索DQN等离散动作空间DRL方法，再引入Double Q-Networks和Dueling Networks改进，最后重点研究连续动作空间模型DDPG和TD3，并在ROS和Gazebo构建的三维动态农业环境中进行实验验证。

Result: 实验表明，连续动作空间DRL算法在动态农业环境中表现优异，特别是预训练的TD3智能体在动态环境中达到95%的成功率，能够有效处理移动障碍物，同时确保作物和机器人的安全。

Conclusion: 深度强化学习在连续动作空间中的应用为农业无人地面车辆提供了有效的自适应路径规划解决方案，TD3算法在动态农业环境中表现出色，证明了该方法在复杂农业场景中的实用性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统观测器无法补偿惯性效应，导致在接触丰富的操作任务中难以实现高精度的零力矩控制，需要一种能清晰分离固有动态反应和真实外部力矩的方法。

Method: 设计动态力矩扰动观测器（DW-DOB），将任务空间惯性嵌入观测器名义模型中，从而清晰分离固有动态反应和真实外部力矩。采用基于无源性的分析确保动态条件下的稳定交互。

Result: 在工业公差（H7/h6）的孔轴配合实验中验证了该方法，实现了更深、更柔顺的插入，残余力矩最小，性能优于传统力矩扰动观测器和PD基线方法。

Conclusion: DW-DOB为接触丰富任务中的高精度零力矩控制提供了一种实用的无学习解决方案，能够保持对小力和力矩的敏感性，同时确保接触力矩的鲁棒调节。
...
TL;DR: 该研究提出了一种配备被动轮的人形机器人进行轮滑运动，通过深度强化学习控制框架实现了比传统双足行走更平滑、更高效的运动模式。
...
Method: 提出了一种每只脚配备四个被动轮的新型人形机器人，并开发了基于深度强化学习的控制框架，针对轮滑的八字步态设计奖励函数。

Result: 在仿真中分析学习策略后部署到物理机器人上，与传统双足行走相比，八字步态在冲击强度和运输成本方面分别减少了75.86%和63.34%。

Conclusion: 轮滑作为一种优越的运动模式，能够显著提高能量效率和延长关节寿命，为人形机器人提供了更平滑高效的运动解决方案。
...
TL;DR: 提出基于校准概率的安全关键触发框架，用于日常生活活动中的多模态下一动作预测，通过校准置信度驱动ACT/HOLD规则，将置信度阈值转化为辅助动作的定量安全参数


<details>
  <summary>Details</summary>
Motivation: 辅助设备在提供支持前需要确定用户的意图以及预测的可靠性。原始模型置信度往往无法反映真实正确性，存在安全风险，需要建立安全关键的触发机制

Method: 引入基于校准概率的安全关键触发框架，通过后处理校准使预测置信度与经验可靠性对齐，使用校准后的置信度驱动简单的ACT/HOLD规则（高可靠性时行动，否则保持）

Result: 后处理校准将误校准降低了约一个数量级，同时不影响准确性。校准后的置信度将置信度阈值转化为辅助动作的定量安全参数，使辅助控制回路具有可验证的行为

Conclusion: 提出的安全关键触发框架通过置信度校准和ACT/HOLD规则，为辅助设备提供了可靠的安全机制，将预测不确定性转化为可量化的安全参数，增强了辅助系统的安全性和可信度
...
<details>
  <summary>Details</summary>
Motivation: 自主系统在开放动态环境（如城市街道、空中和室内空间）中部署时，感知模型需要在传感器噪声、环境变化和平台差异下保持可靠性。现有方法在未见条件下性能下降，需要更鲁棒和可泛化的机器人感知技术。

Method: RoboSense 2025挑战赛设计了五个互补研究赛道：1) 语言基础决策制定，2) 社会合规导航，3) 传感器配置泛化，4) 跨视图和跨模态对应，5) 跨平台3D感知。提供标准化数据集、基线模型和统一评估协议，支持大规模可复现的鲁棒感知方法比较。

Result: 挑战赛吸引了来自16个国家85个机构的143支团队参与，反映了广泛的社区参与度。通过分析23个获胜解决方案，报告突出了各赛道的新兴方法趋势、共享设计原则和开放挑战。

Conclusion: RoboSense 2025挑战赛为评估真实世界感知可靠性提供了全面基准，推动了机器人感知在领域转移、传感器故障和平台差异下的鲁棒性研究，朝着构建能在真实环境中可靠感知、鲁棒行动和跨平台适应的机器人迈出了一步。
...
TL;DR: 提出了一种基于躯干屈伸控制假肢肘部速度的新控制范式CEAC，通过放大躯干与假肢的自然耦合关系，实现连续、精确的协调控制。


<details>
  <summary>Details</summary>
Motivation: 尽管上肢假肢设计有所进步，但实现中间关节（如手腕和肘部）的直观控制仍然具有挑战性，特别是在连续和速度调节运动方面。需要一种能够利用用户自然补偿运动的控制策略。

Method: 提出了补偿效应放大控制（CEAC）范式，利用用户的躯干屈伸作为控制假肢肘部速度的输入。该方法放大躯干与假肢之间的自然耦合，同时引入受控延迟，使用户能够调节假肢关节的位置和速度。

Result: 在12名健全参与者使用具有主动肘部的超数假肢进行的通用绘图任务中，以及10名参与者的多目标到达任务中，CEAC表现出与自然手臂运动相当的任务性能，即使手势速度或绘图大小发生变化，同时保持符合人体工程学的躯干姿势。

Conclusion: CEAC有效地恢复了关节协调动作，在躯干和肘部之间分配运动努力，实现了直观的轨迹控制，无需极端的补偿运动。这为需要连续和精确协调的任务中的上肢假肢中间关节控制提供了有前景的策略。
...
TL;DR: 提出LaST₀框架，通过潜在时空思维链实现高效推理，解决现有VLA模型推理延迟高和语言表示瓶颈问题，在模拟和真实任务中显著提升成功率并加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有VLA方法通过显式生成语言推理轨迹或未来视觉观察来提升动作准确性，但存在推理延迟高、时间分辨率受限的问题，且语言表示难以准确捕捉物理属性的微妙差异。

Method: 提出LaST₀框架：1）构建高效的潜在CoT空间，建模未来视觉动态、3D结构信息和机器人本体感觉状态；2）采用混合Transformer的双系统架构，推理专家进行低频潜在推理，动作专家基于机器人导向的潜在表示生成高频动作；3）通过异构操作频率训练实现推理和动作速率自适应切换。

Result: 在10个模拟和6个真实世界操作任务中，LaST₀相比现有VLA方法分别提升平均成功率8%和13%，同时实现显著更快的推理速度。
