...
Method: GeoPredict引入两个预测模块：1)轨迹级模块编码运动历史并预测机器人手臂的多步3D关键点轨迹；2)预测性3D高斯几何模块预测工作空间几何，并沿未来关键点轨迹进行跟踪引导细化。这些模块仅作为训练时监督，推理时只需轻量级查询令牌。

Result: 在RoboCasa Human-50、LIBERO和真实世界操作任务上的实验表明，GeoPredict持续优于强大的VLA基线，特别是在几何密集和空间要求高的场景中。

Conclusion: GeoPredict通过整合预测性几何和运动学先验，显著提升了VLA模型在3D推理任务中的性能，为机器人操作提供了更可靠的几何感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 人类手部是我们与物理世界的主要交互界面，但自我中心视角感知很少能知道何时、何地以及以多大力度进行接触。目前缺乏鲁棒的穿戴式触觉传感器，也没有野外环境下的第一人称视频与全手触觉对齐的数据集。

Method: 提出了OpenTouch数据集，包含同步的视频-触觉-姿态数据，并基于此数据集引入了检索和分类基准测试，探究触觉如何支撑感知和行动。
...
TL;DR: 提出EgoMAN数据集和模型，用于解决现有3D手部轨迹预测中运动与语义监督分离、推理与动作弱关联的问题


<details>
  <summary>Details</summary>
Motivation: 现有3D手部轨迹预测工作受限于将运动与语义监督解耦的数据集，以及推理与动作弱关联的模型，需要更好的数据集和框架来连接语义推理与运动生成

Method: 首先创建EgoMAN数据集（包含219K 6DoF轨迹和3M结构化QA对），然后提出EgoMAN模型，这是一个推理到运动的框架，通过轨迹-令牌接口连接视觉语言推理和运动生成，采用渐进式训练对齐推理与运动动态

Result: 该方法能够产生准确且阶段感知的轨迹，并在真实场景中展现出良好的泛化能力

Conclusion: 通过EgoMAN数据集和模型，成功解决了3D手部轨迹预测中语义推理与运动生成的连接问题，实现了更准确、阶段感知的轨迹预测
...
TL;DR: 该研究评估了YOLOv8变体在车牌识别和字符识别任务上的性能，提出了一种优化的识别流程，在保持计算效率的同时实现了高精度。


<details>
  <summary>Details</summary>
Motivation: 在交通管理和车辆监控领域，高效的车牌检测与识别至关重要。尽管已有多种方法，但在多样化环境中保持实时准确性仍然是一个挑战。本研究旨在通过评估YOLOv8变体来推进智能交通系统的发展。

Method: 使用两个不同的数据集进行训练和评估。采用YOLOv8 Nano变体进行车牌识别，YOLOv8 Small变体进行字符识别。引入了一种基于x轴位置的自定义字符排序方法，并提出了一个优化的识别流程。

Result: YOLOv8 Nano在车牌识别任务上达到0.964的精确度和0.918的mAP50；YOLOv8 Small在字符识别任务上达到0.92的精确度和0.91的mAP50。提出的优化流程在保持计算效率的同时确保了高准确性。

Conclusion: 提出的YOLOv8 Nano用于车牌识别、YOLOv8 Small用于字符识别的优化配置，为智能交通系统中边缘设备的实际部署建立了坚实基础，是迈向更智能、更高效城市基础设施的重要一步。
...
TL;DR: 提出一种紧凑的图像到文本架构，仅使用单个正面胸片生成放射学报告，通过解剖注意力机制提升临床相关区域的生成质量，在资源受限环境下实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的放射学报告生成系统依赖大规模多模态训练、临床元数据和多个成像视图，资源密集且难以普及。需要开发更紧凑、仅依赖单张图像的解决方案，使更多医疗机构能够使用自动报告生成技术。

Method: 结合冻结的DINOv3 ViT编码器和GPT-2解码器，通过层级解剖注意力机制增强。该机制利用肺部和心脏分割掩码，通过分层高斯平滑将注意力偏向临床相关区域，不增加可训练参数。

Result: 在MIMIC-CXR数据集上评估，关键病理的CheXpert Macro-F1提升168%，Micro-F1提升146%，14种观察指标提升86%。结构连贯性方面，RadGraph F1提升9.7%。

Conclusion: 尽管模型规模小且仅依赖图像条件，但解码器级的解剖引导改善了空间定位能力，增强了临床相关区域的连贯性。证明了在资源受限环境下实现高质量放射学报告生成的可行性。
...
TL;DR: 研究发现文本到图像模型评估基准GenEval存在显著的基准漂移问题，其与人类判断的偏差高达17.7%，已不再有效。为此提出了新基准GenEval 2和评估方法Soft-TIFA，以提供更准确、持久的评估。
...
Result: GenEval基准已严重漂移，与人类判断的绝对误差高达17.7%，表明该基准已饱和。GenEval 2对当前模型更具挑战性，Soft-TIFA评估方法与人类判断更一致，且随时间漂移的可能性更低。

Conclusion: 基准漂移是文本到图像模型评估的重要问题。GenEval 2和Soft-TIFA提供了更可靠的评估方案，但避免基准漂移仍需持续审计和改进评估基准。
...
TL;DR: RePlan是一个用于复杂指令图像编辑的规划-执行框架，通过区域对齐规划和强化学习训练，在复杂视觉场景中实现精确的多区域编辑。
...
Conclusion: RePlan通过区域对齐规划和强化学习训练，有效解决了复杂指令图像编辑中的IV-Complexity问题，在精细定位和知识密集型编辑任务上表现出色，为复杂视觉场景下的指令编辑提供了新思路。
...
TL;DR: ReMeDI-SAM3：一种无需训练的内存增强型SAM3扩展，用于内窥镜视频中的手术器械分割，通过改进内存管理和重识别机制，在遮挡后实现更可靠的恢复。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中的手术器械分割对计算机辅助干预至关重要，但由于频繁遮挡、快速运动、镜面伪影和长期器械重新进入等挑战而困难。SAM3虽然提供了强大的时空框架，但在手术场景中受到非选择性内存更新、固定内存容量和遮挡后弱身份恢复的限制。

Method: 提出ReMeDI-SAM3，包含三个组件：(1) 相关性感知内存过滤，带有专用的遮挡感知内存存储遮挡前帧；(2) 分段插值方案扩展有效内存容量；(3) 基于特征的重识别模块，通过时间投票实现可靠的遮挡后身份消歧。

Result: 在EndoVis17和EndoVis18数据集上的零样本评估显示，相比原始SAM3分别获得约7%和16%的绝对mcIoU提升，甚至超过了之前基于训练的方法。

Conclusion: ReMeDI-SAM3通过改进内存管理和重识别机制，有效缓解了错误累积，实现了遮挡后的可靠恢复，显著提升了内窥镜视频中手术器械分割的性能。
...
TL;DR: M-PhyGs方法从视频中估计复杂多材料自然物体（如花朵）的材料组成和物理参数，解决了现有方法对单一材料、预学习动力学或简单拓扑的限制。


<details>
  <summary>Details</summary>
Motivation: 现实世界物体通常具有复杂的材料组成和几何形状，超出了现有方法对均匀单一材料、预学习动力学或简单拓扑的假设。花朵作为代表性常见物体，其多材料特性需要新的方法来准确估计物理材料参数。
...
Conclusion: M-PhyGs能够从视频中准确估计复杂多材料自然物体的材料组成和物理参数，为现实世界物体的物理特性理解提供了新的解决方案。
...
Method: LinkedOut从原始视频帧中提取语义基础、知识感知的token，使用可提示查询和可选辅助模态引导VLLM。引入跨层知识融合MoE（混合专家），从丰富的VLLM特征中选择适当的抽象层次，实现个性化、可解释、低延迟的推荐。

Result: LinkedOut在标准基准测试中取得了最先进的结果，是第一个无需手工标注、直接在原始帧上操作的VLLM视频推荐方法。可解释性研究和消融实验证实了层多样性和分层融合的好处。

Conclusion: LinkedOut为充分利用VLLM世界知识先验和视觉推理进行下游视觉任务（如推荐）提供了一条实用路径，通过提取知识感知表示解决了VLLM部署的实际限制。
...
Method: 1) 使用现成提取器计算身份无关的面部表情特征；2) 引入归一化面部表情块，通过均值和方差归一化对齐面部特征与扩散潜变量；3) 采用动态滑动窗口方案和重叠区域加权混合确保平滑过渡；4) 基于特定时间步的潜变量变化率和扩散层间导数幅度比，使用高阶潜变量导数直接预测未来时间步的潜变量，跳过多个去噪步骤。

Result: 在基准测试中，FlashPortrait在质量和数量上都显示出有效性，能够合成身份保持的无限长度视频，同时实现高达6倍的推理加速。

Conclusion: FlashPortrait通过创新的归一化面部特征对齐、动态滑动窗口和高阶潜变量预测方法，成功解决了长肖像动画中身份一致性和推理速度的挑战。
...
Method: 1. 引入基于VLM的指导器，将文本指令、源视频第一帧和可选参考图像编码为视觉基础指令表示；2. 提出后训练阶段Edit-GRPO，将组相对策略优化应用于视频编辑领域；3. 设计数据构建管道，合成生成多样高保真度的基础编辑操作配对视频指令数据。

Result: 大量实验表明，VIVA在指令遵循、泛化能力和编辑质量方面优于最先进的方法。

Conclusion: VIVA通过VLM引导编码和奖励优化，解决了基于指令的视频编辑中的泛化差距问题，实现了对复杂真实世界指令的更好处理。
...
TL;DR: 提出SceneDiff Benchmark（首个多视角变化检测基准）和SceneDiff方法（无需训练的跨视角物体变化检测方法），在多个基准上大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决跨视角场景中物体变化检测问题，这对机器人整理、施工进度监控等应用很重要。主要挑战是视角变化会导致物体被错误检测为变化。

Method: 提出无需训练的SceneDiff方法：1）在3D中对齐捕获场景；2）提取物体区域；3）比较空间和语义区域特征来检测变化。利用预训练的3D、分割和图像编码模型。

Result: 在多视角和双视角基准测试中，方法大幅超越现有方法（相对AP提升94%和37.4%）。创建了包含350个视频对、数千个变化物体的SceneDiff Benchmark。

Conclusion: 提出的SceneDiff Benchmark和方法为多视角物体变化检测提供了新基准和有效解决方案，在多个测试中表现优异，代码和基准将公开。
...
Result: 在每张图像仅使用64个令牌的高压缩率下，SFTok在ImageNet上实现了最先进的图像重建质量（rFID = 1.21），在类别到图像生成任务中表现出色（gFID = 2.29）。

Conclusion: SFTok通过创新的多步迭代机制和训练策略，显著提升了离散分词器的图像重建能力，为高分辨率图像生成提供了高效且高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 立体显示设备快速增长，但3D视频制作成本高、复杂；传统多阶段深度-扭曲-修复方法存在误差传播、深度歧义和立体格式不一致问题

Method: 1) 创建UniStereo统一数据集支持两种立体格式；2) 提出StereoPilot前馈模型，无需显式深度图或迭代扩散采样；3) 使用可学习域切换器和循环一致性损失适应不同立体格式

Result: StereoPilot在视觉保真度和计算效率方面显著优于现有最先进方法

Conclusion: 提出的统一数据集和直接合成方法有效解决了单目到立体视频转换的关键挑战，为高质量立体内容生成提供了新方案
...
<details>
  <summary>Details</summary>
Motivation: 现有开源多模态大语言模型存在盲目使用视觉工具的问题，即使不需要工具也会调用，这显著增加了推理开销并降低了模型性能。需要开发能够自适应判断何时真正需要视觉工具的系统。

Method: 提出AdaTooler-V模型，采用AT-GRPO强化学习算法，根据每个样本的工具效益分数自适应调整奖励尺度，鼓励模型仅在工具能真正提供改进时才调用。构建了两个训练数据集：AdaTooler-V-CoT-100k用于SFT冷启动，AdaTooler-V-300k用于带可验证奖励的强化学习。

Result: 在12个基准测试上的实验表明，AdaTooler-V具有强大的推理能力，在多样化的视觉推理任务中优于现有方法。特别是AdaTooler-V-7B在高分辨率基准V*上达到89.8%的准确率，超过了商业专有模型GPT-4o和Gemini 1.5 Pro。

Conclusion: AdaTooler-V通过自适应工具使用机制有效解决了现有模型盲目调用视觉工具的问题，显著提升了推理效率和性能，在多个视觉推理任务上达到了最先进的性能水平。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像编辑技术发展迅速，但视频编辑仍面临一致性、控制和泛化方面的挑战。研究者旨在探索数据、架构和控制的设计空间，开发一个简单有效的指令式视频编辑框架。

Method: 在数据方面：1) 利用现有专家模型和快速反演构建多样化视频对；2) 通过单帧监督和共享仿射运动的伪对将图像编辑对提升为视频；3) 挖掘密集标注的视频片段；4) 添加过渡监督来教授编辑如何展开。在模型方面：发现预训练文本到视频模型已具备编辑能力，采用简单的序列连接条件和轻量LoRA微调。在控制方面：通过单一掩码机制统一时空控制，并支持可选参考图像。

Result: EasyV2V在视频编辑方面实现了最先进的结果，超越了同期研究和商业系统。框架支持多种灵活输入：视频+文本、视频+掩码+文本、视频+掩码+参考图像+文本。

Conclusion: EasyV2V通过系统性的数据、架构和控制设计，提供了一个简单而强大的指令式视频编辑框架，在保持视频一致性的同时实现了灵活的控制和良好的泛化能力。
...
TL;DR: AuditDM是一个自动化框架，通过强化学习训练MLLM作为审计器，主动发现和修复多模态大语言模型的失败模式，生成挑战性问题和反事实图像来揭示模型弱点。
...
Method: 通过强化学习微调MLLM作为审计器，生成最大化目标模型分歧的挑战性问题和反事实图像，从而发现多样化的失败模式。

Result: 在Gemma-3和PaliGemma-2等先进模型上发现了20多种不同的失败类型，基于这些发现进行微调后，所有模型在16个基准测试中均得到提升，3B模型甚至超越了28B模型。

Conclusion: 随着数据扩展收益递减，有针对性的模型审计为模型诊断和改进提供了有效途径，AuditDM框架能够主动发现和修复MLLM的失败模式。
...
TL;DR: NEPA是一种视觉自监督学习方法，通过预测未来图像块嵌入来训练模型，无需像素重建、离散标记或对比损失，在ImageNet和ADE20K上取得良好效果。
...
TL;DR: 提出Generative Refocusing方法，通过两步流程实现单图像重聚焦：先用DeblurNet恢复全焦图像，再用BokehNet生成可控散景，采用半监督训练结合合成数据和真实散景图像。


<details>
  <summary>Details</summary>
Motivation: 景深控制在摄影中很重要，但获得完美焦点通常需要多次尝试或特殊设备。现有单图像重聚焦方法存在显著缺陷：需要全焦输入、依赖模拟器合成数据、光圈控制有限。

Method: 提出两阶段生成式重聚焦方法：1) DeblurNet从各种输入恢复全焦图像；2) BokehNet生成可控散景。主要创新是半监督训练，结合合成配对数据和未配对的真实散景图像，利用EXIF元数据捕捉真实光学特性。

Result: 实验表明，在散焦去模糊、散景合成和重聚焦基准测试中取得最佳性能。此外，该方法支持文本引导调整和自定义光圈形状。

Conclusion: Generative Refocusing方法通过创新的半监督训练策略，有效解决了单图像重聚焦的挑战，实现了高质量的散景控制和灵活的重聚焦功能。
...
TL;DR: WorldCanvas是一个多模态框架，通过结合文本、轨迹和参考图像实现可提示的世界事件生成，支持用户导向的丰富模拟。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：纯文本方法表达能力有限，现有轨迹控制的图像到视频方法无法充分表达复杂世界事件。需要一种能够生成连贯、可控事件（包括多智能体交互、对象进出、参考引导外观和反直觉事件）的框架。

Method: 采用多模态方法，结合轨迹（编码运动、时序和可见性）、自然语言（语义意图）和参考图像（对象身份的视觉基础）。通过这种组合实现连贯可控的事件生成。

Result: 生成的视频不仅具有时间连贯性，还表现出涌现一致性，能够在对象暂时消失后保持对象身份和场景一致性。支持表达性世界事件生成。

Conclusion: WorldCanvas将世界模型从被动预测器推进为交互式、用户可塑造的模拟器，实现了丰富、用户导向的世界事件模拟。
...
### [22] [ReinforceGen: Hybrid Skill Policies with Automated Data Generation and Reinforcement Learning](https://arxiv.org/abs/2512.16861)
...
Method: 1. 将任务分解为多个局部技能，通过运动规划连接；2. 使用10个人类演示生成数据集；3. 通过模仿学习训练技能和运动规划目标；4. 通过在线适应和强化学习进行微调
...
### [23] [PolaRiS: Scalable Real-to-Sim Evaluations for Generalist Robot Policies](https://arxiv.org/abs/2512.16881)
...
<details>
  <summary>Details</summary>
Motivation: 机器人学习研究中准确测量和比较策略性能面临挑战，真实世界评估存在随机性、可重复性差和耗时问题，而现有仿真基准与真实世界存在视觉和物理领域差距，无法可靠指导策略改进。

Method: 提出PolaRiS框架：1) 使用神经重建方法将真实场景的短视频扫描转换为交互式仿真环境；2) 开发仿真数据协同训练方法，弥合剩余的真实到仿真差距，实现未见仿真环境的零样本评估。

Result: 通过仿真与真实世界的广泛配对评估，证明PolaRiS评估与真实世界通用策略性能的相关性远强于现有仿真基准，且能够快速创建多样化的仿真环境。

Conclusion: PolaRiS为下一代机器人基础模型提供了分布式和民主化评估的重要一步，通过高保真仿真评估解决了机器人策略性能测量的关键挑战。
...
### [24] [Sceniris: A Fast Procedural Scene Generation Framework](https://arxiv.org/abs/2512.16896)
*Jinghuan Shang,Harsh Patel,Ran Gong,Karl Schmeckpeper*

Main category: cs.RO

TL;DR: Sceniris是一个高效的3D场景生成框架，相比现有方法实现了234倍以上的加速，支持大规模无碰撞场景生成和机器人可达性检查。


<details>
  <summary>Details</summary>
Motivation: 现有程序化生成方法输出吞吐量低，成为扩展数据集创建的主要瓶颈，需要高效的大规模场景生成框架。

Method: 通过批处理采样和cuRobo的快速碰撞检查优化性能，扩展对象间空间关系支持多样化场景需求，提供可选的机器人可达性检查。

Result: 相比Scene Synthesizer方法实现了至少234倍的加速，能够快速生成大规模无碰撞场景变化。

Conclusion: Sceniris为物理AI和生成模型提供了高效的程序化场景生成框架，解决了现有方法的性能限制问题。
