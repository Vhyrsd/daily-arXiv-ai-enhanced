...
TL;DR: SDSNet是一个可扩展的深度子空间聚类框架，通过地标点近似、自编码器重建与自表达目标的联合优化，以及因子化表示上的直接谱聚类，将复杂度从O(n³)降低到O(n)。


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类方法面临可扩展性限制，因为构建完整的n×n亲和度矩阵和进行谱分解需要O(n³)的计算成本。尽管深度学习方法改进了特征提取，但它们仍然通过穷举的成对相似性计算保持这一计算瓶颈。

Method: 提出SDSNet框架，采用三种关键技术：(1)基于地标点的近似方法，避免构建完整的亲和度矩阵；(2)自编码器重建与自表达目标的联合优化；(3)在因子化表示上直接进行谱聚类。该框架结合了卷积自编码器与子空间保持约束。

Result: 实验结果表明，SDSNet在保持与最先进方法相当的聚类质量的同时，显著提高了计算效率。

Conclusion: SDSNet通过创新的可扩展架构解决了子空间聚类的计算瓶颈问题，为大规模数据集上的子空间聚类提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 异常检测在工业场景中很重要，但现有方法各有缺陷：无监督表示学习方法在领域偏移下特征提取不鲁棒，重构方法训练效率低且因约束不足导致性能下降

Method: 提出CCAD方法，将全局特征作为新模态条件用于重构模型，并设计自适应压缩机制提升泛化能力和训练效率

Result: 大量实验表明CCAD在AUC指标上持续优于最先进方法，同时实现更快收敛；贡献了重新组织和标注的DAGM 2007数据集

Conclusion: CCAD有效结合了两种范式的优势，在异常检测任务中表现出优越性能和效率
...
Method: 从ISIC档案库收集皮肤镜图像，构建包含17,684个分割掩码的数据集，涵盖14,967张图像，其中2,394张图像有多个标注。数据集包含标注者的技能水平和分割工具等元数据。

Result: 创建了ISIC MultiAnnot++数据集，这是目前最大的公开多标注者皮肤病变分割数据集，包含详细元数据，支持标注者特定偏好建模和元数据分析等研究。

Conclusion: ISIC MultiAnnot++填补了多标注者皮肤病变分割数据集的空白，为研究标注者特定偏好建模和元数据分析提供了重要资源，推动了医学图像分割领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有多焦点图像融合方法存在两个主要局限：1) 假设场景中每个空间位置至少在一张输入图像中是清晰的；2) 在复杂现实场景中容易产生边缘伪影。这些限制影响了融合质量，特别是在复杂多焦点内容场景中。

Method: GMFF采用两阶段级联框架：第一阶段使用StackMFF V4进行确定性融合，整合可用焦平面信息生成初始融合图像；第二阶段通过IFControlNet实现生成式修复，利用潜在扩散模型的生成能力重建缺失焦平面内容、恢复细节并消除边缘伪影。

Result: 大量实验表明，GMFF实现了最先进的融合性能，在复杂多焦点内容场景中表现出显著优势，具有重要的实际应用潜力。

Conclusion: GMFF通过结合确定性融合和生成式修复，有效解决了多焦点图像融合中的焦点缺失和边缘伪影问题，为复杂场景下的图像融合提供了创新解决方案。
...
TL;DR: 论文提出了首个视频生成中的社会推理基准，基于发展心理学和社会心理学的30个经典范式，评估了7个核心社会认知维度，发现当前最先进的视频生成模型在表面合理性上表现良好，但在意图识别、信念推理、联合注意和亲社会推理等深层社会推理方面存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成模型在视觉真实性、运动保真度和文本-视频对齐方面取得了显著进展，但在生成社会一致行为方面存在根本性限制。与人类能够轻松从简短视觉线索推断意图、信念、情感和社会规范不同，现有模型倾向于渲染字面场景而无法捕捉潜在的因果或心理逻辑。

Method: 基于发展心理学和社会心理学的研究成果，将30个经典社会认知范式组织成7个核心维度。开发了完全无需训练的基于智能体的流程：1) 提炼每个实验的推理机制；2) 合成多样化的视频就绪场景；3) 通过基于线索的批判强制执行概念中立性和难度控制；4) 使用高容量视觉语言模型(VLM)法官在五个可解释的社会推理维度上评估生成的视频。

Result: 对7个最先进的视频生成系统进行了首次大规模研究，结果显示存在显著的性能差距：现代模型在表面合理性方面表现出色，但在意图识别、信念推理、联合注意和亲社会推理方面系统性失败。

Conclusion: 该研究揭示了当前视频生成模型在社会推理能力方面的根本性限制，为未来开发具有更深入社会认知能力的视频生成系统提供了重要的评估基准和方向指导。
...
TL;DR: 研究比较了多种参数高效训练策略在胸部X光多标签分类任务上的表现，发现冻结编码器等方法在仅使用2.51%参数的情况下，性能优于全微调，但校准性能较差。


<details>
  <summary>Details</summary>
Motivation: 多模态胸部X光分析通常需要微调大型视觉语言模型，计算成本高昂。本研究旨在探索参数高效训练策略，在保持性能的同时大幅降低计算需求。

Method: 使用印第安纳大学胸部X光数据集（3,851个图像-报告对），研究冻结编码器、BitFit、LoRA和适配器等参数高效训练策略。为防数据泄露，从文本输入中删除病理术语但保留临床上下文。在固定参数预算（2.37M参数，占总参数2.51%）下比较性能，并在CheXpert数据集上进行外部验证。

Result: 所有参数高效训练变体在AUROC 0.892-0.908范围内，优于全微调（0.770 AUROC），参数减少40倍。外部验证显示所有方法在<9%可训练参数下达到>0.69 AUROC，适配器表现最佳（0.7214 AUROC）。预算匹配比较显示纯视觉模型（0.653 AUROC）优于预算匹配多模态模型（0.641 AUROC），表明改进主要来自参数分配而非跨模态协同。但参数高效方法校准性能较差（ECE: 0.29-0.34）。

Conclusion: 冻结编码器策略在显著降低计算成本的同时提供优越的判别性能，但校准性能较差，需要通过后处理校准方法解决。多模态模型的改进主要源于参数分配而非跨模态协同效应。
...
TL;DR: 该研究针对AI生成图像检测器在实际部署中的鲁棒性问题，提出固定阈值评估协议，避免传统方法通过重新调参人为夸大鲁棒性估计，揭示了不同架构在照片与艺术内容上的性能差异。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法在评估鲁棒性时存在误导性，通常针对每种后处理条件重新调整决策阈值，这人为夸大了鲁棒性估计，掩盖了实际部署中的性能下降问题。

Method: 引入固定阈值评估协议：在干净验证数据上选择一次决策阈值，并在所有后处理变换中保持固定。采用轻量级CNN-ViT混合架构，结合门控融合和可选频率增强，在三个操作点（低误报率、ROC最优、最佳F1）下进行系统退化测试。

Result: 揭示了法医-语义频谱：频率增强CNN在原始照片上表现优异（93.33%），但在压缩下崩溃（61.49%）；ViT通过语义模式识别降解最小（92.86%到88.36%）。所有架构在艺术内容上的AUROC比照片高15%（0.901-0.907 vs 0.747-0.759）。混合方法在跨域任务中表现均衡：tiny-genimage照片91.4%，AiArtData艺术/图形89.7%，CIFAKE 98.3%。

Conclusion: 固定阈值评估消除了重新调参带来的夸大效应，揭示了真实的鲁棒性差距，并为实际部署提供了指导：干净照片验证优选CNN，压缩内容优选ViT，艺术/图形筛查优选混合方法。语义模式比法医伪影提供更可靠的检测线索。
...
TL;DR: MuS-Polar3D是首个公开的水下偏振3D成像基准数据集，包含42个物体在7种散射条件和5个视角下的偏振图像，支持多种视觉任务，并提出两阶段水下3D重建方法。


<details>
  <summary>Details</summary>
Motivation: 现有水下偏振3D成像方法缺乏多样化的公开数据集，散射和观测条件不足，阻碍了不同方法（包括单视角和多视角偏振成像）的公平比较。

Method: 构建MuS-Polar3D基准数据集，包含42个物体在7种定量控制散射条件和5个视角下的偏振图像，以及高精度3D模型、法线图和前景掩码。从成像链角度提出两阶段水下3D重建流程：先散射抑制，后3D重建。

Result: 数据集支持多种视觉任务，包括法线估计、物体分割、散射抑制和3D重建。在复杂散射条件下使用多种基线方法进行评估，最佳平均角度误差达到15.49度。
...
TL;DR: 提出一种改进GRPO强化学习图像生成的方法，通过分布级创造力奖励和结构感知正则化，解决后期训练中输出同质化问题，在保持质量的同时提升13-18%的语义多样性。


<details>
  <summary>Details</summary>
Motivation: 传统GRPO在训练后期会产生同质化输出，缺乏创造性和视觉多样性，限制了应用场景。问题源于两方面：1）传统GRPO依赖单样本质量作为奖励信号，导致模型收敛于少数高奖励生成模式而忽视分布级多样性；2）传统正则化忽视了早期去噪阶段对保持多样性的主导作用，导致正则化预算错配，限制了质量-多样性权衡。

Method: 从奖励建模和生成动态两个层面改进：1）奖励层面：提出基于语义分组的分布级创造力奖励，通过对同一标题生成的样本进行谱聚类构建分布级表示，根据组大小自适应分配探索奖励以鼓励发现新的视觉模式；2）生成层面：引入结构感知正则化，加强早期阶段约束以保持多样性，同时不影响奖励优化效率。

Result: 实验表明，该方法在匹配质量分数下实现了13%-18%的语义多样性提升，为基于GRPO的图像生成建立了新的质量与多样性帕累托前沿。

Conclusion: 通过从奖励建模和生成动态两个角度重新审视多样性退化问题，提出的分布级创造力奖励和结构感知正则化方法有效解决了GRPO训练后期的同质化问题，在保持图像质量的同时显著提升了生成多样性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通过大规模图像文本预训练学习强大的多模态表示，但将其适应到层次分类任务中尚未充分探索。标准方法将标签视为扁平类别，需要完全微调，成本高昂且在不同分类层级产生不一致的预测。
...
Conclusion: 该方法为将视觉语言模型适应到结构化分类体系提供了一种高效策略，通过少量参数更新实现层次结构一致性。
...
<details>
  <summary>Details</summary>
Motivation: 自注意力机制在视觉Transformer中至关重要，但其二次复杂度在高分辨率场景下带来沉重计算负担，限制了实际应用。现有方法通过引入手工设计的局部性或稀疏性模式来缓解这一问题，但不可避免地会损害模型能力。

Method: 提出循环注意力机制，首先识别出视觉Transformer中自注意力矩阵经常近似于具有循环块的块循环矩阵(BCCB)。利用这种结构化矩阵特性，将注意力图显式建模为其最近的BCCB矩阵，并提出高效计算算法进行快速计算。

Result: 该方法实现了O(NlogN)计算复杂度，同时很大程度上保持了标准自注意力的能力。在多种视觉任务上的广泛实验证明了该方法的有效性。

Conclusion: 循环注意力机制作为视觉Transformer架构中自注意力的有前景替代方案，既保持了高效计算复杂度，又维持了模型能力。
...
TL;DR: EraseLoRA是一个无需训练数据集的物体移除框架，通过背景感知推理和测试时适应，解决了现有方法中非目标前景被误判为背景导致物体重现的问题，以及注意力操作破坏细节的问题。


<details>
  <summary>Details</summary>
Motivation: 物体移除与普通修复不同，需要防止被掩码的目标重新出现，并以结构和上下文保真度重建被遮挡的背景。现有无数据集方法通过重定向掩码内的自注意力存在两个问题：非目标前景常被误判为背景导致不需要的物体重新生成，以及直接的注意力操作破坏细节并阻碍背景线索的连贯整合。
...
Result: EraseLoRA作为预训练扩散模型的插件，在物体移除基准测试中显示出相对于无数据集基线的持续改进，并与数据集驱动方法相比具有竞争力的结果。

Conclusion: EraseLoRA通过背景感知推理和测试时适应，无需数据集即可有效解决物体移除中的关键挑战，避免了注意力操作带来的问题，实现了更好的背景重建效果。
...
### [14] [SymDrive: Realistic and Controllable Driving Simulator via Symmetric Auto-regressive Online Restoration](https://arxiv.org/abs/2512.21618)
*Zhiyuan Liu,Daocheng Fu,Pinlong Cai,Lening Wang,Ying Liu,Yilong Ren,Botian Shi,Jianqiang Wang*

Main category: cs.CV

TL;DR: SymDrive：基于扩散模型的自动驾驶3D仿真框架，同时实现高质量渲染和场景编辑，解决大角度新视角合成与资产操作中的几何/光照问题


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶3D仿真方法难以同时实现逼真渲染和交互式交通编辑，在大角度新视角合成中存在困难，资产操作时容易出现几何或光照伪影

Method: 提出对称自回归在线恢复范式，构建配对对称视图通过地面真值引导的双视图公式恢复细节；采用自回归策略生成一致侧视图；利用无训练协调机制将车辆插入视为上下文感知修复

Result: 实验表明SymDrive在新视角增强和逼真3D车辆插入方面均达到最先进性能
...
Method: 引入了两个新任务：1）上下文感知对象插入，需要预测合适的对象类别、生成对象并合理放置在场景中；2）赞助商产品logo增强，涉及检测产品并插入正确的品牌logo，即使物品未标注品牌或标注错误。为此构建了两个带有类别标注、放置区域和赞助商产品标签的新数据集。

Result: 论文提出了两个新的图像编辑任务框架，并创建了相应的数据集来支持这些任务的开发和评估。

Conclusion: 该研究为广告和数字媒体领域的智能图像编辑提供了新的任务定义和数据集支持，强调上下文适当性和品牌准确性的重要性。
...
TL;DR: 该研究分析了AIGC图像检测技术面临的可复现性和泛化性不足两大核心问题，通过复现代表性方法揭示了问题根源：论文细节缺失和检测方法过度拟合特定生成器特征。


<details>
  <summary>Details</summary>
Motivation: AIGC图像检测技术虽然发展迅速，但在实际应用中面临两大核心挑战：1）可复现性差，难以重现论文结果；2）泛化性不足，检测方法对特定生成器过拟合。这些问题阻碍了技术的实际应用。

Method: 研究方法包括：1）系统回顾7篇AIGC检测关键论文；2）构建轻量级测试数据集；3）复现代表性检测方法；4）分析可复现性问题的根本原因。

Result: 实验结果表明：1）严格遵循论文核心流程可复现基本性能；2）预处理破坏关键特征时检测性能急剧下降；3）跨生成器测试时泛化能力严重不足；4）问题根源在于论文细节缺失和方法过度拟合特定生成器特征。

Conclusion: 该研究为改进AIGC检测技术的可复现性提供了实证证据，建议研究者更全面地披露实验细节，并验证方法的泛化能力，以推动该领域的实际应用。
...
TL;DR: LAid是一种通过知识蒸馏将大模型的长程注意力机制转移到小模型的方法，使小模型的有效上下文窗口扩大3.2倍，同时保持视觉语言基准性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的小分支由于窗口尺寸限制，在语言-图像对齐方面表现不佳。虽然大模型具有强大的长上下文理解能力，但如何将这种能力有效转移到小模型中是一个关键问题
...
Conclusion: LAid不仅为构建更高效的长上下文视觉语言模型提供了实用技术，还提供了关于位置理解在蒸馏过程中如何出现和转移的理论见解，证明了通过知识蒸馏可以有效转移长程注意力机制
...
TL;DR: 提出Pearl：一种无需LLM的监督式图像描述评估指标，适用于有参考和无参考两种设置，通过新颖的图像-描述和描述-描述相似性表示学习机制，在多个数据集上超越现有LLM-free指标。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型（LLM）的图像描述评估指标存在偏向自身生成内容的问题，中立性存疑；而大多数非LLM指标虽然无此问题，但性能表现不够理想。需要开发既保持中立性又具有高性能的评估指标。

Method: 提出Pearl指标，采用监督学习方法，引入新颖的表示学习机制来学习图像-描述和描述-描述之间的相似性表示。同时构建了大规模人工标注数据集，包含约333k个人类判断，来自2,360名标注者对超过75k张图像的评估。
...
TL;DR: UltraLBM-UNet：一种轻量级皮肤病变分割模型，结合双向Mamba全局建模与多分支局部特征感知，参数仅0.034M，在ISIC等数据集上达到SOTA性能，并可通过知识蒸馏进一步压缩。


<details>
  <summary>Details</summary>
Motivation: 现有皮肤病变分割方法存在性能不足和计算复杂度高的问题，难以在临床护理点部署。需要开发既准确又资源高效的轻量级模型。

Method: 提出UltraLBM-UNet，一种轻量级U-Net变体，集成双向Mamba全局建模机制和多分支局部特征感知。采用高效局部特征注入与双向状态空间建模，实现空间维度上的丰富上下文交互。还引入混合知识蒸馏策略训练超紧凑学生模型UltraLBM-UNet-T。

Result: 在ISIC 2017、ISIC 2018和PH2数据集上，UltraLBM-UNet仅用0.034M参数和0.060 GFLOPs就达到最先进的分割精度。蒸馏后的UltraLBM-UNet-T仅0.011M参数和0.019 GFLOPs，仍保持有竞争力的性能。

Conclusion: UltraLBM-UNet在保持高精度的同时实现了极低的计算复杂度，特别适合临床护理点部署，为准确、鲁棒的皮肤病变分析提供了实用解决方案。
...
TL;DR: GaussianEM是一个高斯伪原子框架，用于从冷冻电镜图像中同时建模组成和构象异质性，通过双编码器-单解码器架构将图像映射到高斯组件，提供直观的构象变化描述。
...
Method: GaussianEM采用高斯伪原子框架，使用双编码器-单解码器架构将冷冻电镜图像映射到其各个高斯组件，通过高斯参数的变化来表示结构变异性。

Result: 该方法提供了构象变化的直观可解释描述，保持了沿转变轨迹的局部结构一致性，并自然弥合了基于密度的模型与相应原子模型之间的差距。在模拟和实验数据集上都证明了其有效性。

Conclusion: GaussianEM是一个有效的框架，能够从冷冻电镜图像中同时建模组成和构象异质性，为理解蛋白质动力学提供了新的分析工具。
...
TL;DR: LCMP是首个长上下文MLLM个性化评估基准，用于评估MLLM在长对话中感知个性化概念变化并生成合适响应的能力。TAME作为基线框架，采用双记忆机制和检索对齐增强生成范式，在LCMP上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM个性化方法主要关注简单的视觉识别和文本替换，缺乏对长上下文对话的支持。理想的个性化MLLM助手应该能够进行长上下文对话，并通过学习历史对话持续提升体验质量。

Method: 提出了LCMP评估基准，并引入TAME框架作为强基线。TAME采用训练免费和状态感知的设计，包含双记忆机制（分别管理临时和持久变化）以及检索对齐增强生成（RA2G）范式，通过对齐步骤从多记忆检索的知识中提取与当前问题相关的信息。

Result: 在LCMP基准上的实验表明，TAME取得了最佳性能，在长上下文场景中展现出卓越且不断演进的交互体验。

Conclusion: LCMP填补了长上下文MLLM个性化评估的空白，TAME框架通过创新的双记忆机制和RA2G范式，为MLLM个性化提供了有效的解决方案，能够处理复杂现实世界用户查询并实现持续改进的交互体验。
...
<details>
  <summary>Details</summary>
Motivation: 现有少样本细粒度视觉分类方法通常通过丰富特征表示来提高分类精度，但忽视了少样本支持集作为混杂变量会引入偏差数据分布，误导判别性特征的提取，从而影响分类性能。

Method: 提出CausalFSFG方法，基于结构因果模型分析输入与子类别间的因果关系。包含两个关键组件：1) 干预多尺度编码器(IMSE)进行样本级干预；2) 干预掩码特征重建(IMFR)进行特征级干预，共同揭示从输入到子类别的真实因果关系。

Result: 在CUB-200-2011、Stanford Dogs和Stanford Cars等广泛使用的公开数据集上进行大量实验和深入分析，证明CausalFSFG达到了新的最先进性能。

Conclusion: 通过因果干预方法有效解决了少样本细粒度视觉分类中由支持集引入的偏差分布问题，提出的CausalFSFG方法在多个基准数据集上表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 文本驱动图像编辑中常出现属性纠缠问题，即修改目标属性（如添加刘海）会无意中改变其他语义属性（如身份或外观）。PPE框架虽然尝试解决这一问题，但其正则化策略存在局限性，潜在空间更新仍然密集且容易发生语义泄漏

Method: 1. 分析PPE框架的架构组件，包括基于BERT的属性预测和基于StyleGAN2的图像生成；2. 识别原始正则化策略的局限性；3. 引入基于稀疏性的约束，在潜在空间操作上使用L1正则化；4. 在CelebA-HQ数据集上进行实验验证
...
TL;DR: TrackTeller：一个用于动态3D驾驶场景中基于时间语言的多模态3D目标定位框架，通过整合LiDAR-图像融合、语言条件解码和时间推理，显著提升了语言引导的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 在动态3D驾驶场景中，许多指代表达通过最近的运动或短期交互来描述目标，仅从静态外观或几何特征无法解决。需要研究时间语言基础的3D定位问题，利用多帧观察来识别当前帧中的被指代对象。

Method: 提出TrackTeller框架，整合LiDAR-图像融合、语言条件解码和时间推理。构建与文本语义对齐的共享UniScene表示，生成语言感知的3D提议，并使用运动历史和短期动态细化定位决策。

Result: 在NuPrompt基准测试中，TrackTeller持续改进语言基础跟踪性能，相比强基线，平均多目标跟踪准确率相对提升70%，误报频率降低3.15-3.4倍。

Conclusion: TrackTeller通过统一架构有效解决了动态3D驾驶场景中的时间语言基础定位问题，显著提升了语言引导的3D目标跟踪性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和深度学习算法在自动驾驶领域的广泛应用，需要评估不同模型在实际应用中的性能表现。自动驾驶车辆依赖感知系统来检测、定位和识别其他车辆、行人和道路标志，以确保安全导航和决策。因此，比较新兴深度学习模型在真实场景下的性能对研究社区具有重要意义。

Method: 研究采用自定义数据集，对YOLO-NAS和YOLOv8两种深度学习模型进行对比实验。具体使用YOLOv8s模型与YOLO-NAS模型进行比较，重点关注目标检测任务的性能评估。

Result: 实验结果显示：1）YOLOv8s模型相比YOLO-NAS模型节省了75%的训练时间；2）在目标检测精度方面，YOLOv8s模型达到83%的准确率，优于YOLO-NAS模型的81%。

Conclusion: YOLOv8s模型在训练效率和检测精度方面均优于YOLO-NAS模型，为自动驾驶感知系统的深度学习模型选择提供了实证依据。这项比较分析有助于研究社区理解这些新兴模型在真实应用场景中的性能表现。
...
TL;DR: UniPercept-Bench是一个统一的感知级图像理解框架，涵盖美学、质量、结构和纹理四个关键领域，通过层次化定义系统和数据集构建，开发了基于领域自适应预训练和任务对齐强化学习的基线模型，在视觉评分和视觉问答任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉理解任务上取得了显著进展，但它们在感知级图像特征理解方面仍然有限。本文旨在填补这一空白，定义MLLM时代的感知级图像理解，并提供全面的基准和强基线。

Method: 1. 建立层次化定义系统，涵盖美学、质量、结构和纹理四个感知领域；2. 构建大规模数据集用于评估感知级图像理解；3. 开发UniPercept基线模型，采用领域自适应预训练和任务对齐强化学习方法。

Result: UniPercept在感知级图像理解方面超越了现有的MLLMs，在视觉评分和视觉问答任务上表现出强大的泛化能力，并且可以作为即插即用的奖励模型用于文本到图像生成。

Conclusion: 本文定义了MLLM时代的感知级图像理解，通过引入全面的基准和强基线，为推进感知级多模态图像理解奠定了坚实基础。
...
TL;DR: SlideChain是一个基于区块链的溯源框架，用于验证多模态语义提取的完整性，特别针对教育内容中的视觉-语言模型输出，通过区块链锚定加密哈希确保不可篡改的可审计性。


<details>
  <summary>Details</summary>
Motivation: 现代视觉-语言模型在教育内容解释和生成中应用日益广泛，但其语义输出在验证、重现和长期审计方面存在挑战。不同模型家族、推理设置和计算环境之间的不一致性影响了AI生成教学材料的可靠性，特别是在高风险和定量STEM领域。

Method: SlideChain是一个区块链支持的溯源框架，使用SlideChain Slides Dataset（包含1,117个医学影像讲座幻灯片），从四个最先进的VLM中提取概念和关系三元组，为每个幻灯片构建结构化溯源记录，并将这些记录的加密哈希锚定在本地EVM兼容区块链上。

Result: 研究发现多模型之间存在显著的语义差异，包括低概念重叠和许多幻灯片上关系三元组的接近零一致性。同时评估了模拟部署条件下的gas使用、吞吐量和可扩展性，展示了完美的篡改检测和跨独立提取运行的确定性可重现性。

Conclusion: SlideChain为实现可信、可验证的多模态教育流程提供了实用且可扩展的解决方案，支持AI辅助教学系统的长期可审计性、可重现性和完整性。
...
TL;DR: VGGT的全局自注意力层在处理超过几百帧的输入序列时会出现严重的崩溃现象：注意力矩阵迅速变为近秩一，token几何退化到几乎一维子空间，重建误差超线性累积。本文通过将全局注意力迭代视为退化扩散过程，建立了崩溃现象的严格数学解释。


<details>
  <summary>Details</summary>
Motivation: VGGT在3D重建方面达到了最先进的水平，但其全局自注意力层在处理长序列输入时会出现严重的崩溃现象。这种现象表现为注意力矩阵退化、token几何退化以及重建误差累积，需要从理论上解释这一现象的根本原因。

Method: 将全局注意力迭代视为退化扩散过程，建立数学模型。证明在VGGT中，token特征流以O(1/L)的速率收敛到Dirac型测度，得到闭式平均场偏微分方程，该方程能精确预测经验观察到的秩分布。

Result: 理论定量匹配了注意力热图演化和相关工作中报告的一系列实验结果。解释了token合并补救措施（定期移除冗余token）通过降低有效扩散系数来延迟崩溃的原因，而无需额外训练。

Conclusion: 该分析为解释未来可扩展的3D视觉Transformer提供了原则性视角，并强调了其在多模态泛化方面的潜力。理论框架能够准确预测和解释VGGT中的崩溃现象。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF方法难以准确建模各向异性镜面表面（如金属拉丝表面），限制了文化遗产数字化中高真实感3D重建的质量。
...
Conclusion: ShinyNeRF成功解决了NeRF在建模各向异性反射表面的局限性，为文化遗产数字化提供了更高质量的3D表示工具。
...
TL;DR: 本文提出了一种孟加拉语手写文本生成方法，使用自收集数据集解决了该领域数据稀缺的问题，能够从纯文本输入生成多样化的手写输出。


<details>
  <summary>Details</summary>
Motivation: 手写文本生成是一个新兴领域，但孟加拉语作为世界第五大语言，其手写文本生成研究却很少受到关注。主要挑战在于个体手写风格差异大，且缺乏大规模多样的数据集。

Method: 开发并使用了自收集的孟加拉语手写样本数据集，包含约500名不同年龄和性别的个体贡献。所有图像都经过预处理以确保一致性和质量，然后基于此数据集提出孟加拉语手写词生成方法。

Result: 该方法能够从输入的纯文本生成多样化的手写输出，展示了生成现实手写文本的能力。

Conclusion: 这项工作推动了孟加拉语手写生成领域的发展，能够支持该领域的进一步研究，为世界第五大语言的手写文本生成提供了重要贡献。
...
TL;DR: FUSE是一个用于检测AI生成图像的混合系统，结合了频谱特征和语义特征，通过两阶段训练实现跨多种生成器的强泛化能力。
...
Method: FUSE系统通过快速傅里叶变换提取频谱特征，结合CLIP视觉编码器提取的语义特征，将两种特征融合为联合表示，并采用两阶段渐进式训练方法。

Result: 在GenImage、WildFake、DiTFake、GPT-ImgEval和Chameleon数据集上评估显示：FUSE（第一阶段）在Chameleon基准测试中达到最先进水平，在GenImage数据集上平均准确率为91.36%，在所有测试生成器上平均准确率为88.71%，平均精度为94.96%；第二阶段训练进一步提升了大多数生成器的性能。

Conclusion: 研究结果表明，结合频谱和语义特征对于实现跨多种AI生成器的通用图像检测具有显著优势，相比现有方法在处理高保真图像时保持更好的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要局限：1）依赖位置编码的时空表示不够灵活；2）传统注意力机制的二次时间复杂性导致高计算成本。需要一种既能灵活探索复杂时空依赖又能显著降低计算成本的方法。

Method: 提出时空无约束专家混合模型（ST-MoE），包含四种不同类型的时空专家，每个专家专门捕捉不同的空间或时间依赖。为降低计算开销，引入双向时空Mamba作为专家，通过共享双向时间和空间Mamba实现模型效率和参数经济性。

Result: 在四个多人基准数据集上的实验表明，该方法不仅精度优于现有最佳方法，而且模型参数减少41.38%，训练速度提升3.6倍。

Conclusion: ST-MoE能够灵活探索人类运动的复杂时空依赖，同时显著降低计算成本，为多人运动预测提供了高效且准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频预测面临分辨率、质量和实时性的三难困境，特别是在自主无人机等延迟关键应用中。现有方法（扩散模型、自回归模型）由于迭代生成或二次复杂度注意力机制，无法在边缘硬件上满足高分辨率实时预测的需求

Method: 提出RAPTOR架构：1) 单次前向设计避免迭代方法的误差累积和延迟；2) 核心创新Efficient Video Attention模块，通过沿空间轴和时间轴交替操作分解时空建模，将复杂度从O((ST)²)降至O(S+T)；3) 采用3阶段训练课程，从粗结构逐步细化到清晰的时间相干细节

Result: 在Jetson AGX Orin上首次实现512²分辨率视频超过30 FPS的实时预测，在UAVid、KTH和自定义高分辨率数据集上PSNR、SSIM、LPIPS指标达到新SOTA，在真实无人机导航任务中将任务成功率提升18%

Conclusion: RAPTOR突破了视频预测长期存在的分辨率-质量-速度三难困境，为延迟关键应用（如自主无人机）提供了实时高分辨率预测解决方案，为实现更安全、更具预见性的具身智能体铺平了道路
...
<details>
  <summary>Details</summary>
Motivation: 在开放动态环境中进行具身导航需要准确预测世界如何演变以及动作如何随时间展开。传统"先预测后规划"的解耦方法存在累积误差问题，需要更紧密的视觉-动作耦合来保持决策基于物理一致、任务相关的未来。

Method: 提出AstraNav-World框架，集成基于扩散的视频生成器和视觉语言策略，在统一概率框架下进行同步推演。训练优化两个互补目标：生成动作条件多步视觉预测，以及基于预测视觉推导轨迹。这种双向约束使视觉预测可执行，同时保持决策基于物理一致。

Result: 在多种具身导航基准测试中显示出改进的轨迹准确性和更高的成功率。消融实验证实紧密视觉-动作耦合和统一训练的必要性。在真实世界测试中展示了卓越的零样本能力，无需真实世界微调即可适应未见场景。

Conclusion: 通过将前瞻视觉和控制统一在单个生成模型中，AstraNav-World捕捉了可迁移的空间理解和规划相关的导航动态，而不是仅仅过拟合模拟特定数据分布，向可靠、可解释、通用的具身智能体迈进一步。
...
<details>
  <summary>Details</summary>
Motivation: 实时肖像动画在虚拟助手、直播头像等交互应用中至关重要，需要高视觉保真度、时间连贯性、超低延迟和动态输入的响应控制。现有扩散模型质量好但非因果性阻碍流式部署，因果自回归视频生成方法存在误差累积、块边界运动不连续和长期一致性退化问题。

Method: 提出Knot Forcing流式框架，包含三个关键设计：1) 分块生成策略，通过参考图像的缓存KV状态保持全局身份，使用滑动窗口注意力进行局部时间建模；2) 时间节点模块，重叠相邻块并通过图像到视频条件传播时空线索以平滑块间运动过渡；3) "超前运行"机制，在推理过程中动态更新参考帧的时间坐标，使其语义上下文超前于当前生成帧以支持长期一致性。

Result: Knot Forcing能够实现高保真度、时间一致且交互式的肖像动画，支持无限序列生成，在消费级GPU上达到实时性能并具有强大的视觉稳定性。

Conclusion: Knot Forcing框架有效解决了实时肖像动画中的误差累积、运动不连续和长期一致性退化问题，实现了高质量、时间连贯的流式生成，为交互应用提供了实用解决方案。
...
TL;DR: SyncAnyone是一个两阶段学习框架，通过扩散视频变换器实现音频驱动的唇部同步，解决了传统掩码训练方法破坏时空上下文的问题，在视觉质量、时间一致性和身份保持方面达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码的训练方法虽然能提高唇部同步准确性，但会破坏时空上下文，导致动态面部运动性能下降，面部结构和背景一致性不稳定。需要同时实现准确运动建模和高视觉保真度。

Method: 提出两阶段学习框架：第一阶段训练基于扩散的视频变换器进行掩码唇部修复，利用其强大的时空建模能力生成准确的音频驱动唇部运动；第二阶段开发无掩码微调管道，通过合成伪配对训练样本来解决掩码引起的伪影问题。

Result: 在野外唇部同步场景下，该方法在视觉质量、时间一致性和身份保持方面取得了最先进的结果，能够实现精确的唇部编辑和更好的背景一致性。
...
TL;DR: BertsWin是一种结合BERT风格全令牌掩码和Swin Transformer窗口的混合架构，用于3D医学图像的自监督学习，相比标准ViT-MAE基线实现了5.8倍的语义收敛加速和15倍的训练周期减少。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习和Vision Transformers方法在2D医学图像上表现良好，但在3D体积图像上应用困难。标准的Masked Autoencoders难以捕捉三维空间关系，特别是在预训练期间丢弃75%令牌的情况下。

Method: 提出BertsWin混合架构：结合BERT风格的全令牌掩码和Swin Transformer窗口，保持完整的3D令牌网格（包括掩码和可见令牌）以保留空间拓扑结构。使用单级局部Swin窗口来平滑ViT的二次复杂度，并引入结构优先级损失函数。

Result: BertsWin架构通过保持完整的三维空间拓扑，实现了5.8倍于标准ViT-MAE基线的语义收敛加速。结合GradientConductor优化器，整个框架实现了15倍的训练周期减少（44 vs 660），达到最先进的重建保真度，且没有通常与密集体积处理相关的计算惩罚。

Conclusion: BertsWin架构通过保持完整的3D空间拓扑结构，在3D医学图像自监督学习中实现了显著的收敛加速和计算效率提升，为3D体积图像处理提供了一种有效的解决方案。
...
Method: 提出Scene-VLM框架，联合处理视觉和文本线索（帧、转录文本、可选元数据），实现跨连续镜头的多模态推理；采用因果依赖的序列预测机制和上下文聚焦窗口；从VLM的token级logits提取置信度分数；通过最小化监督使模型生成边界决策的自然语言解释

Result: 在标准场景分割基准上达到最先进性能，在MovieNet上比先前领先方法提升+6 AP和+13.7 F1

Conclusion: Scene-VLM通过多模态推理、因果依赖处理和可解释性生成，显著提升了视频场景分割的性能和实用性，为大规模视频理解提供了更有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LoRA等单适配器的参数高效微调方法在处理多样多条件任务时存在任务干扰问题。MoLE架构提供了模块化解决方案，但其token级路由策略与用户指令的全局性质冲突，导致空间碎片化和语义漂移等伪影。

Method: 提出InstructMoLE框架，采用指令引导的低秩专家混合架构。核心创新包括：1）指令引导路由（IGR），从用户完整指令中提取全局路由信号，统一应用于所有输入token；2）输出空间正交性损失，促进专家功能多样性并防止表示崩溃。

Result: 在具有挑战性的多条件生成基准测试中，InstructMoLE显著优于现有的LoRA适配器和MoLE变体，实现了更好的组合控制和用户意图保真度。

Conclusion: InstructMoLE为生成模型的指令驱动微调提供了一个鲁棒且可泛化的框架，通过全局路由机制解决了局部路由与全局语义的冲突问题，提升了复杂图像生成任务的质量和一致性。
...
TL;DR: 该论文介绍了mAIcetoma挑战赛，旨在通过AI技术解决足菌肿诊断难题，特别是针对资源匮乏地区。挑战赛聚焦于从组织病理图像中分割足菌肿颗粒并分类足菌肿类型，吸引了全球团队参与并取得了良好结果。


<details>
  <summary>Details</summary>
Motivation: 足菌肿是一种被忽视的热带疾病，对贫困农村社区造成严重影响。诊断是足菌肿管理的主要挑战，特别是在资源匮乏地区，专家病理学家有限。需要通过AI解决方案来推进足菌肿诊断。

Method: 组织了mAIcetoma挑战赛，开发自动模型从组织病理图像中分割足菌肿颗粒并分类足菌肿类型。为参与者提供了标准化的足菌肿数据库（MyData），各团队提出了多种深度学习架构，并使用评估指标对模型进行评估。

Result: 所有模型都实现了高分割精度，强调了颗粒检测作为足菌肿诊断关键步骤的必要性。表现最佳的模型在足菌肿类型分类方面显示出显著性能。

Conclusion: mAIcetoma挑战赛成功推进了足菌肿诊断的AI解决方案开发，证明了深度学习模型在足菌肿颗粒分割和类型分类方面的有效性，为解决这一被忽视热带疾病的诊断挑战提供了有前景的技术途径。
...
TL;DR: 该研究探讨了在已知退化模型下使用扩散后验采样进行单图像超分辨率的方法，通过似然引导采样结合无条件扩散先验和基于梯度的条件约束来实现4倍超分辨率，并评估了不同引导尺度和噪声水平下的重建质量。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索如何在不重新训练扩散模型的情况下，通过后验采样方法平衡扩散先验和测量梯度强度，以获得稳定、高质量的单图像超分辨率重建结果。

Method: 采用扩散后验采样方法，结合无条件扩散先验和基于梯度的条件约束来强制测量一致性。通过似然引导采样程序实现4倍超分辨率，并评估不同引导尺度和噪声水平下的性能。

Result: 实验表明中等程度的引导能改善重建质量，最佳配置为PS尺度0.95和噪声标准差σ=0.01（得分1.45231）。所选PS设置相比下采样输入能恢复更清晰的边缘和更连贯的面部细节，而其他条件策略（如MCG和PS-annealed）则展现出不同的纹理保真度权衡。

Conclusion: 研究强调了平衡扩散先验和测量梯度强度的重要性，以获得稳定、高质量的重建结果，而无需为每个操作符重新训练扩散模型。该方法为单图像超分辨率提供了一种有效的后验采样解决方案。
...
TL;DR: 本文提出基于熵的针对性对抗攻击方法（EGA），通过识别自回归生成中的关键决策点（约20%的高熵token），集中对抗扰动在这些位置上，实现用更小预算达到语义退化效果，并显著提高有害内容转化率。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵的对抗攻击方法假设所有token对生成不稳定性贡献相同，但研究发现实际上只有一小部分高熵token（关键决策点）对输出轨迹有决定性影响。通过针对性攻击这些位置，可以在更小预算下实现更有效的攻击，并暴露当前VLM安全机制的脆弱性。

Method: 提出Entropy-bank Guided Adversarial attacks (EGA)方法：1）识别自回归生成过程中的高熵token（约20%的关键决策点）；2）集中对抗扰动在这些关键位置上；3）利用这些高熵分叉在不同VLM架构中的重现性实现可迁移攻击。

Result: 1）在多个代表性VLM上，选择性攻击将35-49%的良性输出转化为有害输出；2）攻击成功率高达93-95%；3）攻击具有良好可迁移性，在未见目标模型上达到17-26%的有害转化率；4）相比全局方法，用更小预算实现可比语义退化。

Conclusion: 自回归生成中的关键决策点（高熵token）对VLM输出轨迹有决定性影响，针对性攻击这些位置能有效暴露当前VLM安全机制的脆弱性。EGA方法展示了对抗攻击的新范式，为VLM安全评估提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，多视角协同感知和多模态融合对于可靠的3D时空理解至关重要，特别是在V2X场景下存在遮挡、有限视角和通信延迟等挑战。

Method: 提出XET-V2X框架，基于多尺度可变形注意力的双层空间交叉注意力模块，先聚合多视角图像特征增强语义一致性，然后通过更新的空间查询引导点云融合，实现有效跨模态交互并降低计算开销。
...
TL;DR: ADT-Tree通过邻接自适应动态草稿树加速自回归图像生成，解决了传统推测解码在视觉AR模型中因区域预测难度差异导致的接受率不一致问题，实现了3倍以上的加速效果。


<details>
  <summary>Details</summary>
Motivation: 自回归图像模型虽然能达到扩散模型的质量水平，但需要约2000步的顺序推理生成576x576图像，推理速度慢。传统的推测解码方法在视觉AR模型中表现不佳，主要原因是图像不同区域的token预测难度差异导致草稿树接受率不一致。

Method: 提出邻接自适应动态草稿树（ADT-Tree），通过利用相邻token状态和先前的接受率动态调整草稿树的深度和宽度。方法首先通过水平邻接初始化，然后通过二分自适应细化深度/宽度，在简单区域生成更深的树，在复杂区域生成更宽的树。

Result: 在MS-COCO 2017和PartiPrompts数据集上的实证评估显示，ADT-Tree分别实现了3.13倍和3.05倍的加速。此外，该方法能与LANTERN等松弛采样方法无缝集成，实现进一步加速。

Conclusion: ADT-Tree有效解决了视觉AR模型中推测解码的接受率不一致问题，通过邻接自适应动态调整草稿树结构，显著加速了自回归图像生成，同时保持生成质量。
...
TL;DR: DIOR是一种无需训练的生成条件图像嵌入的方法，利用大型视觉语言模型根据文本条件生成单字描述，提取隐藏状态作为条件图像嵌入
...
Method: DIOR是一种无需训练的方法，通过提示大型视觉语言模型（LVLM）用与给定条件相关的单个词描述图像，然后提取LVLM最后一个token的隐藏状态向量作为条件图像嵌入

Result: 在条件图像相似性任务中，DIOR优于包括CLIP在内的现有无需训练基线方法，并且在多个设置中超越了需要额外训练的方法

Conclusion: DIOR提供了一种通用的解决方案，无需额外训练或任务特定先验知识即可应用于任何图像和条件，在条件图像嵌入任务中表现出色
...
Method: 提出EasyOmnimatte方法：1）微调预训练视频修复扩散模型，保持原始权重不变；2）采用双专家策略：效果专家（仅在效果敏感的DiT块应用LoRA）捕捉前景和相关效果的粗粒度结构，质量专家（全LoRA微调）细化alpha遮罩；3）采样时早期高噪声步使用效果专家，后期低噪声步使用质量专家

Result: 实验表明EasyOmnimatte在视频omnimatte任务上达到新的最先进水平，在质量和效率上都显著优于基线方法，并支持各种下游任务。消融研究验证了双专家策略的有效性
...
TL;DR: DPAR是一种新型的解码器自回归图像生成模型，通过动态聚合图像token为可变数量的patch来提升效率，利用轻量级无监督自回归模型的next-token预测熵作为token合并准则，显著减少计算成本和内存需求。


<details>
  <summary>Details</summary>
Motivation: 传统解码器自回归图像生成使用固定长度的token化方案，token数量随分辨率平方增长，导致注意力机制的计算和内存需求大幅增加。需要一种更高效的token化方法来降低计算成本。

Method: DPAR提出动态patch聚合方法，利用轻量级无监督自回归模型的next-token预测熵作为信息内容标准，将token合并为更大patch。模型对标准解码器架构进行最小修改，确保与多模态生成框架兼容，并将更多计算资源分配给高信息区域。

Result: 在Imagenet 256和384分辨率下，DPAR分别减少token数量1.81倍和2.06倍，训练成本FLOPs减少高达40%。模型收敛更快，FID相对基线模型提升高达27.1%。动态patch训练使表示对patch边界具有鲁棒性，支持推理时扩展到更大patch尺寸。

Conclusion: DPAR通过动态patch聚合机制有效解决了自回归图像生成中的计算效率问题，利用信息熵指导的token合并策略在保持生成质量的同时显著降低计算成本，为高效图像生成提供了新思路。
...
TL;DR: SLIM-Brain是一个新型的fMRI基础模型，通过两阶段自适应设计解决了现有方法在数据效率和训练效率方面的瓶颈，在保持空间保真度的同时显著降低了计算和内存需求。


<details>
  <summary>Details</summary>
Motivation: 当前fMRI分析的基础模型面临数据效率和训练效率的双重瓶颈：基于图谱的方法会丢失细粒度空间细节，需要大量数据训练；而无图谱方法虽然保持空间保真度，但计算和内存需求过高，难以进行大规模预训练。

Method: 采用两阶段自适应设计：1）轻量级时间提取器捕获全局上下文并对数据窗口进行显著性排序；2）4D分层编码器（Hiera-JEPA）仅从选定的前k个窗口中学习细粒度体素级表示，同时删除约70%的掩码补丁。

Result: 在七个公共基准测试中建立了新的最先进性能，同时仅需4千个预训练会话和约30%的GPU内存（相比传统体素级方法）。

Conclusion: SLIM-Brain成功解决了fMRI基础模型的数据效率和训练效率瓶颈，实现了在保持空间保真度的同时显著降低计算资源需求，为fMRI分析提供了更实用的基础模型解决方案。
...
TL;DR: AutoPP是一个自动化产品海报生成与优化系统，通过生成器和优化器两个模块，无需人工干预即可创建并基于在线点击率反馈优化产品海报。
...
Method: 系统包含生成器和优化器：生成器使用统一设计模块整合海报三要素（背景、文本、布局），然后通过元素渲染模块编码为条件令牌生成海报；优化器通过系统替换元素收集细粒度点击率比较，使用孤立直接偏好优化（IDPO）将点击率提升归因于孤立元素。

Result: AutoPP在离线和在线设置中都取得了最先进的结果，并基于AutoPP1M数据集（包含100万高质量海报和超过100万用户的反馈）进行验证。

Conclusion: AutoPP提供了一个完全自动化的产品海报生成与优化解决方案，通过结合生成和优化模块，有效提升了海报质量和点击率表现。
...
TL;DR: 提出了一种新的脑MRI异常检测方法，通过解耦成像信息与解剖结构，并利用边缘信息重建伪健康图像，显著提升了多模态、多中心数据的泛化能力和检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前无监督学习方法在脑MRI异常检测中存在两个主要问题：1) 对多模态、多中心数据的泛化能力有限，因为模型过度依赖正常训练数据中的特定成像信息；2) 性能受限，因为异常残差会从输入图像传播到重建的伪健康图像中。

Method: 提出两个新模块：1) 解耦表示模块，将脑MRI分解为成像信息和本质的成像不变解剖图像，使用脑解剖先验和可微分one-hot编码算子来约束解耦结果；2) 边缘到图像恢复模块，通过从解剖图像的高频边缘信息恢复解剖表示，然后重新耦合解耦的成像信息来重建高质量伪健康图像。

Result: 在9个公共数据集（来自多个中心的4,443名患者MRI）上评估，方法优于17种最先进方法，在AP指标上绝对提升+18.32%，在DSC指标上绝对提升+13.64%。

Conclusion: 该方法通过解耦成像信息与解剖结构，并利用边缘信息重建伪健康图像，有效解决了当前无监督方法在多模态、多中心脑MRI异常检测中的泛化能力和性能限制问题。
...
TL;DR: 本文提出DRU框架，针对动漫场景图像的低光照增强任务，通过数据相对不确定性方法动态调整目标函数，在数据稀缺情况下实现优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有低光照增强研究主要针对自然图像和视频，而动漫场景图像的低光照质量退化问题尚未充分探索。为解决这一领域差距，需要专门处理动漫场景图像的低光照增强任务。

Method: 1. 从多种来源收集图像，构建包含不同环境和光照条件的无配对动漫场景数据集；2. 提出数据相对不确定性(DRU)框架，受相对论GAN启发，类比光的波粒二象性定义和量化暗/亮样本的光照不确定性；3. 利用不确定性信息动态调整目标函数，重新校准模型在数据不确定性下的学习。

Result: 通过训练多个版本的EnlightenGANs进行广泛实验，DRU框架在感知和美学质量方面超越了现有最先进方法，这些方法无法从数据不确定性角度进行学习。

Conclusion: DRU框架为潜在视觉和语言领域的数据中心学习提供了新范式，能够有效处理动漫场景图像的低光照增强问题，代码已开源。
...
TL;DR: 提出自动化方法寻找紧凑光谱指数用于植被分类，通过构建多项式组合保持光照不变性，在Sentinel-2数据上测试Kochia检测，单个二次指数达到96.26%准确率


<details>
  <summary>Details</summary>
Motivation: 传统植被分类需要人工设计光谱指数，过程繁琐且可能不是最优。需要自动化方法寻找紧凑、可解释的光谱指数，同时保持遥感所需的光照不变性。

Method: 1. 从所有光谱波段对构建归一化差异作为基础特征；2. 构建多项式组合（最高二次）扩展特征空间；3. 使用特征选择方法（ANOVA过滤、递归消除、L1正则化SVM）筛选小型指数集；4. 在Sentinel-2数据上测试Kochia检测。

Result: 对于10波段Sentinel-2配置，二次多项式扩展产生1080个候选特征。单个二次指数（红边波段两个归一化差异的乘积）达到96.26%准确率，8个指数提升至97.70%。所有选定特征都是b4-b8波段的二次乘积，表明判别信号来自光谱交互而非单个波段比值。
...
<details>
  <summary>Details</summary>
Motivation: 在高风险领域中，小型任务特定视觉模型虽然计算需求低且可解释性强，但其解释常显示模型与人类领域知识不一致，依赖虚假相关性而非真实特征，导致在真实世界中部署时行为脆弱。
...
Conclusion: LVLM-VA方法为小型任务特定视觉模型与人类领域知识的对齐提供了一种新颖高效的解决方案，能够增强模型在真实世界部署中的鲁棒性和可靠性。
...
Method: 提出对抗性参数编辑框架ALEAHallu，包含三个步骤：1)构建包含基于视觉特征的正样本和反映LLM先验偏差的负样本的激活数据集；2)通过分析响应对的隐藏状态差异识别幻觉易发参数簇；3)使用注入对抗性调优前缀的提示对这些参数簇进行微调，迫使模型优先考虑视觉证据而非内在参数偏差。

Result: 在生成性和判别性VLM任务上的评估表明，ALEAHallu在缓解幻觉方面具有显著效果。

Conclusion: ALEAHallu通过对抗性参数编辑有效缓解了视觉语言模型的幻觉问题，优于现有的启发式解码校准方法。
...
TL;DR: LongFly是一个用于无人机视觉语言导航的时空上下文建模框架，通过历史感知的时空建模策略解决长时程导航中的语义对齐和路径规划问题，在成功率和路径长度加权成功率上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 无人机在灾后搜救等场景中面临高信息密度、视角快速变化和动态结构等挑战，现有视觉语言导航方法难以建模复杂环境中的长时程时空上下文，导致语义对齐不准确和路径规划不稳定。

Method: 提出LongFly框架，包含三个核心模块：1) 基于槽位的历史图像压缩模块，将多视角历史观测动态压缩为固定长度的上下文表示；2) 时空轨迹编码模块，捕捉无人机轨迹的时间动态和空间结构；3) 提示引导的多模态集成模块，将现有时空上下文与当前观测集成，支持基于时间的推理和稳健的航点预测。

Result: 实验结果表明，LongFly在成功率和路径长度加权成功率上分别比最先进的无人机视觉语言导航基线方法提高了7.89%和6.33%，在已见和未见环境中均表现一致。

Conclusion: LongFly通过有效的时空上下文建模策略，显著提升了无人机在复杂环境中的长时程视觉语言导航性能，为解决灾后搜救等实际应用中的导航挑战提供了有效解决方案。
...
TL;DR: 论文提出了BadVSFM，首个针对提示驱动视频分割基础模型的后门攻击框架，通过两阶段策略实现可控的后门效果，在多种触发器和提示下保持高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着SAM2等提示驱动视频分割基础模型在自动驾驶、数字病理学等关键应用中的部署，后门威胁日益受到关注。研究发现传统后门攻击对这类模型几乎无效（攻击成功率低于5%），需要专门针对其架构特点设计新的攻击方法。

Method: BadVSFM采用两阶段策略：1）引导图像编码器，使触发帧映射到指定目标嵌入，而干净帧保持与参考编码器对齐；2）训练掩码解码器，使触发帧-提示对产生共享目标掩码，同时干净输出接近参考解码器。

Result: 在两个数据集和五个VSFM模型上的实验表明，BadVSFM在多种触发器和提示下实现了强大且可控的后门效果，同时保持了干净的语义分割质量。攻击成功率显著高于传统方法，且对四种代表性防御方法具有鲁棒性。

Conclusion: BadVSFM揭示了当前视频分割基础模型中未被充分探索的漏洞，通过分离触发和干净表示、将注意力转移到触发区域，成功实现了有效的后门攻击，而现有防御方法对此类攻击基本无效。
...
TL;DR: MAI-UI是一系列基础GUI智能体，通过自演进数据管道、原生设备-云协作系统和在线强化学习框架，解决了GUI智能体部署中的关键挑战，在多个基准测试中取得了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: GUI智能体的发展可能彻底改变下一代人机交互。然而，当前GUI智能体面临四个关键挑战：缺乏原生智能体-用户交互、仅限UI操作的局限性、缺乏实用部署架构以及在动态环境中的脆弱性。

Method: MAI-UI采用统一方法：1）自演进数据管道，将导航数据扩展到包括用户交互和MCP工具调用；2）原生设备-云协作系统，根据任务状态路由执行；3）在线强化学习框架，通过扩展并行环境和上下文长度进行优化。

Result: 在GUI基础测试中：ScreenSpot-Pro达到73.5%，MMBench GUI L2达到91.3%，OSWorld-G达到70.9%，UI-Vision达到49.2%，超越Gemini-3-Pro和Seed1.8。在移动GUI导航中：AndroidWorld达到76.7%的新SOTA，超越UI-Tars-2、Gemini-2.5-Pro和Seed1.8；MobileWorld达到41.7%成功率。在线RL实验显示：并行环境从32扩展到512提升5.2个百分点；环境步数预算从15增加到50提升4.3个百分点。设备-云协作系统提升设备性能33%，减少云模型调用40%以上。

Conclusion: MAI-UI通过创新的统一方法解决了GUI智能体部署的关键挑战，在多个基准测试中建立了新的最先进性能，其设备-云协作系统显著提升了性能、效率和隐私保护，为下一代人机交互奠定了基础。
...
TL;DR: 提出一个两阶段自回归适应与加速框架，将高保真人类视频扩散模型适配为实时交互式流媒体系统，实现全身自然对话和倾听行为生成
...
Result: 方法在生成质量、实时效率和交互自然度方面达到最先进水平，能够生成自然对话和倾听行为以及连贯的手势动作

Conclusion: 提出的框架成功解决了实时流媒体虚拟人的挑战，实现了高质量、低延迟的全身交互式虚拟人生成，为数字人研究提供了实用解决方案
...
TL;DR: 提出了一种名为\method的新框架，能够从单张图像或文本提示生成逼真、交互式、连续的世界，支持键盘探索，解决了现有扩散模型在参数规模、推理步骤和历史上下文方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的世界生成方法面临参数规模过大、推理步骤冗长、历史上下文快速增长等关键挑战，严重限制了实时性能，且缺乏文本控制生成能力。

Method: 框架包含三个核心组件：1) 集成统一上下文压缩与线性注意力的长视频生成框架；2) 基于双向注意力蒸馏和增强文本嵌入方案的实时流式加速策略；3) 用于生成世界事件的文本控制方法。

Result: 该框架能够从单张图像或文本提示生成逼真、交互式、连续的世界，支持键盘探索，解决了现有方法的实时性能限制和文本控制能力不足的问题。

Conclusion: \method框架通过创新的架构设计，成功解决了扩散模型在世界生成中的关键挑战，实现了从单张图像或文本提示生成交互式连续世界的能力，并提供了代码实现。
...
TL;DR: TDLP是一种基于检测的跟踪方法，通过链接预测在轨迹和检测之间进行帧级关联，直接学习数据关联而无需手工规则，在计算效率和性能上都优于现有方法。
...
Method: 提出Track-Detection Link Prediction (TDLP)方法，这是一种基于检测的跟踪方法，通过链接预测在轨迹和检测之间进行每帧关联。该方法主要设计用于几何特征（如边界框），同时可选地整合其他线索（如姿态和外观）。与启发式方法不同，TDLP直接从数据中学习关联而无需手工规则。

Result: 在多个基准测试上的广泛实验表明，TDLP在基于检测的跟踪和端到端方法中都持续超越了最先进的性能。详细分析显示，链接预测比基于度量学习的关联更有效，特别是在处理异构特征（如检测边界框）时。

Conclusion: TDLP通过链接预测方法在轨迹和检测之间进行关联，既保持了基于检测跟踪方法的计算效率和模块化优势，又能从数据中学习关联而无需依赖手工规则，在多目标跟踪任务中取得了优越的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于反转的视觉编辑方法在采样过程中过度依赖源图像信息，这虽然保持了编辑一致性，但会负面影响目标图像的编辑效果，导致无法按照用户指令改变主体属性（如姿态、数量、颜色等）。

Method: 提出ProEdit方法，从两个层面解决问题：1）注意力层面：引入KV-mix，在编辑区域混合源图像和目标图像的KV特征，减少源图像对编辑区域的影响同时保持背景一致性；2）潜在空间层面：提出Latents-Shift，扰动源潜在空间中编辑区域，消除反转潜在对采样的影响。

Result: 在多个图像和视频编辑基准测试上的大量实验表明，该方法达到了最先进的性能。此外，该设计是即插即用的，可以无缝集成到现有的反转和编辑方法中，如RF-Solver、FireFlow和UniEdit。

Conclusion: ProEdit有效解决了现有基于反转的视觉编辑方法过度依赖源图像信息的问题，在保持编辑一致性的同时，能够更好地按照用户指令改变图像属性，实现了更好的编辑效果。
...
TL;DR: BiPS通过双向感知塑造，将问题条件化的掩码视图转化为训练时的双向视觉信号，提升视觉语言模型对细粒度视觉证据的依赖，减少文本捷径，在多个基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型虽然使用中间视觉线索，但往往忽略细粒度视觉证据（如图表中的折线），跨领域泛化能力差，且推理成本高。需要一种方法能强制模型依赖细粒度视觉信息，避免仅从文本回答问题。

Method: 提出双向感知塑造（BiPS）：1）KL一致性约束：在原始图像和保留证据的视图之间，鼓励对支持像素的完整覆盖；2）KL分离约束：在原始图像和证据消除的视图之间，阻止文本捷径，强制依赖细粒度视觉信息。

Result: 在8个基准测试中，BiPS将Qwen2.5-VL-7B模型平均提升8.2%，并在未见过的数据集和图像类型上表现出强大的跨领域泛化能力。

Conclusion: BiPS通过双向感知塑造有效解决了视觉语言模型对细粒度视觉证据依赖不足的问题，显著提升了模型性能并增强了跨领域泛化能力，同时避免了高推理成本。
...
### [64] [Safe Path Planning and Observation Quality Enhancement Strategy for Unmanned Aerial Vehicles in Water Quality Monitoring Tasks](https://arxiv.org/abs/2512.21375)
*Yuanshuang Fu,Qianyao Wang,Qihao Wang,Bonan Zhang,Jiaxin Zhao,Yiming Cao,Zhijun Li*

Main category: cs.RO

TL;DR: 提出一种用于无人机水质监测的动态光照阴影干扰主动路径规划方法，通过三维虚拟障碍物建模、改进IFDS算法和MPC框架优化，在复杂光照环境下提高数据采集质量


<details>
  <summary>Details</summary>
Motivation: 无人机光谱遥感技术在水质监测中广泛应用，但动态环境中的光照变化（如阴影和镜面反射）会导致严重的光谱失真，降低数据可用性。需要一种方法在保证飞行安全的同时最大化获取高质量数据。
...
Result: 仿真结果显示，与传统PID和单一避障算法相比，该方法在密集干扰场景中达到98%的避障成功率，显著提高了路径平滑度，并将有效观测数据量增加了约27%。

Conclusion: 该研究为复杂光照环境下精确的无人机水质监测提供了有效的工程解决方案，能够在动态光照干扰条件下实现高质量的遥感数据采集。
...
### [65] [Fast Navigation Through Occluded Spaces via Language-Conditioned Map Prediction](https://arxiv.org/abs/2512.21398)
*Rahul Moorthy Mahesh,Oguzhan Goktug Poyrazoglu,Yukang Cao,Volkan Isler*

Main category: cs.RO

TL;DR: PaceForecaster利用人类指令帮助机器人在遮挡环境中更果断地规划路径，通过预测可见区域地图和指令条件子目标，提高导航性能36%


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中，运动规划器面临安全性和速度之间的权衡，主要由于遮挡和传感器范围有限造成的不确定性。研究探索人类指令是否能帮助机器人更果断地规划同时保持安全。

Method: 提出PaceForecaster方法，将人类指令整合到局部规划器中。系统以机器人局部传感器覆盖范围（Level-1）和人类指令为输入，预测：1）从Level-1可见的所有区域的预测地图（Level-2）；2）基于指令条件化的Level-2内子目标。子目标为规划器提供明确指导，以目标导向方式利用预测环境。

Result: 将PaceForecaster与Log-MPPI控制器集成，在多边形环境中，使用语言条件化预测和目标的导航性能比仅使用局部地图的基线提高了36%。

Conclusion: 人类指令可以帮助机器人在遮挡环境中更有效地规划路径，通过预测未来可见区域和指令条件化子目标，显著提高导航性能。
...
### [66] [Developing a Fundamental Diagram for Urban Air Mobility Based on Physical Experiments](https://arxiv.org/abs/2512.21425)
...
Method: 研究提出了一个通用框架，结合理论分析和物理实验构建UAM基本图。理论方面设计了两种无人机防撞控制律，开发了基于仿真的交通生成方法；实验方面在缩比测试平台上使用Crazyflie无人机进行物理实验，收集轨迹数据构建UAMTra2Flow数据集。

Result: 初步结果表明，地面交通的经典基本图结构同样适用于UAM系统。物理实验获得的基本图曲线与仿真结果存在偏差，突显了实验验证的重要性。研究还将缩比测试结果扩展到实际运行条件，为未来UAM交通系统提供了实用见解。

Conclusion: 该研究首次使用真实世界物理测试数据推导出UAM基本图，验证了经典交通流理论在UAM系统中的适用性，同时强调了物理实验验证的重要性，为未来UAM交通系统的设计和优化提供了理论基础和实用指导。
...
### [67] [Planetary Terrain Datasets and Benchmarks for Rover Path Planning](https://arxiv.org/abs/2512.21438)
*Marvin Chancán,Avijit Banerjee,George Nikolakopoulos*

Main category: cs.RO

TL;DR: 该论文提出了首个基于火星和月球高分辨率地形图像的大规模行星路径规划基准数据集MarsPlanBench和MoonPlanBench，并在统一框架下评估了经典和基于学习的路径规划算法在行星地形上的性能。


<details>
  <summary>Details</summary>
Motivation: 行星探测任务日益增多，但先前任务的大量数据在路径规划和自主导航研究中未得到充分利用。目前缺乏基于太空任务的行星数据集、标准化基准和评估协议，阻碍了相关研究的发展。

Method: 从火星和月球的高分辨率数字地形图像中创建了两个大规模平面基准数据集MarsPlanBench和MoonPlanBench。在统一框架下设置了经典和基于学习的路径规划算法，并在提出的数据集和流行规划基准上进行了评估。

Result: 经典算法在挑战性地形（如月球南北极）上平均能达到100%的全局路径规划成功率，解释了为什么NASA在实践中使用这些算法。基于学习的模型虽然在较简单环境中表现良好，但在行星领域泛化能力仍不足。

Conclusion: 该研究为行星路径规划研究提供了首个标准化基准数据集和评估框架，揭示了经典算法在行星地形上的优势，为未来研究奠定了基础。代码和数据集将开源以促进该领域发展。
...
TL;DR: 将时空管框架扩展到动态不确定环境中的概率时间可达-避障-停留任务，开发实时管合成方法，提供形式化概率安全保证


<details>
  <summary>Details</summary>
Motivation: 解决动态环境中存在不确定障碍物时的概率时间可达-避障-停留任务，需要实时控制方法同时保证概率安全性和任务完成

Method: 扩展时空管框架，将管表示为状态空间中的时变球体，中心和半径基于不确定传感信息在线演化，推导闭式、无近似的控制律将系统轨迹限制在管内

Result: 控制器无模型、无近似、无优化，能高效实时执行，保证收敛到目标，在移动机器人、无人机和7自由度机械臂的仿真和硬件实验中验证有效性和可扩展性

Conclusion: 提出的方法为动态不确定环境中的概率避障和有限时间任务完成提供了形式化保证，实现了实时安全控制
...
### [69] [A Novel Robotic Variable Stiffness Mechanism Based on Helically Wound Structured Electrostatic Layer Jamming](https://arxiv.org/abs/2512.21534)
*Congrui Bai,Zhenting Du,Weibang Bai*

Main category: cs.RO

TL;DR: 提出了一种新型螺旋缠绕结构静电层压紧（HWS-ELJ）可变刚度机制，用于机器人手指设计，通过静电吸附增强层间摩擦实现刚度调节，相比传统平面ELJ具有指数级刚度提升和更小占用空间。


<details>
  <summary>Details</summary>
Motivation: 开发一种更高效的可变刚度机制用于机器人手指设计，解决传统平面静电层压紧（ELJ）在刚度调节范围和空间占用方面的局限性。

Method: 采用螺旋缠绕结构的静电层压紧（HWS-ELJ），利用静电吸引力增强层间摩擦，抑制相对滑动，实现可调刚度。通过改变缠绕角度实现指数级刚度调整。

Result: HWS-ELJ相比传统平面ELJ在相同电极接触面积下获得显著更大的刚度增强，在等效刚度条件下减少所需占用空间。实验验证了其电压控制下的刚度调制特性符合理论预测。

Conclusion: HWS-ELJ机制设计合理有效，成功应用于机器人手指原型，实现了电压驱动的刚度调制，证明了该可变刚度机制在机器人应用中的可行性。
...
### [70] [World-Coordinate Human Motion Retargeting via SAM 3D Body](https://arxiv.org/abs/2512.21573)
...
<details>
  <summary>Details</summary>
Motivation: 为具身智能和机器人学提供从单目视频恢复世界坐标系人体运动的方法，避免复杂的SLAM流程或重型时序模型，实现轻量级工程化解决方案

Method: 使用冻结的SAM 3D Body作为感知骨干，采用Momentum HumanRig作为机器人友好中间表示，通过锁定身份和骨架尺度参数确保时序一致的骨骼长度，在低维MHR潜在空间进行滑动窗口优化平滑预测，使用可微分软脚地接触模型和接触感知全局优化恢复物理合理的全局根轨迹，最后通过运动学感知的两阶段逆运动学管道将重建运动重定向到Unitree G1人形机器人

Result: 在真实单目视频上的结果显示，该方法具有稳定的世界轨迹和可靠的机器人重定向性能，表明结构化人体表示结合轻量级物理约束可以从单目输入产生机器人就绪的运动

Conclusion: 结构化人体表示结合轻量级物理约束能够有效地从单目视频恢复世界坐标系人体运动并成功重定向到人形机器人，为机器人运动生成提供了实用解决方案
...
### [71] [Structural Induced Exploration for Balanced and Scalable Multi-Robot Path Planning](https://arxiv.org/abs/2512.21654)
*Zikun Guo,Adeyinka P. Adedigba,Rammohan Mallipeddi,Heoncheol Lee*

Main category: cs.RO

TL;DR: 本文提出了一种结合结构先验的蚁群优化框架，用于多机器人路径规划，通过空间分布约束搜索空间，改进路径紧凑性、稳定性和负载均衡。


<details>
  <summary>Details</summary>
Motivation: 多机器人路径规划具有组合复杂性，需要在全局效率和公平任务分配之间取得平衡。传统群体智能方法在小规模实例上有效，但容易早熟收敛，难以扩展到复杂环境。

Method: 提出结构诱导探索框架，将结构先验集成到蚁群优化搜索过程中：1) 利用任务空间分布初始化结构先验约束搜索空间；2) 设计信息素更新规则强调结构有意义的连接；3) 引入负载感知目标平衡总行程距离和个体机器人工作量；4) 采用显式重叠抑制策略确保任务区分和团队平衡。

Result: 在多种基准场景（不同实例规模和机器人团队配置）上验证，结果显示在路径紧凑性、稳定性和工作量分布方面相比代表性元启发式基线方法有持续改进。
...
### [72] [MAction-SocialNav: Multi-Action Socially Compliant Navigation via Reasoning-enhanced Prompt Tuning](https://arxiv.org/abs/2512.21722)
*Zishuo Wang,Xinyu Zhang,Zhuonan Liu,Tomohito Kawabata,Daeun Song,Xuesu Xiao,Ling Xiao*

Main category: cs.RO

TL;DR: MAction-SocialNav是一个用于社会合规导航的高效视觉语言模型，能处理动作模糊性，在单一场景中生成多个合理动作，相比GPT-4o和Claude在决策质量和安全性上表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有社会合规导航方法通常假设单一正确动作，无法处理现实世界中社会规范的模糊性和不确定性，限制了在真实社交环境中的应用。

Method: 提出MAction-SocialNav模型，采用元认知提示（MCP）方法增强推理能力，并构建包含789个样本的多动作社会合规导航数据集，每个样本包含三轮对话。

Result: 模型在决策质量（APG: 0.595 vs. 0.000/0.025）和安全对齐（ER: 0.264 vs. 0.642/0.668）上显著优于GPT-4o和Claude，同时保持实时效率（1.524 FPS，快3倍以上）。

Conclusion: MAction-SocialNav通过显式处理动作模糊性，实现了强大的社会推理能力和高效率，在真实人机导航场景中具有应用潜力。
...
### [73] [MoonBot: Modular and On-Demand Reconfigurable Robot Toward Moon Base Construction](https://arxiv.org/abs/2512.21853)
...
TL;DR: MoonBot是一个模块化可重构机器人系统，专为月球表面探索和开发设计，能够在严格的质量限制下适应不同环境条件和任务需求，通过现场演示验证了其在月球基础设施建设中的可行性。


<details>
  <summary>Details</summary>
Motivation: 月球表面探索和开发日益受到全球关注，机器人对于探索未知地形、利用本地资源以及建设未来人类栖息地至关重要。需要一种能够在严格质量限制下适应多变环境和任务需求的机器人系统。

Method: 设计了模块化按需可重构机器人系统（MoonBot），详细介绍了其设计和开发过程，并通过初步现场演示验证概念，模拟月球基础设施建设的关键任务，包括土木工程操作、基础设施组件运输部署以及与充气模块的辅助操作。

Result: 通过现场演示成功验证了MoonBot的概念证明，能够执行模拟月球基础设施建设的里程碑任务。系统总结了测试过程中获得的经验教训，特别是连接器设计方面，为未来月球任务中模块化机器人系统的发展提供了宝贵见解。

Conclusion: MoonBot作为模块化可重构机器人系统，在严格质量限制下展现了适应月球多变环境和任务需求的能力，为未来月球探索和开发任务提供了有前景的解决方案，特别是在连接器设计方面的经验教训对后续系统改进具有重要意义。
...
### [74] [Optimal Trajectory Planning for Orbital Robot Rendezvous and Docking](https://arxiv.org/abs/2512.21882)
*Kenta Iizuka,Akiyoshi Uchida,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出了一种基于非线性优化的轨迹规划方法，用于空间碎片清除任务中接近翻滚目标的近距离交会，采用动态禁区球体确保安全接近，并开发了考虑实际约束的控制策略。


<details>
  <summary>Details</summary>
Motivation: 空间碎片清除任务中，使用机械臂的服务卫星需要安全接近翻滚的目标碎片，这是一个关键挑战。需要开发一种方法将自由漂浮、旋转的碎片物体带入机械臂工作空间，为后续捕获做准备。

Method: 提出基于非线性优化的轨迹规划方法，引入动态禁区球体根据接近条件自适应调整，允许更近更安全地接近目标。同时开发控制策略，考虑实际实现约束，使用离散开关推进器复制优化轨迹。

Result: 该方法能够实现安全接近翻滚空间碎片，动态禁区球体机制提高了接近安全性，控制策略考虑了实际工程约束，为后续捕获操作提供了可行的初步步骤。

Conclusion: 提出的轨迹规划和控制策略为空间碎片清除任务中的安全接近操作提供了有效解决方案，动态禁区球体概念增强了安全性，为实际工程应用奠定了基础。
...
### [75] [Online Inertia Parameter Estimation for Unknown Objects Grasped by a Manipulator Towards Space Applications](https://arxiv.org/abs/2512.21886)
...
<details>
  <summary>Details</summary>
Motivation: 在空间机器人操作中，掌握被抓取物体的惯性参数对于动力学感知操作至关重要，特别是在具有自由漂浮基座的太空机器人应用中

Method: 应用并扩展现有的在线识别方法，通过结合动量守恒原理，使其适用于自由漂浮基座机器人

Result: 通过数值模拟验证了所提方法，将估计参数与真实值进行比较，结果显示在多种场景下都能实现准确识别

Conclusion: 该方法在轨道服务和其它空间任务中具有应用潜力，能够准确识别未知目标物体的惯性参数
...
### [76] [Aerial World Model for Long-horizon Visual Generation and Navigation in 3D Space](https://arxiv.org/abs/2512.21887)
...
TL;DR: ANWM是一个用于无人机导航的世界模型，通过预测未来视觉观测来评估候选轨迹的语义合理性和导航效用，显著提升大规模环境中的导航成功率。


<details>
  <summary>Details</summary>
Motivation: 现有无人机导航策略通常只优化低层目标（如避障和轨迹平滑），缺乏将高层语义融入规划的能力。需要弥合这一差距，使无人机能够基于语义信息进行导航决策。

Method: 提出ANWM（空中导航世界模型），基于4-DoF无人机轨迹训练，引入物理启发的未来帧投影模块（FFP），将过去帧投影到未来视点以提供粗略几何先验，减少长距离视觉生成的表示不确定性，并捕捉3D轨迹与自我中心观测之间的映射关系。

Result: 实证结果表明，ANWM在长距离视觉预测方面显著优于现有世界模型，并在大规模环境中提高了无人机导航成功率。

Conclusion: ANWM通过结合物理启发的几何先验和视觉预测能力，成功将高层语义信息融入无人机导航规划，为大规模三维环境中的自主导航提供了有效解决方案。
...
### [77] [Flexible Multitask Learning with Factorized Diffusion Policy](https://arxiv.org/abs/2512.21898)
*Chaoqi Liu,Haonan Chen,Sigmund H. Høeg,Shaoxiong Yao,Yunzhu Li,Kris Hauser,Yilun Du*

Main category: cs.RO

TL;DR: 提出模块化扩散策略框架，将复杂的机器人动作分布分解为多个专门扩散模型的组合，每个模型捕捉行为空间的不同子模式，从而提高策略效果并实现灵活适应


<details>
  <summary>Details</summary>
Motivation: 多任务学习中机器人动作分布高度多模态且多样化，现有单一模型难以有效拟合这些复杂分布，容易欠拟合且缺乏灵活适应能力

Method: 引入模块化扩散策略框架，将复杂动作分布分解为多个专门扩散模型的组合，每个模型捕捉行为空间的不同子模式，支持通过添加或微调组件实现灵活策略适应

Result: 在仿真和真实世界机器人操作场景中，该方法持续优于强大的模块化和单一模型基线方法

Conclusion: 模块化扩散策略框架能有效处理多任务学习中的复杂动作分布，提供更好的拟合能力和适应灵活性，同时缓解灾难性遗忘问题
...
### [78] [StereoVLA: Enhancing Vision-Language-Action Models with Stereo Vision](https://arxiv.org/abs/2512.21970)
...
<details>
  <summary>Details</summary>
Motivation: 立体视觉能提供丰富的空间线索，对机器人精确操作至关重要，但现有的视觉-语言-动作模型很少探索立体视觉的应用，存在研究空白

Method: 提出StereoVLA模型，包含几何-语义特征提取模块（利用视觉基础模型提取立体视图差异的几何特征和单目视图的语义特征）和交互区域深度估计辅助任务

Result: 在立体视觉设置下的多样化任务中大幅超越基线方法，对相机姿态变化表现出强鲁棒性，模型收敛速度加快

Conclusion: StereoVLA成功整合立体视觉的几何线索，显著提升视觉-语言-动作模型的性能，为机器人精确操作提供了新思路
...
### [79] [Bab_Sak Robotic Intubation System (BRIS): A Learning-Enabled Control Framework for Safe Fiberoptic Endotracheal Intubation](https://arxiv.org/abs/2512.21983)
*Saksham Gupta,Sarthak Mishra,Arshad Ayub,Kamran Farooque,Spandan Roy,Babita Gupta*

Main category: cs.RO

TL;DR: BRIS是一个紧凑的人机协同机器人插管系统，集成了四向可操纵纤维支气管镜、独立的气管导管推进机制和摄像头增强口件，通过学习型闭环控制和单目内窥镜深度估计实现安全、直观的插管操作。


<details>
  <summary>Details</summary>
Motivation: 现有机器人插管系统主要关注气道导航，缺乏对气管导管推进的集成控制和基于隆突的客观深度验证。气管插管是关键技术要求高的操作，失败或不正确放置会导致严重并发症。

Method: 系统整合四向可操纵纤维支气管镜、独立气管导管推进机制和摄像头增强口件。采用学习型闭环控制框架，利用实时形状传感将操纵杆输入映射到支气管镜尖端在笛卡尔空间的运动。使用单目内窥镜深度估计进行气道区域分类，提供可解释的解剖感知引导。

Result: 在高保真气道模型上进行验证，包括标准和困难气道配置，展示了可靠的导航和受控的导管放置性能。

Conclusion: BRIS代表了向更安全、更一致且临床兼容的机器人气道管理迈出的一步，通过集成导航、导管推进和客观深度验证功能，提高了插管操作的安全性和可靠性。
