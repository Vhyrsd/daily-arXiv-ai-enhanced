...
TL;DR: 本文提出了一种用于实时路边交通违规分析的边缘AI感知节点，在资源受限的嵌入式设备上实现高精度多类别违规检测和车牌识别，并通过V2X协议发布安全事件。


<details>
  <summary>Details</summary>
Motivation: 新兴经济体如印度快速机动化导致执法严重不足，2023年记录违规超过1100万起，而警力密度仅为每4000辆车一名警官。传统监控和人工罚单无法应对如此规模，需要自主、协同且节能的边缘AI感知基础设施。

Method: 系统集成YOLOv8 Nano进行高精度多目标检测，DeepSORT实现时间一致性车辆跟踪，以及基于规则的OCR后处理引擎识别符合标准的车牌。部署在NVIDIA Jetson Nano上，通过TensorRT FP16量化优化，支持实时28-30FPS推理。

Result: 系统在9.6W功耗下实现97.7%违规检测准确率和84.9%OCR精度，涵盖信号灯闯行、斑马线违规、逆行、非法掉头和超速五类违规。相比其他模型，平均精度提升10.7%，能效比提升1.4倍。

Conclusion: 路边边缘AI分析节点能够增强协同感知和主动道路安全管理，通过V2X协议向联网车辆和智能交通系统后端发布标准化安全事件，证明了在IEEE智能车辆生态系统中的可行性。
...
TL;DR: 该论文提出了ROAD基准测试，用于评估LiDAR点云分类在同时面临域偏移和标签演化时的持续学习能力，填补了3D感知研究中持续学习和迁移学习的空白。


<details>
  <summary>Details</summary>
Motivation: 3D感知系统在自动驾驶和具身AI等实际应用中需要适应不断变化的对象定义和传感器域，但当前3D点云感知中的持续学习和迁移学习研究相对不足，特别是在同时面临域偏移和标签演化的情况下。

Method: 提出了ROAD基准测试，专门针对LiDAR点云分类任务，考虑域偏移以及三种标签演化形式：类别分裂、类别扩展和类别插入。使用大规模数据集（Waymo、NuScenes、Argoverse2），评估零样本迁移、线性探测和持续学习方法，并分析骨干架构、训练目标和CL方法的影响。

Result: 研究发现现有方法在现实偏移下存在局限性，为未来稳健3D感知研究建立了强基线。揭示了不同方法在应对域偏移和标签演化方面的表现差异。

Conclusion: ROAD基准测试填补了3D点云感知中持续学习和迁移学习研究的空白，为评估模型在同时面临域偏移和标签演化时的鲁棒性提供了全面框架，推动了稳健3D感知系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 2D编辑中的直观拖拽编辑已取得显著进展，但在3D场景编辑中仍面临挑战。现有3D编辑方法在几何相关内容编辑方面存在局限性，需要一种更高效直观的3D编辑方案

Method: 结合两种关键技术：1）基于3D高斯泼溅的变形引导实现一致的几何修改；2）扩散引导用于内容校正和视觉质量提升。采用渐进式编辑策略支持激进3D拖拽编辑
...
TL;DR: 提出AKT模型解决无人机图像中玉米点级定位难题，通过PKAN模块和PAA注意力机制提升小目标特征提取能力，在PML数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 无人机高分辨率摄影测量在精准农业中至关重要，但玉米点级定位面临三大挑战：极小的物体像素比（<0.1%）、超高分辨率图像（>3000×4000）的二次注意力计算成本过高、以及农业场景特有的稀疏分布和环境变化问题

Method: 提出加性Kolmogorov-Arnold Transformer（AKT），用Pade Kolmogorov-Arnold Network（PKAN）模块替代传统多层感知机增强小目标特征提取能力，引入PKAN加性注意力（PAA）以降低计算复杂度建模多尺度空间依赖，并构建包含1928张高分辨率无人机图像和约501,000个点标注的PML数据集

Result: AKT平均F1分数达62.8%，比现有最佳方法提升4.2%，同时减少12.6%的FLOPs并提高20.7%的推理吞吐量；在下游任务中，株数统计的平均绝对误差为7.1，株间距估计的均方根误差为1.95-1.97厘米
...
<details>
  <summary>Details</summary>
Motivation: 开发一个更高效的视觉搜索处理模型，通过减少参数来简化系统，为医学图像感知、计算机视觉、目标检测等应用提供更好的理论基础。

Method: 提出统计理想观察者模型，采用整体视觉搜索处理，通过设置最小可提取图像特征的阈值来减少自由参数。

Result: 开发了一个参数更少、系统更紧凑的视觉搜索模型，为多个应用领域提供了新的理论框架。

Conclusion: 该新型统计理想观察者模型通过阈值设置减少参数，为医学图像感知、计算机视觉、目标检测等领域的系统优化和性能评估提供了有效工具。
...
TL;DR: 该研究探索了Gabor特征和GLCM纹理特征在建模早期视觉搜索行为中的关系，提出了两种特征组合流程来预测人类注视区域，并在模拟乳腺断层合成图像上验证了有效性。
...
Result: 结果显示，提出的流程预测的注视候选区域与基于阈值的模型观察者之间存在定性一致性。观察到GLCM均值和Gabor特征响应之间存在强相关性，表明这些特征编码了相关的图像信息。人类观察者的眼动追踪数据进一步表明预测的注视区域与早期注视行为具有一致性。

Conclusion: 这些发现强调了结合结构性和纹理特征对建模视觉搜索的价值，支持开发感知信息化的观察者模型。特征组合方法能够有效预测人类早期视觉搜索行为。
...
TL;DR: 本文提出CASHEW和CASHEW-RL两种方法，通过推理时聚合多个候选轨迹和强化学习训练，提升视觉语言模型的多步推理稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解和推理任务中表现良好，但其多步推理仍不稳定，相同输入下重复采样会产生不同的推理轨迹和不一致的最终预测。

Method: 提出两种互补方法：1) CASHEW推理时框架，通过迭代聚合多个候选轨迹形成高质量推理轨迹，利用显式视觉验证过滤幻觉步骤；2) CASHEW-RL学习变体，通过Group Sequence Policy Optimization训练，使用复合奖励鼓励基于最小但充分的视觉证据得出正确答案。

Result: 在13个图像理解、视频理解和视频推理基准测试中显示显著性能提升，包括ScienceQA上提升23.6个百分点，EgoSchema上提升8.1个百分点。

Conclusion: CASHEW和CASHEW-RL方法能有效稳定视觉语言模型的多步推理，提高预测一致性和性能，特别是在需要复杂推理的任务中表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于文本条件的扩散编辑器在单个对象替换方面表现良好，但当需要同时引入新对象和新风格时效果不佳。需要一种能够同时处理内容和外观控制的编辑方法。

Method: 提出Twin-Prompt Attention Blend (TP-Blend)框架，包含两个互补的注意力处理器：1) Cross-Attention Object Fusion (CAOF)：通过熵正则化最优传输问题重新分配多头特征向量，实现对象融合；2) Self-Attention Style Fusion (SASF)：通过细节敏感实例归一化和Key/Value矩阵交换注入风格纹理。

Result: 实验表明TP-Blend能够生成高分辨率、照片级真实的编辑结果，在内容和外观控制方面具有精确性，在定量保真度、感知质量和推理速度方面超越现有基线方法。

Conclusion: TP-Blend是一个轻量级、无需训练的双提示融合框架，能够同时实现对象替换和风格注入，为扩散模型的多模态编辑提供了有效的解决方案。
...
TL;DR: 提出了一种无需训练、仅基于视觉的零样本异常检测框架DIVAD，利用预训练DDIM模型的反转和重构过程来检测和定位异常，无需细粒度提示或额外模态。


<details>
  <summary>Details</summary>
Motivation: 当前零样本异常检测方法要么依赖语言等额外模态生成细粒度提示，要么仅能进行图像级分类而缺乏空间定位精度。需要一种简单有效的纯视觉方法，既能精确定位异常又无需依赖提示。

Method: 基于预训练的DDIM模型，给定输入图像和通用文本描述，将图像反转到潜在空间，从固定中间时间步开始去噪重构。由于扩散模型仅在正常数据上训练，重构结果呈现正常外观，输入与重构的差异即指示异常。

Result: 在VISA数据集上达到最先进性能，展示了强大的异常定位能力，无需辅助模态，推动了零样本异常检测研究从提示依赖的转变。

Conclusion: 提出的DIVAD框架是一种简单有效的训练免费、纯视觉的零样本异常检测方法，通过扩散模型反转和重构实现精确异常定位，减少了对提示的依赖，为相关研究提供了新方向。
...
TL;DR: FigEx2是一个视觉条件化框架，能够从科学复合图中定位面板并生成面板级描述，通过噪声感知门控融合模块和强化学习策略提升性能，在跨学科测试中表现出色。
...
Method: 提出FigEx2视觉条件化框架，包含噪声感知门控融合模块来过滤标记级特征以稳定检测查询空间，采用监督学习与强化学习结合的阶段优化策略，使用CLIP对齐和BERTScore语义奖励来确保严格的多模态一致性。

Result: FigEx2在检测任务上达到0.726 mAP@0.5:0.95的优异性能，在METEOR和BERTScore指标上显著优于Qwen3-VL-8B模型，并且在无需微调的情况下展现出对分布外科学领域的卓越零样本迁移能力。

Conclusion: FigEx2框架有效解决了科学复合图的面板定位和描述生成问题，通过创新的融合模块和优化策略实现了高性能，在跨学科应用中表现出强大的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学出版物中的科学图像篡改对研究完整性和可重复性构成威胁，但现有方法难以处理领域特定的伪影、复杂纹理和无结构的图表布局，需要专门针对生物医学领域的伪造检测解决方案。

Method: 提出视觉语言引导框架：1）使用扩散模型结合视觉语言提示生成语义可控的生物医学图像伪造（复制、拼接、区域移除）；2）建立Rescind基准数据集，包含细粒度标注和模态特定划分；3）开发Integscan检测框架，结合注意力增强的视觉编码和提示条件语义对齐的结构化状态空间建模；4）引入视觉语言模型验证循环，基于提示一致性过滤生成的伪造图像。

Result: 在Rescind和现有基准上的大量实验表明，Integscan在伪造检测和定位方面均达到最先进的性能，为自动化科学完整性分析建立了坚实基础。

Conclusion: 该研究首次提出了针对生物医学图像的视觉语言引导伪造生成与检测框架，通过创新的基准数据集和检测方法，显著提升了生物医学图像伪造检测的能力，为维护科学研究的完整性提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 少样本场景下医学图像分割面临标注数据稀缺的挑战，而基于自然图像训练的DINOv3等自监督基础模型虽然具有强大的特征提取能力，但直接应用于医学图像存在领域差异问题
...
Conclusion: 该自动化流水线为领域特定合成数据集的生成提供了有效解决方案，有助于弥合预训练模型与实际部署环境之间的差距
...
### [14] [Edge-Optimized Multimodal Learning for UAV Video Understanding via BLIP-2](https://arxiv.org/abs/2601.08408)
...
Method: 1) 将BLIP-2与YOLO模型深度融合，利用YOLO的精确感知结果进行目标检测和实例分割；2) 设计基于K-Means聚类的内容感知关键帧采样机制，实现智能帧选择和时序特征拼接；3) 实施统一提示优化方案，将YOLO的结构化事件日志作为上下文注入BLIP-2输入，结合输出约束过滤技术细节。

Result: 该平台扩展了BLIP-2在无人机应用中的多任务能力，实现了视频级交互任务处理，能够生成准确且上下文相关的输出，同时保持轻量级架构。

Conclusion: 提出的轻量级多模态任务平台有效解决了无人机边缘设备计算资源有限与视觉语言模型高计算需求之间的矛盾，为无人机复杂场景下的实时视觉理解与交互提供了可行解决方案。
...
TL;DR: PathoGen是一种基于扩散模型的生成模型，能够在良性组织病理学图像中进行可控、高保真的病变修复，解决标注数据稀缺问题，提升下游分割性能。


<details>
  <summary>Details</summary>
Motivation: 组织病理学诊断中AI模型开发面临专家标注病变数据稀缺的严重限制，特别是罕见病理和代表性不足的疾病亚型。现有数据增强方法无法生成足够真实的病变形态，难以保持组织病理学组织特有的复杂空间关系和细胞结构。

Method: 提出PathoGen，一种基于扩散模型的生成模型，利用扩散模型的迭代细化过程，在良性组织病理学图像中进行可控、高保真的病变修复，合成具有自然组织边界、保留细胞结构和真实染色特征的病变。

Result: 在肾脏、皮肤、乳腺和前列腺病理四个不同诊断挑战的数据集上验证。定量评估显示PathoGen在图像保真度和分布相似性方面优于包括条件GAN和Stable Diffusion在内的最先进生成基线。使用PathoGen合成的病变增强训练集相比传统几何增强能提升下游分割性能，特别是在数据稀缺情况下。
...
### [16] [Motion Attribution for Video Generation](https://arxiv.org/abs/2601.08828)
...
<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型快速发展，但数据如何影响运动质量仍不清楚。现有方法主要关注视觉外观，缺乏对时间动态特性的专门分析。

Method: 提出Motive框架：1) 使用运动加权损失掩码分离时间动态和静态外观；2) 基于梯度的数据归因方法；3) 可扩展到现代大规模高质量视频数据集和模型；4) 识别对运动有强影响的微调片段。

Result: 1) 在文本到视频模型中识别出显著影响运动的片段；2) 通过Motive筛选的高影响力数据，在VBench上提升了运动平滑度和动态程度；3) 与预训练基础模型相比获得74.1%的人类偏好胜率。

Conclusion: Motive是首个专门针对视频生成模型中运动而非视觉外观进行归因的框架，可用于指导数据筛选，有效提升视频生成的时间一致性和物理合理性。
...
TL;DR: 本文提出SSP框架，通过光学流和文本提示增强音频-视觉语义分割，解决移动和静止发声物体的分割挑战。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉语义分割（AVSS）需要在像素级别识别发声物体的基础上，进一步理解音频-视觉场景的语义信息。现有方法将任务分解为两个子任务，但面临移动物体和静止发声物体的分割挑战。

Method: 提出SSP协作框架：1）使用光学流捕捉运动动态，为移动物体提供时间上下文；2）引入两种文本提示（物体类别和场景描述）处理静止发声物体；3）设计视觉-文本对齐模块实现跨模态集成；4）采用后掩码技术训练模型学习光学流图。

Result: 实验结果表明，SSP在音频-视觉分割任务上优于现有方法，实现了高效精确的分割结果。

Conclusion: SSP框架通过整合光学流和文本提示，有效解决了音频-视觉语义分割中移动和静止物体的挑战，提供了更连贯和上下文相关的语义解释。
...
TL;DR: SubTTA：通过语义子空间对齐增强视觉语言模型的测试时适应能力，解决模态鸿沟和视觉噪声问题，平均性能提升2.24%


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本能力强大但对分布偏移敏感。现有测试时适应方法依赖零样本预测作为伪标签，但在分布偏移下不可靠，主要受两个限制：1）模态鸿沟：分布偏移导致视觉和文本模态间存在差距，跨模态关系不准确；2）视觉噪声：视觉嵌入编码了丰富但与任务无关的噪声，在分布偏移下会淹没任务特定语义。

Method: 提出SubTTA方法，通过对齐两个模态的语义子空间来增强零样本预测以更好地指导TTA过程。具体包括：1）为弥合模态鸿沟，提取两个模态的主子空间，通过最小化弦距离将视觉流形对齐到文本语义锚点；2）为消除视觉噪声，将对齐后的视觉特征投影到任务特定的文本子空间，通过将视觉嵌入约束在有效语义范围内过滤掉任务无关噪声，然后在净化后的空间上执行标准TTA以细化决策边界。

Result: 在各种基准测试和VLM架构上的广泛实验证明了SubTTA的有效性，相比最先进的TTA方法平均提升了2.24%的性能。

Conclusion: SubTTA通过语义子空间对齐有效解决了视觉语言模型测试时适应中的模态鸿沟和视觉噪声问题，显著提升了模型在分布偏移下的适应能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前可解释视频异常检测方法缺乏空间定位能力，特别是在多实体交互场景中，现有方法产生的解释往往不完整或视觉不对齐，降低了可信度。需要可验证的解释方法来提高异常检测系统的可信度。

Method: 提出实例对齐描述框架，将每个文本声明链接到特定的对象实例，包含外观和运动属性。框架能够识别谁导致了异常、每个实体在做什么、影响了谁以及解释的空间定位。创建了VIEW360+数据集，扩展了八个VAD基准和360度第一人称数据集。

Result: 实验表明，实例级空间定位描述揭示了当前基于LLM和VLM方法的显著局限性，同时为未来可信和可解释的异常检测研究提供了稳健的基准。

Conclusion: 提出的实例对齐描述框架为可解释视频异常检测提供了可验证和可操作的解释方法，通过空间定位提高了异常检测系统的可信度和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 火箭发射过程中，强烈燃烧产生的浓密颗粒雾霾和超过120dB的亮度变化严重降低了图像质量，影响了后续的光学测量和流场分析，需要解决极端成像条件下的测量难题

Method: 采用硬件-算法协同设计框架：1) 定制空间变化曝光传感器，单次拍摄获取多曝光数据；2) 物理感知去雾算法，动态估计雾霾密度，进行区域自适应光照优化，应用多尺度熵约束融合技术分离雾霾与场景辐射

Result: 在真实发射图像和受控实验中验证，该框架在恢复羽流和发动机区域物理准确视觉信息方面表现出优越性能，为提取粒子速度、流动不稳定频率和结构振动等关键机械参数提供了可靠的图像基础

Conclusion: 该硬件-算法协同设计框架能够有效应对火箭发射极端成像条件，支持在极端航空航天环境中进行精确的定量分析
...
TL;DR: 提出SISTA框架，通过图像-报告和补丁-单词两级对齐，改进医学视觉语言预训练，消除假阴性样本并提升细粒度任务性能。
...
Method: 提出SISTA框架，包含两个关键改进：1）通过引入报告间相似性消除假阴性样本；2）在补丁-单词级别对齐图像补丁与相关词标记。

Result: 实验表明该框架在不同数据集的下游任务（图像分类、分割、检测）上提升迁移性能，特别是在有限标注数据的细粒度任务中取得显著改进。

Conclusion: SISTA框架通过多级语义对齐有效改进医学视觉语言预训练，代码和预训练模型将开源。
...
TL;DR: 该技术报告介绍了在RoboSense2025挑战赛中获奖的跨平台3D目标检测解决方案，基于PVRCNN++框架，通过数据增强和自训练策略提升跨平台泛化能力，在挑战赛中获得了第三名。


<details>
  <summary>Details</summary>
Motivation: 解决跨平台3D目标检测中的领域差距问题，提升模型在不同传感器平台间的泛化能力，以应对实际自动驾驶场景中不同传感器配置带来的挑战。

Method: 基于PVRCNN++框架，结合点云和体素特征；采用定制化数据增强策略缩小领域差距；使用带伪标签的自训练策略进一步提升跨平台泛化性能。

Result: 在RoboSense2025挑战赛中获得了第三名：阶段一目标域Car类别的3D AP达到62.67%；阶段二目标域Car和Pedestrian类别的3D AP分别达到58.76%和49.81%。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法缺乏人类认知过程中的记忆和持续学习能力，无法有效处理动态场景和多访视场景。需要开发能够模拟人类认知过程、具有记忆存储和更新能力的动态场景理解框架。

Method: 框架包含三个核心能力：1）多阶段运动线索框架识别动态对象；2）认知映射系统存储、回忆和更新多访视静态场景；3）因子图优化策略精炼相机位姿。通过运动线索结合深度和相机位姿先验识别动态区域，将静态元素与记忆库匹配，在重访时检索存储场景并更新记忆。

Result: 在视频深度估计、相机位姿重建和3D映射任务上展示了最先进的性能，有效支持跨扩展序列和多访视的连续场景理解。

Conclusion: CogniMap3D通过模拟人类认知过程，实现了具有记忆能力的动态3D场景理解，为持续学习和多访视场景理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的3D虚拟形象通常只有有限的几种基本面部表情，缺乏真实情感变化的模拟能力。需要一种能够根据文本指令生成任意表情之间平滑过渡的方法，以增强虚拟形象的表达真实性和多样性。

Method: 1. 提出IFED模块进行多模态数据学习，捕捉文本描述与面部表情特征之间的相关性；2. 提出I2FET方法，利用IFED和顶点重建损失函数来优化潜在向量的语义理解；3. 开发面部表情过渡模型，生成表情之间的平滑过渡序列。

Result: 在CK+和CelebV-HQ数据集上的广泛评估表明，该模型优于现有最先进方法。框架能够根据文本指令生成面部表情轨迹，文本提示允许对人类情感状态进行多样化描述，从而大大扩展了面部表情库及其过渡方式。

Conclusion: 该研究提出了一个创新的文本指令驱动面部表情生成框架，能够实现任意表情之间的平滑过渡，显著增强了3D虚拟形象的情感表达能力，具有广泛的实际应用前景。
...
TL;DR: 本文针对传统角点检测方法在相邻角点灰度信息相互影响的问题，提出了一种基于二阶高斯方向导数滤波器的高分辨率角点检测方法，能够准确检测相邻角点。


<details>
  <summary>Details</summary>
Motivation: 传统角点检测方法（如Zhang等人的方法）使用简单角点模型存在理论缺陷，因为相邻角点的灰度信息会相互影响，导致检测不准确。特别是在高分辨率图像中，这个问题更加明显。

Method: 使用二阶高斯方向导数滤波器对两种典型高分辨率角点模型（END型和L型）进行平滑处理，推导出这些模型的SOGDD表示，发现高分辨率角点的多种特征。基于这些特征，提出了一种新的高分辨率角点检测方法，能够准确检测相邻角点。

Result: 实验结果表明，该方法在定位误差、对图像模糊变换的鲁棒性、图像匹配和3D重建等方面均优于现有最先进的方法。

Conclusion: 提出的基于SOGDD的高分辨率角点检测方法有效解决了相邻角点灰度信息相互影响的问题，在多个计算机视觉任务中表现出优越性能。
...
TL;DR: 多模态大语言模型在胃肠内镜临床工作流中表现出潜力，在诊断推理方面优于实习生并与初级内镜医师相当，但在空间定位和事实准确性方面存在瓶颈。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在胃肠病学中显示出前景，但其在完整临床工作流程中的表现以及与人类基准的比较尚未得到验证。需要系统评估MLLMs在胃肠内镜工作流中的临床效用。

Method: 构建GI-Bench基准测试，包含20个细粒度病变类别。评估12个MLLMs在五阶段临床工作流程中的表现：解剖定位、病变识别、诊断、发现描述和管理。使用Macro-F1、平均交并比和多维度Likert量表将模型性能与3名初级内镜医师和3名住院医师进行比较。

Result: Gemini-3-Pro达到最先进性能。在诊断推理方面，顶级模型（Macro-F1 0.641）优于实习生（0.492），与初级内镜医师相当（0.727；p>0.05）。但存在"空间定位瓶颈"：人类病变定位（mIoU >0.506）显著优于最佳模型（0.345；p<0.05）。定性分析显示"流畅性-准确性悖论"：模型生成报告的语言可读性优于人类（p<0.05），但事实正确性显著较低（p<0.05），存在"过度解释"和视觉特征幻觉。

Conclusion: MLLMs在胃肠内镜诊断推理方面显示出临床潜力，但在空间定位和事实准确性方面仍需改进。GI-Bench建立了动态排行榜，用于追踪MLLMs在临床内镜中的性能演变。
...
<details>
  <summary>Details</summary>
Motivation: 多站点脑MRI数据聚合能增强深度学习模型训练，但站点特异性变化（如扫描仪厂商、采集参数、成像协议差异）会引入非生物异质性，影响模型泛化能力。现有方法依赖有限的配对数据或无法有效分离风格与解剖结构，且大多只处理单序列协调，限制了在实际多序列MRI场景中的应用。

Method: MMH采用两阶段框架：1）基于扩散的全局协调器，使用风格无关的梯度条件将MR图像映射到序列特定的统一域；2）目标特定的微调器，将全局对齐图像适配到期望的目标域。使用三平面注意力BiomedCLIP编码器聚合多视角嵌入来表征体积风格信息，实现无需配对数据的风格与解剖结构显式分离。

Result: 在4,163个T1和T2加权MRI图像上的评估显示，MMH在图像特征聚类、体素级比较、组织分割以及下游年龄和站点分类任务中，均优于现有最先进的方法。

Conclusion: MMH通过统一的序列感知协调框架，有效解决了多站点多序列脑MRI数据的异质性问题，提高了模型的泛化能力，为实际临床环境中的多序列MRI协调提供了实用解决方案。
...
Method: 提出MobiDiary框架，使用统一传感器编码器处理异构物理信号（IMU和Wi-Fi）。编码器采用基于补丁的机制捕捉局部时间相关性，集成异构位置嵌入来统一不同传感器的空间上下文。这些统一信号标记输入到基于Transformer的解码器中，通过自回归机制逐词生成连贯的动作描述。

Result: 在多个公共基准测试（XRF V2、UWash和WiFiTAD）上评估，MobiDiary在跨模态泛化方面表现优异，在描述指标（如BLEU@4、CIDEr、RMC）上达到最先进性能，在连续动作理解方面优于专门的基线方法。
...
TL;DR: 本研究比较了基于Vision Transformer(EVA-ViT)图像编码器与LlaMA或ChatGPT LLM的多模态方法，用于减少幻觉问题并检测胸部X光图像疾病。研究使用NIH胸部X光图像数据集，比较了基于图像的RAG、基于文本的RAG和基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决多模态医学图像分析中的幻觉问题，提高胸部X光图像疾病检测的准确性和可靠性，通过比较不同方法探索减少LLM在医学图像分析中产生错误信息的最佳策略。

Method: 使用EVA-ViT作为图像编码器，结合LlaMA和ChatGPT两种LLM，采用三种方法：基于图像的RAG（使用KNN方法）、基于文本的RAG（利用外部知识信息）和基线方法。使用NIH胸部X光图像数据集进行训练和评估。

Result: 基于文本的RAG能有效减少幻觉问题；基于图像的RAG通过KNN方法提高了预测置信度和校准度；GPT LLM相比LlaMA表现出更好的性能、更低的幻觉率和更好的预期校准误差(ECE)。

Conclusion: 研究揭示了数据不平衡和复杂多阶段结构的挑战，但提出了大规模经验环境和平衡使用示例的建议，为多模态医学图像分析提供了有价值的参考。
...
TL;DR: 提出基于事件分割和置信度估计的零样本ADL识别方法，相比基于时间分割的LLM方法在复杂数据集上表现更好，甚至超越有监督方法。
...
Result: 事件分割方法在复杂现实数据集上持续优于基于时间的LLM方法，甚至超越有监督数据驱动方法；提出的置信度度量能有效区分正确和错误预测。
...
TL;DR: HIPPO提出了一种用于视频LLM的并行推测解码框架，通过语义感知的token保留和并行化设计，在保持输出质量的同时实现高达3.51倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有视频LLM的推测解码方法主要关注剪枝冗余视觉token，但无法达到文本LLM的推理加速效果。主要问题包括：1）剪枝策略无法充分保留视觉语义token，降低草稿质量和接受率；2）即使大幅剪枝（如90%），草稿模型的剩余推理成本仍限制整体加速效果。
...
Conclusion: HIPPO通过语义感知token保留和并行化设计，解决了视频LLM推测解码中的关键限制，实现了显著的推理加速，为视频LLM的高效部署提供了有效方案。
...
TL;DR: KidVis基准测试揭示：尽管多模态大语言模型在高级推理任务上表现出色，但缺乏6-7岁儿童具备的基础视觉感知能力，且模型参数增加并不能线性提升这些基础能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探究多模态大语言模型是否具备与人类直觉相当的基础视觉感知能力，特别是与6-7岁儿童相比的视觉发展水平。

Method: 引入KidVis基准测试，基于人类视觉发展理论，将视觉智能分解为六个原子能力：专注力、追踪、辨别、记忆、空间和闭合性，包含10类低语义依赖的视觉任务，评估20个最先进的MLLM模型并与人类生理基线对比。

Result: 人类儿童平均得分接近完美（95.32），而最先进的GPT-5仅得67.33，存在显著性能差距。发现"缩放定律悖论"：单纯增加模型参数无法线性改善这些基础视觉能力。

Conclusion: 当前多模态大语言模型尽管具备推理能力，但缺乏实现广义视觉智能所需的基本生理感知能力，需要在基础视觉感知方面进行改进。
...
TL;DR: M3SR：一种用于光谱重建任务的多尺度、多感知Mamba架构，通过多感知融合块和U-Net结构有效提取全局、中间和局部特征，在降低计算成本的同时超越现有方法。


<details>
  <summary>Details</summary>
Motivation: Mamba架构在低层视觉任务中表现出色，但在光谱重建中面临两个主要挑战：1）单一空间感知限制了全面理解高光谱图像的能力；2）单尺度特征提取难以捕捉高光谱图像中的复杂结构和精细细节。
...
Conclusion: M3SR通过多尺度、多感知的方法成功解决了Mamba架构在光谱重建中的局限性，实现了更准确的高光谱图像重建，同时保持了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在图像质量评估任务中表现出色，但达到最先进性能通常需要计算成本高昂的微调方法。研究者希望开发一种无需训练的资源高效替代方案。

Method: IQARAG框架包含三个关键阶段：检索特征提取、图像检索、集成与质量分数生成。通过检索语义相似但质量不同的参考图像及其平均意见分数，将这些信息整合到特定提示中，为大型多模态模型提供视觉感知锚点。

Result: 在多个不同的图像质量评估数据集（包括KADID、KonIQ、LIVE Challenge和SPAQ）上的广泛实验表明，IQARAG能有效提升大型多模态模型的图像质量评估性能。

Conclusion: IQARAG为图像质量评估提供了一种资源高效的替代方案，无需进行昂贵的微调即可显著提升大型多模态模型的性能，展示了检索增强生成在视觉质量评估任务中的潜力。
...
TL;DR: 提出YOLOBirDrone架构，用于提高无人机和鸟类的检测分类准确率，解决了现有视觉检测系统难以区分小型鸟类和无人机的问题。


<details>
  <summary>Details</summary>
Motivation: 无人机在商业和国防应用中广泛使用，但也被用于针对性攻击，需要开发无人机检测系统。现有视觉检测系统准确率有限，特别是难以区分无人机和小型鸟类。

Method: 提出YOLOBirDrone架构，包含自适应扩展层聚合(AELAN)、多尺度渐进双注意力模块(MPDA)和反向MPDA(RMPDA)，以保持形状信息并丰富局部和全局空间及通道特征。同时构建了大规模数据集BirDrone。

Result: 实验结果表明，YOLOBirDrone相比其他先进算法在性能指标上有提升，在各种场景下的检测准确率达到约85%。

Conclusion: YOLOBirDrone架构有效提高了无人机和鸟类的检测分类准确率，解决了现有系统难以区分小型鸟类和无人机的问题，为无人机检测系统提供了改进方案。
...
Method: 提出UM-Text统一多模态模型，引入视觉语言模型(VLM)处理指令和参考图像，使文本内容和布局能根据上下文信息精心设计。提出UM-Encoder结合各种条件信息的嵌入，其组合由VLM根据输入指令自动配置。训练中提出区域一致性损失，在潜在空间和RGB空间为字形生成提供更有效的监督，并设计量身定制的三阶段训练策略。此外贡献了UM-DATA-200K大规模视觉文本图像数据集。

Result: 在多个公开基准测试上的广泛定性和定量结果表明，该方法实现了最先进的性能。

Conclusion: UM-Text通过统一的多模态方法解决了视觉文本编辑中的风格一致性问题，通过创新的模型架构和训练策略实现了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法虽然通过建模长距离依赖取得了显著性能，但特征密集型注意力计算成本高。大多数方法采用固定分组注意力，忽略了token相似性的内在不对称性，无法实现灵活的自适应注意力计算
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在多模态基准测试中表现良好，但其在真实感知退化情况下的鲁棒性尚未被充分理解。在自动驾驶和具身AI等安全关键应用中，可靠的感知对于语义推理和决策制定至关重要。

Method: 使用Cityscapes数据集上的语义分割作为代表性感知模块，引入感知现实的退化方法。提出一套语言层面的错位指标来量化幻觉、关键遗漏和安全误判，并分析这些指标与分割质量在多种对比式和生成式VLM中的关系。

Result: 研究发现，即使上游视觉感知只有适度的传统分割指标下降，也会导致下游VLM行为出现严重故障，包括幻觉对象提及、安全关键实体遗漏和不一致的安全判断。像素级鲁棒性与多模态语义可靠性之间存在明显脱节。

Conclusion: 当前基于VLM的系统存在关键局限性，需要在安全关键应用中开发能够明确考虑感知不确定性的评估框架。研究揭示了感知退化对VLM语义可靠性的显著影响，强调了在现实部署中需要更全面的鲁棒性评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有野外图像集合的新视角合成方法虽然效果不错，但缺乏对复杂表面的几何基础，有时会产生不一致的结果

Method: 基于有符号距离函数(SDF)的几何表示来指导渲染过程，并引入新颖的几何保持损失来保留精细结构细节

Result: 实现了竞争力的渲染性能，同时相比类似方法能耗降低了4-5倍，能够生成具有锐利、几何一致细节的逼真结果

Conclusion: Geo-NVS-w是一个鲁棒的野外新视角合成方法，通过几何感知框架实现了高质量、几何一致的渲染结果
...
TL;DR: LoGo：一种针对地理空间点云的新型源自由无监督域适应框架，通过局部-全局双重共识机制解决长尾分布和域偏移问题


<details>
  <summary>Details</summary>
Motivation: 地理空间点云语义分割面临跨区域地理模式差异和数据采集策略变化导致的显著域偏移问题。现有域适应方法通常需要源域数据，但由于数据隐私、监管政策和传输限制，这一要求难以满足，因此需要探索源自由无监督域适应（SFUDA）这一未充分研究的设置。

Method: 提出LoGo框架，包含三个核心模块：1）局部层面：类平衡原型估计模块，采用类内独立锚点挖掘策略而非传统全局阈值过滤，确保样本稀缺的尾部类也能生成鲁棒特征原型；2）全局层面：基于最优传输的全局分布对齐模块，将伪标签分配建模为全局优化问题，通过全局分布约束纠正局部贪婪分配中头部类的过度主导；3）双重一致性伪标签过滤机制，仅保留局部多增强集成预测与全局最优传输分配一致的高置信度伪标签用于自训练。

Result: 该方法有效缓解了长尾分布引起的特征崩溃问题，防止模型预测严重偏向多数类，提高了地理空间点云语义分割在域偏移情况下的性能。

Conclusion: LoGo框架为地理空间点云的源自由无监督域适应提供了一种有效解决方案，通过局部-全局双重共识机制协同工作，解决了长尾分布和域偏移带来的挑战，在数据隐私受限的实际应用场景中具有重要价值。
...
Result: YOLOv8组件达到92%精度和92.5%平均精度；ResNet-50分类器对正常病例和冠周炎病例的F1分数分别为88%和86%；放射科医生报告Grad-CAM与其诊断印象的符合度为84%
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在多视角融合、相机外参不确定性和大规模相机系统可扩展性方面存在困难，需要一种能够同时处理点云融合和外参不确定性的实时多相机3D重建框架。

Method: SPARK包含两个核心模块：(1) 利用多视角先验并强制跨视角和时间一致性的几何感知在线外参估计模块，实现稳定的自校准；(2) 在像素和点级别建模深度可靠性和可见性的置信度驱动点云融合策略，抑制噪声和视角依赖的不一致性。

Result: 在真实世界多相机系统上的广泛实验表明，SPARK在外参精度、几何一致性、时间稳定性和实时性能方面优于现有方法，证明了其在大规模多相机3D重建中的有效性和可扩展性。

Conclusion: SPARK通过联合处理点云融合和外参不确定性，实现了稳定、实时、可扩展的多相机3D重建，适用于动态场景和大规模相机系统。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态地球观测数据的增加，需要有效融合光谱、空间和几何信息并实现语义级理解的方法。现有方法在异构遥感模态对齐和语言引导解释方面存在不足。

Method: 使用模态特定编码器，通过双向对比学习将视觉特征与手工制作的文本嵌入在共享潜在空间中对齐，借鉴CLIP的训练范式。

Result: 在两个基准数据集上，MMLGNet使用简单的CNN编码器就超越了多个已建立的多模态视觉方法，展示了语言监督的显著优势。

Conclusion: MMLGNet成功将高维遥感数据与语言引导解释相结合，证明了语言监督在多模态遥感分析中的重要性，为语义级理解提供了有效框架。
...
TL;DR: 提出一种基于局部补丁的面部动画重定向方法，可将真人表演视频的面部动画转移到风格化3D角色上，同时保持原始表情的语义含义。


<details>
  <summary>Details</summary>
Motivation: 当前面部动画重定向方法在处理风格化或夸张的3D角色时面临挑战，因为这些角色的面部结构与真人差异很大。需要一种方法能够考虑目标角色的面部结构和运动范围，以保持原始面部动作的语义含义。

Method: 提出三模块方法：1) 自动补丁提取模块从源视频帧提取局部补丁；2) 重演模块生成相应的目标局部补丁；3) 权重估计模块计算每帧的动画参数，创建完整的面部动画序列。

Result: 大量实验表明，该方法能够成功将源面部表情的语义含义转移到面部特征比例差异很大的风格化角色上。

Conclusion: 该方法为风格化3D角色的面部动画重定向提供了一种有效的解决方案，能够处理面部结构差异大的情况，保持表情语义的准确性。
...
Method: 提出心脏推理模板（CRT）提供标准化诊断流程，以及CardiacMind强化学习框架，包含三个新颖奖励：程序数量奖励（PQtR）、程序质量奖励（PQlR）和超声心动图语义奖励（ESR）。

Result: 在15种复杂心脏病的多视图超声心动图诊断中性能提升48%，在CardiacNet-PAH上提升5%，用户研究显示93.33%的临床医生认同其心脏病专家式推理逻辑。

Conclusion: 通过引入类似心脏病专家的思维模式，CardiacMind框架显著提升了多模态大语言模型在超声心动图诊断中的推理能力，为医学AI诊断提供了新思路。
...
TL;DR: 提出NAR方法，一种针对遥感多标签分类中不同类型标签噪声的自适应正则化方法，通过置信度机制区分加性、减性和混合噪声，结合半监督学习和早期学习正则化提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 遥感多标签分类中，由于使用主题产品或众包标注来降低成本，常引入部分错误的标注噪声（加性、减性或混合噪声）。现有方法大多忽视这种区别，将噪声标注直接作为监督信号，缺乏针对不同噪声类型的学习机制。

Method: 提出NAR（噪声自适应正则化）方法，在半监督学习框架下明确区分加性和减性噪声。采用基于置信度的标签处理机制：动态保留高置信度标签条目，暂时停用中等置信度条目，通过翻转纠正低置信度条目。这种选择性监督衰减与早期学习正则化（ELR）结合，以稳定训练并减轻对损坏标签的过拟合。

Result: 在加性、减性和混合噪声场景下的实验表明，NAR相比现有方法持续提升鲁棒性。在减性和混合噪声下性能提升最为显著，表明自适应抑制和选择性纠正噪声监督为遥感多标签分类中的噪声鲁棒学习提供了有效策略。

Conclusion: NAR方法通过明确区分不同噪声类型并采用自适应正则化策略，有效解决了遥感多标签分类中的标签噪声问题，特别是在减性和混合噪声场景下表现出色，为噪声鲁棒学习提供了新思路。
...
TL;DR: 本文提出了一种名为静态-动态协作（SDC）的框架，用于解决少样本类增量学习中的稳定性-可塑性困境，通过将学习过程分为静态保留阶段和动态学习阶段来实现更好的平衡。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）需要在有限数据下持续识别新类别，面临稳定性-可塑性困境：如何在保持旧知识的同时获取新知识。为了解决这个问题，本文提出将任务分为两个不同阶段。

Method: 提出静态-动态协作（SDC）框架，将FSCIL分为静态保留阶段（SRS）和动态学习阶段（DLS）。SRS阶段使用基础会话的充足数据训练初始模型，并保留关键部分作为静态记忆；DLS阶段引入额外的动态投影器与静态记忆联合训练。

Result: 在三个公共基准数据集和一个真实世界应用数据集上的广泛实验表明，该方法相比其他竞争者取得了最先进的性能，实现了更好的旧知识保留和新类别适应能力。

Conclusion: 通过将FSCIL分为静态保留和动态学习两个阶段，SDC框架能够更好地平衡稳定性与可塑性，有效解决少样本类增量学习中的关键挑战。
...
TL;DR: 该研究开发了一个结合稳健特征选择的放射组学框架，用于预测高级别浆液性卵巢癌患者对新辅助化疗的治疗反应，最佳模型在体积减少预测上达到AUC 0.83。
...
Method: 采用自动化随机化算法模拟观察者间变异，平衡特征稳健性和预测准确性。使用四种反应指标（CRS、RECIST、体积减少、直径减少），分析不同解剖部位病灶。基于治疗前后CT扫描提取特征，在一个队列训练模型，在独立队列进行外部验证。

Result: 所有病灶组合在体积减少预测上表现最佳（AUC 0.83），网膜病灶在CRS预测上最佳（AUC 0.77），盆腔病灶在直径减少预测上最佳（AUC 0.76）。

Conclusion: 将稳健性整合到特征选择过程中确保了可靠模型的开发，有助于放射组学模型在临床实践中的应用。未来应探索放射组学在卵巢癌中的进一步应用，特别是在实时临床环境中。
...
<details>
  <summary>Details</summary>
Motivation: RGB-T检测在融合模态互补信息方面有优势，但在极端条件下（某一模态质量差）需要模态分离来减少噪声影响。现有方法难以平衡模态互补与分离的需求。

Method: 提出模态解耦的RGB-T检测框架MDQF，使用类似DETR的检测器作为RGB和TIR图像的分支，在每级细化阶段通过查询选择与适配进行跨分支查询融合，排除退化模态并使用高质量查询校正预测。

Result: 大量实验表明，该方法优于现有RGB-T检测器，实现了更好的模态独立性，且无需配对RGB-T数据即可优化各分支。

Conclusion: MDQF框架有效平衡了RGB-T检测中的模态互补与分离需求，在极端条件下通过查询融合排除退化模态干扰，同时支持使用非配对数据进行训练。
...
TL;DR: 提出了一个自动化建筑体量生成框架，并引入了包含详细体量几何、经济数据和场地上下文的CoMa-20K数据集，通过视觉语言模型进行条件生成任务评估。


<details>
  <summary>Details</summary>
Motivation: 建筑和城市规划中的概念设计阶段（特别是建筑体量设计）复杂且高度依赖设计师的直觉和手动工作，需要自动化解决方案。数据驱动方法的主要障碍是缺乏合适的数据集。

Method: 1. 创建CoMa-20K数据集，包含详细体量几何、经济与功能数据、场地视觉上下文；2. 将体量生成制定为视觉语言模型的条件任务；3. 评估微调模型和大型零样本模型。

Result: 实验揭示了任务的固有复杂性，同时展示了视觉语言模型生成上下文敏感体量方案的潜力。数据集和分析为数据驱动的建筑设计建立了基础基准。

Conclusion: 提出的框架和数据集为数据驱动的建筑设计研究奠定了基础，突出了未来研究的重大机遇，特别是在上下文敏感的体量生成方面。
...
Result: 实验表明该方法相比现有基线模型获得了一致的性能提升，证明了该方法在实际道路安全应用中的潜力。

Conclusion: 通过解耦驾驶员外观特征并正交化文本嵌入，能够有效提升视觉语言模型在分心驾驶检测中的零样本分类性能，为实际道路安全应用提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在自动驾驶和移动系统中的部署增加，需要评估其在复杂环境中支持安全决策的能力。现有基准测试未能充分覆盖多样化的危险情况，特别是具有时空动态的异常场景。

Method: 提出了HazardForge可扩展管道，利用图像编辑模型结合布局决策算法和验证模块生成危险场景。基于此构建了MovSafeBench基准测试，包含7,254张图像和相应的问答对，涵盖13个物体类别，包括正常和异常物体。

Result: 实验表明，视觉语言模型在异常物体条件下的性能显著下降，在需要细微运动理解的场景中下降幅度最大。
...
TL;DR: 本文提出了一种基于物联网的智能鱼缸系统，通过ESP32微控制器集成多种传感器和执行器，实现实时水质监测和自动控制，显著提高了鱼缸维护的效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统鱼缸水质监测方法效率低下、劳动密集且容易出错，导致水生环境条件不理想。需要一种能够连续监测多个参数并实现自动控制的解决方案。

Method: 设计并实现了一个物联网智能鱼缸系统，集成ESP32微控制器、pH、TDS、温度和浊度传感器，以及伺服喂食器和水泵执行器。系统采用边缘处理能力，通过Blynk物联网平台实现云连接，并配备具有可配置冷却期的智能警报机制。
...
TL;DR: EfficientFSL是一个专为ViT设计的查询式微调框架，通过极少的可训练参数实现少样本分类，显著降低计算开销


<details>
  <summary>Details</summary>
Motivation: 大型视觉Transformer模型在少样本分类中表现优异，但微调需要大量GPU内存和训练时间，不适用于低资源场景

Method: 提出三个核心模块：轻量级可训练Forward Block合成任务特定查询；Combine Block融合多层输出；Support-Query Attention Block通过调整原型对齐查询分布

Result: 在4个域内少样本数据集和6个跨域数据集上达到最先进性能，显著减少计算开销

Conclusion: EfficientFSL通过查询式微调框架有效平衡了ViT的强大表示能力和实际部署的计算约束，为低资源场景提供了实用解决方案
...
TL;DR: 本文提出了一种名为约束数据集蒸馏（CD²）的框架，用于解决少样本类增量学习中的灾难性遗忘问题，通过数据集蒸馏模块和蒸馏约束模块来保持先前学习的知识。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习面临灾难性遗忘的关键问题。现有方法通常使用外部存储器存储先前知识，并平等对待增量类，无法适当保留先前的基本知识。受最近知识转移蒸馏工作的启发，需要一种更好的方法来保持先前学习的知识。

Method: 提出约束数据集蒸馏（CD²）框架，包括两个核心模块：1）数据集蒸馏模块（DDM）- 在分类器指导下合成高度浓缩的样本，迫使模型从少量增量样本中学习紧凑的基本类相关线索；2）蒸馏约束模块（DCM）- 引入设计的损失函数来约束先前学习的类分布，更充分地保留蒸馏知识。

Result: 在三个公共数据集上进行的广泛实验表明，该方法优于其他最先进的竞争对手。

Conclusion: CD²框架通过数据集蒸馏和约束机制有效解决了少样本类增量学习中的灾难性遗忘问题，能够更好地保留先前学习的基本知识。
...
TL;DR: VideoHEDGE是一个用于视频问答幻觉检测的模块化框架，通过生成多个答案变体、语义聚类和可靠性评分来检测视频视觉语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 视频视觉语言模型（Video-VLMs）经常产生高置信度的幻觉回答，而现有的不确定性度量方法往往无法准确识别这些错误。

Method: 提出VideoHEDGE框架：1）对视频进行光度和时空扰动生成多个变体；2）从原始视频和扰动变体中生成多个高温度答案；3）使用自然语言推理或嵌入方法将文本输出聚类为语义假设；4）基于聚类概率质量计算三种可靠性分数：语义熵、RadFlag和视觉增强语义熵。

Result: 在SoccerChat基准测试中，VASE在三个7B Video-VLM模型上始终获得最高的ROC-AUC，特别是在较大扰动预算时；嵌入聚类与NLI聚类性能相当但计算成本更低；领域微调减少幻觉频率但校准改进有限。

Conclusion: VideoHEDGE能有效检测视频问答中的幻觉，VASE是最可靠的度量方法，嵌入聚类提供了高效替代方案，领域微调对校准改善有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频角色替换方法依赖逐帧分割掩码和显式结构指导（如骨骼、深度），在遮挡、人物-物体交互、异常姿态或挑战性光照等复杂场景中泛化能力差，容易产生视觉伪影和时间不一致问题。

Method: 提出MoCha框架：1）仅需单帧任意掩码；2）引入条件感知RoPE适配多模态输入条件；3）使用RL后训练阶段增强面部身份；4）构建三个专门数据集：UE5渲染的高保真数据集、肖像动画技术合成的表情驱动数据集、现有视频-掩码对的增强数据集。

Result: 大量实验表明，该方法在复杂场景中显著优于现有最先进方法，能够处理遮挡、交互、异常姿态和挑战性光照等情况，同时保持时间一致性。

Conclusion: MoCha通过简化输入要求（仅需单帧掩码）和创新的训练策略，有效解决了视频角色替换在复杂场景中的泛化问题，为相关研究提供了新思路。
...
TL;DR: 提出两种互补策略（放射组学指纹和健康人格）来提升膝关节MRI自动评估的准确性和可解释性，通过患者特异性特征选择和健康基线合成实现个性化诊断。


<details>
  <summary>Details</summary>
Motivation: 传统放射组学使用群体级预定义特征，虽然可解释性强但过于局限，无法捕捉患者特异性变异；端到端深度学习虽然性能好但缺乏可解释性。需要兼顾准确性和临床可解释性的解决方案。

Method: 1. 放射组学指纹：动态构建患者特异性特征集，通过图像条件预测器估计特征使用概率，结合透明逻辑回归进行分类。2. 健康人格：使用扩散模型合成无病理的膝关节MRI基线，通过对比病理图像与健康基线的特征差异来定位疾病。

Result: 两种方法在三个临床任务上的表现均达到或超过最先进的深度学习模型，同时支持多层次的可解释性。案例研究展示了这些方法如何促进人类可解释的生物标志物发现和病理定位。

Conclusion: 放射组学指纹和健康人格是互补策略，能够实现个性化、可解释的膝关节MRI自动评估，在保持高性能的同时提供临床可接受的解释机制。
...
<details>
  <summary>Details</summary>
Motivation: 源自由域自适应（SFDA）在现实应用中面临数据隐私和存储限制的挑战，现有方法在感知范围和计算效率之间存在权衡。虽然Mamba通过选择性扫描机制提供了长距离依赖建模的解决方案，但Visual Mamba在捕获通道频率特征和保持空间鲁棒性方面仍有局限。

Method: 提出SfMamba框架，包含两个核心组件：1）通道视觉状态空间块，实现通道序列扫描以提取域不变特征；2）语义一致打乱策略，在2D选择性扫描中打乱背景补丁序列同时保持预测一致性，以减少错误累积。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在医疗保健、自动驾驶等关键决策系统中的广泛应用，其不确定性估计的校准变得至关重要。然而，现有的VLM测试时提示调优文献主要关注提升判别性能，而忽视了校准这一维度。现有方法强制文本提示嵌入完全正交，但作者理论证明这会过度推动语义相关类别分离，导致模型过度自信。

Method: 提出语义正交校准（SoC）方法，采用Huber-based正则化器，在强制原型分离的同时保持语义邻近性。该方法避免了完全正交约束的缺陷，实现平滑的原型分离，从而改善校准性能。

Result: 通过全面的实证验证，证明SoC方法在多个数据集上一致地改善了校准性能，同时保持了竞争力的判别能力。相比之前基于正交性的方法，SoC在校准方面表现更优。

Conclusion: 语义正交校准（SoC）是一种有效的VLM测试时提示调优方法，能够显著改善模型的不确定性校准，同时不牺牲判别性能。该方法通过平衡原型分离和语义保持，解决了现有正交方法导致的过度自信问题。
...
TL;DR: CtrlFuse：一个可控的红外与可见光图像融合框架，通过掩码提示实现交互式动态融合，同时提升融合质量和下游任务性能
...
Result: 实验表明在融合可控性和分割准确性方面均达到最先进水平，适应后的任务分支甚至超越了原始分割模型的性能

Conclusion: CtrlFuse通过交互式动态融合机制有效解决了现有方法的局限性，实现了任务感知与图像融合的协同优化，为智能无人系统的环境感知提供了更灵活的解决方案
...
TL;DR: 本文提出了一个基于四大支柱（可迁移性、鲁棒性、可解释性、计算效率）的深度伪造检测可靠性评估框架，分析了五种最先进方法，揭示了显著进展和关键局限性。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术具有教育创意等积极应用，但也带来欺诈、虚假信息和隐私侵犯等严重负面影响。虽然检测技术已有进步，但超越分类性能的综合评估方法仍然缺乏。

Method: 提出了一个基于四大支柱的可靠性评估框架：1) 可迁移性（在不同数据集和伪造方法间的泛化能力）；2) 鲁棒性（对抗攻击和扰动的稳定性）；3) 可解释性（检测结果的可解释性）；4) 计算效率（资源消耗和速度）。使用该框架分析了五种最先进的深度伪造检测方法。

Result: 对五种最先进方法的分析显示，在深度伪造检测方面取得了显著进展，但也揭示了关键局限性。不同方法在四大评估支柱上表现各异，存在可迁移性不足、鲁棒性脆弱、可解释性差或计算效率低等问题。

Conclusion: 提出的可靠性评估框架为深度伪造检测方法提供了超越传统分类性能的综合评估标准。分析结果表明当前方法在可迁移性、鲁棒性、可解释性和计算效率方面仍存在重要挑战，为未来研究指明了改进方向。
...
TL;DR: Salience-SGG通过迭代显著性解码器强调具有显著空间结构的三元组，解决了场景图生成中的长尾分布问题，同时避免了传统去偏方法对空间理解的削弱。


<details>
  <summary>Details</summary>
Motivation: 场景图生成存在长尾分布问题，少数谓词类别占主导而许多其他类别代表性不足，导致模型在罕见关系上表现不佳。现有的无偏SGG方法虽然解决了偏置问题，但往往以牺牲空间理解为代价，过度依赖语义先验。

Method: 提出Salience-SGG框架，包含迭代显著性解码器，强调具有显著空间结构的三元组。同时提出语义无关的显著性标签来指导ISD，确保模型关注空间关系而非语义先验。

Result: 在Visual Genome、Open Images V6和GQA-200数据集上的评估显示，Salience-SGG实现了最先进的性能，并通过Pairwise Localization Average Precision指标证明能够改善现有无偏SGG方法的空间理解能力。

Conclusion: Salience-SGG通过强调空间显著性，在解决长尾分布偏置问题的同时保持了空间理解能力，为场景图生成提供了更平衡的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 急性缺血性卒中病变在MRI中的准确分割对卒中诊断和管理至关重要。虽然已有基于U-Net的深度学习模型，但大多数实现不公开，且最优配置不明确，需要开发更鲁棒、可复现的分割框架。

Method: 提出ISLA模型，系统优化损失函数、卷积架构、深度监督和注意力机制，使用三个多中心数据库（超过1500名AIS参与者）进行训练，并研究无监督域适应以提高对外部临床数据集的泛化能力。

Result: ISLA在外部测试集上优于两种最先进的AIS病变分割方法，证明了其优越性能。代码和训练模型将公开以促进重用和可复现性。

Conclusion: ISLA为急性缺血性卒中病变分割提供了一个鲁棒且可复现的深度学习框架，通过系统优化实现了优异的性能，其公开可用性将促进该领域的研究和应用。
...
TL;DR: DentalX是一个基于上下文感知的牙科疾病检测方法，通过利用口腔结构信息来减轻X光片中的视觉模糊性，显著提升了细微牙科疾病的检测性能。
...
Method: 提出DentalX方法，引入结构上下文提取模块，学习牙科解剖结构的语义分割作为辅助任务。该模块提取有意义的结构上下文，并将其整合到主要的疾病检测任务中，以增强对细微牙科疾病的检测能力。

Result: 在专用基准测试上的广泛实验表明，DentalX在两个任务上都显著优于先前的方法。这种相互益处是在模型优化过程中自然产生的，因为两个任务之间的相关性被有效捕捉。

Conclusion: DentalX通过利用口腔结构信息来减轻X光片中的视觉模糊性，为牙科疾病检测提供了一种有效的上下文感知方法，能够显著提升对细微疾病的检测性能。
...
TL;DR: 本文提出了一种自动合成3D视觉定位数据及其推理过程的管道，并利用该数据微调LLM，开发出性能超越先前方法的3D视觉定位大语言模型Reason3DVG-8B。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉定位任务面临两大挑战：1) 现有模型推理能力有限；2) 需要大量人工标注的3D数据，成本高昂。虽然已有研究尝试使用合成数据训练3D视觉定位LLM，但性能提升有限且与数据收集成本不成比例。

Method: 1) 开发自动化的3D视觉定位数据合成管道，能够生成包含相应推理过程的训练数据；2) 利用生成的数据对大语言模型进行微调；3) 提出Reason3DVG-8B模型，这是一个8B参数的3D视觉定位大语言模型。

Result: Reason3DVG-8B仅使用先前方法3D-GRAND 1.6%的训练数据，就在性能上超越了该方法，证明了所提出数据管道的有效性以及推理在3D视觉定位中的重要性。

Conclusion: 通过自动化合成包含推理过程的3D视觉定位数据，可以高效训练出性能优异的3D视觉定位大语言模型，显著降低数据收集成本，同时验证了推理能力在3D视觉理解中的关键作用。
...
### [68] [3AM: Segment Anything with Geometric Consistency in Videos](https://arxiv.org/abs/2601.08831)
*Yang-Che Sun,Cheng Sun,Chin-Yang Lin,Fu-En Yang,Min-Hung Chen,Yen-Yu Lin,Yu-Lun Liu*

Main category: cs.CV

TL;DR: 3AM通过将MUSt3R的3D感知特征集成到SAM2中，解决了视频对象分割在视角变化下的问题，仅需RGB输入无需相机位姿，在宽基线运动数据集上显著优于现有方法。
...
Method: 提出3AM训练时增强方法：1）集成MUSt3R的3D感知特征到SAM2中；2）轻量级特征融合器融合多级MUSt3R特征（编码隐式几何对应）；3）结合SAM2外观特征实现基于空间位置和视觉相似性的几何一致识别；4）提出视场感知采样策略确保帧观察空间一致对象区域。
...
Conclusion: 3AM通过融合3D感知特征，仅需RGB输入就能实现几何一致的视频对象分割，在大视角变化下显著提升性能，为实际应用提供了更实用的解决方案。
...
### [69] [RAVEN: Erasing Invisible Watermarks via Novel View Synthesis](https://arxiv.org/abs/2601.08832)
*Fahad Shamshad,Nils Lukas,Karthik Nandakumar*

Main category: cs.CV

TL;DR: 该论文提出了一种基于视角合成的零样本扩散框架，通过语义保持的视角变换来移除不可见水印，揭示了当前水印方案在语义层面存在的根本性漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像内容认证需求的增长，各大平台大规模部署了不可见水印方案。然而，评估这些方案对抗复杂移除攻击的脆弱性对于评估其可靠性和指导鲁棒设计至关重要。
...
Conclusion: 揭示了不可见水印的根本性漏洞：即使对像素空间和频域攻击具有鲁棒性的水印，仍然容易受到语义保持的视角变换攻击。这表明当前水印方案存在关键缺陷，需要更鲁棒的设计。
...
### [70] [Fiducial Exoskeletons: Image-Centric Robot State Estimation](https://arxiv.org/abs/2601.08034)
*Cameron Smith,Basile Van Hoorick,Vitor Guizilini,Yue Wang*

Main category: cs.RO

TL;DR: Fiducial Exoskeletons是一种基于图像的3D机器人状态估计方法，通过单张RGB图像估计机器人各连杆的6D位姿，替代了传统繁琐的手眼标定流程。
...
Method: 1. 将机器人状态估计重新定义为从单张RGB图像估计每个连杆的6D位姿；2. 引入"基准外骨骼"：在每个连杆上安装带有已知几何关系的基准标记的3D打印支架；3. 通过轻量级全局优化确保观测到的连杆位姿与运动学一致性，从而获得机器人-相机基座变换和关节状态。

Result: 该方法在低成本机器人手臂上验证，显著简化了设置流程，同时提高了标定精度、状态准确性和下游3D控制性能。即使在断电的机器人上也能实现鲁棒的状态估计。

Conclusion: Fiducial Exoskeletons提供了一种简单有效的机器人状态估计解决方案，通过算法-硬件协同设计，实现了从单张图像获取鲁棒的相机-机器人外参、连杆SE(3)位姿和关节角度状态。
...
### [71] [Efficient Incremental SLAM via Information-Guided and Selective Optimization](https://arxiv.org/abs/2601.08110)
*Reza Arablouei*

Main category: cs.RO

TL;DR: 提出了一种高效的增量SLAM后端方法，结合信息引导门控和选择性部分优化，在保持批量优化精度的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统增量SLAM方法在计算效率和全局一致性之间存在权衡。需要一种既能保持批量优化精度，又能显著降低计算成本的方法，以适应实时动态数据丰富的环境。

Method: 提出两种互补机制：1) 信息引导门控(IGG)：基于信息矩阵对数行列式的信息理论准则，量化新测量的贡献，仅在观察到显著信息增益时触发全局优化；2) 选择性部分优化(SPO)：执行多轮高斯牛顿更新，但每轮仅优化受新测量影响最大的变量子集，动态细化活动集直至收敛。

Result: 在基准SLAM数据集上的广泛实验表明，该方法始终匹配批量求解器的估计精度，同时相比传统增量方法实现显著的计算节省。理论分析显示该方法保持了完整高斯牛顿的收敛保证。

Conclusion: 该方法在精度和效率之间提供了原则性的平衡，为动态数据丰富环境中的实时操作提供了稳健且可扩展的解决方案。
...
### [72] [A Pin-Array Structure for Gripping and Shape Recognition of Convex and Concave Terrain Profiles](https://arxiv.org/abs/2601.08143)
*Takuya Kato,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出一种用于移动机器人的抓取器，能够在极端环境中抓取并识别地形形状，采用针阵列结构同时实现自适应抓取和地形测量。


<details>
  <summary>Details</summary>
Motivation: 多肢攀爬机器人在崎岖地形（如悬崖、洞穴墙壁）上有效，但在未知自然环境中可能因误抓表面或失去可抓取点而摔倒或卡住。需要一种既能自适应抓取不规则地形，又能准确测量地形表面形状的抓取器。

Method: 开发了一种采用针阵列结构的抓取器，能够抓取凸凹地形并同时测量地形形状。通过原型机验证了抓取器的机制，并评估了其抓取和地形识别性能。

Result: 提出的针阵列设计不仅适用于不规则地形的自适应抓取，还能很好地用于3D地形映射。原型机验证了抓取和地形识别性能。

Conclusion: 该针阵列抓取器解决了极端环境中移动机器人面临的抓取和地形识别问题，为在未知自然环境中安全导航提供了有效解决方案。
...
### [73] [Robust Subpixel Localization of Diagonal Markers in Large-Scale Navigation via Multi-Layer Screening and Adaptive Matching](https://arxiv.org/abs/2601.08161)
*Jing Tao,Banglei Guan,Yang Shang,Shunkun Liang,Qifeng Yu*

Main category: cs.RO

TL;DR: 提出一种针对复杂背景干扰下飞行导航定位失效问题的鲁棒高精度定位方法，通过多层角点筛选和自适应模板匹配实现快速精确的标记定位


<details>
  <summary>Details</summary>
Motivation: 解决大规模飞行导航中复杂背景干扰导致的定位失效问题，以及传统滑动窗口匹配技术计算效率低下的局限性
...
Conclusion: 该方法为复杂背景干扰下的飞行导航定位提供了一种鲁棒、高精度且计算高效的解决方案
...
### [74] [FSAG: Enhancing Human-to-Dexterous-Hand Finger-Specific Affordance Grounding via Diffusion Models](https://arxiv.org/abs/2601.08246)
*Yifan Han,Pengfei Yi,Junyan Li,Hanqing Wang,Gaojing Zhang,Qi Peng Liu,Wenzhao Lian*

Main category: cs.RO

TL;DR: 提出了一种数据高效框架，利用预训练扩散模型中的语义先验，从人类视频演示中提取抓取功能，实现无需特定硬件抓取数据集的多指灵巧抓取合成


<details>
  <summary>Details</summary>
Motivation: 灵巧抓取合成面临高维度和运动多样性挑战，现有方法依赖特定硬件的大型抓取数据集，限制了新灵巧手设计的可扩展性

Method: 从人类视频演示中提取时间对齐的细粒度抓取功能，与深度图像的3D场景几何融合，通过运动学感知的重定向模块映射到不同灵巧手

Result: 系统产生稳定、功能适当的多接触抓取，在常见物体和工具上可靠成功，对未见物体实例、姿态变化和不同手部实现具有强泛化能力

Conclusion: 通过人类演示和预训练生成模型，为可扩展、硬件无关的灵巧操作开辟了新路径，证明单深度模态结合基础模型语义足以实现高性能抓取合成
...
### [75] [ActiveVLA: Injecting Active Perception into Vision-Language-Action Models for Precise 3D Robotic Manipulation](https://arxiv.org/abs/2601.08325)
*Zhenyang Liu,Yongchong Gu,Yikai Wang,Xiangyang Xue,Yanwei Fu*

Main category: cs.RO

TL;DR: ActiveVLA是一个新颖的视觉-语言-动作框架，通过主动感知能力实现机器人高精度细粒度操作，采用粗到细的两阶段方法：关键区域定位和主动感知优化。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作方法通常依赖静态腕部摄像头，提供末端执行器中心视角，无法在任务执行过程中自适应选择最优视角或分辨率，限制了在长视野任务和细粒度操作场景中的性能。
...
Conclusion: ActiveVLA通过引入主动感知能力，解决了现有视觉-语言-动作方法在视角选择和分辨率适应方面的局限性，为机器人高精度细粒度操作提供了有效解决方案。
...
### [76] [Large Language Models to Enhance Multi-task Drone Operations in Simulated Environments](https://arxiv.org/abs/2601.08405)
*Yizhan Feng,Hichem Snoussi,Jing Teng,Abel Cherouat,Tian Wang*

Main category: cs.RO

TL;DR: 提出了一种基于微调CodeT5模型与AirSim无人机模拟器的自然语言控制方法，实现多任务操作的自动化代码生成与执行


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，人机交互面临前所未有的机遇。传统无人机操作门槛较高，需要专业编程知识，限制了普通用户的使用。本文旨在通过自然语言命令降低无人机操作门槛，实现更直观的人机交互

Method: 1) 使用微调的CodeT5模型；2) 结合Unreal Engine的AirSim无人机模拟器；3) 利用ChatGPT生成的自然语言-程序代码对数据集与开发者编写的无人机代码作为训练数据；4) 实现自然语言到可执行无人机任务代码的自动翻译

Result: 实验结果表明，该方法在模拟环境中表现出优越的任务执行效率和命令理解能力。能够通过自然语言提示或命令描述灵活控制无人机状态，显著降低操作门槛

Conclusion: 该方法为无人机自然语言控制提供了有效解决方案。未来计划以模块化方式扩展模型功能，增强对复杂场景的适应性，推动无人机技术在现实环境中的应用
...
### [77] [Teaching Robots Like Dogs: Learning Agile Navigation from Luring, Gesture, and Speech](https://arxiv.org/abs/2601.08422)
*Taerim Yoon,Dongho Kang,Jin Cheng,Fatemeh Zargarbashi,Yijiang Huang,Minsung Ahn,Stelian Coros,Sungjoon Choi*

Main category: cs.RO

TL;DR: 提出人机交互框架，让四足机器人通过少量人类演示数据学习理解社交线索和导航行为，结合手势和语音指令实现敏捷导航


<details>
  <summary>Details</summary>
Motivation: 让四足机器人能够理解人类社交线索并产生适当行为，但物理交互学习需要大量人类数据，给用户带来负担，需要数据高效的学习方法

Method: 提出人在回路框架，使用物理仿真重建交互场景并聚合数据缓解分布偏移；采用渐进式目标提示策略，在训练中自适应提供指令和导航目标；支持多模态自然输入（手势和语音）

Result: 在6个真实世界敏捷导航场景（跳跃障碍、避障等）中测试，成功率97.15%，总演示数据少于1小时

Conclusion: 提出的框架能够以数据高效的方式让机器人学习导航行为，实现人类输入与机器人行为的高度对齐，在复杂场景中表现优异
...
### [78] [Large Multimodal Models for Embodied Intelligent Driving: The Next Frontier in Self-Driving?](https://arxiv.org/abs/2601.08434)
*Long Zhang,Yuchen Xia*

Main category: cs.RO

TL;DR: 本文提出了一种语义与策略双驱动的混合决策框架，将大型多模态模型（LMMs）的语义理解能力与深度强化学习（DRL）的实时策略优化相结合，以解决自动驾驶在开放世界场景中的持续学习和联合决策问题。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶的模块化设计在需要持续环境理解和逻辑推理的开放世界场景中存在局限性。虽然大型多模态模型（LMMs）和具身人工智能为自动驾驶提供了增强理解能力和持续学习能力，但仅依赖LMMs无法实现有效的联合决策，限制了向具身智能驾驶的发展。

Method: 提出语义与策略双驱动的混合决策框架：1）利用LMMs进行语义理解和认知表征；2）采用深度强化学习（DRL）进行实时策略优化；3）将两者结合实现持续学习和联合决策。通过变道规划任务的案例研究验证框架性能。

Result: 实验验证表明，该框架在完成变道规划任务方面表现出性能优势。框架能够有效结合LMMs的语义理解能力和DRL的实时决策能力，为具身智能驾驶提供了可行的技术路径。

Conclusion: 提出的语义与策略双驱动混合决策框架能够有效解决自动驾驶在开放世界场景中的持续学习和联合决策挑战。该框架为具身智能驾驶的发展提供了新的研究方向，并识别了多个未来研究方向以指导后续工作。
...
### [79] [Real2Sim based on Active Perception with automatically VLM-generated Behavior Trees](https://arxiv.org/abs/2601.08454)
*Alessandro Adami,Sebastian Zudaire,Ruggero Carli,Pietro Falco*

Main category: cs.RO

TL;DR: 提出一个自主生成行为树的Real2Sim框架，通过任务特定的物理交互获取仿真所需参数，无需预定义任务模板或专家设计的探索程序


<details>
  <summary>Details</summary>
Motivation: 传统Real2Sim流水线依赖手动测量或固定的预编程探索程序，限制了其对不同任务和用户意图的适应性，需要更灵活自主的参数估计方法

Method: 使用视觉语言模型进行多模态推理，识别相关对象、推断所需物理参数，并生成结构化行为树；通过扭矩控制的机器人执行接触丰富的交互来估计参数

Result: 在真实机械臂上成功估计了物体质量、表面高度和摩擦相关量，包括遮挡物体和不完整先验模型等多种场景
...
### [80] [AME-2: Agile and Generalized Legged Locomotion via Attention-Based Neural Map Encoding](https://arxiv.org/abs/2601.08485)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：端到端感觉运动模型泛化能力和可解释性有限；针对泛化运动的方法敏捷性不足且难以处理视觉遮挡。需要一种能同时实现敏捷性和泛化性的方法。

Method: 提出AME-2框架，包含：1）注意力机制的地图编码器，提取局部和全局地图特征，关注关键区域；2）基于学习的映射管道，将深度观测转换为带不确定性的局部高程图，并与里程计融合，对噪声和遮挡具有鲁棒性；3）与并行仿真集成，支持在线映射训练。

Result: 在四足和双足机器人上验证，控制器在仿真和真实实验中表现出强大的敏捷性，并能泛化到未见过的地形。

Conclusion: AME-2框架通过注意力机制的地图编码器和鲁棒的映射管道，成功实现了机器人在复杂地形上的敏捷和泛化运动，解决了现有方法的局限性。
...
### [81] [AUV Trajectory Learning for Underwater Acoustic Energy Transfer and Age Minimization](https://arxiv.org/abs/2601.08491)
*Mohamed Afouene Melki,Mohammad Shehab,Mohamed-Slim Alouini*

Main category: cs.RO

TL;DR: 该论文提出了一种可持续的水下物联网解决方案，通过自主水下车辆同时进行信息上行和声能传输，使设备能够无限期运行。采用深度强化学习算法优化年龄信息和公平性指标。
...
Method: 提出通过自主水下车辆同时进行信息上行和声能传输的方法。采用年龄信息（AoI）和Jain公平性指数作为评估指标。开发了两种深度强化学习算法：高性能的频分双工（FDD）解决方案和中等性能的时分双工（TDD）方法。

Result: 提出的FDD和TDD解决方案相比基线方法显著降低了平均年龄信息（AoI），提高了能量收集效率和数据收集的公平性。

Conclusion: 该研究为水下物联网提供了一种可持续的解决方案，通过自主水下车辆同时进行信息传输和能量补给，有望实现设备的无限期运行，并通过深度强化学习优化了系统性能。
...
### [82] [Simplifying ROS2 controllers with a modular architecture for robot-agnostic reference generation](https://arxiv.org/abs/2601.08514)
*Davide Risi,Vincenzo Petrone,Antonio Langella,Lorenzo Pagliara,Enrico Ferrentino,Pasquale Chiacchio*

Main category: cs.RO

TL;DR: 提出了一种新的ROS2模块化架构，将参考获取、验证和插值逻辑与控制律解耦，通过专门的Reference Generator组件处理参考输入，减少控制器中的重复代码，提高跨平台复用性。


<details>
  <summary>Details</summary>
Motivation: 现有ROS2控制器中，参考处理代码（获取、验证、插值）与控制律代码耦合，导致代码重复和跨平台复用性差。需要一种架构来分离这些关注点，提高控制系统的模块化和可重用性。

Method: 设计了一个专门的Reference Generator组件，接收来自规划器等外部节点的参考输入（单点或轨迹），通过ros2_control链式机制以控制器采样周期向下游控制器写入单点参考。实现了两种参考生成器：关节空间参考生成器和笛卡尔空间参考生成器，并开发了新的控制器（PD重力补偿、笛卡尔位姿、导纳控制器）。

Result: 在模拟和真实机器人（Universal Robots和Franka Emika机械臂）上验证了该方法。结果显示：(1)在所有测试场景中参考都能可靠跟踪；(2)参考生成器减少了链式控制器中的重复参考处理代码，有利于构建和复用复杂控制器管道；(3)控制器实现仅专注于控制律。

Conclusion: 提出的模块化架构成功将参考处理逻辑与控制律解耦，通过专门的Reference Generator组件提高了代码复用性和系统模块化，为构建复杂控制器管道提供了更好的支持。
...
### [83] [Keyframe-based Dense Mapping with the Graph of View-Dependent Local Maps](https://arxiv.org/abs/2601.08520)
*Krzysztof Zielinski,Dominik Belter*

Main category: cs.RO

TL;DR: 提出基于关键帧的RGB-D建图系统，使用局部NDT地图，通过2D视图相关结构存储单元，利用RGB-D相机特性实现距离相机越近精度越高的自然表示，支持闭环检测后的全局地图校正。


<details>
  <summary>Details</summary>
Motivation: 现有建图方法未能充分利用RGB-D相机的特性和不确定性模型，需要一种能够自然表示距离相机越近精度越高的建图系统，同时支持局部地图的合并和全局地图的优化。

Method: 基于关键帧的建图系统，使用局部NDT地图，将NDT单元存储在2D视图相关结构中，利用RGB-D传感器数据更新局部地图，将局部地图存储在姿态图中支持闭环检测后的全局校正，并提出局部地图合并和过滤流程以获得全局环境地图。

Result: 与Octomap和NDT-OM方法进行比较，展示了所提建图方法的示例应用，验证了方法的有效性。

Conclusion: 提出的基于关键帧的RGB-D建图系统能够有效利用RGB-D相机特性，实现距离相机越近精度越高的自然表示，支持闭环检测和全局地图优化，为环境建图提供了有效的解决方案。
...
### [84] [QP-Based Control of an Underactuated Aerial Manipulator under Constraints](https://arxiv.org/abs/2601.08523)
...
<details>
  <summary>Details</summary>
Motivation: 针对欠驱动空中机械臂的控制问题，需要在实现精确末端执行器轨迹跟踪的同时，明确考虑安全性和可行性约束，并增强系统对扰动、建模不确定性和稳态误差的鲁棒性。

Method: 将控制问题表述为二次规划问题，计算满足欠驱动、执行器边界和系统约束的动态一致广义加速度。在扭矩层面加入基于无源性的积分作用以增强鲁棒性，同时不损害可行性。

Result: 通过高保真物理仿真验证了方法的有效性，包括参数扰动、粘性关节摩擦以及真实的传感和状态估计效应。实现了精确跟踪、平滑控制输入和可靠的约束满足。

Conclusion: 提出的约束感知控制框架能够在现实操作条件下为欠驱动空中机械臂提供准确跟踪、平滑控制和可靠的约束满足，增强了系统的鲁棒性和实用性。
...
### [85] [VLingNav: Embodied Navigation with Adaptive Reasoning and Visual-Assisted Linguistic Memory](https://arxiv.org/abs/2601.08665)
*Shaoan Wang,Yuanfei Luo,Xingyu Chen,Aocheng Luo,Dongyue Li,Chang Liu,Sheng Chen,Yangang Zhang,Junzhi Yu*

Main category: cs.RO

TL;DR: VLingNav是一种基于语言驱动认知的视觉语言行动模型，通过自适应思维链机制和视觉辅助语言记忆模块，在具身导航任务中实现最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型主要依赖从观察到动作的被动映射，缺乏显式推理能力和持久记忆，难以处理复杂、长时程的导航任务。

Method: 1. 引入自适应思维链机制，动态触发显式推理；2. 开发视觉辅助语言记忆模块，构建跨模态语义记忆；3. 构建Nav-AdaCoT-2.9M数据集；4. 结合在线专家引导强化学习。

Result: VLingNav在多个具身导航基准测试中达到最先进性能，能够零样本迁移到真实机器人平台，执行各种导航任务，展示出强大的跨域和跨任务泛化能力。

Conclusion: VLingNav通过语言驱动的认知方法，结合自适应推理和持久记忆，显著提升了VLA模型在复杂具身导航任务中的性能，实现了从仿真到真实世界的有效迁移。
...
### [86] [Real-Time Localization Framework for Autonomous Basketball Robots](https://arxiv.org/abs/2601.08713)
*Naren Medarametla,Sreejon Mondal*

Main category: cs.RO

TL;DR: 提出了一种用于篮球场机器人定位的混合算法，结合经典技术与基于学习的方法，仅利用场地地板视觉数据进行自定位


<details>
  <summary>Details</summary>
Motivation: 在Robocon 2025竞赛中，精确可靠的定位对于提高射击精度、避免与其他机器人碰撞以及高效导航比赛场地至关重要。自主机器人在动态环境中需要基础定位能力才能有效运行。

Method: 提出了一种混合定位算法，将经典定位技术与基于学习的方法相结合。该方法仅依赖从篮球场地板获取的视觉数据进行自定位。

Result: 论文未在摘要中提供具体实验结果，但描述了该方法旨在在篮球场上实现机器人自定位。

Conclusion: 该混合定位方法结合了经典技术和基于学习的视觉方法，为Robocon 2025竞赛中的机器人提供了准确可靠的定位解决方案，有助于提高整体性能。
...
### [87] [Older Adults' Preferences for Feedback Cadence from an Exercise Coach Robot](https://arxiv.org/abs/2601.08819)
*Roshni Kaushik,Reid Simmons*

Main category: cs.RO

TL;DR: 研究探索老年人对机器人运动教练不同节奏的言语和非言语反馈的反应，通过在线视频评估发现改变一种模态的节奏会影响两种模态的感知，为设计适合老年人的机器人教练反馈频率提供指导。


<details>
  <summary>Details</summary>
Motivation: 人们以不同方式回应反馈和指导，机器人需要个性化互动并利用言语和非言语沟通线索。本研究旨在了解老年人如何回应机器人运动教练不同节奏的言语和非言语反馈。

Method: 对老年人进行在线研究，参与者评估机器人以不同节奏提供每种模态反馈的视频，分析节奏变化对感知的影响。

Result: 结果显示改变一种模态的节奏会影响该模态和另一种模态的感知，节奏调整对老年人对机器人反馈的接受度有显著影响。

Conclusion: 研究结果可用于更好地设计机器人教练在老年人运动训练中的反馈频率，优化人机互动体验。
